...
# Add repo root to path for bucket-aware imports
def find_repo_root() -> Path:
    p = Path(__file__).resolve()
    for parent in p.parents:
        if (parent / "AGENTS.md").exists() or (parent / ".git").exists():
            return parent
    return p.parents[3]  # Default for CAPABILITY/TOOLS/catalytic/
...
class CatalyticRuntime:
    ...
    def __init__(
        ...
        self.ledger_dir = PROJECT_ROOT / "LAW" / "CONTRACTS" / "_runs" / run_id
        self.ledger_dir.mkdir(parents=True, exist_ok=True)
        self.cas = CatalyticStore(self.ledger_dir / "CAS")
        self.ledger = Ledger(self.ledger_dir / "LEDGER.jsonl")
        self.memo_cache = JobMemoCache(PROJECT_ROOT / "LAW" / "CONTRACTS" / "_runs" / "_cache" / "jobs")
    ...
    def write_canonical_artifacts(
        ...
        try:
            ...
            proof_schema_path = PROJECT_ROOT / "LAW" / "SCHEMAS" / "proof.schema.json"
