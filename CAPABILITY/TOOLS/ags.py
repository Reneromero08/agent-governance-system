...
from CAPABILITY.PRIMITIVES.registry_validators import (
...
def _pipeline_dir(pipeline_id: str) -> Path:
    ...
    path = Path("LAW") / "CONTRACTS" / "_runs" / "_pipelines" / slugged
...
DEFAULT_CAPABILITIES_PATH = "CAPABILITY/CONFIG/CAPABILITIES.json"
DEFAULT_PINS_PATH = "CAPABILITY/CONFIG/CAPABILITY_PINS.json"
DEFAULT_REVOKES_PATH = "CAPABILITY/CONFIG/CAPABILITY_REVOKES.json"
DEFAULT_SKILLS_PATH = "CAPABILITY/SKILLS/registry.json"
...
def ags_route(*, plan_path: Path, pipeline_id: str, runs_root: str) -> int:
    if runs_root != "LAW/CONTRACTS/_runs":
        raise RuntimeError("UNSUPPORTED_RUNS_ROOT (only LAW/CONTRACTS/_runs is supported)")
...
def ags_run(*, pipeline_id: str, runs_root: str, strict: bool, repo_write: bool, allow_repo_write: bool, allow_dirty: bool, skip_preflight: bool = False) -> int:
    ...
    catalytic = [sys.executable, str(REPO_ROOT / "CAPABILITY" / "TOOLS" / "catalytic" / "catalytic.py")]
    ...
    if not skip_preflight:
        preflight_cmd = [sys.executable, str(REPO_ROOT / "CAPABILITY" / "PRIMITIVES" / "preflight.py"), "--json"]
        if allow_dirty:
    ...
    admit_cmd = [sys.executable, str(REPO_ROOT / "CAPABILITY" / "TOOLS" / "governance" / "admission.py"), "--intent", str(intent_path)]
...
def main(argv: List[str] | None = None) -> int:
    ...
    route_p.add_argument("--runs-root", default="LAW/CONTRACTS/_runs", help="Runs root (default: LAW/CONTRACTS/_runs)")
    ...
    run_p.add_argument("--runs-root", default="LAW/CONTRACTS/_runs", help="Runs root (default: LAW/CONTRACTS/_runs)")
    ...
    try:
        ...
        if ensure_script.exists():
            subprocess.run(
                ["bash", str(ensure_script), str(REPO_ROOT)],
                cwd=str(REPO_ROOT),
                check=False,
    ...
    try:
        ...
        if args.cmd == "preflight":
            preflight = [sys.executable, str(REPO_ROOT / "CAPABILITY" / "PRIMITIVES" / "preflight.py")]
            forwarded: List[str] = []
            if bool(args.strict):
        ...
        if args.cmd == "admit":
            admit = [sys.executable, str(REPO_ROOT / "CAPABILITY" / "TOOLS" / "governance" / "admission.py"), "--intent", str(args.intent)]
