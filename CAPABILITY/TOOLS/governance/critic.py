...
ALLOWED_OUTPUT_ROOTS = {
    str(PROJECT_ROOT / "LAW" / "CONTRACTS" / "_runs"),
    str(PROJECT_ROOT / "NAVIGATION" / "CORTEX" / "_generated"),
    ...
    for category_dir in SKILLS_DIR.iterdir():
        ...
        for skill_dir in category_dir.iterdir():
            ...
            if not fixtures_dir.exists() or not any(fixtures_dir.iterdir()):
                violations.append(f"Skill '{category_dir.name}/{skill_dir.name}' has no fixtures")
    return violations
            ...
            for py_file in skill_dir.glob("*.py"):
                ...
                for pattern in RAW_FS_PATTERNS:
                    if re.search(pattern, content):
                        ...
                        if skill_dir.name in ("artifact-escape-hatch", "pack-validate", "llm-packer-smoke", "cas-integrity-check", "system1-verify", "agi-hardener"):
                            continue
                        violations.append(
    ...
    for category_dir in SKILLS_DIR.iterdir():
        ...
        for skill_dir in category_dir.iterdir():
            ...
            if not manifest.exists():
                violations.append(f"Skill '{category_dir.name}/{skill_dir.name}' missing SKILL.md manifest")
    return violations
...
def check_schema_validation() -> List[str]:
    ...
    # Check ADRs
    adr_dir = PROJECT_ROOT / "LAW" / "CONTEXT" / "decisions"
    adr_schema = PROJECT_ROOT / "CAPABILITY" / "MCP" / "schemas" / "governance" / "adr.schema.json"
    if adr_dir.exists():
    ...
    # Check Skills
    skill_schema = PROJECT_ROOT / "CAPABILITY" / "MCP" / "schemas" / "governance" / "skill.schema.json"
    for category_dir in SKILLS_DIR.iterdir():
        if not category_dir.is_dir() or category_dir.name.startswith("_"):
            continue
    ...
    # Check Styles
    style_dir = PROJECT_ROOT / "LAW" / "CONTEXT" / "preferences"
    style_schema = PROJECT_ROOT / "CAPABILITY" / "MCP" / "schemas" / "governance" / "style.schema.json"
    if style_dir.exists():
...
def check_log_output_roots() -> List[str]:
    ...
    disallowed_patterns = [
        (r'["\']LOGS/', "References LOGS/ directory (should be LAW/CONTRACTS/_runs/<purpose>_logs/)"),
        (r'["\']CAPABILITY/MCP/logs/', "References CAPABILITY/MCP/logs/ (should be LAW/CONTRACTS/_runs/mcp_logs/)"),
    ]

    # Scan Python files that may write logs
    scan_paths = [
        PROJECT_ROOT / "CAPABILITY" / "TOOLS",
        PROJECT_ROOT / "CAPABILITY" / "MCP",
        PROJECT_ROOT / "CAPABILITY" / "SKILLS",
    ]
    ...
    for canon_file in canon_files:
        ...
        if re.search(r'`LOGS/[^`]+`', content):
            violations.append(
                f"{canon_file.relative_to(PROJECT_ROOT)}: References LOGS/ (should use LAW/CONTRACTS/_runs/)"
            )
        # Check for MCP/logs/ references (but skip ADR context)
        if re.search(r'`CAPABILITY/MCP/logs/[^`]+`', content):
            violations.append(
                f"{canon_file.relative_to(PROJECT_ROOT)}: References CAPABILITY/MCP/logs/ (should use LAW/CONTRACTS/_runs/)"
            )
    ...
    # Check for bare excepts (see CANON/STEWARDSHIP.md "No Bare Excepts" rule)
            if re.search(r'`LOGS/[^`]+`', unreleased_section):
                violations.append(
                    f"LAW/CANON/CHANGELOG.md [Unreleased]: References LOGS/ (should use LAW/CONTRACTS/_runs/)"
                )
            if re.search(r'`CAPABILITY/MCP/logs/[^`]+`', unreleased_section):
                violations.append(
                    f"LAW/CANON/CHANGELOG.md [Unreleased]: References CAPABILITY/MCP/logs/ (should use LAW/CONTRACTS/_runs/)"
                )
...
def check_output_roots(changed_files: List[str]) -> List[str]:
    ...
    disallowed_patterns = [
        ...
        (r'["\']LOGS/', "References LOGS/ (moved to LAW/CONTRACTS/_runs/ per ADR-015)"),
        (r'["\']CAPABILITY/MCP/logs', "References CAPABILITY/MCP/logs/ (moved to LAW/CONTRACTS/_runs/mcp_logs/ per ADR-015)"),
    ]
