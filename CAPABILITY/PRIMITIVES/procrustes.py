...
def out_of_sample_mds(
    ...
    # Row means of anchor distance matrix: r_i = mean_j D^2[i,j]
    r = D2_anchors.mean(axis=1)  # (n,)

    # Grand mean: r_bar = mean_i r_i
    r_bar = r.mean()
    ...
    for i in range(m):
        ...
        # Mean of new point's distances: d_bar = mean_i d^2_i
        d_bar = d2.mean()

        # Gower's formula: b = -1/2 * (d^2 - d_bar - r + r_bar)
        b = -0.5 * (d2 - d_bar - r + r_bar)

        # Project: y = L^(-1/2) * V^T * b
