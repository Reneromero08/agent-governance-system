# File: D:\CCC 2.0\AI\agent-governance-system\CAPABILITY\TESTBENCH\test_ags_phase6_bridge.py
        ...
        "steps": [
            {
                ...
                "jobspec": jobspec,
                "memoize": False,
                "cmd": [sys.executable, "-c", f"from pathlib import Path; Path('{out1}').parent.mkdir(parents=True, exist_ok=True); Path('{out1}').write_text('ONE', encoding='utf-8')"]
            },
            {
                "step_id": "s2",
                "cmd": [sys.executable, "-c", f"from pathlib import Path; Path('{out1}').parent.mkdir(parents=True, exist_ok=True); Path('{out1}').write_text('ONE', encoding='utf-8')"],
...
def test_ags_run_calls_verify_ok(tmp_path: Path) -> None:
    ...
    try:
        ...
        rr = _run_ags(["run", "--pipeline-id", pipeline_id, "--runs-root", "LAW/CONTRACTS/_runs", "--strict", "--skip-preflight"])
