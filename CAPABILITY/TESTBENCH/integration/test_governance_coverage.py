...
def test_roadmap_deliverables_exist():
    ...
    roadmap_path = REPO_ROOT / "MEMORY" / "archive" / "catalyticdpt" / "CATALYTIC_ROADMAP_V2.3.md"
    assert roadmap_path.exists()
    
    # Key files promised in Phase 7, 8, 9
    required_files = [
        "LAW/SCHEMAS/swarm.schema.json",
        "CAPABILITY/PIPELINES/swarm_runtime.py",
        "LAW/SCHEMAS/VERSIONING_POLICY.md",
        "MEMORY/archive/catalyticdpt/CATALYTIC_RELEASE_CHECKLIST.md",
        "LAW/CONTEXT/decisions/ADR-023-capability-revocation-semantics.md",
...
def test_canon_mentions_key_concepts():
    ...
    contract = (REPO_ROOT / "LAW" / "CANON" / "CONTRACT.md").read_text(encoding="utf-8")
...
def test_router_receipt_artifacts_in_schema():
    ...
    schema_path = REPO_ROOT / "LAW" / "SCHEMAS" / "ags_plan.schema.json"
    import json
...
def test_policy_snapshot_logic_exists():
    ...
    ags_py = (REPO_ROOT / "CAPABILITY" / "TOOLS" / "ags.py").read_text(encoding="utf-8")
    
    assert "revoked_capabilities" in ags_py, "TOOL/ags.py missing revocation snapshot logic"
    assert "POLICY.json" in ags_py, "TOOL/ags.py does not write POLICY.json"

def test_swarm_schema_validity():
    """Verify swarm.schema.json is valid JSON and contains expected structure (Phase 7)."""
    schema_path = REPO_ROOT / "LAW" / "SCHEMAS" / "swarm.schema.json"
    import json
    schema = json.loads(schema_path.read_text(encoding="utf-8"))
