...
def _setup_pipeline(tmp_root: Path, *, pipeline_id: str) -> tuple[Path, Path]:
    ...
    domain_rel = f"LAW/CONTRACTS/_runs/_tmp/{tmp_root}/domain"
    out1 = f"LAW/CONTRACTS/_runs/_tmp/pipeline-{pipeline_id}-s1-a1_out.txt"
    out2 = f"LAW/CONTRACTS/_runs/_tmp/pipeline-{pipeline_id}-s2-a1_out.txt"
...
def test_pipeline_chain_valid(tmp_path: Path) -> None:
    ...
    try:
        ...
        _rm(pipeline_dir)
        _rm(REPO_ROOT / "LAW" / "CONTRACTS" / "_runs" / "_tmp" / "pipeline_chain_valid")
...
def test_pipeline_chain_tamper(tmp_path: Path) -> None:
    ...
    try:
        ...
        proof = REPO_ROOT / "LAW" / "CONTRACTS" / "_runs" / run_id_s1 / "PROOF.json"
        ...
        _rm(pipeline_dir)
        _rm(REPO_ROOT / "LAW" / "CONTRACTS" / "_runs" / "_tmp" / "pipeline_chain_tamper")
...
def test_pipeline_chain_reorder(tmp_path: Path) -> None:
    ...
    try:
        ...
        _rm(pipeline_dir)
        _rm(REPO_ROOT / "LAW" / "CONTRACTS" / "_runs" / "_tmp" / "pipeline_chain_reorder")
...
def test_pipeline_chain_determinism(tmp_path: Path) -> None:
    ...
    try:
        ...
        _rm(pipeline_dir)
        for rid in run_ids:
            _rm(REPO_ROOT / "LAW" / "CONTRACTS" / "_runs" / rid)
        ...
        _rm(pipeline_dir)
        _rm(REPO_ROOT / "LAW" / "CONTRACTS" / "_runs" / "_tmp" / "pipeline_chain_determinism")
