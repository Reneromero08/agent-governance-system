...
```json
{
  ...
  "inputs": {"test": true},
  "outputs": {
    "durable_paths": ["CAPABILITY/TESTBENCH/_output"],
    "validation_criteria": {"schema_is_valid": true}
  },
  "catalytic_domains": ["CAPABILITY/TESTBENCH/_tmp"],
...
def test_store_retrieve():
    store = CatalyticStore("CAPABILITY/TESTBENCH/_store")
...
def test_domain_to_spectrum():
    # Create test files
    test_dir = Path("CAPABILITY/TESTBENCH/_domain")
...
def test_append_entry():
    ledger = Ledger("CAPABILITY/TESTBENCH/test_ledger.jsonl")
...
def test_deterministic_ordering():
    ledger = Ledger("CAPABILITY/TESTBENCH/test_ledger2.jsonl")
...
3. Run `python CAPABILITY/TESTBENCH/run_poc.py --all`
4. Check `CAPABILITY/TESTBENCH/report.md`
