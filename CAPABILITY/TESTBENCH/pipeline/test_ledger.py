...
def test_append_single_record_one_line(tmp_path: Path) -> None:
    ...
    ledger_inst = Ledger(path)
    ledger_inst.append(_record(job_id="job-1", run_id="run-1"))
...
def test_append_multiple_records_order_preserved(tmp_path: Path) -> None:
    ...
    ledger_inst = Ledger(path)
    ledger_inst.append(_record(job_id="job-1", run_id="run-1"))
    ledger_inst.append(_record(job_id="job-1", run_id="run-2"))

    records = ledger_inst.read_all()
...
def test_corrupt_line_detection(tmp_path: Path) -> None:
    ...
    assert ledger_inst.verify_append_only() is False
    with pytest.raises(ValueError):
        ledger_inst.read_all()