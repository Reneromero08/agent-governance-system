...
Run:
    pytest CAPABILITY/TESTBENCH/test_runtime_guard.py -v
"""
...
from CAPABILITY.PRIMITIVES.fs_guard import FilesystemGuard
...
def test_allowed_write_succeeds(fs_guard, project_root):
    ...
    (project_root / "LAW" / "CONTRACTS" / "_runs" / "test").mkdir(parents=True)

    # This should succeed
    valid, error = fs_guard.validate_write_path("LAW/CONTRACTS/_runs/test/output.json")
...
def test_guarded_write_text_succeeds(fs_guard, project_root):
    ...
    fs_guard.guarded_write_text(
        project_root / "LAW" / "CONTRACTS" / "_runs" / "test" / "output.json",
        "test content"
    )

    # Verify file was written
    assert (project_root / "LAW" / "CONTRACTS" / "_runs" / "test" / "output.json").read_text() == "test content"


def test_guarded_mkdir_succeeds(fs_guard, project_root):
    """Test that guarded_mkdir succeeds for allowed path."""
    # Create parent directory
    (project_root / "LAW" / "CONTRACTS" / "_runs").mkdir(parents=True)

    # This should succeed
    fs_guard.guarded_mkdir(
        project_root / "LAW" / "CONTRACTS" / "_runs" / "test_dir",
        parents=False,
        exist_ok=False
    )

    # Verify directory was created
    assert (project_root / "LAW" / "CONTRACTS" / "_runs" / "test_dir").is_dir()
...
def test_guarded_write_bytes_succeeds(fs_guard, project_root):
    ...
    fs_guard.guarded_write_bytes(
        project_root / "LAW" / "CONTRACTS" / "_runs" / "test" / "binary.dat",
        b"\x00\x01\x02\x03"
    )

    # Verify file was written
    assert (project_root / "LAW" / "CONTRACTS" / "_runs" / "test" / "binary.dat").read_bytes() == b"\x00\x01\x02\x03"
...
def test_path_normalization(fs_guard, project_root):
    ...
    # Test with backslashes (Windows-style)
    valid, error = fs_guard.validate_write_path("LAW\\CONTRACTS\\_runs\\test\\output.json")
...
def test_forbidden_path_fails(fs_guard):
    ...
    # Attempt to write to LAW/CANON (forbidden)
    valid, error = fs_guard.validate_write_path("LAW/CANON/test.md")

    assert valid is False
    assert error is not None
    assert error["code"] == "WRITE_GUARD_PATH_FORBIDDEN"
    assert "LAW/CANON" in error["message"].replace("\\", "/")
...
def test_forbidden_path_guarded_write_fails(fs_guard, project_root):
    ...
    # Create LAW/CANON directory (forbidden)
    (project_root / "LAW" / "CANON").mkdir(parents=True)

    # This should raise RuntimeError
    with pytest.raises(RuntimeError) as exc_info:
        fs_guard.guarded_write_text(
            project_root / "LAW" / "CANON" / "test.md",
            "forbidden content"
        )
...
def test_path_traversal_fails(fs_guard):
    ...
    valid, error = fs_guard.validate_write_path("LAW/CONTRACTS/_runs/../../../etc/passwd")
...
def test_multiple_allowed_roots_work(fs_guard, project_root):
    ...
    for root in ["LAW/CONTRACTS/_runs", "NAVIGATION/CORTEX/_generated", "LAW/MEMORY/LLM_PACKER/_packs", "CAPABILITY/PRIMITIVES/_scratch"]:
        (project_root / root).mkdir(parents=True)

    # Test each allowed root
    test_paths = [
        "LAW/CONTRACTS/_runs/test/file.json",
        "NAVIGATION/CORTEX/_generated/index.json",
        "LAW/MEMORY/LLM_PACKER/_packs/pack.tar.gz",
        "CAPABILITY/PRIMITIVES/_scratch/temp.txt",
    ]
...
def test_guarded_mkdir_forbidden_fails(fs_guard, project_root):
    ...
    with pytest.raises(RuntimeError) as exc_info:
        fs_guard.guarded_mkdir(
            project_root / "LAW" / "CANON" / "test_dir",
            parents=False,
            exist_ok=False
        )
