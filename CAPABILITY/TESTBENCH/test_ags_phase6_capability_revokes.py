...
def test_verify_rejects_revoked_capability(tmp_path):
    ...
    try:
        ...
        cmd_verify = [sys.executable, str(REPO_ROOT / "CAPABILITY" / "TOOLS" / "catalytic" / "catalytic.py"), "pipeline", "verify", 
...
def test_historical_pipeline_verifies_after_revocation(tmp_path):
    ...
    try:
        ...
        r2 = _run([sys.executable, str(REPO_ROOT / "CAPABILITY" / "TOOLS" / "catalytic" / "catalytic.py"), "pipeline", "run", "--pipeline-id", pipeline_id], env)
        ...
        r3 = _run([sys.executable, str(REPO_ROOT / "CAPABILITY" / "TOOLS" / "catalytic" / "catalytic.py"), "pipeline", "verify", "--pipeline-id", pipeline_id, "--strict"], env)
        assert r3.returncode == 0, f"Historical verify failed: {r3.stdout}\n{r3.stderr}"

        # 6. Attempt new route (Should FAIL because it is now revoked)
        r4 = _run([sys.executable, "-m", "CAPABILITY.TOOLS.ags", "route", "--plan", str(plan_path), "--pipeline-id", "test-new-fail"], env)
