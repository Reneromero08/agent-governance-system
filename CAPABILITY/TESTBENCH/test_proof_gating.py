...
Run:
    pytest CAPABILITY/TESTBENCH/test_proof_gating.py -v
"""
...
def create_minimal_ledger(ledger_dir: Path, proof: dict) -> None:
    ...
    pre_manifest = {"CAPABILITY/PRIMITIVES/_scratch": {"file.txt": "a" * 64}}
    (ledger_dir / "PRE_MANIFEST.json").write_text(json.dumps(pre_manifest, indent=2))

    post_manifest = {"CAPABILITY/PRIMITIVES/_scratch": {"file.txt": "a" * 64}}
    (ledger_dir / "POST_MANIFEST.json").write_text(json.dumps(post_manifest, indent=2))

    restore_diff = {"CAPABILITY/PRIMITIVES/_scratch": {"added": {}, "removed": {}, "changed": {}}}
...
def proof_schema_path():
    return Path(__file__).resolve().parents[2] / "LAW" / "SCHEMAS" / "proof.schema.json"
...
def test_missing_proof_rejected():
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        pre_manifest = {"CAPABILITY/PRIMITIVES/_scratch": {}}
        (ledger_dir / "PRE_MANIFEST.json").write_text(json.dumps(pre_manifest, indent=2))

        post_manifest = {"CAPABILITY/PRIMITIVES/_scratch": {}}
        (ledger_dir / "POST_MANIFEST.json").write_text(json.dumps(post_manifest, indent=2))
...
def test_proof_is_single_source_of_truth(proof_validator):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        proof = proof_validator.generate_proof(
            ...
            catalytic_domains=["CAPABILITY/PRIMITIVES/_scratch"],
            pre_state={"CAPABILITY/PRIMITIVES/_scratch": {"file.txt": "a" * 64}},
            post_state={"CAPABILITY/PRIMITIVES/_scratch": {"file.txt": "b" * 64}},  # Mismatch
            timestamp="2025-12-25T00:00:00Z",
        )
