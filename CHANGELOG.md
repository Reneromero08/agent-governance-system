...
- **Skill Compliance:** Brought `hermes-harness` `run.py` into ADR-017 compliance. Switched from `/v1/chat/completions` to `/v1/responses` with named `conversation`. Added `session_chat` transport via `POST /api/sessions/{id}/chat`. Added `--transport`, `--session-id` CLI args. Note: real Hermes `/goal` only dispatches via CLI/gateway, not API. The session_chat transport gives session-scoped turns but cannot invoke judge-model auto-continue.
