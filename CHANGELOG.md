...
- **Commit-and-base-bound verification:** Expanded `ci_local_gate.py` receipts to include the exact tested commit, immutable resolved base SHA, selected suites and risk groups, plan hash, mode, and timestamp. Added strict receipt validation, stable-`HEAD` enforcement, remote-base drift detection, and safe receipt reuse for unchanged network retries.
- **Risk-complete canonical planner:** Rebuilt `push_test_plan.py` around explicit risk-group ownership for write firewall, symbol resolution, MCP/capability contracts, skill discovery, cassette network, and real embeddings. Every normal full-gate exclusion now has a conditional owner and changed-path trigger. Added fail-closed base handling, merge-base/direct-diff unioning, initial-history handling, machine-independent plan hashes, and per-suite xdist control.
- **Faster clean-state gate:** Full verification now rejects dirty non-lab trees before expensive work, includes untracked files, preserves the `THOUGHT/` exemption, checks both sides of renames, safely restores known generated-index churn, prefers the repository virtualenv, and rejects commits that move during verification.
- **Per-suite CI evidence:** GitHub Actions now freezes one canonical plan, installs semantic dependencies only when needed, runs selected suites in individually named steps, retains per-suite logs, and reports the complete failing-suite set.
...
- **MCP drift guards:** Tool dispatch moved to `AGSMCPServer.tool_handlers` registry (`_build_tool_registry`). `verify_governance.py` rewritten as a CI-usable smoke test (was testing three tools removed long ago; exits non-zero on failure) and now enforces: tools.json definitions == registered handlers == README docs; canon_read enum == canon.json stems (bijection, each entry resolves); traversal rejection; context_review shape; clamp_limit bounds. New CI step `MCP governance smoke test` in `.github/workflows/contracts.yml`.
- **MCP limits + rotation:** New `clamp_limit()` in `CAPABILITY/MCP/primitives.py` caps client-supplied `limit`/`top_k` at `MAX_RESULTS_PER_PAGE` (applied at 8 sites across server.py and semantic_adapter.py). Audit log rotates at 5MB (`audit.jsonl` -> `audit.jsonl.1`; `AGS_AUDIT_LOG_MAX_BYTES` override).
- **canon_read enum synced:** `schemas/tools.json` enum expanded from 9 stale flat-layout names to all 31 `canon.json` stems; name regex widened to accept hyphenated stems (`SPECTRUM-02_RESUME_BUNDLE`, `CMP-01_...`). Dead `CHANGELOG` entry dropped (broken since the canon reorg; the project changelog lives at repo root).
...
- **Hermes-harness edge-case hardening (round 2):** CLI now returns a clean `{"error": ...}` + exit 1 instead of a raw Python traceback on bad input (unknown/busy worker, nothing to judge). `continue_worker` gained the same atomic busy-guard as `submit_task` (rejects a concurrent continue/submit instead of starting a second loop on the shared conversation). `judge()` now claims the worker atomically so two concurrent `judge(accept)` calls can't double-finalize / double-summarize. Added CLI clean-error, valid-example-JSON, and continue-busy-guard tests; live-verified the HTTP API serves the new schema (122 hermetic tests total).
- **Hermes-harness edge-case hardening:** Fixed concurrency corruption -- `save_worker`/`_save_task`/manifest writes are now atomic (temp + `os.replace`) with a bounded retry for the Windows "file held open" `PermissionError`, an in-process `_io_lock` (RLock) serializing controller reads/writes, and a resilient retry-reader; `list_workers` no longer silently drops a worker being concurrently written. Fixed a manager-judgment ACCEPT path that finalized an `execution_required` task without summarizing its execution-lane result back into the persistent conversation. Fixed a judge-mode `verify_command` rejection that wrongly referenced a `GOAL_COMPLETE` marker (judge mode emits none). Legacy `transport="runs"` worker records now coerce to the `responses` persistent lane instead of breaking. `get_state` now exposes `persistent_transport`/`execution_transport`/`session_id`/`last_response_id`/`last_run_id`/`session_key_present`. 118 hermetic tests (thread-exception warnings treated as errors); live-verified the execution-lane round-trip (a `/v1/runs` result is recalled on the persistent lane with no client-side history).
- **Hermes-harness persistent-sessions-primary correction (supersedes the same-day Worker API entry below):** Restored persistent worker session identity as the core of the architecture. Two lanes now: the **persistent reasoning lane** (default `persistent_transport="responses"` -> `/v1/responses` named `conversation` + `session_key`, server-side memory) is the canonical worker memory; `/v1/runs` is demoted to an **opt-in execution lane** (`execution_required=True`) whose result is summarized back into the persistent conversation via `record_execution_result_to_persistent_worker` -- runs are never the memory layer. The goal loop sends every turn to the same `conversation`+`session_key` (server-side continuity); client-side `conversation_history` is fallback/debug only, not canonical. Worker schema adds `persistent_transport`, `execution_transport`, `session_id` (session_chat only; distinct from `conversation`), `last_response_id`, `last_run_id`. Completion is now `completion_mode="marker"` (default) or `"judge"`; a configured-but-unavailable judge **fails fast** (status `error`) instead of silently fail-open burning the budget; judge-mode continuation no longer contradicts judge mode. "Write firewall" wording corrected to **postflight scope audit** (observe/optional-revert after the turn, NOT a pre-write block). 110 hermetic tests; **verified live** that Hermes preserves the named `/v1/responses` conversation server-side across separate tasks (cross-task token recall with no client-side history). `worker_id`/`conversation`/`session_key`/`session_id`/`run_id` documented as distinct (truth table in `WORKER_API.md`).
...
- **Hermes Port:** Switched default API port from 8642 (WSL) to 8643 (Windows native).
...
- **Hermes-harness fixtures:** `run.py` now checks `CI=true` and skips Hermes API call (returns `{"skipped": true}`). `hermes_harness.py` uses `GuardedWriter` for governance-compliant file I/O. `expected.json` fixtures updated for `run_task()` output format.
- **Wormhole CI skip:** `run.py` CI check moved to module level before `torch`/`numpy` imports; `CI=true` skips at process start, not only in `main()`. Removed duplicate CI check from `main()`.
...
- `THOUGHT/LAB/CAT_CAS/4_holographic/33_mera_compression/_ds_integrated.py`: Reference implementation of the integrated wormhole pipeline for DeepSeek V4 Flash experts. 5-stage pipeline: catalytic SVD extraction, rotation chain construction, boundary stress decomposition, phase cavity sieve, LoRA compression. Proves chain fidelity is invariant to rank (0.085 at r=1 equals 0.085 at r=124).
- `THOUGHT/LAB/CAT_CAS/4_holographic/33_mera_compression/_ds_wormhole.py`: Fast expert-0 wormhole compressor using representative anchor approach.
...
- `THOUGHT/LAB/CAT_CAS/4_holographic/33_mera_compression/7_modular_compress.py`: Added D_f block compression (B3) with re-anchoring on fidelity drop, LoRA rank parameter, anchors/blocks tracking in stats.
...
- **Preserved in git**: Our tests in `qgt_lib/python/` (test_q34_*.py, test_q43_*.py, etc.)
- Added to `.gitignore` - library can be restored locally for experiments if needed
- Note: History rewrite changed commit hashes; required force push
