...
## 0.2 Run artifact set (canonical) ✅ PARTIAL
Goal: Every run writes the same minimal artifact set to a stable location.

Location
- [x] CONTRACTS/_runs/<run_id>/ (existing runs use this)

Files (minimum set) - Generated by catalytic_runtime.py:
- [x] RUN_INFO.json (metadata: run_id, timestamp, intent, catalytic_domains, exit_code)
- [x] PRE_MANIFEST.json (domain -> {path -> sha256})
- [x] POST_MANIFEST.json (domain -> {path -> sha256})
- [x] RESTORE_DIFF.json (domain -> {added, removed, changed})
- [x] OUTPUTS.json (list of durable outputs with hashes)
- [x] STATUS.json (status, restoration_verified flags)
- [x] PROOF.json (restoration proof: verified flag, condition, mismatches)
- [ ] JOBSPEC.json (input jobspec - not yet auto-saved)
- [ ] VALIDATOR_ID.json (not yet auto-saved)

Exit Criteria
- [x] PROOF.json is generated and validates against proof.schema.json
- [x] Acceptance is proof-gated: run accepted iff PROOF.json.verified == true
