...
from MEMORY.LLM_PACKER.Engine import pack_hygiene
...
def test_repo_state_manifest_validation_and_dedup() -> None:
    ...
    repo_state = {"files": [a, b]}
    pack_hygiene.validate_repo_state_manifest(repo_state, allow_duplicate_hashes=False)

    with pytest.raises(ValueError, match=r"PACK_MANIFEST_INVALID:unsorted_files"):
        pack_hygiene.validate_repo_state_manifest({"files": [b, a]}, allow_duplicate_hashes=False)

    with pytest.raises(ValueError, match=r"PACK_DEDUP_DUPLICATE_PATH:a\.txt"):
        pack_hygiene.validate_repo_state_manifest({"files": [a, a]}, allow_duplicate_hashes=False)

    with pytest.raises(ValueError, match=r"PACK_DEDUP_DUPLICATE_HASH:"):
        pack_hygiene.validate_repo_state_manifest(
            {"files": [{"path": "a.txt", "hash": "f" * 64, "size": 1}, {"path": "b.txt", "hash": "f" * 64, "size": 1}]},
...
def test_verify_manifest_detects_tamper(tmp_path: Path) -> None:
    ...
    # Must be sorted by (path, hash) even for a single entry.
    pack_hygiene.validate_repo_state_manifest(repo_state, allow_duplicate_hashes=True)
    (pack_dir / "meta" / "REPO_STATE.json").write_text(pack_hygiene.canonical_json_bytes(repo_state).decode("utf-8") + "\n", encoding="utf-8")
