...
def test_pipeline_init_is_deterministic_and_resume_safe() -> None:
    ...
    base = REPO_ROOT / "CONTRACTS" / "_runs" / "_tmp" / "pipelines_test"
    pipeline_dir = REPO_ROOT / "CONTRACTS" / "_runs" / "_pipelines" / pipeline_id

    jobspec1_rel = "CONTRACTS/_runs/_tmp/pipelines_test/jobspec_step1.json"
    jobspec2_rel = "CONTRACTS/_runs/_tmp/pipelines_test/jobspec_step2.json"
    jobspec1 = REPO_ROOT / jobspec1_rel
    jobspec2 = REPO_ROOT / jobspec2_rel

    out1 = "CORTEX/_generated/_tmp/pipelines_step1_out.txt"
    out2 = "CORTEX/_generated/_tmp/pipelines_step2_out.txt"
    marker1 = "CONTRACTS/_runs/_tmp/pipelines_test/step1_marker.txt"
    ...
    try:
        _write_jobspec(jobspec1, job_id="pipe-step1", intent="step1", catalytic_domains=["CONTRACTS/_runs/_tmp/pipelines_test/domain"], durable_paths=[out1])
        _write_jobspec(jobspec2, job_id="pipe-step2", intent="step2", catalytic_domains=["CONTRACTS/_runs/_tmp/pipelines_test/domain"], durable_paths=[out2])
        ...
        for rid in [run_id_s1, run_id_s2]:
            rdir = REPO_ROOT / "CONTRACTS" / "_runs" / rid
