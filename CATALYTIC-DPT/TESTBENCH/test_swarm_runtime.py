...
def test_swarm_runtime_happy_path(tmp_path: Path) -> None:
    ...
    jobspec1_rel = f"CONTRACTS/_runs/_tmp/swarm_test/{p1}_jobspec.json"
    jobspec2_rel = f"CONTRACTS/_runs/_tmp/swarm_test/{p2}_jobspec.json"
    ...
    try:
        ...
        _rm(pipeline1_dir)
        _rm(pipeline2_dir)
        _rm(REPO_ROOT / "CONTRACTS" / "_runs" / "_tmp" / "swarm_test")
        _rm(REPO_ROOT / out1)
        _rm(REPO_ROOT / out2)

        _write_jobspec(REPO_ROOT / jobspec1_rel, job_id=f"{p1}-job", intent=p1, catalytic_domains=["CONTRACTS/_runs/_tmp/swarm_test/domain"], durable_paths=[out1])
        _write_jobspec(REPO_ROOT / jobspec2_rel, job_id=f"{p2}-job", intent=p2, catalytic_domains=["CONTRACTS/_runs/_tmp/swarm_test/domain"], durable_paths=[out2])
