...
def _setup_pipeline(tmp_root: Path, *, pipeline_id: str) -> tuple[Path, Path]:
    ...
    out1 = f"CORTEX/_generated/_tmp/{pipeline_id}_verify_step1_out.txt"
    out2 = f"CORTEX/_generated/_tmp/{pipeline_id}_verify_step2_out.txt"
...
def test_pipeline_verify_ok(tmp_path: Path) -> None:
    ...
    try:
        ...
        _rm(pipeline_dir)
        _rm(REPO_ROOT / "CONTRACTS" / "_runs" / "_tmp" / tmp_root)
        _rm(REPO_ROOT / "CONTRACTS" / "_runs" / f"pipeline-{pipeline_id}-s1-a1")
        _rm(REPO_ROOT / "CONTRACTS" / "_runs" / f"pipeline-{pipeline_id}-s2-a1")
...
def test_pipeline_verify_missing_artifact(tmp_path: Path) -> None:
    ...
    try:
        ...
        ledger = REPO_ROOT / "CONTRACTS" / "_runs" / run_id / "LEDGER.jsonl"
        _rm(ledger)
...
def test_pipeline_verify_chain_tamper(tmp_path: Path) -> None:
    ...
    try:
        ...
        state = json.loads((pipeline_dir / "STATE.json").read_text(encoding="utf-8"))
        run_id = state["step_run_ids"]["s1"]
        proof = REPO_ROOT / "CONTRACTS" / "_runs" / run_id / "PROOF.json"
