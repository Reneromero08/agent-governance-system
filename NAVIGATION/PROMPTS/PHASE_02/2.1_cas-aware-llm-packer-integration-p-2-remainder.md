...
phase: 2
task_id: "2.1"
slug: "cas-aware-llm-packer-integration-p-2-remainder"
...
depends_on: []
primary_model: "GPT-5.1-Codex-Max (Reasoning: Medium)"
fallback_chain: ["Claude Sonnet (Reasoning: High)", "Gemini Pro (Reasoning: High)"]
receipt_path: "LAW/CONTRACTS/_runs/_tmp/prompts/2.1_cas-aware-llm-packer-integration-p-2-remainder/receipt.json"
report_path: "LAW/CONTRACTS/_runs/_tmp/prompts/2.1_cas-aware-llm-packer-integration-p-2-remainder/REPORT.md"
...
- Primary model: GPT-5.1-Codex-Max (Reasoning: Medium)
- Fallback chain: use only if required to reach DONE = GREEN.

## GOAL
Execute task **2.1** (cas-aware-llm-packer-integration-p-2-remainder) exactly as specified, without scope drift.
...
```text
ROLE + MODEL
- Primary: GPT-5.1-Codex-Max (Thinking: Medium)
- Fallback chain: GPT-5.2-Codex -> Claude Sonnet 4.5 (non-thinking) -> Gemini 3 Pro (High)

GOAL
Implement **2.1 CAS-aware LLM Packer Integration (Z.2.6 + P.2 remainder)** by completing its unchecked checklist items from the roadmap. Do not start adjacent sections or “helpful extras”.
...
- Allowed writes (minimal and necessary only):
  ...
  - LAW/CONTRACTS/_runs/_tmp/prompts/2-1-cas-aware-llm-packer-integration-z-2-6-p-2-remainder/  (receipts + report only)
...
title = "2.1 CAS-aware LLM Packer Integration (Z.2.6 + P.2 remainder)"
start = None
...
- Identify the minimal touch set BEFORE editing:
  - `rg -n "2.1" -S .`
  - `rg -n "2.1" -S NAVIGATION/ROADMAPS/AGS_ROADMAP_MASTER_REPHASED_TODO_UPDATED.md`
...
- Receipt:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/2-1-cas-aware-llm-packer-integration-z-2-6-p-2-remainder/receipt.json
  ...
  - Include:
    ...
    - errors: ordered list
- Report:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/2-1-cas-aware-llm-packer-integration-z-2-6-p-2-remainder/REPORT.md
  - Must include:
...
- receipt_path: LAW/CONTRACTS/_runs/_tmp/prompts/2.1_cas-aware-llm-packer-integration-z-2-6-p-2-remainder/receipt.json
- report_path: LAW/CONTRACTS/_runs/_tmp/prompts/2.1_cas-aware-llm-packer-integration-z-2-6-p-2-remainder/REPORT.md
