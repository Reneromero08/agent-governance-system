...
phase: 3
task_id: "3.1"
slug: "router-fallback-stability"
...
depends_on: []
primary_model: "Claude Sonnet 4.5 (non-thinking) (Reasoning: Medium)"
fallback_chain: ["Claude Sonnet (Reasoning: High)", "Gemini Pro (Reasoning: High)"]
receipt_path: "LAW/CONTRACTS/_runs/_tmp/prompts/3.1_router-fallback-stability/receipt.json"
report_path: "LAW/CONTRACTS/_runs/_tmp/prompts/3.1_router-fallback-stability/REPORT.md"
...
- Primary model: Claude Sonnet 4.5 (non-thinking) (Reasoning: Medium)
- Fallback chain: use only if required to reach DONE = GREEN.

## GOAL
Execute task **3.1** (router-fallback-stability) exactly as specified, without scope drift.

Source prompt body (carried forward, normalized):

---
Model: Claude Sonnet 4.5 (non-thinking)
Thinking: Medium

```text
ROLE + MODEL
- Primary: Claude Sonnet 4.5 (non-thinking) (Thinking: Medium)
- Fallback chain: GPT-5.2-Codex -> Gemini 3 Pro (Low) -> Claude Opus 4.5 (Thinking)

GOAL
Implement **3.1 Router & Fallback Stability (Z.3.1)** by completing its unchecked checklist items from the roadmap. Do not start adjacent sections or “helpful extras”.
...
- Allowed writes (minimal and necessary only):
  ...
  - LAW/CONTRACTS/_runs/_tmp/prompts/3-1-router-fallback-stability-z-3-1/  (receipts + report only)
...
title = "3.1 Router & Fallback Stability (Z.3.1)"
start = None
...
- Identify the minimal touch set BEFORE editing:
  - `rg -n "3.1" -S .`
  - `rg -n "3.1" -S NAVIGATION/ROADMAPS/AGS_ROADMAP_MASTER_REPHASED_TODO_UPDATED.md`
...
- Always run at least:
  - `pytest -q`
- Run router/skill execution tests:
  - `pytest -q CAPABILITY/TESTBENCH/core -q` (if present)
  - plus any tests you touch/add under `CAPABILITY/TESTBENCH/`.

ARTIFACTS (RECEIPTS + REPORT)
- Receipt:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/3-1-router-fallback-stability-z-3-1/receipt.json
  ...
  - Include:
    ...
    - errors: ordered list
- Report:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/3-1-router-fallback-stability-z-3-1/REPORT.md
  - Must include:
...
- receipt_path: LAW/CONTRACTS/_runs/_tmp/prompts/3.1_router-fallback-stability-z-3-1/receipt.json
- report_path: LAW/CONTRACTS/_runs/_tmp/prompts/3.1_router-fallback-stability-z-3-1/REPORT.md
