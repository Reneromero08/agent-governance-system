...
receipt_path: "LAW/CONTRACTS/_runs/_tmp/prompts/7.2_logging-infrastructure/receipt.json"
report_path: "LAW/CONTRACTS/_runs/_tmp/prompts/7.2_logging-infrastructure/REPORT.md"
...
## GOAL
Execute task **7.2** (logging-infrastructure) exactly as specified, without scope drift.
...
GOAL
Implement **7.2 Logging Infrastructure (E.1) (P0)** by completing its unchecked checklist items from the roadmap. Do not start adjacent sections or “helpful extras”.
...
- Allowed writes (minimal and necessary only):
  ...
  - LAW/CONTRACTS/_runs/_tmp/prompts/7-2-logging-infrastructure-e-1-p0/  (receipts + report only)
...
title = "7.2 Logging Infrastructure (E.1) (P0)"
start = None
...
- Identify the minimal touch set BEFORE editing:
  - `rg -n "7.2" -S .`
  - `rg -n "7.2" -S NAVIGATION/ROADMAPS/AGS_ROADMAP_MASTER_REPHASED_TODO_UPDATED.md`
...
- Always run at least:
  - `pytest -q`
- Run the smallest relevant test subset first, then full suite:
  - `pytest -q`

ARTIFACTS (RECEIPTS + REPORT)
- Receipt:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/7-2-logging-infrastructure-e-1-p0/receipt.json
  ...
  - Include:
    ...
    - errors: ordered list
- Report:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/7-2-logging-infrastructure-e-1-p0/REPORT.md
  - Must include:
...
- receipt_path: LAW/CONTRACTS/_runs/_tmp/prompts/7.2_logging-infrastructure-e-1-p0/receipt.json
- report_path: LAW/CONTRACTS/_runs/_tmp/prompts/7.2_logging-infrastructure-e-1-p0/REPORT.md
