...
receipt_path: "LAW/CONTRACTS/_runs/_tmp/prompts/7.3_elo-engine/receipt.json"
report_path: "LAW/CONTRACTS/_runs/_tmp/prompts/7.3_elo-engine/REPORT.md"
...
## GOAL
Execute task **7.3** (elo-engine) exactly as specified, without scope drift.
...
GOAL
Implement **7.3 ELO Engine (E.2)** by completing its unchecked checklist items from the roadmap. Do not start adjacent sections or “helpful extras”.
...
- Allowed writes (minimal and necessary only):
  ...
  - LAW/CONTRACTS/_runs/_tmp/prompts/7-3-elo-engine-e-2/  (receipts + report only)
...
- Identify the minimal touch set BEFORE editing:
  - `rg -n "7.3" -S .`
  - `rg -n "7.3" -S NAVIGATION/ROADMAPS/AGS_ROADMAP_MASTER_REPHASED_TODO_UPDATED.md`
...
- Receipt:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/7-3-elo-engine-e-2/receipt.json
  ...
  - Include:
    ...
    - errors: ordered list
- Report:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/7-3-elo-engine-e-2/REPORT.md
  - Must include:
...
- receipt_path: LAW/CONTRACTS/_runs/_tmp/prompts/7.3_elo-engine-e-2/receipt.json
- report_path: LAW/CONTRACTS/_runs/_tmp/prompts/7.3_elo-engine-e-2/REPORT.md
