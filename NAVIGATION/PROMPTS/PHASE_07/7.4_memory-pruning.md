...
receipt_path: "LAW/CONTRACTS/_runs/_tmp/prompts/7.4_memory-pruning/receipt.json"
report_path: "LAW/CONTRACTS/_runs/_tmp/prompts/7.4_memory-pruning/REPORT.md"
...
## GOAL
Execute task **7.4** (memory-pruning) exactly as specified, without scope drift.
...
GOAL
Implement **7.4 Memory Pruning (E.3)** by completing its unchecked checklist items from the roadmap. Do not start adjacent sections or “helpful extras”.
...
- Allowed writes (minimal and necessary only):
  ...
  - LAW/CONTRACTS/_runs/_tmp/prompts/7-4-memory-pruning-e-3/  (receipts + report only)
...
- Identify the minimal touch set BEFORE editing:
  - `rg -n "7.4" -S .`
  - `rg -n "7.4" -S NAVIGATION/ROADMAPS/AGS_ROADMAP_MASTER_REPHASED_TODO_UPDATED.md`
...
- Receipt:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/7-4-memory-pruning-e-3/receipt.json
  ...
  - Include:
    ...
    - errors: ordered list
- Report:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/7-4-memory-pruning-e-3/REPORT.md
  - Must include:
...
- receipt_path: LAW/CONTRACTS/_runs/_tmp/prompts/7.4_memory-pruning-e-3/receipt.json
- report_path: LAW/CONTRACTS/_runs/_tmp/prompts/7.4_memory-pruning-e-3/REPORT.md
