...
phase: 6
task_id: 6.3
slug: cross-cassette-queries-m-3
legacy_id: P06_6-3
...
receipt_path: LAW/CONTRACTS/_runs/_tmp/prompts/6.3_cross-cassette-queries-m-3/receipt.json
report_path: LAW/CONTRACTS/_runs/_tmp/prompts/6.3_cross-cassette-queries-m-3/REPORT.md
...
## GOAL
Execute task **6.3** (cross-cassette-queries-m-3) exactly as specified, without scope drift.
...
- Allowed writes (minimal and necessary only):
  ...
  - LAW/CONTRACTS/_runs/_tmp/prompts/6-3-cross-cassette-queries-m-3/  (receipts + report only)
...
title = "6.3 Cross-Cassette Queries (M.3)"
start = None
...
- Identify the minimal touch set BEFORE editing:
  - `rg -n "6.3" -S .`
  - `rg -n "6.3" -S NAVIGATION/ROADMAPS/AGS_ROADMAP_MASTER_REPHASED_TODO_UPDATED.md`
...
- Receipt:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/6-3-cross-cassette-queries-m-3/receipt.json
  ...
  - Include:
    ...
    - errors: ordered list
- Report:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/6-3-cross-cassette-queries-m-3/REPORT.md
  - Must include:
