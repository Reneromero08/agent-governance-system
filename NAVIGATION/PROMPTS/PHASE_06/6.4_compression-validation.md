...
receipt_path: "LAW/CONTRACTS/_runs/_tmp/prompts/6.4_compression-validation/receipt.json"
report_path: "LAW/CONTRACTS/_runs/_tmp/prompts/6.4_compression-validation/REPORT.md"
...
## GOAL
Execute task **6.4** (compression-validation) exactly as specified, without scope drift.
...
GOAL
Implement **6.4 Compression Validation (M.4)** by completing its unchecked checklist items from the roadmap. Do not start adjacent sections or “helpful extras”.
...
- Allowed writes (minimal and necessary only):
  ...
  - LAW/CONTRACTS/_runs/_tmp/prompts/6.4_compression-validation/  (receipts + report only)
  - NAVIGATION/PROOFS/COMPRESSION/COMPRESSION_PROOF_REPORT.md (optional: publish proof)
  - NAVIGATION/PROOFS/COMPRESSION/COMPRESSION_PROOF_DATA.json (optional: publish proof)
...
- Token measurement availability (no estimates):
  - Verify `CAPABILITY/TOOLS/utilities/tokenizer_harness.py` exists
  - Verify `tiktoken` imports successfully in the active environment
  - Record: tokenizer version + encoding name used for all measurements
- Semantic retrieval readiness (if semantic retrieval is part of “compressed context”):
  - Verify `NAVIGATION/CORTEX/db/system1.db` exists
  - Verify DB has table `section_vectors` AND row count > 0
  - Record: embedding model id / dimensions (from DB metadata if available)

ARTIFACTS (RECEIPTS + REPORT)
- Receipt:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/6.4_compression-validation/receipt.json
  ...
  - Include:
    ...
    - errors: ordered list
- Report:
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/6.4_compression-validation/REPORT.md
  - Must include:
...
- Data (required for math audit):
  - Path: LAW/CONTRACTS/_runs/_tmp/prompts/6.4_compression-validation/DATA.json
  - Must include: baselines, per-benchmark token counts (baseline vs compressed), tokenizer/encoding/version,
    retrieval params (top_k/thresholds), retrieved hashes/pointers, formulas used, and commands run.
...
- [ ] 6.4.5 Define token measurement (tokenizer+encoding+version) and use it for all counts (M.4.5)
- [ ] 6.4.6 Baseline corpus is explicit (paths + integrity anchors) (M.4.6)
- [ ] 6.4.7 Compressed context definition includes retrieval method + params + retrieved hashes (M.4.7)
- [ ] 6.4.8 Proof bundle emitted (REPORT.md + DATA.json) (M.4.8)
