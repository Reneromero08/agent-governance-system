<!-- CONTENT_HASH: 7fede0be6a13b91c0174a041cba77197f2580868874556ac4d62983848e3ae18 -->
...
3. **Query cortex** - Instead of scanning files, agents query the shadow index (`NAVIGATION/CORTEX/_generated/cortex.db`) via MCP tools or `TOOLS/cortex.py`. The cortex returns structured metadata about pages, assets and tokens.
4. **Select entrypoint** - Using the maps, agents determine which files or skills need to change to achieve the goal.
5. **Execute skill** - The agent invokes a skill (a script under `CAPABILITY/SKILLS/`) to perform the action. Skills operate on data provided by the cortex and abide by the canon constraints.
...
1. **Load pipeline definition** - Read DAG from `CAPABILITY/PIPELINES/` specifying nodes and dependencies.
2. **Validate against schemas** - Pipeline structures are validated against `LAW/SCHEMAS/`.
3. **Schedule nodes** - The DAG scheduler determines execution order based on dependencies.
4. **Execute primitives** - Core operations from `CAPABILITY/PRIMITIVES/` are invoked.
