...
try:
    ...
    ollama = None
    # We don't exit here to allow running --help or other non-inference commands
...
def main():
    ...
    parser.add_argument(
        "pos_args",
        nargs='*',
        help="Question to ask OR fixture input/output files"
    )
    ...
    # Check for Fixture Mode (2 args, second ends in .json)
    if len(args.pos_args) == 2 and args.pos_args[1].endswith('.json'):
        input_path = args.pos_args[0]
        output_path = args.pos_args[1]
        
        # In fixture mode, we just need to verify the script runs.
        # Write "valid" to output if we can.
        try:
    ...
    if args.pos_args:
        # Treat all positional args as the question (join them)
        question = " ".join(args.pos_args)
        response = cli.ask(
            question,
            file_path=args.file,
