...
- ADR-001: Button Colors → Blue (#4A90E2)
- ADR-002: Navigation → Flat, max 2 levels
- ADR-003: API Versioning → Semantic, URL-based
...
### Overdue Review
- ADR-004: Review was 2025-11-15 ⚠️

## Supersession Chain

```
ADR-050 → ADR-045 → ADR-023 (current)
ADR-012 → ADR-018 (current)
```
```
...
def generate_pack():
    ...
    print(f"✓ Generated pack: {manifest['pack_version']}")
...
- ✓ Update CHANGELOG.md for every behavior change
- ✓ Create ADR before implementing significant changes
- ✓ Add fixtures before implementing new features
- ✓ Use the pre-commit hook to catch violations early
- ✓ Query CONTEXT when restarting work: `python CONTEXT/query-context.py --review-due 30`
- ✓ Document rejected approaches (prevents rearguing)
- ✓ Version your Skills explicitly
- ✓ Generate memory packs before long breaks
- ✓ Keep canon files under 10KB each (split if needed)
- ✓ Set review dates on ADRs (3-12 months typical)
...
- ✗ Change behavior without updating canon
- ✗ Skip fixture validation
- ✗ Hardcode paths (use config files)
- ✗ Mix canon and implementation in same file
- ✗ Let secondary docs become authoritative
...
def main():
    ...
    if args.superseded:
        ...
        for chain in results:
            ...
            if chain['supersedes']:
                print(f"  ← Supersedes: {chain['supersedes']}")
            if chain['superseded_by']:
                print(f"  → Superseded by: {chain['superseded_by']}")
            print()
    else:
...
def main():
    ...
    if results['overdue']:
        print(f"\n⚠️  {len(results['overdue'])} OVERDUE REVIEW(S):\n")
    ...
    if results['upcoming']:
        print(f"\n📅 {len(results['upcoming'])} UPCOMING REVIEW(S):\n")
    ...
    if results['no_review']:
        print(f"\n⚠️  {len(results['no_review'])} RECORD(S) WITH NO REVIEW DATE:\n")
        for record in results['no_review']:
            print(f"  {record.id}: {record.title}")
        print()
    
    if not any(results.values()):
        print("✓ All reviews current")
...
**Do:**
- ✓ Create ADR *before* implementing significant changes
- ✓ Update INDEX.md when adding new records
- ✓ Set realistic review dates (3-12 months)
- ✓ Document *why* something was rejected (prevents rearguing)
- ✓ Link to evidence (test results, benchmarks, user feedback)
- ✓ Close OPEN questions once decided (convert to ADR)

**Don't:**
- ✗ Skip context docs for "obvious" decisions (they're never obvious later)
- ✗ Copy-paste ADRs without customizing
- ✗ Let review dates slip (use the review tool)
- ✗ Delete rejected approaches (learning compounds)
- ✗ Make context docs authoritative over canon (context informs, canon governs)
...
2. Research & Discussion
   ↓
3. Decision Made → ADR Created
   ↓
4. Alternative Options → REJECT Records Created
   ↓
5. Implementation → Linked to Canon/Fixtures
   ↓
...
- Does CONTEXT have a decision about this?
  → python CONTEXT/query-context.py "[topic]"
...
1. What system am I working on?
   → Read CANON/CONTRACT.md introduction
2. What's the current focus?
   → Read ROADMAP.md
3. What decisions are relevant?
   → python CONTEXT/query-context.py --tag [current-area]
4. What am I allowed to change?
   → Read MAPS/ENTRYPOINTS.md"
```
