...
function isBehaviorChange(p) {
    ...
    if (p.startsWith("TOOLS/") && !p.endsWith(".md")) return true;

    // PRIMITIVES folder (core runtime)
    if (p.startsWith("CATALYTIC-DPT/PRIMITIVES/") && !p.endsWith(".md")) return true;
    if (p.startsWith("CATALYTIC-DPT/PIPELINES/") && !p.endsWith(".md")) return true;

    // SKILLS folder (agent capabilities)
    if (p.startsWith("SKILLS/") && !p.endsWith(".md")) return true;
...
function isRuleChange(p) {
    ...
    if (p.startsWith("CANON/") && p.endsWith(".md")) {
        // Exclude planning and archive
        if (p.startsWith("CANON/planning/")) return false;
        if (p.startsWith("CANON/archive/")) return false;
        return true;
    }

    // SPECTRUM specifications (frozen law)
    if (p.startsWith("CATALYTIC-DPT/SPECTRUM/") && p.endsWith(".md")) return true;

    // Schemas (contract definitions)
    if (p.startsWith("CATALYTIC-DPT/SCHEMAS/") && p.endsWith(".json")) return true;
...
function isDecisionChange(p) {
    ...
    if (p.startsWith("CONTEXT/decisions/") && p.endsWith(".md")) return true;
...
function isPlanningChange(p) {
    ...
    if (p.startsWith("CANON/planning/")) return true;
    if (p.startsWith("CONTEXT/planning/")) return true;
...
function isCatDptChangelogUpdate(p) {
    return p === "CATALYTIC-DPT/CHANGELOG.md";
}
