...
class MCPTerminalServer:
    ...
    def _verify_post_run_outputs(self, run_id: str) -> Dict:
        ...
        for idx, raw_path in enumerate(durable_paths):
            ...
            # Path.is_absolute() does not treat Windows drive paths as absolute on Linux/WSL,
            # but CMP-01 must reject them consistently.
            is_windows_abs = bool(re.match(r"^[A-Za-z]:[\\\\/]", str(raw_path)))
            if Path(raw_path).is_absolute() or is_windows_abs:
                errors.append({
                    "code": "PATH_ESCAPES_REPO_ROOT",
                    "message": f"Absolute paths are not allowed: {raw_path}",
                    "path": json_pointer,
