...
def squared_distance_matrix(embeddings: np.ndarray) -> np.ndarray:
    ...
    Returns:
        (n, n) squared distance matrix D² where D²[i,j] = ||e_i - e_j||²
    """
    # For normalized embeddings: ||a - b||² = 2(1 - a·b)
...
def classical_mds(
    ...
    The key formula:
        ...
        B = V Λ V^T            (eigendecomposition)
        X = V √Λ               (MDS coordinates)
...
def stress(D_original: np.ndarray, D_reconstructed: np.ndarray) -> float:
    ...
    Formula: sqrt(Σ(d_ij - d̂_ij)² / Σd_ij²)
