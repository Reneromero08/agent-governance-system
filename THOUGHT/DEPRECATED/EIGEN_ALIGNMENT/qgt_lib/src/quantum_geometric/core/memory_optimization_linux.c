...
// Memory block header with guard pages (for internal use)
typedef struct MemoryBlockInternal {
    size_t size;
    bool is_free;
    struct MemoryBlockInternal* next;
    struct MemoryBlockInternal* prev;
...
// Internal memory pool structure with monitoring
typedef struct {
    void* base_address;
    size_t total_size;
    size_t used_size;
...
struct MemoryManager {
    InternalMemoryPool pools[MAX_MEMORY_POOLS];
    size_t num_pools;
    bool numa_enabled;
    size_t total_memory;
    size_t peak_memory;
...
static void update_memory_performance_metrics_internal(MemoryManager* manager);
...
size_t get_cache_line_size(CacheLevel level) {
    // On Linux, cache line size is typically uniform across levels
    // Could be enhanced to query actual cache sizes via sysfs
...
// Update performance metrics (internal version)
static void update_memory_performance_metrics_internal(MemoryManager* manager) {
...
static bool init_memory_pool(InternalMemoryPool* pool,
                           size_t size,
                           int numa_node) {
...
void* optimized_malloc(MemoryManager* manager, size_t size) {
    ...
    if (!pool) {
        ...
        update_memory_performance_metrics_internal(manager);
...
const memory_stats_t* get_memory_stats(const MemoryManager* manager) {
    ...
    for (size_t i = 0; i < manager->num_pools; i++) {
        const InternalMemoryPool* pool = &manager->pools[i];
...
void cleanup_memory_manager(MemoryManager* manager) {
    ...
    for (size_t i = 0; i < manager->num_pools; i++) {
        InternalMemoryPool* pool = &manager->pools[i];
