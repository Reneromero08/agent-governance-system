...
// Optional curl support for downloading datasets
#ifdef HAVE_CURL
#include <curl/curl.h>
#endif

// Optional zlib support for decompression
#ifdef HAVE_ZLIB
#include <zlib.h>
#endif
...
static memory_buffer_t* download_file(const char* url) {
    fprintf(stderr, "Error: Dataset downloading not available. libcurl not installed.\n");
    fprintf(stderr, "Please manually download the dataset from: %s\n", url);
    return NULL;
}
#endif
...
#else
// Stub implementation when zlib is not available
static memory_buffer_t* decompress_gzip(const memory_buffer_t* compressed) {
    fprintf(stderr, "Error: GZIP decompression not available. zlib not installed.\n");
    return NULL;
}
#endif
