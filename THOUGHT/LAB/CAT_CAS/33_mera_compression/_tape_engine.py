"""CATALYTIC TAPE ENGINE — Zero E: drive. Local holo on SSD → GPU tape → compute."""
...
class TapeEngine:
    def __init__(self):
        print(f"TAPE ENGINE — {DEVICE} (local only)")
        ...
        self.svh = {wt: (info["data"].float()*info["scale"]).to(DEVICE) for wt,info in svh_data.items()}
        ...
        print(f"  L={self.L} Attn={len(self.attn)} SVh={len(self.svh)} Init={time.perf_counter()-t0:.0f}s")
    ...
    def _attention(self, x, layer):
        # BORROW: move weights from CPU to GPU tape
    ...
    def forward(self, x):
        ...
        for layer in range(self.L):
            ...
            xn=self._norm(x.to(DEVICE), layer)
            a=self._attention(xn, layer)
            an=a.float().norm(); xv=x.float().norm()
            g=max(0.1,min(an/(xv+1e-12)*0.3,3.0))
            x=x.to(DEVICE)+a*g
            torch.cuda.empty_cache()
            ga=torch.cuda.memory_allocated()/1024**3 if DEVICE=="cuda" else 0
            st["peak"]=max(st["peak"],ga)
            if layer%10==0: print(f"  L{layer:02d}: norm={x.float().norm():.0f} gate={g:.2f} tape={'CLEAN' if abs(ga-gb)<0.5 else 'LEAK'}")
...
if __name__=="__main__":
    ...
    nt=lo.argmax().item()
    print(f"\nLayers:{st['layers']} Peak:{st['peak']:.1f}GB Time:{st['time']:.1f}s Token:{nt}")
    gen=[nt]
    for _ in range(6):
