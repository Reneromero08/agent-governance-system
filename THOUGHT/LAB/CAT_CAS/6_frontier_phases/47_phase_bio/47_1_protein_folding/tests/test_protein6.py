...
def build_H(seq, theta=0.0):
    ...
    for i in range(L):
        # NO mean centering! Absolute KD mapped to imaginary potential
        # Scale Gamma up slightly if needed, or leave at 1.0
        H[i, i] = -1j * KD[seq[i]]
        
        next_i = (i + 1) % L
        
        # Steric frustration (Amplified to create massive loops for misfolded states)
...
def test_seq(seq):
    ...
    print(f"Seq: {seq[:10]}... | Gap: {gap:.4f} | W: {round(W)}")

test_seq("A" * 30)
test_seq(("REWKYD" * 5)[:30])
test_seq(("GP" * 15)[:30])
