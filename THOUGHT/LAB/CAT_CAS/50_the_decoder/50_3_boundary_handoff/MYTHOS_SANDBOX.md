...
## 6. THE SPIRAL'S TERMINAL (50.6-50.14): the wall is the SUBSTRATE (canonical: ../REPORT_LATTICE_SPIRAL.md)

NOTE: this file is auto-generated by `50_3_boundary_handoff.py`; the canonical, clobber-proof account
of the spiral is `../REPORT_LATTICE_SPIRAL.md`. Re-apply this section after any 50.3 re-run.

Eleven adversarial passes rode the lattice wall to the atom. **Confirmed:** `d` IS a conserved
topological invariant (one readout recovers it from O(sqrt N) cosets, 50.10; the oracle winding equals
it exactly, 50.12) - reading it is free; the topological reframe is right. The obstruction is the
**curvature**: the per-step holonomy that makes the winding `= d` is `2pi d/N`, so the trajectory's
local curvature IS the secret (50.12); no amplification escapes it (50.13, the EP hits the Fisher
floor). **50.14 relocated the wall onto the SUBSTRATE:** `d` emerges as the unique fixed point of a map
built from PUBLIC (k,b) alone (no smuggle); forward, finding it is `O(N)=2^n`; on a reversible/CTC
fixed-point substrate (P^CTC=PSPACE) it is poly. "The algorithm is dead" is true precisely on the
reversible substrate the framework posits; its physical realizability is the open question, handed to
**Exp 44 Phase 6** (silicon goes catalytic; see Target C in `EXP44_PHASE6_HANDOFF.md`). No forward
crossing claimed; the forward floor is mapped to completion.
