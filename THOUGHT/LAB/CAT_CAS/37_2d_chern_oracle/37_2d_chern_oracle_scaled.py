...
def spectral_projector(H, E_fermi=-0.5j, n_pts=32, radius=2.0):
    ...
    Discrete approximation:  z_k = E_fermi + radius * e^{i*theta_k}
      P = (1/n_pts) * sum_k  (z_k*I - H)^{-1} * radius * e^{i*theta_k}

    Returns P_occ — projector onto states with eigenvalues inside C.
    """
    N = H.shape[0]
...
def scale_sweep():
    ...
    Uses FIXED Fermi energy from L=8 calibration to avoid the auto-gap
    detection edge case at L=10."""
    print(f"\n{'=' * 78}")
    ...
    for L in [4, 6, 8, 10, 12, 14, 16]:
        ...
        # Auto-detect Fermi energy per L
        im_s = torch.sort(torch.linalg.eigvals(H_loop).imag).values
        gaps = im_s[1:] - im_s[:-1]
        Ef = complex(0, (im_s[gaps.argmax()] + im_s[gaps.argmax()+1]).item()/2)
        n_pts = 32 if N <= 100 else 48

        t0 = time.time()
        P_loop = spectral_projector(H_loop, E_fermi=Ef, n_pts=n_pts)
        proj_time = time.time() - t0
        C_loop = bott_index(P_loop, L)

        P_halt = spectral_projector(H_halt, E_fermi=Ef, n_pts=n_pts)
        C_halt = bott_index(P_halt, L)
    ...
    print(f"  {'='*78}")
    print(f"  Auto-detected Fermi energy per lattice size")
    print(f"  L=6,8 confirmed.  Larger L needs higher Gamma for edge destruction.")
    print(f"  {'='*78}")

    # Gamma sweep: at what sink strength does the edge collapse?
    print(f"\n{'=' * 78}")
    print("  GAMMA SWEEP — C_halt vs EP sink strength (L=8)")
    print(f"{'=' * 78}")
    print(f"  {'Gamma':>8s}  {'C_halt':>7s}  {'Im(E)_min':>10s}  {'Verdict'}")
    print("  " + "-" * 45)

    H8_loop = build_2d_hamiltonian(8, gamma_halt=0.0)

    for g in [0.0, 0.5, 1.0, 2.0, 3.0, 5.0, 10.0, 20.0]:
        Hh = build_2d_hamiltonian(8, gamma_halt=g)
        # Auto-detect Fermi for each gamma (spectrum shifts with Gamma)
        im_sg = torch.sort(torch.linalg.eigvals(Hh).imag).values
        gaps_g = im_sg[1:] - im_sg[:-1]
        Efg = complex(0, max(
            float((im_sg[gaps_g.argmax()] + im_sg[gaps_g.argmax()+1])/2), -0.1))
        Ph = spectral_projector(Hh, E_fermi=Efg, n_pts=48)
        Ch = bott_index(Ph, 8)
        im_min = float(im_sg.min())
        verdict = "LOOPS (edge survives)" if Ch != 0 else "HALTS (edge destroyed)"
        print(f"  {g:8.1f}  {Ch:+7d}  {im_min:10.4f}  E={Efg.imag:.3f}j  {verdict}")

    print(f"  {'='*78}")

    # Gamma sweep at L=12: find edge destruction threshold
    print(f"\n{'=' * 78}")
    print("  GAMMA SWEEP — C_halt vs sink strength (L=12)")
    print(f"{'=' * 78}")
    print(f"  {'Gamma':>8s}  {'C_halt':>7s}  {'Verdict'}")
    print("  " + "-" * 35)
    for g in [0, 5, 10, 20, 30, 50]:
        Hh = build_2d_hamiltonian(12, gamma_halt=g)
        im_sg = torch.sort(torch.linalg.eigvals(Hh).imag).values
        gaps_g = im_sg[1:] - im_sg[:-1]
        Efg = complex(0, max(float((im_sg[gaps_g.argmax()]+im_sg[gaps_g.argmax()+1])/2),-0.1))
        Ph = spectral_projector(Hh, E_fermi=Efg, n_pts=48)
        Ch = bott_index(Ph, 12)
        v = "LOOPS" if Ch != 0 else "HALTS"
        print(f"  {g:8.1f}  {Ch:+7d}  {v}")
    print(f"  {'='*78}")
