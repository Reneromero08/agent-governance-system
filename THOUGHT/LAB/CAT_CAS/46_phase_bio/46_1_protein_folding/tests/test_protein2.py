...
def build_H(seq, theta=0.0):
    ...
    for i in range(L):
        # On-site potential: -i * Gamma * Hydrophobicity
        H[i, i] = -1j * KD[seq[i]]
        
        # Hopping
        next_i = (i + 1) % L
        
        # Complex phase based on steric bulk difference
        phi = (BULK[seq[i]] - BULK[seq[next_i]]) / 100.0 * np.pi
        
        t_fwd = 1.0 + (BULK[seq[i]] / 200.0)
        t_bwd = 1.0 - (BULK[seq[next_i]] / 200.0)
