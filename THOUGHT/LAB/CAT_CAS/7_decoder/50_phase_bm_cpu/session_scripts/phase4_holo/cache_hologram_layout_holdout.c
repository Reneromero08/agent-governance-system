...
static void apply_schedule(tape_t *t, int layout, int family, int declared_mode, int actual_mode,
                           int trial, const int canonical_lines[TOUCHES]) {
    uint64_t mask = 0x4C41594F55544C48ULL ^ (uint64_t)layout ^ ((uint64_t)family << 5) ^
...
int main(void) {
    ...
    printf("layout,family,declared_mode,actual_mode,trial,hash_restored");
        ...
        for (int layout = 0; layout < LAYOUTS; layout++) {
            for (int family = 0; family < FAMILIES; family++) {
                for (int declared = 0; declared < MODES; declared++) {
                    ...
                    init_tape(&t, 0x4404B000C0000000ULL ^ (uint64_t)trial ^ ((uint64_t)layout << 52));
                    uint64_t h0 = tape_hash(&t);
                    flush_tape(&t);

                    apply_schedule(&t, layout, family, declared, actual, trial, canonical_lines);

                    double samples[LINES];
                    for (int probe = 0; probe < LINES; probe++) {
                        int physical = (probe * 29 + trial * 13 + layout * 11 + family * 7 + declared * 3) &
                                       (LINES - 1);
                    ...
                    printf("%d,%s,%s,%s,%d,%d", layout, family_name(family), mode_name(declared),
    ...
    fprintf(stderr, "PHASE4B_CACHE_HOLOGRAM_LAYOUT_HOLDOUT restored=%d/%d sink=%llu\n",
