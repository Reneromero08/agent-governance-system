#!/usr/bin/env python3
"""Standalone M226 direct repeated public-unit action reference."""
...
def public_unit_descriptor() -> dict[str, int]:
    field_values = tuple(
        value
        for unit in m222.UNITS
        for value in (unit.value, unit.reciprocal, unit.norm, unit.reciprocal_norm)
    )
    parameters = tuple(prior.m220.PARAMETERS)
    return {
        "field_cells": len(field_values),
        "field_payload_bits": sum(prior.scalar_payload(value) for value in field_values),
        "parameter_integer_cells": len(parameters),
        "parameter_payload_bits": sum(prior.signed_bits(value) for value in parameters),
    }
...
def direct_action_controls() -> dict[str, object]:
    residual = [base.ONE]
    ledger = [0] * len(m222.UNITS)
    scratch = [base.ZERO]

    def run(value: base.E, exponent: int) -> tuple[base.E, Work]:
        local_work = Work()
    ...
    unit = m222.UNITS[0].value
    expected = (unit * unit) * unit
    correct, correct_work = run(unit, 3)
    wrong_base, _ = run(m222.UNITS[1].value, 3)
    wrong_sign, _ = run(m222.UNITS[0].reciprocal, 3)
    ...
    return {
        ...
        "zero_exponent_is_identity": zero == base.ONE,
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "schema": "cat_cas.su2_level8_cubic_skein_direct_unit_action_reference.v1",
                ...
                "reuse": prior.reuse(),
                "direct_action_controls": direct_action_controls(),
            },
            indent=2,
            sort_keys=True,
        )
