...
#define RELATION_SPATIAL_CANONICAL_SCHEDULE_SHA256 "5378ae183daec17a2ad5c6b9cecb954c1d7de4bf834dfa8e02d5188da5b159cf"
...
static int execute_schedule_common(const char *schedule_path, const char *output_root, int synthetic) {
    ...
    while (fgets(line, sizeof(line), schedule) != NULL) {
        ...
        for (sample = 0u; sample < FAMILY10H_RELATION_SPATIAL_PAIR_SAMPLE_COUNT; ++sample) {
            ...
            if (fprintf(
                    pairs,
                    "{\"tuple_id\":\"%s\",\"execution_ordinal\":%d,\"block_id\":\"%s\",\"block_local_position\":%d,\"row_role\":\"%s\",\"relation_cell\":\"%s\",\"relation_match\":%s,\"query\":\"%s\",\"sample_index\":%u,\"pair_index\":%u,\"A_line_index\":%u,\"B_line_index\":%u,\"measurement_order\":\"%s\",\"A_first_touch_cycles\":%" PRIu64 ",\"B_first_touch_cycles\":%" PRIu64 ",\"source_alive_at_pair_measurement\":true,\"source_cpu_expected\":%d,\"receiver_cpu_expected\":%d,\"timer_method\":\"rdtscp_lfence_serialized\",\"physical_measurement\":%s,\"positive_physical_claim\":false}\n",
                    row.tuple_id,
                    row.execution_ordinal,
                    row.block_id,
                    row.block_local_position,
        ...
        if (fprintf(
                deaths,
                "{\"tuple_id\":\"%s\",\"execution_ordinal\":%d,\"source_lifetime\":\"alive_during_query\",\"source_pid\":%d,\"source_ready_monotonic_ns\":%" PRIu64 ",\"source_exit_monotonic_ns\":%" PRIu64 ",\"query_start_monotonic_ns\":%" PRIu64 ",\"query_end_monotonic_ns\":%" PRIu64 ",\"source_alive_at_pair_measurement\":true,\"source_alive_during_query\":true,\"post_observation_query_or_window_selection\":false,\"process_custody\":\"source_alive_during_spatial_pair_probe\",\"source_cpu_before\":%d,\"source_cpu_after\":%d,\"physical_measurement\":%s}\n",
                row.tuple_id,
                row.execution_ordinal,
                synthetic ? 0 : source_pid_record,
                source_ready_ns,
