...
RUN_ID = "family10h_relation_rspatial_shift_sweep_discovery_v1_0"
...
RELATION_SHIFT_VALUES = [1, 2, 4, 8, 16, 32, 64, 128]
VARIANTS = [
    f"shift_{shift:04d}_{kind}"
    for shift in RELATION_SHIFT_VALUES
    for kind in ["primary", "sham"]
]
QUERY_BY_VARIANT = {
...
MATCHED_PERMUTATION_SEED = "family10h-rspatial-shift-sweep-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "RSPATIAL_SHIFT_SWEEP_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_RSPATIAL_SHIFT_SWEEP_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_RSPATIAL_SHIFT_SWEEP_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_rspatial_shift_sweep() -> None:
    ...
    shifted_map_function = (
        map_function
        + "\n"
        "uint32_t relation_spatial_map_index_with_shift(relation_spatial_relation_id relation, uint32_t logical_a_index, uint32_t relation_shift) {\n"
        "    uint32_t shift = relation_shift % FAMILY10H_RELATION_SPATIAL_LINE_COUNT;\n"
        "    if (shift == 0u) {\n"
        "        shift = 1u;\n"
        "    }\n"
        "    if (relation == RELATION_SPATIAL_R0) {\n"
        "        return (logical_a_index + shift) % FAMILY10H_RELATION_SPATIAL_LINE_COUNT;\n"
        "    }\n"
        "    return (logical_a_index + FAMILY10H_RELATION_SPATIAL_LINE_COUNT - shift) % FAMILY10H_RELATION_SPATIAL_LINE_COUNT;\n"
        "}\n"
    )
    require(map_function in text, "relation_spatial_map_index anchor not found")
    text = text.replace(map_function, shifted_map_function, 1)
    text = text.replace(
    ...
    text = text.replace(
        ...
        "        uint32_t b_index = relation_spatial_map_index_with_shift(prep.relation, a_index, prep.bank_a_work);\n",
        1,
    )
    text = text.replace(
        "    return relation_spatial_map_index(row->r_query, a_index);\n",
        "    return relation_spatial_map_index_with_shift(row->r_query, a_index, row->bank_a_work);\n",
        1,
    )
    prep_block = (
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        relation_shift = SHIFT_BY_VARIANT[variant]
        rows = []
        for index, row in enumerate(base_rows):
            copied = dict(row)
            copied["execution_ordinal"] = str(index)
            copied["query"] = query
            copied["bank_A_work"] = str(relation_shift)
        ...
        schedules[variant] = {
            "query": query,
            "relation_shift_lines": relation_shift,
            "path": f"RSPATIAL_SHIFT_SWEEP_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        ...
        "relation_shift_values": RELATION_SHIFT_VALUES,
        "variants": schedules,
        "claim_boundary": {
        ...
        "mechanism_question": (
            "Whether varying the relation-spatial A-to-B shift exposes an oriented phase/selectivity "
            "coordinate after local primary-minus-sham subtraction."
        ),
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_RSPATIAL_SHIFT_SWEEP_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_RSPATIAL_SHIFT_SWEEP_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/RSPATIAL_SHIFT_SWEEP_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_RSPATIAL_SHIFT_SWEEP_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_RSPATIAL_SHIFT_SWEEP_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_RSPATIAL_SHIFT_SWEEP_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_RSPATIAL_SHIFT_SWEEP_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_RSPATIAL_SHIFT_SWEEP_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(RUN_ROOT / "RELATION_RSPATIAL_SHIFT_SWEEP_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- shift sweep candidate: `{result['interpretation']['shift_sweep_candidate']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    patch_runtime_for_rspatial_shift_sweep()
    schedule_manifest = make_rows()
    build = compile_runtime()
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_SHIFT_SWEEP_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_RSPATIAL_SHIFT_SWEEP_PREPARE_RESULT.json", result)
    return result
