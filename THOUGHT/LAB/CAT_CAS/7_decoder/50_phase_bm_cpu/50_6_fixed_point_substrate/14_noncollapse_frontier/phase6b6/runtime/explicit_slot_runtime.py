...
from contracts.contract import AUTHORITY, NOMINAL_SAMPLES_PER_SLOT, canonical_json, digest
...
def mock_slot_capture(session: dict[str, Any], slot: dict[str, Any], measured_tsc_hz: int) -> dict[str, Any]:
...
def run_mock(schedule: dict[str, Any]) -> dict[str, Any]:
    ...
    for session in schedule["sessions"]:
        ...
        runtime_session = {
            key: session[key]
            for key in ("session_index", "route", "reboot_block", "sender_core", "receiver_core", "split")
        }
        ...
        runtime_session["slots"] = [mock_slot_capture(runtime_session, slot, measured_tsc_hz) for slot in session["slots"]]
