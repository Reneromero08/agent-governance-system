#!/usr/bin/env python3
"""M253 public controller; imports no backend or exact-field implementation."""
...
RESULT = "PASS_CATVM_GRASSMANN_GAUSSIAN_OPEN_RELATION_STRICT_SCOPE"
CLAIM = (
    "CATVM_ENFORCED_BOUNDED_EXACT_QZETA8_FOUR_PORT_GRASSMANN_GAUSSIAN_OPEN_"
    "RELATION_INTERSECTS_BY_ANTISYMMETRIC_COEFFICIENT_ADDITION_AND_CLOSES_BY_"
    "AN_INVERTIBLE_BEREZIN_PFAFFIAN_TRANSFORM_ON_ONE_SEVEN_CELL_RESIDENT_"
    "SIGNATURE_WITH_FINAL_TOP_FORM_ONLY_ATOMIC_EXACT_SAME_BACKING_RESTORATION_"
    "AND_GENERATION2_REUSE_BUT_THE_IDENTICAL_SEVEN_CELL_CLASSICAL_RECURRENCE_"
    "AND_GENERAL_PFAFFIAN_MATCHGATE_ALGORITHMS_BISIMULATE_THE_DECLARED_LAW_"
    "WITH_NO_ADVANTAGE"
)
CLAIM_CEILING = (
    "EXACT_SOFTWARE_FOUR_TYPED_GRASSMANN_PORT_QZETA8_GAUSSIAN_RELATIONS_"
    "PUBLIC_PROGRAM_LENGTH_AT_MOST8_ON_AN_ABSTRACT_UNIX_SOCKET_CATVM_ONLY"
)


def socket_address(name: str) -> str:
    if not name.startswith("@catvm-m253-"):
        raise RuntimeError("M253 client requires declared abstract socket")
    return "\0" + name[1:]
...
def exchange(name: str, request: dict[str, object]) -> tuple[dict[str, Any], int, int]:
    ...
    connection.close()
    if not response:
        raise RuntimeError("M253 service returned no response")
...
def disconnect_run(name: str, case: dict[str, Any]) -> int:
    request = run_request(case, 1, "M253_DISCONNECT")
...
def comparable(case: dict[str, Any]) -> dict[str, Any]:
    return {key: case[key] for key in (
        "module_kinds", "top_form_boundary", "hidden_relation_field_cells",
...
def contains_forbidden(value: object) -> bool:
    forbidden = {
        ...
        "intermediate", "inverse_history_values",
    }
...
def main(name: str) -> None:
    ...
    if set(cases) != expected:
        raise RuntimeError("invalid M253 public case set")

    total_request_bytes = 0
    total_response_bytes = 0
    accepted: list[dict[str, Any]] = []
    for label, generation in (("primary", 1), ("reuse", 2), ("fresh", 1)):
        response, sent, received = exchange(name, run_request(cases[label], generation, f"M253_{label.upper()}"))
        total_request_bytes += sent
        total_response_bytes += received
        if response.get("status") != "OK":
            raise RuntimeError(f"M253 accepted case rejected: {label}")
    ...
    if disconnect_status is None:
        raise RuntimeError("M253 disconnect restoration timeout")
    ...
    for case_id, field in (("partial", "inject_failure_after_partial"), ("postprojection", "inject_failure_after_projection")):
        request = run_request(cases[case_id], 1, f"M253_{case_id.upper()}")
        request[field] = True
    ...
    base = run_request(cases["descriptor_control"], 1, "M253_CONTROL")
    controls_response, sent, received = exchange(name, {**base, "command": "CONTROLS"})
    total_request_bytes += sent
    total_response_bytes += received
    if controls_response.get("status") != "OK":
        raise RuntimeError("M253 backend controls rejected")
    ...
    mutations.append(("stale_generation_rejected", run_request(cases["primary"], 2, "M253_STALE")))
    skipped = dict(base)
    skipped["generation"] = 2
    mutations.append(("skipped_generation_rejected", skipped))
    same_id = json.loads(json.dumps(base))
    same_id["descriptor"]["modules"][0]["coefficients"][0] = [[2, 1], [0, 1], [0, 1], [0, 1]]
    ...
    malformed_rational["descriptor"]["modules"][0]["mu"][0] = [1, 0]
    ...
    malformed_transaction["transaction_id"] = 253
    ...
    malformed_carrier["carrier_id"] = 253
    ...
    if not shutdown.get("shutdown"):
        raise RuntimeError("M253 shutdown failed")
    ...
    controls = {
        ...
        **descriptor_controls, **protocol_controls,
        ...
        "backend_responses_exclude_relation_coefficients_scratch_and_dense_signatures": not any(
            contains_forbidden(case) for case in accepted
        ),
    }
    if not all(controls.values()):
        raise RuntimeError(f"M253 control failure: {controls}")

    work_fields = tuple(accepted[0]["work"])
    resource_law = {
        ...
        **{f"accepted_{field}": sum(case["work"][field] for case in accepted) for field in work_fields},
        ...
        "stronger_general_algorithmic_ceiling_not_claimed_as_implemented_baseline": "GENERAL_PLANAR_MATCHGATE_OR_GRASSMANN_GAUSSIAN_CONTRACTION_BY_POLYNOMIAL_TIME_PFAFFIAN_ALGORITHMS",
        "accepted_catvm_path_has_space_work_or_query_advantage": False,
        "comparison_basis": "QZETA8_FIELD_CELLS_PUBLIC_PLAN_FIELD_CELLS_FIELD_OPERATIONS_WRITES_FINAL_BOUNDARY_RESTORATION_REUSE_AND_PROTOCOL_TRAFFIC_NOT_WHOLE_PROCESS_RSS",
    ...
    output = {
        ...
        "claim_limits": {
            ...
            "non_gaussian_relation_closure": False,
            "fixed_bounded_width_exact_state_across_growing_interfaces": False,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("usage: client.py @catvm-m253-NAME")
    main(sys.argv[1])
