...
if [[ $# -ne 1 ]]; then
  echo "usage: qualify_f17_cubic_chain_period17_pi_unit_exact_coordinate_descent.sh EVIDENCE_DIR" >&2
  exit 2
fi
...
source_path="$frontier_dir/f17_cubic_chain_period17_pi_unit_exact_coordinate_descent.py"
oracle_path="$frontier_dir/f17_cubic_chain_period17_pi_unit_exact_coordinate_descent_oracle.py"
...
qualifier_path="$frontier_dir/qualify_f17_cubic_chain_period17_pi_unit_exact_coordinate_descent.sh"
expected_path="$frontier_dir/F17_CUBIC_CHAIN_PERIOD17_PI_UNIT_EXACT_COORDINATE_DESCENT_RESULTS.json"
oracle_expected_path="$frontier_dir/F17_CUBIC_CHAIN_PERIOD17_PI_UNIT_EXACT_COORDINATE_DESCENT_ORACLE_RESULTS.json"
provenance_path="$frontier_dir/F17_CUBIC_CHAIN_PERIOD17_PI_UNIT_EXACT_COORDINATE_DESCENT_PROVENANCE.json"
review_path="$frontier_dir/F17_CUBIC_CHAIN_PERIOD17_PI_UNIT_EXACT_COORDINATE_DESCENT_INDEPENDENT_REVIEW.md"
...
if rg -n '^[[:space:]]*(from|import)[[:space:]]+f17_cubic_chain_period17_pi_unit_exact_coordinate_descent' \
  "$oracle_path"; then
...
jq -e '
  ...
  and .period1_named_component_maxima_sums_beat_raw
  and .period64_named_component_maxima_sums_remain_above_raw
  and all(.cases[];
    .restored_exactly
    and .same_backing
    ...
    and .balanced_stats.coordinatewise_certified_calls
      == .balanced_stats.balance_calls
    and .balanced_stats.maximum_search_power_live_pair_payload_bits > 0
    and .balanced_stats.maximum_accepted_move_scale_payload_bits > 0
    and .balanced_stats.maximum_unit_scale_payload_bits > 0
    and .balanced_stats.maximum_unit_materialization_payload_bits > 0
  )
  ...
  and all(.controls[]; .)
  and all(.exact_controls[]; .)
  and .matched_classical.identical_exact_coordinate_search_available
  and .matched_classical.identical_exact_recurrence_available
  and (.matched_classical.comparison_establishes_advantage|not)
  and .resource_law.search_power_result_factor_live_pairs_counted
  and .resource_law.accepted_move_scale_payload_counted
  and .resource_law.unit_scale_payload_counted
  and .resource_law.unit_materialization_counted
...
jq -e '
  ...
  and .oracle_search_method
    == "INDEPENDENT_EXACT_DIRECTION_BRACKET_AND_DISCRETE_DIFFERENCE_BINARY_SEARCH"
  ...
  and all(.case_checks[];
    ...
    and .named_exact_search_temporary_maxima_sum_equal
...
echo "QUALIFIED_F17_CUBIC_CHAIN_PERIOD17_PI_UNIT_EXACT_COORDINATE_DESCENT"
