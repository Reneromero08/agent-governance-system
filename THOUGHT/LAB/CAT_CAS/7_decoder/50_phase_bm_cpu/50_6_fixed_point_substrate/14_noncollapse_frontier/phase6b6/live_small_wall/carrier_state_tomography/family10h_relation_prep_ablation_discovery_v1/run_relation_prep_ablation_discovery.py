...
RUN_ID = "family10h_relation_prep_ablation_discovery_v1_0"
...
VARIANTS = [
    ...
    "prep_ablation_control",
    "carrier_off_control",
    "relation_sham",
]
QUERY_BY_VARIANT = {
    "primary_relation_pair": "query_relation_pair",
    "prep_ablation_control": "prep_ablation_control",
...
MATCHED_PERMUTATION_SEED = "family10h-prep-ablation-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "PREP_ABLATION_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_PREP_ABLATION_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_PREP_ABLATION_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_transform_kill() -> None:
    ...
    text = text.replace(
        ...
        "RELATION_SPATIAL_CONTROL_DISTANCE_MATCHED = 4,\n"
        "    RELATION_SPATIAL_CONTROL_PREP_ABLATION = 5,\n"
    ...
    text = text.replace(
        ...
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "prep_ablation_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_PREP_ABLATION;\n"
        "        return 1;\n"
        "    }\n"
    ...
    prepare_ablation = (
        insert_after_prepare
        + "\n"
        + "static int relation_spatial_prepare_ablation(relation_spatial_preparation prep, relation_spatial_carrier_state *state) {\n"
        + "    uint32_t step = 0u;\n"
        + "    if (state == NULL || prep.cyclic_origin >= FAMILY10H_RELATION_SPATIAL_LINE_COUNT) {\n"
        + "        return 0;\n"
        + "    }\n"
        ...
        + "    for (step = 0u; step < FAMILY10H_RELATION_SPATIAL_TOTAL_WORK; ++step) {\n"
        + "        uint32_t a_index = relation_spatial_origin_index(prep.cyclic_origin, step);\n"
        + "        uint32_t b_index = (a_index + 1024u) % FAMILY10H_RELATION_SPATIAL_LINE_COUNT;\n"
        + "        volatile uint8_t *a = &state->lane_a[a_index * FAMILY10H_RELATION_SPATIAL_LINE_BYTES];\n"
        + "        volatile uint8_t *b = &state->lane_b[b_index * FAMILY10H_RELATION_SPATIAL_LINE_BYTES];\n"
        + "        if (prep.source_order == RELATION_SPATIAL_ORDER_AB) {\n"
        + "            *a = (uint8_t)(*a + 1u);\n"
    ...
    require(insert_after_prepare in text, "canonical prepare function not found for ablation insertion")
    text = text.replace(insert_after_prepare, prepare_ablation)
    prep_block = (
    ...
    patched_prep_block = (
        "            if (row.control == RELATION_SPATIAL_CONTROL_CARRIER_OFF) {\n"
        ...
        "                prep.source_order = row.source_order;\n"
        "                prep.cyclic_origin = row.cyclic_origin;\n"
        "                shared->preparation_ok = row.control == RELATION_SPATIAL_CONTROL_PREP_ABLATION\n"
        "                    ? relation_spatial_prepare_ablation(prep, &shared->state)\n"
        "                    : relation_spatial_prepare(prep, &shared->state);\n"
        "            }\n"
    )
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_PREP_ABLATION_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"PREP_ABLATION_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_PREP_ABLATION_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        ...
        "mechanism_question": "Does changing only source preparation relation move the local coordinate while preserving receiver query geometry?",
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_PREP_ABLATION_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_PREP_ABLATION_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_PREP_ABLATION_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_PREP_ABLATION_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_PREP_ABLATION_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/PREP_ABLATION_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_PREP_ABLATION_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_PREP_ABLATION_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_PREP_ABLATION_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_PREP_ABLATION_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_PREP_ABLATION_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_PREP_ABLATION_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_PREP_ABLATION_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_PREP_ABLATION_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_PREP_ABLATION_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_PREP_ABLATION_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    prep_ablation = reports["prep_ablation_control"]["R_spatial"]["mean"]
    carrier_off = reports["carrier_off_control"]["R_spatial"]["mean"]
    sham = reports["relation_sham"]["R_spatial"]["mean"]
    prep_ablation_delta = primary - prep_ablation
    result = {
        "schema": "FAMILY10H_RELATION_PREP_ABLATION_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "coordinates": {
            "R_primary": primary,
            "R_prep_ablation": prep_ablation,
            "R_carrier_off": carrier_off,
            "R_relation_sham": sham,
            "D_primary_minus_sham": primary - sham,
            "prep_ablation_primary_minus_ablation": prep_ablation_delta,
            "prep_ablation_half_difference": 0.5 * prep_ablation_delta,
            "prep_ablation_abs_to_primary_abs": abs(prep_ablation) / abs(primary) if primary else None,
            "prep_ablation_distance_to_sham_abs": abs(prep_ablation - sham),
            "prep_ablation_distance_to_sham_to_D": abs(prep_ablation - sham) / abs(primary - sham) if primary != sham else None,
        ...
        "interpretation": {
            "prep_ablation_reduces_primary": primary > 0 and prep_ablation < primary,
            "prep_ablation_delta_above_25pct_primary": primary > 0 and abs(prep_ablation_delta) > 0.25 * abs(primary),
            "prep_ablation_approaches_sham_within_25pct_D": (
                primary != sham and abs(prep_ablation - sham) <= 0.25 * abs(primary - sham)
            ),
            "carrier_off_kills_primary_to_below_25pct": primary > 0 and abs(carrier_off) <= 0.25 * abs(primary),
    ...
    write_json(RUN_ROOT / "RELATION_PREP_ABLATION_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- prep ablation reduces primary: `{result['interpretation']['prep_ablation_reduces_primary']}`",
            f"- prep ablation delta above 0.25 x primary: `{result['interpretation']['prep_ablation_delta_above_25pct_primary']}`",
            f"- prep ablation approaches sham within 0.25 x D: `{result['interpretation']['prep_ablation_approaches_sham_within_25pct_D']}`",
            f"- carrier-off below 0.25 x primary: `{result['interpretation']['carrier_off_kills_primary_to_below_25pct']}`",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_PREP_ABLATION_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_PREP_ABLATION_PREPARE_RESULT.json", result)
    return result
