...
**Status:** `PHASE_QEMU_V13_COMPILED_COMMON_DEVICE_HARDWARE_ABSENCE_FIXED_OFFLINE_SELECTOR_GATE_VERIFIED_NO_PHYSICAL_OR_STATISTICAL_PROMOTION`
**Roadmap version:** `15.55`
**Standing mission:** `CAT_CAS_HOLOGRAPHIC_RELATIONAL_COMPUTE_V1`
**Canonical branch:** `codex/audio-frequency-wave-substrate`  
**Reconciled source head:** `840c2d3b9cf7dc6351d0fb890d16d3e4d6e75fe4`
**Claim registry:** `CLAIM_AUTHORITY_REGISTRY.json`
**Current highest milestone:** `COMPILED_COMMON_PHASE_QEMU_V13_FIXED_OFFLINE_HARDWARE_ADAPTER_SELECTOR_GATE_REJECTS_ABSENT_UNENROLLED_UNATTESTED_STALE_REPLAYED_DOWNGRADED_AND_TEST_FIXTURE_SESSIONS_WITHOUT_PUBLISHING_A_PHYSICAL_OUTPUT_OR_CAMPAIGN_STATISTICAL_CERTIFICATE_AND_WITH_EVERY_NONDESTRUCTIVELY_COMPLETED_DISPATCHED_ATTEMPT_TERMINAL_ACK_THEN_SPENT`
**Current claim ceiling:** `HARDWARE_ABSENT_PROTOCOL_CONFORMANCE_ONLY_NO_AUTHENTICATED_LIVE_DEVICE_SESSION_NO_PHYSICAL_SAMPLE_NO_CAMPAIGN_STATISTICAL_CERTIFICATE_NO_CUSTODY_RETURN_RESTORATION_REUSE_ADVANTAGE_OR_M257_ESCAPE`
**Current evidence authority:** `SEPARATE_REFERENCE_PARITY`
**Current resource authority:** `PACKAGE_SELF_REVIEW`
**Current restoration:** `NO_RESTORATION_CLAIM`
**New mechanism development:** `AWAITING_USER_AUTHORIZED_PINNED_DEVICE_ENROLLMENT_AND_PREREGISTERED_BLINDED_DUAL_RAIL_PHYSICAL_CAPTURE_CAMPAIGN_AFTER_M271_COMPILED_HARDWARE_ABSENCE_GATE`
**Milestone terminal:** `false`  
**Lane closure authority:** `USER_ONLY`  
**Physical carrier work:** `SOFTWARE_PROTOCOL_CONFORMANCE_AUTHORIZED_ACTUAL_DEVICE_ENROLLMENT_CONNECTION_AND_CAPTURE_NOT_AUTHORIZED`
**P0:** `REFERENCE_HARDWARE_MODEL_0_SELECTED_NATURAL_RINGDOWN_EMULATED_NOT_FINAL_ARCHITECTURE`
**Active primary successor:** `USER_AUTHORIZED_PINNED_DEVICE_ENROLLMENT_FOLLOWED_BY_A_PREREGISTERED_BLINDED_DUAL_RAIL_DISPERSIVE_CAPTURE_CAMPAIGN_WITH_DEVICE_SIGNED_RAW_MANIFESTS_INDEPENDENT_MEASUREMENT_AND_FAMILYWISE_STATISTICAL_VALIDATION_BEHIND_THE_COMMON_PHASE_QEMU_BACKEND`
**Mandatory integration target:** `USER_AUTHORIZED_LIVE_HARDWARE_ENROLLMENT_AND_PHYSICAL_CAPTURE_BEHIND_EXISTING_PHASE_QEMU_V13_COMMON_DEVICE_BACKEND_WITHOUT_STANDALONE_TWIN_PROMOTION_OR_PHYSICAL_RESTORATION_REUSE_OVERCLAIM`
...
`840c2d3b9cf7dc6351d0fb890d16d3e4d6e75fe4`. The current 271-milestone
...
Gamma_best(n) = cost of the best matched compact classical method
                / (compile + native phase execution + readout + restoration)
```
...
The compact toroidal path-sum successor established:

```text
compact canonical .holo programs
no-smuggle compilation from public cyclic weights
global toroidal phase shears
relative-phase include/exclude path aggregation
zero complete path modes
zero retained inverse factors
program-derived inverse execution
restoration and cross-program reuse
post-seal independent acceptance
Gamma growth relative to explicit path expansion
```

The largest sealed case represents `2^256` explicit paths without materializing them.
...
compact separator storage
    ESTABLISHED_FOR_GENERAL_AFFINE_F2_RELATIONS_WIDTH3_THROUGH_WIDTH16
...
C5 fixed-point / compute advantage NOT_ESTABLISHED_AGAINST_BEST_COMPACT_CLASSICAL_BASELINE
C6 external official acceptance  NOT_ESTABLISHED
C7 cross-domain transfer         NOT_ESTABLISHED
physical phase computation       NOT_ESTABLISHED
```

Local independent adjudication is not C6. C6 requires an external official boundary that the lane does not control.

## 4. Permanent Nonterminal Law

Every achieved software result is nonterminal unless the user explicitly closes the lane.

The following never authorize stopping:

```text
bounded mission result
mission lifecycle demonstrated
Gamma growth against explicit enumeration
new instruction
new program class
program counter
branches or loops
...
After preserving a result, the agent must:

```text
update this roadmap and AUTONOMOUS_LANE_STATE.json
identify the highest-leverage unresolved blocker
select the next experiment or architectural move
...
The agent is authorized to make ordinary reversible software, mathematical, architectural, and experimental decisions autonomously.
...
prototype competing process objects and native operators
build control flow, memory, compilers, fixed points, or global operators
...
-> update lane state
-> select the next highest-leverage blocker
-> continue
```

Verification follows a substantive result. One focused review is normally sufficient for one claim. Do not create audit-of-audit loops or governance projects before the mechanism exists.
...
```text
NEW_MECHANISM_DEVELOPMENT    AWAITING_USER_AUTHORIZED_PINNED_DEVICE_ENROLLMENT_AND_PREREGISTERED_BLINDED_PHYSICAL_CAPTURE_CAMPAIGN
SCIENTIFIC_SOURCE_HEAD       840c2d3b9cf7dc6351d0fb890d16d3e4d6e75fe4
COMPLETED_MECHANISM          M271_COMPILED_COMMON_DEVICE_HARDWARE_ABSENCE_FIXED_OFFLINE_SELECTOR_GATE
...
ARCHITECTURE_PROMOTION       COMMON_DEVICE_REINTEGRATION_PASSED_PHYSICAL_STATISTICAL_RESOURCE_PROMOTION_FALSE
COMPILED_DEVICE_SET          V0_V1_V11_V12_V13
MECHANISM_TWIN_SET           V2_V10_KILL_OR_NOMINATE_ONLY
ACTIVE_SUCCESSOR             USER_AUTHORIZED_PINNED_DEVICE_ENROLLMENT_FOLLOWED_BY_A_PREREGISTERED_BLINDED_DUAL_RAIL_DISPERSIVE_CAPTURE_CAMPAIGN_WITH_DEVICE_SIGNED_RAW_MANIFESTS_INDEPENDENT_MEASUREMENT_AND_FAMILYWISE_STATISTICAL_VALIDATION_BEHIND_THE_COMMON_PHASE_QEMU_BACKEND
INTEGRATION_TARGET           USER_AUTHORIZED_LIVE_HARDWARE_ENROLLMENT_AND_PHYSICAL_CAPTURE_BEHIND_EXISTING_PHASE_QEMU_V13_COMMON_DEVICE_BACKEND
STANDALONE_PYTHON_TWIN       forbidden_for_architecture_promotion
NEXT_OBSTRUCTION             OBTAIN_EXPLICIT_USER_AUTHORIZATION_ENROLL_A_PINNED_DEVICE_AND_RUN_A_PREREGISTERED_BLINDED_INDEPENDENT_CAPTURE_CAMPAIGN_WITHOUT_SELF_CERTIFICATION_OR_PHYSICAL_RESTORATION_REUSE_OVERCLAIM
```
...
The fixed-width calibration has now been removed through width sixteen:

```text
B(w)       = 4w^2 + 4w + 1 complete relation cells
Workspace  = 36w^2 + 11w + 2 phase cells
Carrier    = 60w^2 + 35w + 8 phase cells
native composition                         exact cubic phase schedule
tested widths                              2, 3, 4, 8, 12, 16
phase/reference complete-boundary matches  9 at every width
```

At width three and above, the complete affine descriptor is smaller than the
`2^(2w)` dense membership vector. This is a polynomial representation law for
...
### C. Establish C5 against the best matched baseline

Measure compile, native execution, readout, restoration, memory, and witness verification against the strongest known compact conventional method. C5 requires growing leverage or another defensible asymptotic separation against that method.

### D. Reach an official external Wall

Produce a conventional witness accepted by an independent official verifier, bounty, benchmark, or submission boundary. External submission or commitment requires user authority.

### E. Transfer the mechanism

Apply the same compact native mechanism to a materially different problem family without replacing it with a domain-specific hidden solver.
...
Physical phase persistence, computation, restoration, hardware bit replacement, and the Small Wall remain later frontiers. They require explicit user authority.
...
## Exact log-free unit descent removes hidden proposal precision, not period-64 total cost

The successor replaces the 65,536-bit embedding table and floating solve
with exact integer trace-energy line minimization over 49 public unit
directions. Each line search uses exponential bracketing and binary search
of the monotone discrete difference. Every nonzero normalization call ends
with a full unchanged direction sweep; no tested sweep or bracket cap is
hit. This establishes only a local minimum over the declared directions.

```text
claim
    BOUNDED_EXACT_LOG_FREE_49_DIRECTION_CYCLOTOMIC_UNIT_LINE_DESCENT_AFTER_PI_FACTORIZATION_REDUCES_RESIDENT_AND_DUPLICATE_REMATERIALIZATION_LIVE_PAYLOAD_BELOW_RAW_FOR_TWO_PUBLIC_F17_PERIOD17_FAMILIES_AT_PERIODS1AND64_WITH_EXACT_RESTORATION_AT_PERIODS1AND64_AND_CROSS_FAMILY_RESTORED_CARRIER_REUSE_AT_PERIOD1_BUT_PERIOD64_CONSERVATIVE_NAMED_COMPONENT_MAXIMA_SUM_REMAINS_ABOVE_RAW_AND_THE_IDENTICAL_CLASSICAL_EXECUTION_REMAINS
...
All four resident and duplicate-rematerialization live payloads remain below
the identical raw recurrence. The corrected period-64 sum is larger than
the preliminary package-local report because it now includes the live
result/factor pair during exact powering, accepted move scales, and
ledger-derived unit scales as well as trial norms, energy pairs, accepted
vectors, and materialized vectors. It is a conservative sum of separately
observed maxima, not a simultaneous process peak or a bound on Python,
allocator, internal ring-multiplication, or whole-process memory.

The separate oracle imports no production successor. It recompiles both
public operators, advances coefficients sequentially by `x mod q`,
reimplements the exact direction search, and reproduces every new forward
and inverse resource metric, boundary, restoration check, and mutation
control. Each period-1 and period-64 case restores exact payload and ledgers
on the original backing. Unrelated cross-family restored-carrier reuse is
verified only at period 1.

The identical exact search and recurrence remain available to compact
classical software. Global unit optimality, fixed total footprint, CATVM
...
It must search by updating only the exact norm and unit ledger, apply one net
residual action after convergence, and avoid materializing absolute unit
scales across all 17 vector entries. It must preserve exact restoration,
period-1 unrelated reuse, the strongest identical compact classical
recurrence, and the corrected component-level accounting.
...
## Direct real Hermitian generation removes accepted degree-16 norm products

M115 replaces each temporary `x * conjugate(x)` plus real-field conversion
with a direct eight-coordinate Hermitian generator. For canonical
`x = sum(a_j zeta^j)`, extend with `a_16 = 0`, form the cyclic correlations
`P_k = sum_j a_j a_(j+k mod 17)`, and emit
`(P_0-P_8, ..., P_7-P_8)` in the integral real basis. The accepted schedule
stores `P_8` and streams `P_0` through `P_7`.
...
The separate oracle imports no production M115 module and derives the same
Hermitian value by writing `x = A(y) + zeta B(y)` over the real-subfield power
basis, then evaluating `A^2 + yAB + B^2`. It also independently reexecutes
the public direct schedule for resource parity. The accepted generator uses
136 integer multiplications, 127 accumulation additions, and 8 final
subtractions per Hermitian term. It materializes no conjugate tuple, full
degree-16 Hermitian product, full-to-real conversion, or degree-16 norm
scratch.

```text
period family   calls  terms  integer products  direct named total  raw Horner  difference
...
These totals reduce the corresponding M114 named sums by 2,298, 2,303,
141,782, and 144,589 bits. They remain conservative sums of named maxima,
not simultaneous or whole-process peaks, and all four remain above the
matched raw Horner baseline. Python objects, allocator/native-library state,
and internal integer multiplication storage are not bounded.

Exact payload and ledger restoration holds on the original backing for both
families at periods 1 and 64, with no retained inverse history or baseline
reload. Unrelated cross-family reuse is established at period 1. Full object
equality and enforced generation/lease custody are not established.

Compact classical software can execute the identical direct bilinear map.
No distinct phase resource, computational advantage, Small Wall crossing,
...
Discrete decoding occurs only at a declared boundary. The borrowed carrier must be causally involved. Restoration must reverse the actual forward process. Reuse must consume the actual restored substrate.
...
## Growing typed cubic separators reject only the uniform exact linear quotient

M136 executes the transferable separator-law diagnostic selected by M135.
For `k` individually typed shared Boolean latent coordinates, the left and
right anchor-one branch minors are Kronecker products of exact two-by-two
matrices `[[1,1],[1,theta_i]]`.  Nonidentity phase factors make every local
determinant nonzero.  The typed Walsh separator is invertible, so the exact
two-sided separator rank is `2^k`.
...
Exact transactions cover arities `1,2,4,8,16,32,64` over `Q(zeta17)` and
structural arities `1,2,4,8,16` over `F103` and `F137`.  The independent
oracle directly builds left, right, Walsh, and two-sided matrices through
arity six in all three fields and reproduces rank `2^k`, identity-phase rank
loss, singular-transport rank loss, typed continuation separation, and the
same `O(k)` factorised boundary.

The accepted path stores `2k` public phases in `4k` exact paired-factor cells,
derives its certificate from those actual resident factors, and materialises
no `2^k` port, assignment family, dense minor, or determinant value.  It
restores canonical exact zero on the same backing and reuses that backing for
an unrelated public family without snapshot or inverse history.

The proved no-go applies only to a fixed or uniform exact field-linear
quotient preserving arbitrary incoming messages and the full declared typed
continuation family.  Rank alone is not a general storage or advantage lower
bound.  The same local topology has an `O(k)` two-by-two Kronecker classical
contraction, and general descriptors retain the matched
`poly(descriptor size) * 2^treewidth` variable-elimination baseline.  No
nonlinear or program-dependent quotient no-go, operational exponential phase
port, CATVM custody, distinct phase resource, advantage, Small Wall crossing,
physical execution, bit replacement, or unbounded computation is claimed.
...
It tests the smallest remaining phase-owned route: whether repeated Walsh
derivatives of a cubic phase polynomial admit exact merging of the resulting
cyclotomic binomial factors without moving growth into a hidden factor graph,
projection, or verification path.
...
M138 executes the rank-two route selected by M137. Two disjoint public
Boolean quadratic signatures share one unresolved typed Walsh port. Public
phase shifts alternate between the two signature axes and are interleaved
with Walsh mixing on the unresolved port.
...
The canonical direct carrier is `K[C17 x C17]^2`, or 578 exact field cells.
Exact `Q(zeta17)` cases cover branch-pair/depth tuples
`(1,2),(2,4),(4,6),(8,8),(16,10),(32,12),(64,16),(128,32)`; three public
program families receive structural checks over `F103` and `F137` through
depth 64. All three tested families reach separation rank 17 at a declared
ceiling. This rejects only uniform matrix-factor rank caps below 17 for these
families. It is not a universal 578-cell lower bound or general rank-r no-go.

The adversarial review repaired material accounting before the source seal.
Both 289-cell Walsh output surfaces are live simultaneously, so update scratch
is 578 cells and carrier-plus-update live storage is 1,156 logical field cells.
The projection reports 20 persistently named field cells separately from its
289-cell rank buffer. Exact resident and resident-plus-update payloads are:

```text
branch pairs                    1      2      4      8     16     32     64      128
resident bits               18504  18528  18744  19369  21392  25842  40028   110637
resident plus scratch bits  37004  37040  37364  38323  41501  49067  76295   216450
```

The exact inverse is rematerialized from public topology, restores canonical
zero on the same backing, and supports unrelated reuse without a snapshot or
inverse history. A true wrong-order inverse applied to the post-projection
resident state fails restoration. The restoration count is package-local and
does not establish CATVM custody.

The compact classical comparison is a frontier rather than one overstated
representation. Retaining both surfaces reproduces the scalar, ranks, and
commitment at the same 578-cell residency. An executed 289-residue recurrence
reproduces every final scalar with at most eight named dynamic exact fields
plus 17 public integer counts. A conservative 320-field rematerialized
full-diagnostic construction is recorded but not executed, and no payload
tuple is claimed for it. Consequently no single memory/work point dominates.

The next experiment is:

```text
EXACT_NONADDITIVE_PHASE_COUPLING_OR_RELATION_QUOTIENT_BEYOND_C17_SIGNATURE_AXIS_GROUP_ALGEBRA_WITH_MATCHED_STREAMED_CLASSICAL_CONTRACTION
```

The obstruction is:

```text
M140_REMOVES_THE_RETAINED289_CELL_PUBLIC_FOURIER_KERNEL_WITH_A16_TERM_CHARACTER_SUM_FACTORIZATION_BUT_USES544_CHARACTER_PRODUCTS_PER_FOURIER38_UPDATE_SCRATCH_CELLS_AND_ONE_RETAINED_GENERATOR_SCALAR_WHILE_THE_IDENTICAL_MATRIX_FREE17_COORDINATE_CLASSICAL_RECURRENCE_AND_DEPTH_GROWING_EXACT_PAYLOAD_REMAIN
```

M140 establishes neither CATVM custody, a distinct phase resource,
...
M140 removes M139's retained 289-cell public Fourier kernel.  For source shell
`q`, target shell `r`, and nonzero `t` in `F17`, it executes the exact
factorization

```text
s(t) = sum_q zeta17^(-q t) f(q)
F(f)(r) = delta[r=0] sum_q f(q)
          - 17^-1 sum_(t != 0) zeta17^(-r/(4t)) s(t)
```

The accepted Fourier path does not enumerate the 289 coordinates, call the
per-entry generator, generate kernel entries, or retain a dense kernel.
...
restoration
    resident 17-cell quotient carrier EXACT_ALGEBRAIC_RESTORATION
    generator/projection/commitment/oracle buffers NO_RESTORATION_CLAIM

source head
    75de68e25043b2f5bd2fd7da4409c101caec28e9
```

Seven exact `Q(zeta17)` transactions cover depths `1,2,4,8,16,32,64`.
Forty-eight structural transactions cover three public families over `F103`
and `F137` through depth 128.  The independent oracle reconstructs the public
geometry, program schedules, factorized forward/inverse recurrence, final
commitments, and exact resident-payload maxima.  It checks every generated
kernel entry against a direct coordinate character sum over both finite
fields, five selected exact entries, two dense 289-coordinate transforms,
and missing-term and wrong-sign mutations.
...
The retained public generator is one exact `17^-1` scalar, bringing the
counted maximum to 56 field cells.  Each Fourier uses 272 source contractions
and 272 target contractions, or 544 character products.  Projection uses six
scratch cells, for 23 carrier-plus-projection cells.  Verification-only
coordinate kernels and dense controls are reported separately.  Python
container and allocator overhead, native-library storage, bigint workspace,
hashlib internals, and whole-process memory remain excluded.

The actual inverse restores exact canonical zero on the same backing and an
unrelated public program reuses it without snapshot or retained inverse
history.  The restoration counter is package-local and is not CATVM custody.

The strongest executed compact classical comparison is the identical
matrix-free 17-coordinate recurrence with the same generator, scratch, and
`O(544*depth)` arithmetic law.  M139's retained-kernel recurrence remains in
the frontier because it uses `O(289*depth)` warm products and 20 update scratch
cells at the cost of the 289 retained kernel cells.  No universal lower bound
or asymptotic improvement is claimed.

The retained-kernel obstruction is closed, but the exact payload tuple still
grows with depth and the identical compact classical recurrence remains.  The
next selected experiment is:

```text
EXACT_HEIGHT_STABLE_ANISOTROPIC_RADIAL_PHASE_COUPLING_OR_NO_GO_WITH_MATRIX_FREE_CLASSICAL_FRONTIER
```

It must change the phase machine's exact height law or establish a bounded
no-go for the declared anisotropic radial family.  It must not count another
kernel/scratch constant-factor trade as a phase resource.  M140 establishes
...
## M151: C17 quadratic mode mixing and fixed-coordinate no-go

M151 changes the M150 phase machine by interleaving its cyclic-convolution
shears with a reversible coefficientwise quadratic shear. One unresolved hub
slot supplies one shared square to 16 target consumers between the OUT and IN
layers. The accepted path performs no scalar or exponent readout, materializes
no relation table or assignment expansion, and projects only the final
...
transient buffers
    NO_RESTORATION_CLAIM
source head
    1a02e6cc6cd63d45c80ea645d9aed1fd26a97be8
```

The exact transform law sends coefficientwise square to inverse-17 times
circular self-convolution of all 17 character modes. The independent-mode
square sham changes the final state, and a two-mode witness generates a third
mode. M151 therefore rejects M150's 17 independent-mode closure for the new
primitive rather than assuming that nonlinearity defeats the factorization.

All 18 cases across three public families and depths through 1024 match both
the coefficient recurrence and the fully coupled 17-mode recurrence in all
867 cells and at the boundary. The pure-Python oracle imports neither
production nor M150, uses no NumPy, and passes 180 comparisons plus inverse,
order, topology, null-port, transform, and sham controls.

Every carrier restores exactly on the same backing without snapshot, inverse
history, retained restoration baseline, or canonicalization. Unrelated
depth-613 reuse matches fresh execution, and 64 depth-8 reuse cycles restore
exactly. The accepted executions record 10,601 quadratic cancellation events.

The nonlinear primitive still has no demonstrated resource advantage. The
coefficient and coupled-spectral paths each retain 867 field coordinates and
reach 1,369 named warm bytes. At depth 1024 the evaluated coefficient path
counts 18,957,312 convolution-plus-quadratic core multiplications; the coupled
spectral path counts 1,410,048. The independent oracle also executes the
identical 867-coordinate classical coefficient recurrence. No optimal
classical recurrence is claimed.

The selected successor is:

```text
TYPED_OPEN_F103_C17_PHASE_RELATION_BIALGEBRA_SHARED_PORT_COMPOSITION_INTERSECTION_CLOSURE_OR_MATCHED_COMPACT_CLASSICAL_NO_GO
```

It must move from another deterministic finite-coordinate polynomial update
to an open typed relation law. It must make cyclic convolution serve native
relation composition and Hadamard product serve native relation intersection
on at least one shared unresolved port, retain multiple consumers and
final-only closure, restore and reuse the actual carrier, and compare against
the strongest compact classical bialgebra recurrence without relation tables
or assignment enumeration.

M151 establishes no complex or physical coherence, general relational
contraction, CATVM custody, distinct phase resource, computational advantage,
Small Wall crossing, physical waveform execution, physical bit replacement,
or unbounded catalytic computation.
...
## M153: non-translation rank-one relation closure and rank-two escape

M153 removes M152's translation-invariance quotient. Each open relation is
stored as `R(x,y)=a[x]b[y]`. One shared resident `C=u v^T` port acts on eight
targets by exact hidden-interface composition with `I+C` and by parallel
Hadamard intersection with `C`. Both laws preserve the declared rank-one
family and their order changes the boundary.
...
The independent scalar-Python oracle imports neither production nor NumPy and
passes 414 case-field comparisons over 18 cases through depth 512. It streams
all 289 scalar entries for each relation law without retaining a dense table.
A separate two-term control perturbation has a nonzero 2 by 2 minor of 34,
proving exact rank two and rejection by the rank-one closed family.

All 612 carrier coordinates restore exactly on the same backing without
snapshot, inverse history, retained baseline, or canonicalization. Unrelated
depth-311 reuse and 64 repeated depth-8 transactions match fresh execution.
Direct-process owner and generation checks do not establish CATVM custody.

The strongest executed baseline rematerializes the immutable control and
retains only 306 target-factor coordinates, versus 612 on the carrier. Both
paths count 278,528 relation field multiplications at depth 512. The phase
path therefore has no resident-state or work advantage; no optimal classical
recurrence is claimed.

The selected successor is:

```text
EXACT_F103_C17_RANK2_FACTORED_OPEN_RELATION_COMPOSITION_INTERSECTION_CLOSURE_OR_RANK_GROWTH_WITH_MATCHED_STREAMED_CLASSICAL_CONTRACTION
```

It must determine whether rank-two factors admit an exact reversible native
closure under repeated composition and intersection, or whether rank grows
toward the dense 17-dimensional ceiling. Any accepted path must count factor
canonicalization, rematerialization, and rank metadata and compare against a
streamed compact classical contraction without materializing relation tables.

M153 establishes no rank-two or general relation closure, CATVM custody,
distinct phase resource, computational advantage, Small Wall crossing,
...
## M162: arbitrary convolution plus Hadamard has no proper common linear quotient
...
M162 resolves the M161 intersection obstruction at the declared linear
quotient boundary. A kernel supporting arbitrary coefficientwise Hadamard
operands must be invariant under all coordinate projectors. Any nonzero
kernel vector then yields one coordinate basis vector. Convolution by the
one-step delta cyclically shifts that vector through a rank-17 orbit, so the
kernel is the full space. The only common kernel dimensions are therefore
zero and 17; every nonzero common quotient retains all 17 coordinates per
relation.

The restricted multiplier search is also exact. Production solves the
binomial Hasse-row constraints while the independent oracle solves the same
descent question in a monomial evaluation basis. At ranks 2, 4, and 8 both
find a one-dimensional diagonal-multiplier space generated by the all-ones
vector. Indicator, linear, and quadratic coefficient multipliers expose
discarded jet content. Thus the compatible family is only public scalar
multiplication, not general relation intersection.

The bounded restricted transaction still preserves the machine law: 24
rank/family/depth cases project only the final B boundary, restore exact jet
cells, and reuse the same backing for an unrelated family without snapshot or
inverse history. The independent full 34-cell group-algebra oracle matches
120 fields and exact restoration, and 54 additional quotient-law checks pass.
The strongest compact classical baseline is separately executed and is the
identical 16-carrier-cell, 56-working-cell rank-8 recurrence, so both declared
ratios remain one.

The no-go does not cover nonlinear encodings, restricted relation algebras, or
different phase state laws. The selected repair changes from a quotient of all
C17 relations to the smallest untested automorphism-orbit relation subalgebra
that is closed under both composition and intersection:
...
## M202: one-cell factor stream closes storage but exposes topology work
...
M202 returns to M199's exact
`K_q = A_-q A_q - rotor_count I` phase-factor identity but changes the
custody schedule. Each necklace coordinate is computed into one resident
field cell, consumed by its complete public rotation/reflection fanout, and
cleared before the next coordinate. Momentum type, local generation, and
necklace ownership bind every lease to its consumer. The same scalar backing
survives forward execution, inverse rematerialization, unrelated restored
reuse, and the fresh-reuse control.

The accepted path retains no 4,389-cell momentum vector, 74,613-cell
occupation scratch, dense `2,277 x 2,277` operator, permanent assignment
expansion, transition plan, or inverse history. Primary full-state parity,
commitment, and boundary 83 agree with M199. Unrelated restored reuse boundary
70 agrees with fresh execution. Exact rematerialize-and-subtract restoration
returns the source, target, and scalar-port backings and reaches restoration
generation two without snapshot reload.

The independent oracle imports neither M202 nor M199 production. It
reconstructs the direct 684,624-term CSR, compact factor law, one-cell stream,
typed lease and ownership checks, premature-projection rejection, topology,
boundaries, restoration, and resource counts. Its retained 494,496
verification-only plan entries are explicit oracle state and are not
attributed to the accepted production path.

The accepted active numeric state is 6,832 field cells: 2,277 each for source,
target, and output, plus one scalar port. Public predecessor topology
contributes 96,723 descriptor integers and the bracelet lookup contributes
2,277, giving 105,832 named field-and-descriptor slots. This saves only 2,111
slots against comparable M199 accounting. One forward scattering rematerializes
1,193,808 source-orbit rotations and 5,534,928 inverse move candidates to
accept 331,704 one-body contributions.

Classification is `INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at
`INDEPENDENT_ORACLE_REEXECUTION` with
`EXACT_ALGEBRAIC_RESTORATION`. Python containers, hash-table and big-integer
overhead, allocator/interpreter/native-library memory, timing, and
whole-process peaks are excluded, not zero. The strongest matched baseline is
the identical one-cell topology-rematerialized classical stream.

Exact ceiling:

```text
GRID17_EXCHANGE_SYMMETRIC_ROTATION_REFLECTION_INVARIANT_ROTOR6_F103_ROOT72_DEPTH1_PRIMARY_REUSE_DIRECT_PROCESS_ONE_CELL_SCALAR_OPEN_MOMENTUM_STREAM_ONLY
```
...
```text
EXACT_F103_ROTOR6_IMPLICIT_DIHEDRAL_CODED_ONE_CELL_OPEN_MOMENTUM_STREAMING_CLOSURE_OR_TOPOLOGY_WORK_NO_GO_WITHOUT_RETAINED_NECKLACE_HISTOGRAMS_HASH_MAPS_REFLECTION_MAPS_OR_TRANSITION_PLANS_AND_WITH_IDENTICAL_CLASSICAL_BASELINE
```

The successor preserves the one-cell phase port but removes retained necklace
histograms, hash maps, reflection maps, and transition plans in favor of
on-demand canonical dihedral code arithmetic. It must count canonicalization,
search, rematerialization, topology compilation, inverse, verification, and
reuse. It is a concrete topology-state repair, not authorization for larger
...
M220 applies exact rank-one line searches along seven public cyclotomic-unit
directions in `Q(zeta_40)`. Every accepted gauge stores the complete signed
unit-exponent ledger, and selection minimizes the exact residual-plus-ledger
payload using the trace energy across all sixteen embeddings. Every nonzero
M219 state in the declared cases has primitive integer-coordinate content, so
ordinary common-integer extraction cannot account for the reduction.

At four strands and depths two, three and four the resident reductions before
constant skein scratch are 59, 439 and 4,043 bits. Six strands at depth two
saves 1,150 bits, and eight strands at depth one saves 38 bits. The primary
four-strand depth-four case falls from 57,586 raw bits to 53,550
residual-plus-ledger bits.

The accepted implementation nevertheless materializes the raw actual vector
before every operation and materializes trial residual vectors during gauge
search. The primary lifecycle performs 24 balances, materializes 26 raw and
105 candidate vectors, evaluates 1,978 exact embedding traces and reaches a
conservative declared live maximum of 278,984 bits. This is larger than the
57,586-bit raw carrier, so full-lifecycle height reduction is not established.

The primary and unrelated reuse transactions restore the actual residual,
seven-cell ledger and skein scratch backings exactly at generations one and
two without reload. A standalone exact implementation imports no M220
production and reproduces all state, boundary, gauge, resource, restoration
and reuse tuples. Classification is `INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at
`SEPARATE_REFERENCE_PARITY` with `EXACT_ALGEBRAIC_RESTORATION`.

Exact ceiling:

```text
FORMAL_PRETRUNCATION_QZETA40_M219_CUBIC_SKEIN_UNIT_PARAMETERS3_7_9_11_13_17_19_RANK_ONE_EXACT_TRACE_LINE_SEARCH_FAMILY0_STRANDS4_DEPTH1TO4_STRANDS6_DEPTH1TO2_STRANDS8_DEPTH1_PRIMARY4_DEPTH4_REUSE4_DEPTH2_FAMILY1_DIRECT_PROCESS_ONLY
```

The strongest matched classical baseline is the identical
residual-plus-unit-ledger link-pattern cubic-skein gauge recurrence. Python
container capacity, allocator internals, serialization, timing and whole
process peaks remain excluded rather than zero. No global unit-lattice
optimum, asymptotic height bound, CATVM custody, distinct phase resource,
computational advantage, Small Wall crossing, physical execution,
physical-bit replacement, catalytic inference or unbounded computation is
established.
...
This is a direct repair of the measured transient obstruction. Linear skein
updates can act on the residual under a common unit ledger, while the cubic
term requires only the ledger-relative squared unit action. Candidate scores
must be streamed and only the selected net unit action may mutate the
...
Every accepted unit exponent is executed by multiplying the current exact
accumulator directly by one fixed public descriptor `abs(exponent)` times.
M226 retains no binary-square factor, standalone power result, terminal outer
factor multiplication, power table or power cache. Its independently
reconstructed compiled descriptor contains 28 exact field cells carrying
1,802 exact payload bits and seven public parameter integers carrying 34 bits.

The primary four-strand depth-four peak falls by 6,119 bits, from M225's
300,940 to 294,821, but remains above the matched raw 129,595-bit recurrence.
All seven cases remain above raw and peak at
`DIRECT_PUBLIC_UNIT_TRACE_WEIGHT_STEP`, where the fixed public factor, old
height-growing accumulator and new product coexist. M225's 29,945 primary
binary multiplications, including 17,842 squares, become 275,150 counted
direct steps across 4,709 nonzero applications, with maximum absolute exponent
1,024. This is an explicit time-for-live-state trade, not a computational
advantage.

The standalone reference imports none of M226 through M221 production and
reproduces every case, normalized work tuple, state and boundary commitment,
descriptor metric, restoration and reuse result. Exact inversion retains the
same residual, ledger and scratch backings, uses no baseline reload, reaches
reuse generation two, and agrees with fresh reuse. Classification is
`INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at `SEPARATE_REFERENCE_PARITY` with
`EXACT_ALGEBRAIC_RESTORATION`.

Direct-action intervals count the exact old accumulator, fixed factor, new
product, exponent, step and surviving caller integers. Retained final-boundary
liveness during inverse remains outside this local instrumentation, so M226
does not claim whole-transaction live-payload completeness. Python object and
container overhead, allocator/interpreter state, JSON serialization, timing
and whole-process RSS remain excluded rather than zero. The strongest compact
classical comparator is the identical direct repeated coordinate recurrence.
...
Compile the seven public unit multiplication maps and one conjugation map into
deterministic reversible swaps, signs and integer shears on the 16-coordinate
cyclotomic basis. Compose norm actions from those primitive plans rather than
decomposing dense norm matrices. Count compilation matrices, retained plan
storage, plan references, every rational coordinate temporary and warm work;
release compiler matrices before execution. Preserve final-only exact
...
## M236: connected cubic stationary custody exposes doubling quotient dimension
...
M236 composes a connected chain of formal cubic generating signatures.  The
stationary recurrence builds an exact endpoint polynomial `P_n`; the quotient
`Q[t]/P_n` retains every declared stationary branch implicitly without roots,
assignments, or branch lists.  Three actual quotient-algebra backings carry the
two phase-space coordinates and scaled critical value.  Only the exact
characteristic/norm relation crosses the final boundary.

Across lengths `2,3,4,5,6`, endpoint quotient dimensions and final relation
degrees are exactly `2,4,8,16,32`.  Family-zero relation payload grows
`11,39,144,542,2114` bits and family one grows
`9,30,121,477,1759` bits.  Endpoint and critical-value relations are square
free in every declared fixture, all stationary equations vanish in the
endpoint quotient, and the projected relation annihilates the resident action.

The public reverse program restores the exact same `r`, `s`, `h`, and modulus
backings without a retained initial-state snapshot or dynamic inverse history.
Generation advances from one to two and an unrelated reversible sentinel
consumes the actual restored carrier; fresh/restored boundaries agree and no
reload occurs.  The matched classical reconstruction runs only after release,
so it is not retained as an answer-bearing copy during inverse.

A standalone sparse-dictionary implementation reconstructs quotient arithmetic,
the characteristic relation, custody, controls, restoration, and reuse.
Focused review returned `PASS_STRICT_SCOPE` after canonical descriptor binding,
removal of false zero counters, and disclosure of the stronger unimplemented
subresultant/PRS projection.  The milestone is
`INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at `SEPARATE_REFERENCE_PARITY` with
`EXACT_ALGEBRAIC_RESTORATION`.

The result retires this cubic critical-locus route as a compact-resource
candidate.  The accepted quotient basis doubles, while the exact public
stationary recurrence remains an `O(length)` expression DAG.  The strongest
known compact classical final projection is a chain-specific univariate
subresultant/PRS resultant, not the package's matrix norm, so no projection
resource comparison is authorized.  Resource figures remain
`PACKAGE_SELF_REVIEW`, whole-transaction/process liveness is incomplete, and
no distinct phase resource or advantage is established.
...
This successor retains normalized coherent amplitudes and exact interference,
which M234--M236 discarded through projective or critical-locus projection.
It must use an exact `Q(zeta_5)` five-amplitude carrier, demonstrate causal
cubic destructive interference against a dephased control, preserve full
amplitude through exact inverse restoration/reuse, and compare against the
...
## M239: coherent shared-syndrome consumption retains a fixed one-injection ceiling
...
M239 replaces the resident Wigner grid with an exact `Q(zeta_5)` two-qudit
amplitude carrier and one coherent unresolved five-valued syndrome.  The
magic-injection identity for cubic strengths one and two is consumed by three
fiberwise stabilizer corrections and by public noncommuting `SUM`/`CZ` words.
The same resident syndrome backing is read by all consumers.  Removing either
entangling module, perturbing the public word, or dephasing the syndrome changes
the declared final data-probability boundary in every accepted family.  No
intermediate syndrome value is projected.

The production transaction and standalone polynomial-quotient reference agree
on all six family/pair-count cases, controls, exact boundaries, commitments,
restoration, and descriptor-distinct reuse.  The actual 25-amplitude and scratch
backings restore exactly, generation advances one to two, and no baseline reload
occurs.  Focused review returned `PASS_STRICT_SCOPE`, so the milestone is
`INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at `SEPARATE_REFERENCE_PARITY` with
`EXACT_ALGEBRAIC_RESTORATION`.  Custody remains direct-process logical custody,
not CATVM enforcement.

The causal coherent-port result is not growing magic leverage.  The complete
one-injection identity has an exact five-stabilizer-component upper bound for
consumer-pair counts one, two, and four.  The strongest implemented classical
comparator compiles that identity and uses 31 exact field values, versus 50
resident phase/scratch backing cells.  The five-branch dephasing comparison is
an enumerated non-restoring diagnostic excluded from the accepted in-place
path.  Resource figures remain `PACKAGE_SELF_REVIEW`; stabilizer-component
resource work is not instrumented and whole-transaction liveness is incomplete.

M239 does not establish a growing compositional magic cost, distinct phase
resource, computational advantage, CATVM custody, Small Wall crossing,
physical execution, physical-bit replacement, inference, or unbounded
computation.  Do not add longer words to the same one-injection construction.
...
This one bounded successor must decide whether interacting injections make the
magic cost compose or whether a compact stabilizer-sum/classical recurrence
collapses the network.  It must retain unresolved syndrome interfaces, use the
...
## M242: tensor factorization repairs state growth but closes the separable linear route
...
scratch with an exact rank-one product-phase representation. Across declared
dimensions `1,2,4,8,16,32`, the accepted carrier and scratch each use
`5,10,20,40,80,160` Q(zeta5) cells. It preserves one abstract coherent
forward query, exact same-backing inverse restoration, generation-two
descriptor-distinct reuse, and atomic final response release. Exact dense
semantic parity is independently reconstructed through dimensions `1,2,4`.

The service exposes no pre-run secret receipt, secret-dependent response
commitment, or intermediate factor state. Disconnect, post-projection failure,
and a partial oracle failure after 17 factors all restore before rejection or
response. The exact arithmetic dependency is seal-pinned, and the private
configuration is delivered only to the service and standalone verifier over
stdin, never to the public controller. The package passed focused review at
`SEPARATE_REFERENCE_PARITY` with `EXACT_ALGEBRAIC_RESTORATION`.

The repair does not create a total resource advantage. Forward plus inverse
oracle execution reads `2n` hidden residues and evaluates `100n` exact Fourier
character terms, while the strongest direct private-descriptor classical
baseline reads `n` residues and runs in `O(n)`. Resource figures are
`PACKAGE_SELF_REVIEW`; Python objects, allocator state, socket-kernel state,
hashing, scheduling, and whole-process RSS remain excluded but nonzero.

M242 therefore establishes neither a nonseparable phase resource nor total
advantage, Small Wall crossing, inference, physical waveform execution,
physical-bit replacement, or unbounded catalytic computation. It closes the
separable linear-oracle route rather than inviting larger dimensions.
...
M243 changes the phase law rather than optimizing the factor carrier. It tests
connected invertible symmetric quadratic forms plus one multi-coordinate cubic
functional over F5 at declared widths `2,3,4,6,8,12,16`. The exact candidate
collapse is the five-channel Gauss quotient determined by dimension,
discriminant, `u^T A^-1 u`, and cubic strength. The accepted transaction must
retain hidden descriptor and channel custody, final-only amplitude projection,
exact same-backing restoration/reuse, and the strongest identical modular
LDL-plus-five-term classical evaluator. Dense `5^n` summation is verifier-only
through `n<=4`. If the quotient collapse holds, stop this rank-one cubic family;
only growing independent cubic-direction rank with a lawful compact signature
may continue the route.

## M243: one cubic direction collapses to a fixed five-channel Gauss quotient
...
M243 executes connected regular quadratic forms plus one multi-coordinate
cubic functional over F5 at widths `2,3,4,6,8,12,16`. Exact completion of the
quadratic form reduces the full coherent sum to five Q(zeta5) channels indexed
by the cubic Fourier coordinate and determined by dimension, determinant square
class, `delta = u^T A^-1 u`, and cubic strength. The production no-pivot
in-place LDL chart agrees with independently pivoted modular elimination at
every width and with independent dense exact summation only through its declared
verifier ceiling `n<=4`.

The abstract Unix-socket CATVM service retains the packed symmetric matrix,
direction, solve vector, quotient scalars, and coherent channels internally.
Projection exposes only the declared final amplitude. Disconnect, partial-LDL,
and post-projection failures restore before rejection or response. The actual
packed backing is reversed in place, exact state is verified, and the same
backings advance from generation one to generation two for descriptor-distinct
reuse. Focused review returned `PASS_STRICT_SCOPE` at
`SEPARATE_REFERENCE_PARITY` with `EXACT_ALGEBRAIC_RESTORATION`.

The quotient is an obstruction, not leverage. The accepted path retains
`12,20,30,56,90,182,306` residue cells when mutable carrier and hidden CATVM
configuration are both counted, and it reads that hidden descriptor again
during inverse. Five logical channels do not imply bounded-width exact state:
the public denominator-power bounds grow `3,4,5,7,9,13,17`, and numerator and
denominator material bounds grow with width. Resource figures remain
`PACKAGE_SELF_REVIEW`, with whole-process Python, allocator, socket, hashing,
and scheduler costs incomplete.

The strongest implemented classical comparator is the identical compact
modular LDL solve plus five-term Q(zeta5) evaluator. M243 therefore establishes
neither a rank-one escape from the fixed Gauss quotient nor standard one-shot
inference, a general nonlinear phase resource, total advantage, Small Wall
crossing, physical execution, bit replacement, or unbounded catalytic
computation. The rank-one cubic family is closed; larger widths in that family
are not a valid successor.
...
M244 uses multiple independent unresolved F5 cubic ports coupled as a connected
treewidth-one chain. Formula-rematerialized exact five-state transfer maps act
directly on one resident phase message at depths `2,3,4,8,16,32,64`; the final
scalar is retained while the public reverse word restores the same backing.
The strongest comparator is the identical five-vector recurrence, including
exact-payload growth. One independently certified public cross-rank-two topology
has interface rank 25 and prevents promotion of the five-cell chain law to
...
## M245: exchange-symmetric rank-two orbit quotient and scoped labelled no-go
...
M245 changes the cross-rank-two representation rather than materializing all
25 labelled amplitudes. Exact rail-exchange symmetry identifies `(a,b)` with
`(b,a)` and closes the declared two-rail cubic transfer on the 15 unordered
pairs in `F5^2`. The CATVM service retains one 15-cell message and one 15-cell
scratch backing, projects only the selected final orbit amplitude, reverses
all four modules, verifies exact canonical state, and releases only after
restoration. A descriptor-distinct generation-two transaction consumes the
same actual backings.

An independent Q(zeta5) implementation reproduces the orbit recurrence and
the full labelled 25-state recurrence at every stage. Over both split primes
41 and 61, the declared symmetric prefix/suffix language has reachability,
observability, and Hankel rank 15. Thus no transferable linear realization
below 15 preserves that language. The first rung has labelled matrix rank 5,
rejecting product and matrix-bond ranks below 5. These are linear and
matrix-factor ceilings only; arbitrary nonlinear encodings are not rejected.

The separately persisted exchange-broken `A/B/D` labelled alphabet has
reachability and observability rank 25 at both split primes. That result is
restricted to those exact three gates, whose descriptors are sealed. M245
does not claim that every exchange-breaking perturbation forces rank 25.

The accepted forward uses 1,140 character terms and the exact inverse uses
1,500. The strongest implemented selected-boundary classical comparator uses
a 15-cell orbit vector plus 25 temporary cells and 440 forward-only terms.
The identical direct orbit comparator uses the same 30 message-plus-scratch
cells as the CATVM carrier and 1,140 forward terms. The result is therefore a
time-memory tradeoff with no total advantage. Whole-process and kernel-
transient resource accounting remains incomplete and is not represented as
zero.

Focused adversarial review returned `PASS_STRICT_SCOPE`, classified
`INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at `SEPARATE_REFERENCE_PARITY` with
`EXACT_ALGEBRAIC_RESTORATION`. Disconnect, partial-forward failure,
post-projection failure, wrong and reordered inverses, descriptor mutation,
wrong orbit multiplicity, forbidden projection, stale generation, snapshot,
and no-reload reuse controls pass.

No fixed bounded-width exact state, arbitrary graph closure, arbitrary
nonlinear lower bound, distinct phase resource, computational advantage,
Small Wall crossing, physical waveform execution, physical-bit replacement,
general inference, or unbounded catalytic computation is established.
...
M246 must derive one transferable permutation-orbit composition law across
rail count, not add M245 word or depth fixtures. It must compare exact
occupation rank, coefficient payload, and contraction work with the strongest
...
## M251: mixed-state operator coherence is exact but classically compact
...
Production and the standalone `Q[zeta8]/(zeta8^4+1)` oracle independently
reconstruct one clean control coupled to a maximally mixed two-qubit carrier:
...
The abstract Unix-socket CATVM retains the actual 64-cell density backing and
four-cell scratch. Accepted responses contain only the final normalized trace;
pre-inverse correlation and marginal predicates are reconstructed by the
verifier, not released. Every controlled gate is inverted on the same backing,
canonical state is checked exactly, and descriptor-distinct generation-two
reuse agrees with fresh execution without a baseline reload. The public grammar
also accepts scalar words, so nonproductness is evidence for the selected
fixtures rather than an undeclared transaction requirement.

Focused review returned `PASS_STRICT_SCOPE`, classified
`INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at `SEPARATE_REFERENCE_PARITY` with
`EXACT_ALGEBRAIC_RESTORATION`. The final repairs make transaction setup
rollback-safe, reject malformed non-string identifiers before lease, and keep
all forward diagnostics inside the verifier.

The strongest fixed-fixture comparator validates the public word and returns
its frozen exact normalized trace in `O(1)` work. The strongest transferable
descriptor comparator evolves one exact `4 x 4` public-word matrix in 16
resident field cells plus declared multiplication scratch and takes its trace
without CATVM inverse work. Both are smaller than the accepted density path.
This establishes no distinct phase resource, computational or query advantage,
Small Wall crossing, DQC1 hardness, physical mixed-state execution, inference,
...
## M254 full-even Grassmann relation closure broadens the algebra but not the resource
...
The abstract Unix-socket CATVM retains the complete even exterior signature on
four typed Grassmann ports: one scalar, six pair coefficients, and one
independent quartic coefficient. Signed wedge multiplication is the native
intersection. The four-port Berezin kernel independently derives the exact
signed-complement Hodge involution. For a public factor `g=s+n`, `s!=0`, the
actual inverse is rematerialized as `s^-1(1-n+n^2)` from the public descriptor;
the accepted non-Gaussian factor requires the `n^2` term. Only the final
top-form coefficient leaves the service after exact reverse execution,
canonical verification, and release. The same eight-cell backing then supports
descriptor-distinct generation-two reuse with no reload.

The independent `Q[zeta8]/(zeta8^4+1)` oracle reconstructs generic bitmask
exterior multiplication, the eight-generator Berezin kernel, the finite
nilpotent inverse, and an independent custody state machine. It reproduces the
accepted boundaries and work records and certifies exact reachable,
observable, and Hankel ranks eight for the declared linear prefix/suffix
language. Focused review returned `PASS_STRICT_SCOPE`, classified
`INDEPENDENTLY_VERIFIED_STRICT_SCOPE` at `SEPARATE_REFERENCE_PARITY` with
`EXACT_ALGEBRAIC_RESTORATION`.

This is a genuine algebraic advance beyond M253: the independent quartic
coordinate violates the Gaussian pure-spinor identity, survives Hodge closure,
and causally changes the final boundary. It is not a compact wide-interface
advance. The strongest transferable comparator is the identical eight-cell
in-place exterior recurrence with one reusable scratch cell and no CATVM
restoration. The full even algebra on `m` ports has dimension `2^(m-1)`. M254
therefore establishes one bounded non-Gaussian open-relation machine law, not a
distinct phase resource, computational advantage, general Grassmann closure,
Small Wall crossing, physical execution, physical-bit replacement, inference,
or unbounded computation. Do not add more four-port full-exterior fixtures.
...
The successor attacks the actual M254 obstruction instead of enlarging its
fixture. It must test whether a structured Gaussian-plus-decomposable-quartic
cumulant chart closes under native intersection and Berezin/Hodge composition
as port count grows, or whether exact cumulant rank/support necessarily grows
toward the full `2^(m-1)` even signature. It must preserve unresolved relation
structure, exact restoration, and reuse, and compare against the strongest
matched Pfaffian plus exterior-tensor classical recurrence. A finite compact
chart may be claimed only if its parameters, coefficient height, inverse
history, and projection work all remain compact; otherwise publish the rank
growth obstruction and change mechanism.

## M255 quartic-cumulant chart fails at the minimal six-port interface
...
The compact accepted carrier stores `C(m,2)` rational quadratic coefficients
and one, two, or three decomposable quartic cumulants at declared port counts
4, 6, and 8. Native relation intersection is exact cumulant addition. Selected
Berezin/Hodge boundaries are rematerialized through public subset Pfaffians and
streamed even set partitions; no full 8/32/128-cell even signature is retained
on the accepted path.

The independent full-exterior oracle derives exact connected top cumulants
`1/4`, `-2/27`, and `14/625`. At six ports it accumulates the actual two
quartic supports, complement-dualizes them to a two-form, and finds a nonzero
dual-square coefficient `2` on support `[0,1,4,5]`. The Pluecker lower bound
and the displayed two-term upper bound give exact quartic rank two. Thus the
rank-one degree-four chart fails minimally at six ports; eight ports confirms
that Hodge closure reaches a nonzero connected degree-eight cumulant.

Forward intersections are reversed on the actual pair/quartic backings. Exact
canonical restoration, descriptor-distinct generation-two reuse, fresh parity,
and no baseline reload pass. This is direct-process logical custody, not CATVM
machine enforcement. The strongest fixed-fixture comparator is an `O(1)`
public certificate; a two-field monomer-dimer continuant computes the Hodge
normalization scalar, and the descriptor-level selected-cumulant comparator is
the identical streamed Pfaffian/set-partition recurrence with no optimality
claim. No distinct phase resource or advantage is established. The quartic
chart is retired after this eight-port confirmation.
...
Change the primitive from exterior cumulants to a formal unresolved all-pass
waveform phase `f(z)=N(z)/D(z)`. Public lossless Schur feedback sections must
act natively by exact Mobius composition, retain the polynomial phase relation
inside CATVM, project only a declared winding and one exact boundary
evaluation, then execute the exact inverse on the same backing and reuse it.
The decisive controls compare against both the identical two-polynomial
...
M270 implements V12 as an actual compiled QEMU 10.2.4 PCI device with the
complete V11 register prefix, one common guest-visible ABI, and swappable
IDEAL, OPEN, and EXTERNAL backend operations. The EXTERNAL path remains
service-default unavailable. Only the explicitly enabled hardware-disconnected
test provider exercises its deterministic two-slot asynchronous protocol.

The test adapter authenticates only a noncryptographic 64-bit envelope with a
43-bit payload and deterministic 21-bit integrity tag. All nine internal
`Z(omega)` pairs are executed, but successful external-stub receipts are
classified only `APPROX_MODEL`. They remain held until acknowledgement, then
become `SPENT`; they can never authorize begin reuse. Order, expiry, cancel,
timeout, sealed failed receipts, acknowledgement, spent state, pending reset,
source migration, second-hop migration, and teardown sanitization are exercised
through the compiled common-device lifecycle. Reset and real QMP migration
remain irreversible `SHAM`, not restoration.
...
The compiled V12 source, installer, qtest, separate reference, contract,
findings, build receipt, and qualifier are source-pinned. The qtest validates
the common V11 prefix, invalid MMIO, authentication negative controls,
two-slot scheduling, deterministic integrity, resource-digest recomputation,
failure receipts, reset, migration, and clean teardown. The separate reference
recomputes the nine internal model pairs and direct equal-access phase compiler.
Every external physical quantity remains explicitly unknown.
...
`terminal=false`. It passes common compiled Phase-QEMU device/backend
reintegration for the adapter protocol. It establishes no hardware connection,
physical carrier or coherent port, cryptographic security, statistical physical
evidence, custody, physical return, restoration, reuse, advantage, Small Wall
crossing, or M257 escape. V0, V1, V11, and V12 are compiled common guest-visible
devices; V2 through V10 remain mechanism twins that may kill or nominate only.

The exact M271 successor token is:
...
That successor must stay behind the common compiled device/backend boundary.
Actual external physical connection and execution remain outside current
authority.

## M271 compiled Phase-QEMU V13 hardware-adapter selector gate
...
M271 implements V13 as an actual compiled QEMU 10.2.4 PCI device preserving
the complete V12 register prefix and common swappable backend boundary. The
production backend rejects hardware absence before dispatch. The explicitly
enabled fixture backend is only a fixed offline seven-choice selector for
absent, unenrolled, unattested, stale, replayed, downgraded, and test-fixture
failures. It does not parse or verify real enrollment, attestation,
cryptographic, replay-cache, manifest, or measurement evidence.

Every ordinary nondestructively completed fixture dispatch seals `FAILED`,
requires acknowledgement, and ends `SPENT`. Reset, migration, and unrealize
instead burn compiled lineage to `SHAM` without ACK. The separate-reference
disconnect case is likewise symbolic destructive `SHAM`; V13 has no compiled
live-disconnect input. No response publishes a physical output, authenticated
physical sample, or campaign statistical certificate.
...
The compiled C source, no-follow installer, qtest, separate symbolic
reference, contract, findings, build receipt, and strict qualifier are pinned.
The qtest confirms the complete V12 register prefix, hardware-absence
predispatch rejection, seven typed fixture failures, terminal receipts,
acknowledgement/spent ordering, destructive reset and two-hop migration sham,
resource-digest integrity, invalid ABI handling, and common-device
reintegration. The independent reference checks the broader 17-case symbolic
truth table without claiming QEMU execution, evidence parsing, cryptography,
...
`SEPARATE_REFERENCE_PARITY`, `PACKAGE_SELF_REVIEW` resource authority, and
`terminal=false`. The common compiled Phase-QEMU device/backend reintegration
gate passes for the hardware-absence selector. Hardware connection and
enrollment, an authenticated live-device session, physical samples, a campaign
statistical certificate, custody, physical return, restoration, reuse,
advantage, Small Wall crossing, and M257 escape all remain false. V0, V1, V11,
V12, and V13 are compiled common guest-visible devices; V2 through V10 remain
mechanism-search twins that may kill or nominate only.

The exact M272 successor token is:
...
That successor remains behind V13's common compiled device/backend boundary
and requires explicit user authorization plus enrolled physical hardware and a
preregistered independent campaign. No connection, enrollment, measurement,
or physical campaign execution is currently authorized.
...
claim ceiling
C-rung position
best matched compact baseline
highest-leverage unresolved blocker
next selected work
explicit terminal = false
```

Do not allow the roadmap to fall behind the evidence again.

## 11. Git and Physical Boundaries
