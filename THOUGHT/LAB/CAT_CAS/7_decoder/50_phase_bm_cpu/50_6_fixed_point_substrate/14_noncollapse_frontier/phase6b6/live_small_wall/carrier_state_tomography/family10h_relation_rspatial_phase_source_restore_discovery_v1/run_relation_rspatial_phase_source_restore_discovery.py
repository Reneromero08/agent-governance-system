...
RUN_ID = "family10h_relation_rspatial_phase_source_restore_discovery_v1_0"
...
MATCHED_PERMUTATION_SEED = "family10h-rspatial-phase-source-restore-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "RSPATIAL_PHASE_SOURCE_RESTORE_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_rspatial_full_schedule_lifetime_reset() -> None:
    ...
    text = text.replace(
        ...
        "                ? \"source_dead_then_double_flush_then_source_cpu_restore_same_relation_before_spatial_pair_probe\"\n"
        "                : (restore_opposite_after_reset_control(row.control)\n"
        "                    ? \"source_dead_then_double_flush_then_source_cpu_restore_opposite_relation_before_spatial_pair_probe\"\n"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"RSPATIAL_PHASE_SOURCE_RESTORE_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
    ...
    write_json(SOURCE_ROOT / "RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/RSPATIAL_PHASE_SOURCE_RESTORE_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "interpretation": {
            ...
            "phase_source_restore_candidate": (
    ...
    write_json(RUN_ROOT / "RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- phase source-restore candidate: `{result['interpretation']['phase_source_restore_candidate']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_RSPATIAL_PHASE_SOURCE_RESTORE_PREPARE_RESULT.json", result)
    return result
