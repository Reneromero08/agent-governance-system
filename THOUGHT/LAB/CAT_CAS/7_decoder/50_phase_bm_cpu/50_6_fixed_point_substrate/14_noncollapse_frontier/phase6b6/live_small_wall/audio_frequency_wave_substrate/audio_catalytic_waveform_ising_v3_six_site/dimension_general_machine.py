...
TARGET_SITE_COUNT = 6
SITE_COUNT = TARGET_SITE_COUNT
MODE_COUNT = 1 << SITE_COUNT
COMPLEX_SAMPLES_PER_MODE = 8
SAMPLE_COUNT = COMPLEX_SAMPLES_PER_MODE * MODE_COUNT
SAMPLE_RATE_HZ = 48_000
ACTIVE_BIN_START = 8
ACTIVE_BINS = np.asarray(
    tuple(range(ACTIVE_BIN_START, ACTIVE_BIN_START + MODE_COUNT)), dtype=np.int64
)
GEOMETRY_ANCHOR_OFFSET = 11
GEOMETRY_ANCHOR_STRIDE = 7
GEOMETRY_SAMPLES = np.asarray(
    tuple(
        (GEOMETRY_ANCHOR_OFFSET + GEOMETRY_ANCHOR_STRIDE * index) % SAMPLE_COUNT
        for index in range(MODE_COUNT)
    ),
    dtype=np.int64,
)
RELATION_PHASE_DENOMINATOR = 2 * MODE_COUNT
...
def maximum_penalty_bound(
    site_count: int,
    maximum_abs_coupling: float,
    maximum_abs_field: float,
) -> float:
    return float(
        site_count * (site_count - 1) * maximum_abs_coupling
        + 2.0 * site_count * maximum_abs_field
    )

@dataclass(frozen=True)
class SpectralPhaseLaw:
    relation_phase_scale: float = math.pi / RELATION_PHASE_DENOMINATOR
    ...
    def validate(self) -> None:
        ...
        maximum_penalty = maximum_penalty_bound(
            SITE_COUNT,
            self.maximum_abs_coupling,
            self.maximum_abs_field,
        )
...
def validate_problem(
    ...
    if coupling.shape != (SITE_COUNT, SITE_COUNT) or field.shape != (SITE_COUNT,):
        raise ValueError("six-site problem shape required")
...
def machine_contract(law: SpectralPhaseLaw = DEFAULT_LAW) -> dict[str, Any]:
    return {
        ...
        "dimension_law": {
            "active_bin_start": ACTIVE_BIN_START,
            "complex_samples_per_mode": COMPLEX_SAMPLES_PER_MODE,
            "geometry_anchor_offset": GEOMETRY_ANCHOR_OFFSET,
            "geometry_anchor_stride": GEOMETRY_ANCHOR_STRIDE,
            "maximum_penalty_bound": metric(
                maximum_penalty_bound(
                    SITE_COUNT,
                    law.maximum_abs_coupling,
                    law.maximum_abs_field,
                )
            ),
            "phase_denominator": RELATION_PHASE_DENOMINATOR,
            "phase_denominator_law": "2 * mode_count",
            "sample_count_law": "8 * mode_count",
        },
        ...
        "source_closure": "self-contained dimension-general V3 source plus frozen CPython/NumPy environment",
