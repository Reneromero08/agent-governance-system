#!/usr/bin/env python3
"""Deploy, run, copy back, verify, and clean one balanced-transducer calibration."""
...
FILES = {
    HERE / "BALANCED_TRANSDUCER_CONTRACT.md": "BALANCED_TRANSDUCER_CONTRACT.md",
    HERE / "FIRST_LIGHT_TRANSDUCER_AUDIT.md": "FIRST_LIGHT_TRANSDUCER_AUDIT.md",
    HERE / "PUBLIC_TRIAL_SCHEDULE.json": "PUBLIC_TRIAL_SCHEDULE.json",
    HERE / "PUBLIC_TRIAL_SCHEDULE.sha256": "PUBLIC_TRIAL_SCHEDULE.sha256",
    HERE / "PUBLIC_TRIAL_SCHEDULE.tsv": "PUBLIC_TRIAL_SCHEDULE.tsv",
    HERE / "balanced_transducer_target.py": "balanced_transducer_target.py",
    HERE / "balanced_transducer_public.py": "balanced_transducer_public.py",
    HERE / "balanced_transducer_runtime.c": "balanced_transducer_runtime.c",
    HERE / "balanced_transducer_runtime.h": "balanced_transducer_runtime.h",
}
...
def verify_copy(local_root: Path) -> dict[str, Any]:
    ...
    require(manifest["schema_id"] == "CAT_CAS_BALANCED_TRANSDUCER_FILE_MANIFEST_V1", "manifest schema mismatch")
...
def default_run_id() -> str:
    return dt.datetime.now(dt.timezone.utc).strftime("balanced_transducer_%Y%m%dT%H%M%SZ")
...
def execute(run_id: str, *, keep_remote: bool) -> dict[str, Any]:
    ...
    remote_command = (
        ...
        f"{shlex.quote(remote_source + '/balanced_transducer_target.py')} "
    ...
    copied = run(["scp", "-q", "-r", f"{TARGET}:{remote_output}/.", str(local_run)], timeout=90, check=False)
    ...
    adjudication = (
        json.loads((local_run / "TRANSDUCER_ADJUDICATION.json").read_text(encoding="utf-8"))
        if (local_run / "TRANSDUCER_ADJUDICATION.json").is_file()
        else None
    )
    controller = {
        "schema_id": "CAT_CAS_BALANCED_TRANSDUCER_CONTROLLER_V1",
        "run_id": run_id,
        "target": TARGET,
        ...
        "primary_coordinate": None if adjudication is None else adjudication["primary_coordinate"],
        "eligible_coordinates": None if adjudication is None else adjudication["eligible_coordinates"],
        "raw_capture_sha256": final.get("raw_capture_sha256"),
        "restoration_sentinels_sha256": final.get("restoration_sentinels_sha256"),
        "public_trial_schedule_sha256": final.get("public_trial_schedule_sha256"),
        "source_bundle_sha256": final.get("source_bundle_sha256"),
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"run_balanced_transducer_calibration: {exc}", file=sys.stderr)
        return 1
    return 0 if result["final_status"] == "BALANCED_TRANSDUCER_TARGET_COMPLETE" else 1
