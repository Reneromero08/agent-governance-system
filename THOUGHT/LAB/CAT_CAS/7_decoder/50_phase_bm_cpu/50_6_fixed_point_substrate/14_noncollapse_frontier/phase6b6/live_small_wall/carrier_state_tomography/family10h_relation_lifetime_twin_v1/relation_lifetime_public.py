#!/usr/bin/env python3
"""Public relation-lifetime matched-permutation build-readiness helpers.

The package freezes a future Family 10h relation-lifetime experiment, but this
...
SCIENCE_PACKAGE_ID = "family10h_relation_lifetime_twin_v1"
TRANSACTION_RUN_ID = "family10h_relation_lifetime_twin_v1_0"
PUBLIC_RANDOMIZATION_SEED = "family10h-relation-lifetime-twin-v1-public-seed-7d2f0c91"
...
SCALAR_EVIDENCE_PROVENANCE = {
    ...
    "role": "sealed scalar q-readout evidence provenance for threshold derivation only",
}
...
DEPLOYMENT_CUSTODY_FILENAME = "RELATION_LIFETIME_DEPLOYMENT_CUSTODY.json"
SENSOR_AUTHORITY_BINDING_FILENAME = "RELATION_LIFETIME_SENSOR_AUTHORITY_BINDING.json"
...
OWNED_OUTPUT_PARENT_NAME = "_relation_lifetime_owned_outputs"
ATTEMPT_CEILING = 1

PACKAGE_DECISION_BLOCKED = "FAMILY10H_RELATION_LIFETIME_BUILD_READINESS_BLOCKED"
PACKAGE_DECISION_BUILD_READY = "FAMILY10H_RELATION_LIFETIME_BUILD_READY_AWAITING_AUTHORIZATION"

FUTURE_RESULT_CLASSES = [
    "FAMILY10H_RELATION_LIFETIME_COORDINATE_CONFIRMED_PROSPECTIVE",
    "FAMILY10H_RELATION_LIFETIME_COORDINATE_NOT_CONFIRMED_PROSPECTIVE",
    "FAMILY10H_RELATION_LIFETIME_COORDINATE_CANDIDATE_PROSPECTIVE",
    "FAMILY10H_RELATION_LIFETIME_COORDINATE_CUSTODY_INVALID",
]
MAXIMUM_FUTURE_CLAIM = "PUBLIC_POST_SOURCE_LIFETIME_DEPENDENT_RELATION_MATCH_COORDINATE_CONFIRMED"
NEGATIVE_FUTURE_CLAIM = "PUBLIC_POST_SOURCE_LIFETIME_DEPENDENT_RELATION_MATCH_COORDINATE_NOT_ESTABLISHED"
...
SOURCE_LIFETIMES = ["alive_during_query", "dead_before_query"]
...
def relation_grammar(package_decision: str = PACKAGE_DECISION_BLOCKED) -> dict[str, Any]:
    ...
    grammar = {
        "schema": "FAMILY10H_RELATION_LIFETIME_GRAMMAR_V2",
        ...
        "primary_relation_law": {
            "name": "Delta_R_lifetime",
            "formula": "R_match(alive_during_query) - R_match(dead_before_query)",
            "positive_or_negative_allowed": True,
            "primary_endpoint": "dirty_probe_response",
            "source_lifetime_levels": SOURCE_LIFETIMES,
...
def relation_marginal_equality_proof(
    ...
    lifetime_counts = Counter(row["source_lifetime"] for row in schedule_rows) if schedule_rows else Counter()
    ...
    checks = {
        ...
        "same_window_only": all(row.get("delay_label") == "same_window" and row.get("delay_ns") == 0 for row in schedule_rows)
        if schedule_rows
        else True,
        "source_lifetime_factor_balanced": bool(lifetime_counts)
        and sorted(lifetime_counts) == SOURCE_LIFETIMES
        and len(set(lifetime_counts.values())) == 1
        if schedule_rows
        else True,
        "source_lifetime_runtime_bound": runtime_gates.get("source_lifetime_factor_implemented", False),
    ...
    proof = {
        ...
        "source_lifetime_counts": dict(sorted((str(k), v) for k, v in lifetime_counts.items())),
                        ...
                        for source_order in SOURCE_ORDERS:
                            for query_order in QUERY_ORDERS:
                                for origin in CYCLIC_ORIGINS:
                                    ...
                                    pair_id = f"lifetime_pair_{stable_token(base, 12)}"
                                    order = SOURCE_LIFETIMES[:]
                                    if int(stable_token({"lifetime_order": base}, 2), 16) % 2:
                                        order = list(reversed(order))
                                    for source_lifetime in order:
                                        conditions.append(
                                            {
                                                **base,
                                                "source_lifetime": source_lifetime,
                                                "lifetime_pair_id": pair_id,
                                                "lifetime_execution_order": "alive_first" if order[0] == "alive_during_query" else "dead_first",
...
def schedule_row_base(condition: dict[str, Any], common_hashes: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_lifetime": condition["source_lifetime"],
        "lifetime_pair_id": condition["lifetime_pair_id"],
        "lifetime_execution_order": condition["lifetime_execution_order"],
        "lifetime_hold_ns": condition["lifetime_hold_ns"],
...
def build_schedule(grammar: dict[str, Any]) -> dict[str, Any]:
    ...
    schedule = {
        "schema": "FAMILY10H_RELATION_LIFETIME_PUBLIC_SCHEDULE_V2",
        "science_package_id": SCIENCE_PACKAGE_ID,
        "transaction_run_id": TRANSACTION_RUN_ID,
        "public_randomization_seed": PUBLIC_RANDOMIZATION_SEED,
        ...
        "cyclic_origins": CYCLIC_ORIGINS,
        "source_lifetime_levels": SOURCE_LIFETIMES,
        "source_lifetime_order_law": "within each lifetime_pair_id, alive/dead execution order is frozen by public_randomization_seed and counterbalanced",
        "expected_tuple_count_derivation": {
            "sessions": len(SESSIONS),
            "replicates": len(REPLICATES),
            "mappings": len(MAPPINGS),
            "timing_conditions": len(DELAY_GRID),
            "q_values": len(Q_VALUES),
            "source_orders": len(SOURCE_ORDERS),
            "query_orders": len(QUERY_ORDERS),
            "cyclic_origins": len(CYCLIC_ORIGINS),
            "source_lifetime_levels": len(SOURCE_LIFETIMES),
            "rows_per_base_condition": 12,
            "expected_tuple_count": 21504,
        },
...
def validate_grammar(grammar: dict[str, Any]) -> dict[str, Any]:
    failures: list[str] = []
    if grammar.get("schema") != "FAMILY10H_RELATION_LIFETIME_GRAMMAR_V2":
...
def validate_schedule(schedule: dict[str, Any], grammar: dict[str, Any]) -> dict[str, Any]:
    failures: list[str] = []
    if schedule.get("schema") != "FAMILY10H_RELATION_LIFETIME_PUBLIC_SCHEDULE_V2":
    ...
    if schedule.get("tuple_count") != 21504:
        failures.append("tuple count is not the frozen 21504-row lifetime-twin schedule")
    ...
    for row in rows:
        ...
        if row.get("post_observation_scheduling") is not False:
            ...
            break
        if row.get("source_lifetime") not in SOURCE_LIFETIMES:
            failures.append("invalid source_lifetime")
            break
        if row.get("delay_label") != "same_window" or row.get("delay_ns") != 0:
            failures.append("non-immediate timing condition present")
            break
        if type(row.get("lifetime_pair_id")) is not str or not row["lifetime_pair_id"].startswith("lifetime_pair_"):
            failures.append("invalid lifetime_pair_id")
            break
        if row.get("lifetime_execution_order") not in {"alive_first", "dead_first"}:
            failures.append("invalid lifetime_execution_order")
            break
        if row.get("lifetime_hold_ns") != LIFETIME_HOLD_NS:
            failures.append("lifetime hold interval drift")
            break
    ...
    pair_lifetimes: dict[str, set[str]] = defaultdict(set)
    for block_id, block in block_rows.items():
        ...
        lifetime_counts[block[0]["source_lifetime"]] += 1
        pair_lifetimes[block[0]["lifetime_pair_id"]].add(block[0]["source_lifetime"])
    ...
    if sorted(lifetime_counts) != SOURCE_LIFETIMES or len(set(lifetime_counts.values())) != 1:
        failures.append("source lifetime imbalance")
    if pair_lifetimes and not all(levels == set(SOURCE_LIFETIMES) for levels in pair_lifetimes.values()):
        failures.append("lifetime pair missing alive/dead twin")
    return {
        "passed": not failures,
        "failures": failures,
        "tuple_count": len(rows),
        ...
        "origin_counts": dict(sorted((str(k), v) for k, v in origin_counts.items())),
        "source_lifetime_counts": dict(sorted(lifetime_counts.items())),
        "lifetime_pair_count": len(pair_lifetimes),
