#!/usr/bin/env python3
"""Deploy, run, copy back, verify, and clean one Family 10h PMU first-light probe."""
...
def default_run_id() -> str:
    return dt.datetime.now(dt.timezone.utc).strftime("f10_pmc_first_light_%Y%m%dT%H%M%SZ")
...
def execute(run_id: str, *, keep_remote: bool) -> dict[str, Any]:
    ...
    completed = run(["ssh", "-o", "BatchMode=yes", TARGET, remote_command], timeout=60, check=False)
    ...
    worker = json.loads((local_run / "F10_PMC_FIRST_LIGHT_RESULT.json").read_text(encoding="utf-8"))
    controller = {
        "schema_id": "CAT_CAS_F10_PMC_FIRST_LIGHT_CONTROLLER_V1",
        "run_id": run_id,
        "target": TARGET,
        ...
        "first_light": bool(worker["acceptance"]["first_light"]),
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"run_f10_pmc_first_light: {exc}", file=sys.stderr)
        return 1
    return 0 if result["target_status"] == "F10_PMC_FIRST_LIGHT_TARGET_COMPLETE" else 1
