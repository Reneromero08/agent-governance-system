#!/usr/bin/env python3
"""M225 fused dynamic unit-power direct coordinate trace.
...
M225 keeps the M224 direct coordinate trace and rematerialization law, but
folds every dynamic public-unit power directly into its consuming accumulator.
No accepted path first returns ``base ** exponent`` as a standalone field
value and then multiplies that value into a simultaneously live accumulator.
The identical fused recurrence remains classical and every exact
multiplication is counted.
"""
...
class Work(m222.Work):
    ...
    fused_unit_power_calls: int = 0
    fused_unit_power_accumulate_multiplications: int = 0
    fused_unit_power_square_multiplications: int = 0
    fused_ledger_norm_power_calls: int = 0
    fused_ledger_scale_power_calls: int = 0
    fused_direct_trace_weight_power_calls: int = 0
    fused_candidate_multiplier_power_calls: int = 0
    fused_selected_multiplier_power_calls: int = 0
...
def fused_accumulate_power(
    ...
    *,
    ...
    """Return ``accumulator * base**exponent`` without a standalone power."""

    if exponent < 0:
        raise ValueError("negative exponent in fused exact power")
    if exponent == 0:
        return accumulator
    work.fused_unit_power_calls += 1
    result = accumulator
    factor = base
    ...
    while remaining:
        if remaining & 1:
            ...
            work.fused_unit_power_accumulate_multiplications += 1
            work.observe(
                residual,
                ledger,
                scratch,
                scalars=live_scalars + (result, factor, product),
                integers=live_integers + (exponent, remaining),
                context=f"{context_prefix}_ACCUMULATE",
            )
            result = product
            del product
        remaining >>= 1
        if remaining:
            ...
            work.unit_power_field_multiplications += 1
            work.fused_unit_power_square_multiplications += 1
            work.observe(
                residual,
                ledger,
                scratch,
                scalars=live_scalars + (result, factor, squared),
                integers=live_integers + (exponent, remaining),
                context=f"{context_prefix}_SQUARE",
            )
            factor = squared
            del squared
    return result
...
def reject_standalone_power(*_args: Any, **_kwargs: Any) -> braid.K:
    raise RuntimeError("standalone dynamic power is forbidden on the M225 path")
...
def fused_ledger_scale(
    ...
    *,
    ...
    for index, (exponent, direction) in enumerate(
        ...
        work.fused_ledger_scale_power_calls += 1
        scale = fused_accumulate_power(
            scale,
            base,
            abs(exponent),
            residual,
            ...
            context_prefix="FUSED_LEDGER_SCALE_POWER",
        )
        del base
    return scale
...
def rematerialize_ledger_norm(
    ...
    *,
    ...
    for index, (exponent, direction) in enumerate(
        ...
        work.fused_ledger_norm_power_calls += 1
        result = fused_accumulate_power(
            result,
            base,
            abs(exponent),
            residual,
            ...
            work,
            live_integers=live_integers + (index, exponent),
            context_prefix="FUSED_LEDGER_NORM_POWER",
        )
        del base
    return result


def fused_unit_power_energy(
    residual: list[braid.K],
    ledger: list[int],
    scratch: list[braid.K],
    work: Work,
    *,
    ...
    if direction is not None and exponent != 0:
        ...
        work.fused_direct_trace_weight_power_calls += 1
        weight = fused_accumulate_power(
            weight,
            base,
            abs(exponent),
            residual,
            ...
            work,
            live_integers=live_integers + (exponent,),
            context_prefix="FUSED_DIRECT_TRACE_WEIGHT_POWER",
        )
        del base
...
def rematerialized_line_minimum(
    ...
    *,
    ...
    def energy(exponent: int, integers: tuple[int, ...]) -> int:
        return fused_unit_power_energy(
            residual,
            ledger,
            scratch,
            work,
...
def rematerialized_scaled_payload(
    ...
    *,
    ...
    if candidate:
        ...
        work.fused_candidate_multiplier_power_calls += 1
        multiplier = fused_accumulate_power(
            multiplier,
            base,
            ...
            work,
            live_integers=live_integers + (candidate_exponent,),
            context_prefix="FUSED_CANDIDATE_MULTIPLIER_POWER",
        )
...
def rematerialized_apply_selected(
    ...
    *,
    ...
    for index, (exponent, direction) in enumerate(
        ...
        work.fused_selected_multiplier_power_calls += 1
        multiplier = fused_accumulate_power(
            multiplier,
            base,
            abs(exponent),
            ...
            context_prefix="FUSED_SELECTED_MULTIPLIER_POWER",
        )
        del base
...
def rematerialized_balance_resident(
    ...
    identity_total = raw_payload + m221.m220.ledger_payload_bits(zero_ledger)
    identity_energy = fused_unit_power_energy(
        residual,
        ledger,
        scratch,
        work,
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit(
            "usage: su2_level8_cubic_skein_fused_unit_power.py "
            "SEPARATE_REFERENCE_JSON"
        )
    here = Path(__file__).resolve().parent
    reference_path = Path(sys.argv[1]).resolve()
    if str(reference_path).startswith(("/dev/shm/", "/run/shm/")):
        raise ValueError("RAM-backed M225 reference is forbidden")
    reference = json.loads(reference_path.read_text())
    if reference.get("schema") != "cat_cas.su2_level8_cubic_skein_fused_unit_power_reference.v1":
        raise RuntimeError("M225 separate-reference schema changed")
    ...
    if [m221.reference_case_view(case) for case in cases] != reference.get("cases"):
        raise RuntimeError("M225 independent case and resource parity failed")
    ...
    for section in ("primary", "reuse", "fresh_reuse"):
        for key in (
            ...
            if reuse[section][key] != reference["reuse"][section][key]:
                raise RuntimeError(
                    f"M225 independent reuse parity failed: {section}.{key}"
                )
        normalized_work = m221.reference_case_view(reuse[section])["work"]
        if normalized_work != reference["reuse"][section]["work"]:
            raise RuntimeError(
                f"M225 independent normalized work parity failed: {section}"
            )
    for key in (
        ...
        if reuse[key] != reference["reuse"][key]:
            raise RuntimeError(f"M225 independent reuse parity failed: {key}")
    ...
    if (
        ...
        raise RuntimeError("M225 control failed")
    selected = [
        {
            ...
            "fused_unit_power_maximum_declared_live_payload_bits": case["work"][
            ...
            "fused_unit_power_maximum_context": case["work"][
    ...
    m224_result = json.loads(m224_result_path.read_text())
    m224_primary = next(
        case
        for case in m224_result["cases"]
        if (case["strands"], case["rounds"], case["family"]) == (4, 4, 0)
    )
    predecessor_primary_bits = m224_primary["work"][
        "maximum_declared_live_payload_bits"
    ]
    predecessor_terminal_multiplications = (
        m224_primary["work"]["rematerialization_field_multiplications"]
        + m224_primary["work"]["ledger_scale_field_multiplications"]
    )
    current_primary_bits = current_primary["work"][
        "maximum_declared_live_payload_bits"
    ]
    if all_nontrivial_smaller:
        result_name = "PASS_BOUNDED_EXACT_FUSED_UNIT_POWER_LIFECYCLE_REDUCTION"
        claim = "BOUNDED_EXACT_DYNAMIC_BINARY_PUBLIC_UNIT_POWER_FUSION_ELIMINATES_STANDALONE_POWER_RESULTS_AND_ONE_TERMINAL_FIELD_MULTIPLICATION_PER_NONZERO_FACTOR_APPLICATION_WHILE_REDUCING_DECLARED_EXACT_LIVE_PAYLOAD_BELOW_THE_MATCHED_RAW_RECURRENCE_ON_EVERY_DECLARED_DEPTH_ABOVE_ONE_WITH_FINAL_ONLY_BOUNDARY_EXACT_RESTORATION_REUSE_BUT_THE_IDENTICAL_CLASSICAL_FUSED_COORDINATE_RECURRENCE_REMAINS"
    elif every_case_above:
        result_name = "PASS_BOUNDED_EXACT_FUSED_UNIT_POWER_PERSISTING_HEIGHT_NO_GO"
        claim = (
            "BOUNDED_EXACT_DYNAMIC_BINARY_PUBLIC_UNIT_POWER_FUSION_ELIMINATES_"
            "STANDALONE_POWER_RESULTS_AND_ONE_TERMINAL_FIELD_MULTIPLICATION_PER_"
            "NONZERO_FACTOR_APPLICATION_AND_REDUCES_PRIMARY_DECLARED_EXACT_"
            f"LIVE_PAYLOAD_FROM{predecessor_primary_bits}_TO"
            f"{current_primary_bits}_BITS_WITH_FINAL_ONLY_BOUNDARY_EXACT_"
            "RESTORATION_REUSE_BUT_FUSED_UNIT_POWER_ACCUMULATION_"
            "DOMINATES_AND_ALL_SEVEN_DECLARED_CASES_REMAIN_ABOVE_THE_MATCHED_"
            "RAW_RECURRENCE_WHILE_THE_IDENTICAL_CLASSICAL_FUSED_"
            "COORDINATE_RECURRENCE_REMAINS"
        )
    else:
        result_name = "PASS_BOUNDED_EXACT_FUSED_UNIT_POWER_MIXED_LIFECYCLE_RESULT"
        claim = "BOUNDED_EXACT_DYNAMIC_BINARY_PUBLIC_UNIT_POWER_FUSION_ELIMINATES_STANDALONE_POWER_RESULTS_AND_ONE_TERMINAL_FIELD_MULTIPLICATION_PER_NONZERO_FACTOR_APPLICATION_WITH_FINAL_ONLY_BOUNDARY_EXACT_RESTORATION_REUSE_AND_A_MIXED_DECLARED_LIVE_PAYLOAD_RESULT_AGAINST_THE_MATCHED_RAW_RECURRENCE_WHILE_THE_IDENTICAL_CLASSICAL_FUSED_COORDINATE_RECURRENCE_REMAINS"
    result = {
        "schema": "cat_cas.su2_level8_cubic_skein_fused_unit_power.v1",
        "result": result_name,
        "claim": claim,
        "classification": "INDEPENDENTLY_VERIFIED_STRICT_SCOPE",
        "verification_level": "SEPARATE_REFERENCE_PARITY",
        "restoration_classification": "EXACT_ALGEBRAIC_RESTORATION",
        "claim_ceiling": "FORMAL_PRETRUNCATION_QZETA40_M219_CUBIC_SKEIN_M220_UNIT_M221_LEDGER_NATIVE_M222_STREAMED_M223_DIRECT_M224_REMATERIALIZED_PARAMETERS3_7_9_11_13_17_19_FUSED_DYNAMIC_BINARY_PUBLIC_UNIT_POWER_POWER_BASIS_RAMANUJAN_TRACE_LINE_SEARCH_FAMILY0_STRANDS4_DEPTH1TO4_STRANDS6_DEPTH1TO2_STRANDS8_DEPTH1_PRIMARY4_DEPTH4_REUSE4_DEPTH2_FAMILY1_DIRECT_PROCESS_ONLY",
        "mechanism": {
            ...
            "dynamic_public_unit_power_fused_into_consuming_accumulator": True,
        ...
        "predecessor_comparison": {
            "m224_primary_declared_live_payload_bits": predecessor_primary_bits,
            "m225_primary_declared_live_payload_bits": current_primary_bits,
            "reduction_bits": predecessor_primary_bits - current_primary_bits,
            "m224_primary_terminal_factor_multiplications": (
                predecessor_terminal_multiplications
            ),
            "m225_primary_terminal_factor_multiplications": (
                current_primary["work"]["rematerialization_field_multiplications"]
                + current_primary["work"]["ledger_scale_field_multiplications"]
            ),
            "m225_primary_nonzero_fused_factor_applications": current_primary[
                "work"
            ]["fused_unit_power_calls"],
        },
        "lifecycle_law": {
            ...
            "all_accepted_dynamic_powers_fused": all(
                case["work"]["fused_unit_power_calls"] > 0
            ...
            "source_complete_scope": "FUSED_DYNAMIC_POWER_INTERVALS_ONLY",
        ...
        "matched_classical_baselines": {
            "strongest_compact": "IDENTICAL_FUSED_DYNAMIC_BINARY_PUBLIC_UNIT_POWER_LEDGER_AND_DIRECT_POWER_BASIS_RAMANUJAN_TRACE_COORDINATE_RECURRENCE",
        ...
        "resource_law": {
            "all_fused_power_products_squares_accumulators_and_loop_integers_counted": True,
        ...
        "source_dependencies": {
            ...
            "m225_production_sha256": sha256_file(Path(__file__).resolve()),
            "m225_separate_reference_code_sha256": sha256_file(
                here / "su2_level8_cubic_skein_fused_unit_power_separate_reference.py"
            ),
            "m225_separate_reference_result_sha256": sha256_file(reference_path),
        },
        "terminal": False,
    }
