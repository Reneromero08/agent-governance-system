#!/usr/bin/env python3
"""Deploy, run, copy back, verify, and clean one dual-observed Family 10h path pilot."""
...
def default_run_id() -> str:
    return dt.datetime.now(dt.timezone.utc).strftime("f10_path_dual_%Y%m%dT%H%M%SZ")
...
def execute(run_id: str, *, keep_remote: bool) -> dict[str, Any]:
    ...
    worker = json.loads((local_run / "F10_PATH_DUAL_OBSERVE_RESULT.json").read_text(encoding="utf-8"))
    controller = {
        "schema_id": "CAT_CAS_F10_PATH_DUAL_OBSERVE_CONTROLLER_V1",
        "run_id": run_id,
        ...
        "path_dual_observe_candidate": bool(worker["acceptance"]["path_dependence_pilot"]),
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"run_f10_path_dual_observe: {exc}", file=sys.stderr)
        return 1
    return 0 if result["target_status"] == "F10_PATH_DUAL_OBSERVE_TARGET_COMPLETE" else 1
