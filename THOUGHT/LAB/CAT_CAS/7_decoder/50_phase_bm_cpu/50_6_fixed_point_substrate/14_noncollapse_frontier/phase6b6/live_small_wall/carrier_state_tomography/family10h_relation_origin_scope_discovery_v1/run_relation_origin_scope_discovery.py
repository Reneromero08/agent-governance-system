...
RUN_ID = "family10h_relation_origin_scope_discovery_v1_0"
...
VARIANTS = [f"origin_{origin}_{base}" for origin in ORIGINS for base in BASE_VARIANTS]
QUERY_BY_BASE_VARIANT = {
...
ROWS_PER_ORIGIN_VARIANT = 512
PAIR_SAMPLE_COUNT = 256
EXPECTED_PAIRS_PER_VARIANT = ROWS_PER_ORIGIN_VARIANT * PAIR_SAMPLE_COUNT
MATCHED_PERMUTATION_COUNT = 63
MATCHED_PERMUTATION_SEED = "family10h-origin-scope-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "ORIGIN_SCOPE_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_ORIGIN_SCOPE_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_ORIGIN_SCOPE_DISCOVERY_SUMMARY.md"


def variant_origin(variant: str) -> str:
    for origin in ORIGINS:
        prefix = f"origin_{origin}_"
        if variant.startswith(prefix):
            return origin
    raise KeyError(variant)


def variant_base(variant: str) -> str:
    origin = variant_origin(variant)
    return variant[len(f"origin_{origin}_") :]


def variant_query(variant: str) -> str:
    return QUERY_BY_BASE_VARIANT[variant_base(variant)]
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    rows_by_origin: dict[str, list[dict[str, str]]] = {origin: [] for origin in ORIGINS}
    for row in source_rows:
        origin = row.get("cyclic_origin", "")
        if origin in rows_by_origin:
            rows_by_origin[origin].append(row)
    for origin in ORIGINS:
        require(len(rows_by_origin[origin]) >= ROWS_PER_ORIGIN_VARIANT, f"not enough source rows for origin {origin}")
    SCHEDULE_DIR.mkdir(parents=True, exist_ok=True)
    schedules: dict[str, Any] = {}
    for variant in VARIANTS:
        origin = variant_origin(variant)
        base_variant = variant_base(variant)
        query = variant_query(variant)
        base_rows = rows_by_origin[origin][:ROWS_PER_ORIGIN_VARIANT]
        rows = []
        for index, row in enumerate(base_rows):
            ...
            copied["control_semantics_id"] = "none" if base_variant == "primary_relation_pair" else query
        ...
        schedules[variant] = {
            ...
            "query": query,
            "path": f"ORIGIN_SCOPE_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        "source_schedule": str(source_schedule),
        "source_schedule_sha256": sha256_file(source_schedule),
        "origins": ORIGINS,
        "base_variants": BASE_VARIANTS,
        "rows_per_origin_variant": ROWS_PER_ORIGIN_VARIANT,
        ...
        "mechanism_question": "Is the relation-minus-carrier-off orientation residual stable across cyclic origin/route scope?",
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_ORIGIN_SCOPE_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_ORIGIN_SCOPE_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/ORIGIN_SCOPE_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_ORIGIN_SCOPE_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_ORIGIN_SCOPE_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_ORIGIN_SCOPE_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_ORIGIN_SCOPE_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_ORIGIN_SCOPE_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    for origin in ORIGINS:
        def r(base_variant: str) -> float:
            return reports[f"origin_{origin}_{base_variant}"]["R_spatial"]["mean"]

        primary = r("primary_relation_pair")
        inverted = r("query_inversion_control")
        carrier_off = r("carrier_off_control")
        carrier_off_inverted = r("carrier_off_inversion_control")
        sham = r("relation_sham")
        ...
        origin_reports[origin] = {
            ...
            "net_orientation_positive": net_orientation > 0,
            "relation_sham_below_25pct_primary": primary > 0 and abs(sham) <= 0.25 * abs(primary),
        }
    aggregate = {
        "mean_relation_orientation_half_difference": mean(
            [origin_reports[origin]["relation_orientation_half_difference"] for origin in ORIGINS]
        ),
        "mean_carrier_off_orientation_half_difference": mean(
            [origin_reports[origin]["carrier_off_orientation_half_difference"] for origin in ORIGINS]
        ),
        "mean_net_relation_minus_carrier_off_orientation": mean(
            [origin_reports[origin]["net_relation_minus_carrier_off_orientation"] for origin in ORIGINS]
        ),
        "net_positive_origins": [
            origin for origin in ORIGINS if origin_reports[origin]["net_relation_minus_carrier_off_orientation"] > 0
        ],
        "net_nonpositive_origins": [
            origin for origin in ORIGINS if origin_reports[origin]["net_relation_minus_carrier_off_orientation"] <= 0
        ],
    }
    result = {
        "schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "origin_coordinates": origin_reports,
        "aggregate_coordinates": aggregate,
        "interpretation": {
            "all_origins_query_inversion_reverse_primary": all(
                origin_reports[origin]["query_inversion_reverses_primary"] for origin in ORIGINS
            ),
            "all_origins_net_orientation_positive": all(
                origin_reports[origin]["net_orientation_positive"] for origin in ORIGINS
            ),
            "nonzero_origins_net_orientation_positive": all(
                origin_reports[origin]["net_orientation_positive"] for origin in ["1024", "2048", "3072"]
            ),
            "all_origins_relation_sham_below_25pct_primary": all(
                origin_reports[origin]["relation_sham_below_25pct_primary"] for origin in ORIGINS
            ),
            "route_scope_candidate_origins": aggregate["net_positive_origins"],
            "exploratory_only": True,
            "small_wall_crossed": False,
        },
        "claim_boundary": {
    ...
    write_json(RUN_ROOT / "RELATION_ORIGIN_SCOPE_DISCOVERY_ANALYSIS.json", result)
    write_json(SUMMARY_JSON, result)
    lines = [
        "# Relation Origin-Scope Discovery",
        "",
        f"Run ID: `{RUN_ID}`",
        f"Archive SHA-256: `{result['archive_sha256']}`",
        f"Analysis SHA-256: `{result['analysis_sha256']}`",
        "",
        "| Origin | R primary | R inverted | R off | R off inverted | relation orient | off orient | net | net/relation |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for origin in ORIGINS:
        coord = origin_reports[origin]
        ratio = coord["net_orientation_to_relation_orientation"]
        lines.append(
            f"| `{origin}` | {coord['R_primary']:.9f} | {coord['R_query_inversion']:.9f} | "
            f"{coord['R_carrier_off']:.9f} | {coord['R_carrier_off_inversion']:.9f} | "
            f"{coord['relation_orientation_half_difference']:.9f} | "
            f"{coord['carrier_off_orientation_half_difference']:.9f} | "
            f"{coord['net_relation_minus_carrier_off_orientation']:.9f} | "
            f"{ratio:.3f} |"
        )
    lines.extend(
        [
            ...
            f"- all origins query inversion reverse primary: `{result['interpretation']['all_origins_query_inversion_reverse_primary']}`",
            f"- all origins net orientation positive: `{result['interpretation']['all_origins_net_orientation_positive']}`",
            f"- nonzero origins net orientation positive: `{result['interpretation']['nonzero_origins_net_orientation_positive']}`",
            f"- route-scope candidate origins: `{result['interpretation']['route_scope_candidate_origins']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_ORIGIN_SCOPE_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_ORIGIN_SCOPE_PREPARE_RESULT.json", result)
    return result
        ...
        json.dumps(
            {
                ...
                "aggregate_coordinates": analysis["aggregate_coordinates"],
                "origin_coordinates": analysis["origin_coordinates"],
