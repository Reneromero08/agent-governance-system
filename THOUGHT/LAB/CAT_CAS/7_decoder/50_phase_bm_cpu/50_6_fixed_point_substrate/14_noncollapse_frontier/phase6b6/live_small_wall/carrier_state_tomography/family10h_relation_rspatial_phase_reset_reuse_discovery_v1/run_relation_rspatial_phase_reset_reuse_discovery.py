...
RUN_ID = "family10h_relation_rspatial_phase_reset_reuse_discovery_v1_0"
...
VARIANTS = [
    ...
    "restore_same_primary",
    "restore_same_sham",
    "restore_opposite_primary",
    "restore_opposite_sham",
]
QUERY_BY_VARIANT = {
    ...
    "restore_same_primary": "restore_same_query_relation_pair_control",
    "restore_same_sham": "restore_same_relation_sham_control",
    "restore_opposite_primary": "restore_opposite_query_relation_pair_control",
    "restore_opposite_sham": "restore_opposite_relation_sham_control",
}
...
MATCHED_PERMUTATION_SEED = "family10h-rspatial-phase-reset-reuse-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "RSPATIAL_PHASE_RESET_REUSE_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_rspatial_full_schedule_lifetime_reset() -> None:
    ...
    text = text.replace(
        ...
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "restore_same_query_relation_pair_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESTORE_SAME_PRIMARY;\n"
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "restore_same_relation_sham_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESTORE_SAME_SHAM;\n"
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "restore_opposite_query_relation_pair_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESTORE_OPPOSITE_PRIMARY;\n"
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "restore_opposite_relation_sham_control") == 0) {\n'
    ...
    lifecycle_helpers = (
        ...
        "        || control == RELATION_SPATIAL_CONTROL_RESTORE_OPPOSITE_SHAM;\n"
        "}\n\n"
        "static int restore_same_after_reset_control(relation_spatial_control_id control) {\n"
        "    return control == RELATION_SPATIAL_CONTROL_RESTORE_SAME_PRIMARY\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESTORE_SAME_SHAM;\n"
        "}\n\n"
        "static int restore_opposite_after_reset_control(relation_spatial_control_id control) {\n"
    ...
    text = text.replace(
        ...
        "                        flush_state_lines(&shared->state);\n"
        "                    }\n"
        "                    if (restore_same_after_reset_control(row.control)\n"
        "                        || restore_opposite_after_reset_control(row.control)) {\n"
        "                        relation_spatial_preparation restore;\n"
        "                        restore.bank_a_work = row.bank_a_work;\n"
        "                        restore.bank_b_work = row.bank_b_work;\n"
        "                        restore.relation = restore_opposite_after_reset_control(row.control)\n"
    ...
    text = text.replace(
        ...
        "            : (restore_same_after_reset_control(row.control)\n"
        "                ? \"source_dead_then_double_flush_then_receiver_restore_same_relation_before_spatial_pair_probe\"\n"
        "                : (restore_opposite_after_reset_control(row.control)\n"
        "                    ? \"source_dead_then_double_flush_then_receiver_restore_opposite_relation_before_spatial_pair_probe\"\n"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"RSPATIAL_PHASE_RESET_REUSE_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
    ...
    write_json(SOURCE_ROOT / "RELATION_RSPATIAL_PHASE_RESET_REUSE_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_RSPATIAL_PHASE_RESET_REUSE_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/RSPATIAL_PHASE_RESET_REUSE_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_RSPATIAL_PHASE_RESET_REUSE_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    method_pairs = {
        ...
        "restore_same": ("restore_same_primary", "restore_same_sham"),
        "restore_opposite": ("restore_opposite_primary", "restore_opposite_sham"),
    }
    ...
    reuse_methods = ["restore_same", "restore_opposite"]
    ...
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "coordinates": {
            ...
            "restore_same_D_primary_minus_sham": method_reports["restore_same"]["D_primary_minus_sham"],
            "restore_same_D_abs_to_alive_D_abs": method_reports["restore_same"]["D_abs_to_alive_D_abs"],
            "restore_opposite_D_primary_minus_sham": method_reports["restore_opposite"]["D_primary_minus_sham"],
            "restore_opposite_D_abs_to_alive_D_abs": method_reports["restore_opposite"]["D_abs_to_alive_D_abs"],
        ...
        "interpretation": {
            ...
            "restore_same_rebuilds_positive_D": method_reports["restore_same"]["D_primary_minus_sham"] > 0,
            "restore_same_rebuilds_at_least_50pct_alive_D": (
                method_reports["restore_same"]["D_abs_to_alive_D_abs"] is not None
                and method_reports["restore_same"]["D_abs_to_alive_D_abs"] >= 0.50
            ),
            "restore_opposite_flips_D_negative": method_reports["restore_opposite"]["D_primary_minus_sham"] < 0,
            "restore_opposite_magnitude_at_least_50pct_alive_D": (
                method_reports["restore_opposite"]["D_abs_to_alive_D_abs"] is not None
                and method_reports["restore_opposite"]["D_abs_to_alive_D_abs"] >= 0.50
            ),
            "phase_reset_reuse_candidate": (
                best_reset["D_abs_to_alive_D_abs"] is not None
                and best_reset["D_abs_to_alive_D_abs"] <= 0.25
                and method_reports["restore_same"]["D_primary_minus_sham"] > 0
                and method_reports["restore_opposite"]["D_primary_minus_sham"] < 0
            ),
            "exploratory_only": True,
            "small_wall_crossed": False,
        },
    ...
    write_json(RUN_ROOT / "RELATION_RSPATIAL_PHASE_RESET_REUSE_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- restore-same rebuilds positive D: `{result['interpretation']['restore_same_rebuilds_positive_D']}`",
            f"- restore-opposite flips D negative: `{result['interpretation']['restore_opposite_flips_D_negative']}`",
            f"- phase reset/reuse candidate: `{result['interpretation']['phase_reset_reuse_candidate']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_PHASE_RESET_REUSE_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_RSPATIAL_PHASE_RESET_REUSE_PREPARE_RESULT.json", result)
    return result
