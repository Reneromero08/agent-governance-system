...
RUN_ID = "family10h_relation_temporal_lifetime_mutation_discovery_v1_0"
...
MATCHED_PERMUTATION_SEED = "family10h-relation-temporal-lifetime-mutation-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "TEMPORAL_LIFETIME_MUTATION_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_TEMPORAL_LIFETIME_MUTATION_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_TEMPORAL_LIFETIME_MUTATION_DISCOVERY_SUMMARY.md"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"TEMPORAL_LIFETIME_MUTATION_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        ...
        "mechanism_question": (
            "Whether the temporal-order relation coordinate survives source death, is killed by reset, "
            "and can be rebuilt or flipped by source-CPU same/opposite relation mutation."
        ),
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_TEMPORAL_LIFETIME_MUTATION_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_TEMPORAL_LIFETIME_MUTATION_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/TEMPORAL_LIFETIME_MUTATION_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_TEMPORAL_LIFETIME_MUTATION_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_TEMPORAL_LIFETIME_MUTATION_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_TEMPORAL_LIFETIME_MUTATION_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_TEMPORAL_LIFETIME_MUTATION_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_TEMPORAL_LIFETIME_MUTATION_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(RUN_ROOT / "RELATION_TEMPORAL_LIFETIME_MUTATION_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- temporal lifetime mutation candidate: `{result['interpretation']['phase_mutation_candidate']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_LIFETIME_MUTATION_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_TEMPORAL_LIFETIME_MUTATION_PREPARE_RESULT.json", result)
    return result
