...
RUN_ID = "family10h_relation_offset2_signed_projection_source_off_screen_discovery_v1_0"
...
VARIANTS = [
    ...
    "source_off_offset2_signed",
    "dead_offset2_signed",
    "reset_double_flush_offset2_signed",
]
QUERY_BY_VARIANT = {
    "alive_offset2_signed": "source_on_offset_2_control",
    "source_off_offset2_signed": "source_off_offset_2_control",
    "dead_offset2_signed": "dead_offset_2_control",
    "reset_double_flush_offset2_signed": "reset_double_flush_offset_2_control",
}
LIFECYCLE_BY_VARIANT = {
    "alive_offset2_signed": "alive",
    "source_off_offset2_signed": "source_off",
    "dead_offset2_signed": "dead",
    "reset_double_flush_offset2_signed": "reset_double_flush",
}
...
MATCHED_PERMUTATION_SEED = "family10h-offset2-signed-projection-source-off-screen-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_offset2_signed_projection_source_off_screen() -> None:
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        ...
        "mechanism_question": (
            "Does the signed offset-2 receiver projection preserve a source-written relation-matrix "
    ...
    write_json(SOURCE_ROOT / "RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_ARCHIVE, "r:gz") as tf:
        ...
        for variant in VARIANTS:
            ...
            reports[variant] = {
                ...
                "C_offset2_signed": distribution(values),
    ...
    source_off = value_for("source_off_offset2_signed")
    dead = value_for("dead_offset2_signed")
    reset = value_for("reset_double_flush_offset2_signed")
    ...
    for variant in VARIANTS:
        ...
        stratum_reports[variant] = {
            ...
            "all_same_sign_as_alive_aggregate": all((value > 0) == (alive > 0) for value in values)
            if values and alive
            else False,
    ...
    candidate = (
        alive != 0.0
        and abs(source_off) <= 0.25 * abs(alive)
        and abs(dead) >= 0.25 * abs(alive)
        and stratum_reports["source_off_offset2_signed"]["all_one_factor_strata_below_25pct_alive_matched"]
        and stratum_reports["alive_offset2_signed"]["all_same_sign_as_alive_aggregate"]
    )
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "coordinates": {
            "C_alive_offset2_signed": alive,
            "C_source_off_offset2_signed": source_off,
            "C_dead_offset2_signed": dead,
            "C_reset_double_flush_offset2_signed": reset,
            "source_off_abs_to_alive_abs": ratio_to_alive(source_off),
            "dead_abs_to_alive_abs": ratio_to_alive(dead),
            "reset_double_flush_abs_to_alive_abs": ratio_to_alive(reset),
            "alive_minus_source_off": alive - source_off,
            "dead_minus_source_off": dead - source_off,
            "reset_minus_source_off": reset - source_off,
        },
        "interpretation": {
            "alive_offset2_signed_nonzero": alive != 0.0,
            "alive_all_one_factor_strata_same_sign": stratum_reports["alive_offset2_signed"][
                "all_same_sign_as_alive_aggregate"
            ],
            "source_off_collapses_below_25pct_alive": abs(source_off) <= 0.25 * abs(alive) if alive else False,
            "source_off_all_one_factor_strata_below_25pct_alive": stratum_reports["source_off_offset2_signed"][
                "all_one_factor_strata_below_25pct_alive_matched"
            ],
            "dead_preserves_at_least_25pct_alive": abs(dead) >= 0.25 * abs(alive) if alive else False,
            "reset_double_flush_collapses_below_25pct_alive": abs(reset) <= 0.25 * abs(alive) if alive else False,
            "offset2_signed_projection_source_off_screen_candidate": candidate,
            "exploratory_only": True,
            "small_wall_crossed": False,
        },
        "claim_boundary": {
    ...
    write_json(RUN_ROOT / "RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_DISCOVERY_ANALYSIS.json", result)
    ...
    for variant in VARIANTS:
        ...
        lines.append(
            f"| {variant} | {LIFECYCLE_BY_VARIANT[variant]} | {value:.9f} | "
            f"{(ratio_to_alive(value) or 0.0):.3f} | "
    ...
    lines.extend(
        [
            ...
            f"- source-off collapses below 0.25 x alive: `{result['interpretation']['source_off_collapses_below_25pct_alive']}`",
            f"- source-off all one-factor strata below 0.25 x alive: `{result['interpretation']['source_off_all_one_factor_strata_below_25pct_alive']}`",
            f"- dead preserves at least 0.25 x alive: `{result['interpretation']['dead_preserves_at_least_25pct_alive']}`",
            f"- reset double flush collapses below 0.25 x alive: `{result['interpretation']['reset_double_flush_collapses_below_25pct_alive']}`",
            f"- offset2 signed projection candidate: `{result['interpretation']['offset2_signed_projection_source_off_screen_candidate']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    patch_runtime_for_offset2_signed_projection_source_off_screen()
    schedule_manifest = make_rows()
    build = compile_runtime()
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_OFFSET2_SIGNED_PROJECTION_SOURCE_OFF_SCREEN_PREPARE_RESULT.json", result)
    return result
