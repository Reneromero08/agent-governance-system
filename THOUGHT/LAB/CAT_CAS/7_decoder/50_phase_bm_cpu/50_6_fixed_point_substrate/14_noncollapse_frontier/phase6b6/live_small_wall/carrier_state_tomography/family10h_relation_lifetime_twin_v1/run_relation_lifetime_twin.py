#!/usr/bin/env python3
"""Prepare and validate the relation-lifetime matched-permutation package."""
...
import relation_lifetime_adjudication as synthetic_adjudication
import relation_lifetime_physical_adjudication as physical_adjudication
...
SELF_TEST_JSON = HERE / "RELATION_LIFETIME_SELF_TEST.json"
ADVERSARY_JSON = HERE / "RELATION_LIFETIME_ADVERSARY_TEST.json"
TRANSPORT_JSON = HERE / "RELATION_LIFETIME_TRANSPORT_SIMULATION.json"
VALIDATE_JSON = HERE / "RELATION_LIFETIME_OFFLINE_VALIDATE.json"
MANIFEST_JSON = HERE / "RELATION_LIFETIME_IMPLEMENTATION_MANIFEST.json"
MANIFEST_SHA = HERE / "RELATION_LIFETIME_IMPLEMENTATION_MANIFEST.sha256"
SOURCE_HASHES_JSON = HERE / "RELATION_LIFETIME_SOURCE_HASHES.json"
SOURCE_BUNDLE = HERE / "RELATION_LIFETIME_SOURCE_BUNDLE.tar.gz"
...
PMU_PREFLIGHT_BINARY = HERE / "relation_lifetime_pmu_preflight"
SCHEDULE_JSON = HERE / "RELATION_LIFETIME_PUBLIC_SCHEDULE.json"
SCHEDULE_TSV = HERE / "RELATION_LIFETIME_PUBLIC_SCHEDULE.tsv"
SCHEDULE_SHA = HERE / "RELATION_LIFETIME_PUBLIC_SCHEDULE.sha256"
RUNTIME_BUILD_JSON = HERE / "RELATION_LIFETIME_RUNTIME_BUILD_SELF_TEST.json"
TARGET_SELF_TEST_JSON = HERE / "RELATION_LIFETIME_TARGET_SELF_TEST.json"
PHYSICAL_ADJUDICATOR_SELF_TEST_JSON = HERE / "RELATION_LIFETIME_PHYSICAL_ADJUDICATOR_SELF_TEST.json"
PHYSICAL_THRESHOLD_CONTRACT_JSON = HERE / "RELATION_LIFETIME_PHYSICAL_THRESHOLD_CONTRACT.json"
BUILD_READINESS_JSON = HERE / "RELATION_LIFETIME_BUILD_READINESS.json"
TOOLCHAIN_DISCOVERY_JSON = HERE / "RELATION_LIFETIME_TOOLCHAIN_DISCOVERY.json"
SYNTHETIC_EXECUTOR_SELF_TEST_JSON = HERE / "RELATION_LIFETIME_SYNTHETIC_EXECUTOR_SELF_TEST.json"
TARGET_PREFLIGHT_SELF_TEST_JSON = HERE / "RELATION_LIFETIME_TARGET_PREFLIGHT_SELF_TEST.json"
RUNTIME_CONTROL_FLOW_AUDIT_JSON = HERE / "RELATION_LIFETIME_RUNTIME_CONTROL_FLOW_AUDIT.json"
SYNTHETIC_TARGET_WRAPPER_SELF_TEST_JSON = HERE / "RELATION_LIFETIME_SYNTHETIC_TARGET_WRAPPER_SELF_TEST.json"
LIVE_CONTROLLER_SELF_TEST_JSON = HERE / "RELATION_LIFETIME_LIVE_CONTROLLER_SELF_TEST.json"

SOURCE_FILES = [
    HERE / "relation_lifetime_public.py",
    HERE / "relation_lifetime_adjudication.py",
    HERE / "relation_lifetime_physical_adjudication.py",
    HERE / "relation_lifetime_runtime.c",
    HERE / "relation_lifetime_runtime.h",
    HERE / "relation_lifetime_pmu_preflight.c",
    HERE / "relation_lifetime_target.py",
    HERE / "relation_lifetime_live_controller.py",
    HERE / "run_relation_lifetime_twin.py",
]
CONTRACT_FILE = HERE / "RELATION_LIFETIME_CONTRACT.md"
...
def schedule_json_manifest(schedule: dict[str, Any], tsv_sha256: str) -> dict[str, Any]:
    manifest = {
        "schema": "FAMILY10H_RELATION_LIFETIME_PUBLIC_SCHEDULE_MANIFEST_V3",
        ...
        "canonical_expanded_schedule_artifact": "RELATION_LIFETIME_PUBLIC_SCHEDULE.tsv",
        ...
        "deterministic_generator": "relation_lifetime_public.build_schedule",
...
def validate_schedule_json_manifest(manifest: dict[str, Any], generated_schedule: dict[str, Any], tsv_sha256: str) -> dict[str, Any]:
    failures: list[str] = []
    if manifest.get("schema") != "FAMILY10H_RELATION_LIFETIME_PUBLIC_SCHEDULE_MANIFEST_V3":
...
def source_authority_validation(
    ...
    *,
    ...
    return {
        "schema": "FAMILY10H_RELATION_LIFETIME_SOURCE_AUTHORITY_VALIDATION_V1",
...
def source_authority_regression_tests(
    ...
    return {
        "schema": "FAMILY10H_RELATION_LIFETIME_SOURCE_AUTHORITY_REGRESSION_TESTS_V1",
        "results": results,
...
def write_sensor_authority_binding() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_LIFETIME_SENSOR_AUTHORITY_BINDING_V1",
...
def source_hashes(
    ...
    result = {
        "schema": "FAMILY10H_RELATION_LIFETIME_SOURCE_HASHES_V3",
        "science_package_id": pub.SCIENCE_PACKAGE_ID,
        "files": files,
...
def run_command(command: list[str], *, timeout: int = 30, cwd: Path | None = None) -> dict[str, Any]:
    try:
        completed = subprocess.run(
            ...
            timeout=timeout,
            cwd=cwd,
            env={key: value for key, value in os.environ.items() if "RELATION_LIFETIME" not in key},
        )
        return {
            "command": command,
...
def discover_toolchains() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_LIFETIME_TOOLCHAIN_DISCOVERY_V1",
...
def runtime_raw_schema_validation(
    ...
    return {
        "schema": "FAMILY10H_RELATION_LIFETIME_RUNTIME_RAW_SCHEMA_INTEGRATION_V1",
...
def runtime_static_inspection() -> dict[str, Any]:
    return {
        "schema": "FAMILY10H_RELATION_LIFETIME_RUNTIME_SOURCE_INVENTORY_V1",
        "gates": {},
        "gate_policy": "readiness gates are derived from compile, execution, disassembly, fixtures, and exact hash comparisons",
        "runtime_c_sha256": pub.sha256_file(HERE / "relation_lifetime_runtime.c"),
        "runtime_h_sha256": pub.sha256_file(HERE / "relation_lifetime_runtime.h"),
        "pmu_preflight_helper_c_sha256": pub.sha256_file(HERE / "relation_lifetime_pmu_preflight.c"),
        "target_py_sha256": pub.sha256_file(HERE / "relation_lifetime_target.py"),
    }
...
def compile_pmu_preflight_helper(target_compiler: dict[str, Any], distro: str) -> dict[str, Any]:
    ...
    if helper.exists():
        helper.unlink()
    source = HERE / "relation_lifetime_pmu_preflight.c"
    result: dict[str, Any] = {
        "schema": "FAMILY10H_RELATION_LIFETIME_PMU_PREFLIGHT_HELPER_BUILD_V1",
        "source": source.name,
...
def disassembly_control_flow_audit(distro: str, wsl_binary: str) -> dict[str, Any]:
    ...
    prepare_primary = extract_disassembly_function(disassembly, "relation_lifetime_prepare_primary")
    query_primary = extract_disassembly_function(disassembly, "relation_lifetime_query_primary")
    prepare_wrapper = extract_disassembly_function(disassembly, "relation_lifetime_prepare")
    query_wrapper = extract_disassembly_function(disassembly, "relation_lifetime_query_with_table")
    ...
    checks = {
        ...
        "prepare_primary_has_no_relation_map_call": "relation_lifetime_map_index" not in prepare_primary,
        "query_primary_has_no_relation_map_call": "relation_lifetime_map_index" not in query_primary,
        "prepare_primary_has_no_table_selection_call": "relation_lifetime_table_for" not in prepare_primary,
        "query_primary_has_no_table_selection_call": "relation_lifetime_table_for" not in query_primary,
        "both_relations_enter_same_prepare_primary_loop": "relation_lifetime_prepare_primary" in prepare_wrapper,
        "query_uses_preselected_table_entrypoint": "relation_lifetime_query_primary" in query_wrapper
        and "relation_lifetime_query_with_table" in query_dispatch
        and "execute_query_for_row" in executor,
        "table_selection_before_query_entrypoint": "relation_lifetime_table_for" in executor
        and executor.find("relation_lifetime_table_for") < executor.find("execute_query_for_row"),
    ...
    audit = {
        "schema": "FAMILY10H_RELATION_LIFETIME_RUNTIME_CONTROL_FLOW_AUDIT_V1",
        "command": command,
        "returncode": result["returncode"],
        "stderr": result["stderr"],
        "checks": checks,
        "function_body_line_counts": {
            "relation_lifetime_prepare_primary": len(prepare_primary.splitlines()) if prepare_primary else 0,
            "relation_lifetime_query_primary": len(query_primary.splitlines()) if query_primary else 0,
            "relation_lifetime_prepare": len(prepare_wrapper.splitlines()) if prepare_wrapper else 0,
            "relation_lifetime_query_with_table": len(query_wrapper.splitlines()) if query_wrapper else 0,
...
def compile_runtime(toolchain: dict[str, Any]) -> dict[str, Any]:
    ...
    binary = HERE / "relation_lifetime_runtime"
    if binary.exists():
        binary.unlink()
    receipt: dict[str, Any] = {
        "schema": "FAMILY10H_RELATION_LIFETIME_RUNTIME_BUILD_SELF_TEST_V2",
    ...
    wsl_c = wsl_path(HERE / "relation_lifetime_runtime.c", distro)
    ...
    missing_auth = run_command(
        ["wsl.exe", "-d", distro, "--", wsl_binary, "--execute-schedule", wsl_schedule, wsl_path(HERE / "_relation_lifetime_noauth_output", distro)],
        timeout=30,
        cwd=HERE,
    )
    ...
    output_root = HERE / "_relation_lifetime_synthetic_executor_self_test_output"
    ...
    implementation_gates = {
        ...
        "target_linux_binary_bound_to_source": binary.exists() and pub.sha256_file(HERE / "relation_lifetime_runtime.c") == receipt["runtime_binary_authority"]["runtime_c_sha256"],
        ...
        "raw_record_output_implemented": synthetic_summary.get("raw_record_count") == 21504,
...
def run_target_self_test() -> dict[str, Any]:
    completed = subprocess.run(
        [sys.executable, str(HERE / "relation_lifetime_target.py"), "--self-test", "--source-root", str(HERE)],
        text=True,
        capture_output=True,
        check=False,
        timeout=3600,
    ...
    try:
        ...
        payload = {
            "schema": "FAMILY10H_RELATION_LIFETIME_TARGET_SELF_TEST_V1",
...
def run_live_controller_self_test() -> dict[str, Any]:
    completed = subprocess.run(
        [
            sys.executable,
            str(HERE / "relation_lifetime_live_controller.py"),
            "--self-test",
            "--source-root",
            str(HERE),
    ...
    try:
        ...
        payload = {
            "schema": "FAMILY10H_RELATION_LIFETIME_SYNTHETIC_LIVE_CONTROLLER_SELF_TEST_V1",
            "passed": False,
            "parse_failure": completed.stdout,
        }
    payload["subprocess_returncode"] = completed.returncode
...
def negative_regressions(grammar: dict[str, Any], schedule: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": "FAMILY10H_RELATION_LIFETIME_NEGATIVE_REGRESSIONS_V2",
        "tests": tests,
...
def transport_simulation(schedule: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": "FAMILY10H_RELATION_LIFETIME_TRANSPORT_SIMULATION_V2",
...
def self_test(
    ...
    result = {
        "schema": "FAMILY10H_RELATION_LIFETIME_SELF_TEST_V2",
        "tests": tests,
...
def build_readiness(
    ...
    result = {
        "schema": "FAMILY10H_RELATION_LIFETIME_BUILD_READINESS_V1",
...
def implementation_manifest(
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_LIFETIME_IMPLEMENTATION_MANIFEST_V2",
        "science_package_id": pub.SCIENCE_PACKAGE_ID,
        "transaction_run_id": pub.TRANSACTION_RUN_ID,
...
def validate_artifacts() -> dict[str, Any]:
    ...
    required = [
        ...
        "RELATION_LIFETIME_SELF_TEST.json",
        "RELATION_LIFETIME_ADVERSARY_TEST.json",
        "RELATION_LIFETIME_TRANSPORT_SIMULATION.json",
        "RELATION_LIFETIME_OFFLINE_VALIDATE.json",
        ...
        "RELATION_LIFETIME_SOURCE_BUNDLE.tar.gz",
        ...
        "RELATION_LIFETIME_RUNTIME_CONTROL_FLOW_AUDIT.json",
        "RELATION_LIFETIME_SYNTHETIC_EXECUTOR_SELF_TEST.json",
        "RELATION_LIFETIME_TARGET_SELF_TEST.json",
        "RELATION_LIFETIME_TARGET_PREFLIGHT_SELF_TEST.json",
        "RELATION_LIFETIME_SYNTHETIC_TARGET_WRAPPER_SELF_TEST.json",
        "RELATION_LIFETIME_LIVE_CONTROLLER_SELF_TEST.json",
        "RELATION_LIFETIME_PHYSICAL_ADJUDICATOR_SELF_TEST.json",
        "RELATION_LIFETIME_PHYSICAL_THRESHOLD_CONTRACT.json",
        "RELATION_LIFETIME_BUILD_READINESS.json",
    ]
    ...
    validate = {
        "schema": "FAMILY10H_RELATION_LIFETIME_OFFLINE_VALIDATE_V2",
...
def normalized_relation_source_authority(value: str | None) -> str:
    return value or os.environ.get("FAMILY10H_RELATION_LIFETIME_RELATION_SOURCE_AUTHORITY") or pub.RELATION_SOURCE_AUTHORITY_UNSET
...
def prepare(relation_source_authority: str | None = None) -> dict[str, Any]:
    ...
    pub.write_json(
        ...
        {
            "schema": "FAMILY10H_RELATION_LIFETIME_SYNTHETIC_EXECUTOR_SELF_TEST_RECEIPT_V1",
    ...
    provisional_validate = {"schema": "FAMILY10H_RELATION_LIFETIME_OFFLINE_VALIDATE_V2", "passed": False}
