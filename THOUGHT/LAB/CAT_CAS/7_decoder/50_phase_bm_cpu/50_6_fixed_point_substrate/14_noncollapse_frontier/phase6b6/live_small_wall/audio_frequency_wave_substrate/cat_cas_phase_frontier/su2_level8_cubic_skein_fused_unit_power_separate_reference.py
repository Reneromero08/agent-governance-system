#!/usr/bin/env python3
"""Standalone M225 fused dynamic unit-power reference."""
...
def reject_standalone_power(*_args: object, **_kwargs: object) -> base.E:
...
def fused_ledger_scale(
    ...
    *,
    ...
    for index, (exponent, unit) in enumerate(zip(ledger, m222.UNITS)):
        if exponent == 0:
            continue
        value = unit.value if exponent > 0 else unit.reciprocal
...
def fused_unit_power_energy(
    ...
    if unit is not None and exponent != 0:
        value = unit.reciprocal_norm if exponent > 0 else unit.norm
...
def rematerialized_scaled_payload(
    ...
    if candidate:
        value = (
            candidate_unit.reciprocal
            if candidate_exponent > 0
            else candidate_unit.value
        )
...
def rematerialized_apply_selected(
    ...
    for index, (exponent, unit) in enumerate(zip(selected_ledger, m222.UNITS)):
        if exponent == 0:
            continue
        value = unit.reciprocal if exponent > 0 else unit.value
...
def main() -> None:
    ...
    prior.Work = Work
    prior.counted_power = reject_standalone_power
    prior.ledger_scale = fused_ledger_scale
    prior.balance = rematerialized_balance
    prior.apply_operation = rematerialized_apply_operation
    prior.project = fused_project
    print(
        json.dumps(
            {
                "schema": "cat_cas.su2_level8_cubic_skein_fused_unit_power_reference.v1",
                "imports_m225_production": False,
