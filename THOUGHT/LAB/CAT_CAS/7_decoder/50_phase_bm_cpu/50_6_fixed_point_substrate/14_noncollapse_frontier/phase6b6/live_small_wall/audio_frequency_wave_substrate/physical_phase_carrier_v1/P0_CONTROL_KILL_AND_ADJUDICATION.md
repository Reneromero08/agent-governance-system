...
| Reference-only leakage | Provide reference and trigger with zero physical drive | CH1 remains at noise | Reference alone produces admitted carrier | Reference wiring, raw channels, I/Q | Yes |
...
| Timing mismatch | Test dependence on the source-off epoch | Quarter-master-cycle delay remains master-gauge neutral ideally; any change quantifies switching sensitivity, and the arm stays categorically unmatched | Mismatched arm is treated as matched or its schedule is normalized away | Gate witness, master-clock record, raw evidence | Yes |
...
|---|---|
| Residual source drive | Upstream CH0 plus analog/relay witnesses; removed-resonator and dummy arms; guarded termination |
| Direct electrical feedthrough | Dummy matched to C0; source kept active upstream; `epsilon_feed` upper bound |
| Driver energy storage | No driver component downstream of K2; CH0 plus CH2 contact witnesses; sealed continuity/injection evidence; wrong-termination arm |
...
| Digitizer memory or trigger correlation | Continuous pre/post capture, raw native bytes, zero-drive and controller-buffer controls |
...
5. A source-off witness is missing, contradictory, late, or changes after
   `t_admit`.
6. Any primary raw byte is altered after its first SHA-256 seal.
7. Any analysis window touches pre-guard data.
8. Any primary signal clips, contains nonfinite data, loses samples, or violates
   the frozen channel map.
9. The resonator-removed or dummy control exceeds `T_feed`.
10. Fewer than 256 consecutive usable cycles satisfy SNR and uncertainty.
11. Calibration requires any threshold above its hard cap.
12. A threshold, filter, frequency, window, guard, or arm mapping changes after
    primary data are inspected.
13. The source-left-on, wrong-termination, wrong-frequency, pi/2, timing, or
    amplitude control is admitted as matched primary evidence.
14. A result depends on filenames, metadata, expected labels, reference-only
    data, controller buffers, or synthetic replay.
15. The primary pair exceeds 0.20 C temperature difference or lacks the frozen
    environmental record.
16. A scientific failure is retried or an integrity replacement exceeds the
    one-replacement law.
17. Any claimed contact count is inconsistent with the immutable run ledger.
...
timing mismatch at +0.250 master cycle
wrong phase at pi/2
...
four independent architecture reviews pass with zero open material findings
