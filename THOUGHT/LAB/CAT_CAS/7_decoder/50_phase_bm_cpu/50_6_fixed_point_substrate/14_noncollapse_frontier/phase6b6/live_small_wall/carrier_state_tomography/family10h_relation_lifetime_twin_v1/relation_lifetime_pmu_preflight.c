...
static int self_test(void) {
    ...
    printf("{\"schema\":\"FAMILY10H_RELATION_LIFETIME_PMU_PREFLIGHT_HELPER_SELF_TEST_V1\",");
    printf("\"passed\":%s,", passed ? "true" : "false");
    printf("\"events\":");
    print_event_json();
    printf(",\"perf_event_attr_size\":%zu,", sizeof(struct perf_event_attr));
...
int main(int argc, char **argv) {
    ...
    if (argc == 3 && strcmp(argv[1], "--disabled-group-preflight") == 0) {
        ...
        print_state_json("FAMILY10H_RELATION_LIFETIME_DISABLED_PMU_PREFLIGHT_V1", passed, cpu, &state);
        return passed ? 0 : 1;
    }
