...
static const char *coded_preprojection_warm_query_scramble_schedule_sha256 =
...
static const char *coded_preprojection_warm_phase_local_sham_schedule_sha256 =
    "51f3fb66cd4f03dff2d3e9aab9196d4f94d85e221cd552b04eda4929669cca2e";
static const char *coded_preprojection_warm_phase_local_schedule_sha256 =
    "1144b929905e30f3da1261fdedf5e6393c30d31a41c9a4dd2dc39e8573f4cbc4";
static const char *coded_preprojection_active_query_schedule_sha256 =
    "5a0ac285435ba33a80a3272020f19c85004fc63949f1df4325a3ad90fdcd87f2";
static const char *coded_preprojection_source_phase_chop_schedule_sha256 =
    "0308e6518c6e8e4fd60862f3825750a3865d3f8cb4cbef59d140eefb6d2e0fb1";
static const char *coded_preprojection_active_source_chop_schedule_sha256 =
    "f1a5da565e0f0cc6da880bf27918ae3d29fde52745f03d237ffb8d0b54caa292";
...
static const char *coded_preprojection_warm_query_scramble_sequence[16] = {
    "WU", "WU", "N0", "QS0", "QS1", "QS2", "QS3", "QM0",
    "QM1", "QM2", "QM3", "C0", "C1", "C2", "C3", "N1"
};

static const char *coded_preprojection_warm_query_off_sequence[16] = {
    "WU", "WU", "N0", "QO0", "QO1", "QO2", "QO3", "QO4",
    "QO5", "QO6", "QO7", "C0", "C1", "C2", "C3", "N1"
};

static const char *coded_preprojection_warm_phase_local_sham_sequence[16] = {
...
static int coded_preprojection_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_LOOP ||
           ...
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP;
}
...
static int coded_preprojection_query_scramble_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_QUERY_SCRAMBLE_LOOP;
}

static int coded_preprojection_query_off_variant(int pilot) {
...
static int coded_preprojection_phase_local_sham_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP;
}

static int coded_preprojection_phase_local_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP ||
...
static int coded_preprojection_active_query_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP;
}

static int coded_preprojection_source_phase_chop_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP ||
           pilot == GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP;
}

static int coded_preprojection_active_source_chop_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP;
}
...
static int coded_preprojection_warm_restored_variant(int pilot) {
    return pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_RESTORED_LOOP ||
           ...
           coded_preprojection_phase_local_variant(pilot);
}
...
static int validate_coded_preprojection_warm_query_scramble_schedule_semantics(void) {
    return string_equal(coded_preprojection_warm_query_scramble_sequence[0], "WU") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[1], "WU") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[2], "N0") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[3], "QS0") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[4], "QS1") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[5], "QS2") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[6], "QS3") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[7], "QM0") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[8], "QM1") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[9], "QM2") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[10], "QM3") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[11], "C0") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[12], "C1") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[13], "C2") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[14], "C3") &&
           string_equal(coded_preprojection_warm_query_scramble_sequence[15], "N1") ? 0 : 1;
}

static int validate_coded_preprojection_warm_query_off_schedule_semantics(void) {
    return string_equal(coded_preprojection_warm_query_off_sequence[0], "WU") &&
           string_equal(coded_preprojection_warm_query_off_sequence[1], "WU") &&
           string_equal(coded_preprojection_warm_query_off_sequence[2], "N0") &&
           string_equal(coded_preprojection_warm_query_off_sequence[3], "QO0") &&
           string_equal(coded_preprojection_warm_query_off_sequence[4], "QO1") &&
           string_equal(coded_preprojection_warm_query_off_sequence[5], "QO2") &&
           string_equal(coded_preprojection_warm_query_off_sequence[6], "QO3") &&
           string_equal(coded_preprojection_warm_query_off_sequence[7], "QO4") &&
           string_equal(coded_preprojection_warm_query_off_sequence[8], "QO5") &&
           string_equal(coded_preprojection_warm_query_off_sequence[9], "QO6") &&
           string_equal(coded_preprojection_warm_query_off_sequence[10], "QO7") &&
           string_equal(coded_preprojection_warm_query_off_sequence[11], "C0") &&
           string_equal(coded_preprojection_warm_query_off_sequence[12], "C1") &&
           string_equal(coded_preprojection_warm_query_off_sequence[13], "C2") &&
           string_equal(coded_preprojection_warm_query_off_sequence[14], "C3") &&
...
static int validate_coded_preprojection_warm_phase_local_sham_schedule_semantics(void) {
    return string_equal(coded_preprojection_warm_phase_local_sham_sequence[0], "WU") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[1], "WU") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[2], "N0") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[3], "P0") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[4], "C0") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[5], "M0") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[6], "M1") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[7], "C1") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[8], "P1") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[9], "P2") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[10], "C2") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[11], "M2") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[12], "M3") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[13], "C3") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[14], "P3") &&
           string_equal(coded_preprojection_warm_phase_local_sham_sequence[15], "N1") ? 0 : 1;
}

static int validate_coded_preprojection_active_query_schedule_semantics(void) {
    return validate_coded_preprojection_warm_phase_local_sham_schedule_semantics();
}

static int validate_coded_preprojection_source_phase_chop_schedule_semantics(void) {
    return validate_coded_preprojection_warm_phase_local_sham_schedule_semantics();
}

static int validate_coded_preprojection_active_source_chop_schedule_semantics(void) {
    return validate_coded_preprojection_warm_phase_local_sham_schedule_semantics();
}
...
static int validate_only(void) {
    if (validate_schedule_semantics() != 0 ||
        ...
        validate_coded_preprojection_warm_phase_local_sham_schedule_semantics() != 0 ||
        validate_coded_preprojection_active_query_schedule_semantics() != 0 ||
        validate_coded_preprojection_source_phase_chop_schedule_semantics() != 0 ||
        validate_coded_preprojection_active_source_chop_schedule_semantics() != 0) {
...
static int pilot_variant_value(const char *text) {
    ...
    if (string_equal(text, "coded-preprojection-warm-query-scramble-loop")) {
        return GATE_A_PILOT_CODED_PREPROJECTION_WARM_QUERY_SCRAMBLE_LOOP;
    }
    ...
    if (string_equal(text, "coded-preprojection-warm-phase-local-sham-loop")) {
        return GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP;
    }
    if (string_equal(text, "coded-preprojection-warm-phase-local-loop")) {
        return GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP;
    }
    if (string_equal(text, "coded-preprojection-active-query-loop")) {
        return GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP;
    }
    if (string_equal(text, "coded-preprojection-source-phase-chop-loop")) {
        return GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP;
    }
    if (string_equal(text, "coded-preprojection-active-source-chop-loop")) {
        return GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP;
    }
    return -1;
}
...
static size_t pilot_working_set_bytes(int slot, int pilot) {
    if (coded_preprojection_variant(pilot)) {
        ...
        if (coded_preprojection_phase_local_variant(pilot)) {
            const char *token = slot >= 0 && slot < 16
                ? coded_preprojection_warm_phase_local_sham_sequence[slot] : "";
            if (coded_preprojection_phase_local_sham_variant(pilot)) {
...
static void emit_slot_records(int pilot) {
    ...
    for (int slot = 0; slot < slot_count; slot++) {
        ...
        const char *token = coded_preprojection_variant(pilot)
            ? (coded_preprojection_query_scramble_variant(pilot)
                ? coded_preprojection_warm_query_scramble_sequence[slot]
                : (coded_preprojection_query_off_variant(pilot)
                    ? coded_preprojection_warm_query_off_sequence[slot]
                : (coded_preprojection_phase_local_variant(pilot)
                    ? coded_preprojection_warm_phase_local_sham_sequence[slot]
...
static int execute_authorized(int argc, char **argv) {
    ...
    const char *required_schedule = coded_preprojection_query_scramble_variant(pilot)
        ...
        : (coded_preprojection_phase_local_sham_variant(pilot)
            ? coded_preprojection_warm_phase_local_sham_schedule_sha256
        : (coded_preprojection_active_source_chop_variant(pilot)
            ? coded_preprojection_active_source_chop_schedule_sha256
        : (coded_preprojection_source_phase_chop_variant(pilot)
            ? coded_preprojection_source_phase_chop_schedule_sha256
        : (coded_preprojection_active_query_variant(pilot)
            ? coded_preprojection_active_query_schedule_sha256
        : (pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP
            ? coded_preprojection_warm_phase_local_schedule_sha256
        : (coded_preprojection_warm_restored_variant(pilot)
    ...
    if (!compiled_authority_sha256 || !compiled_output_root ||
        ...
        pilot < GATE_A_PILOT_PN || pilot > GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP ||
        (coded_preprojection_query_scramble_variant(pilot)
            ...
            : (coded_preprojection_phase_local_sham_variant(pilot)
                ? validate_coded_preprojection_warm_phase_local_sham_schedule_semantics()
            : (coded_preprojection_active_source_chop_variant(pilot)
                ? validate_coded_preprojection_active_source_chop_schedule_semantics()
            : (coded_preprojection_source_phase_chop_variant(pilot)
                ? validate_coded_preprojection_source_phase_chop_schedule_semantics()
            : (coded_preprojection_active_query_variant(pilot)
                ? validate_coded_preprojection_active_query_schedule_semantics()
            : (pilot == GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP
                ? validate_coded_preprojection_warm_phase_local_sham_schedule_semantics()
            : (coded_preprojection_warm_restored_variant(pilot)
...
static int cache_response_self_test(void) {
    ...
    for (int slot = 3; slot <= 14; ++slot) {
        if (pilot_working_set_bytes(
                ...
                GATE_A_OCCUPANCY_EQUAL_BYTES) return 1;
        if (pilot_working_set_bytes(
                slot, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP) !=
                GATE_A_OCCUPANCY_EQUAL_BYTES) return 1;
    }
    if (pilot_working_set_bytes(3, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(5, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(8, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(11, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES ||
        pilot_working_set_bytes(14, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES) return 1;
    if (pilot_working_set_bytes(3, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(5, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(8, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(11, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES ||
        pilot_working_set_bytes(14, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES) return 1;
    if (pilot_working_set_bytes(3, GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(5, GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(8, GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(11, GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES ||
        pilot_working_set_bytes(14, GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES) return 1;
    if (pilot_working_set_bytes(3, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(5, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(8, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_LARGE_BYTES ||
        pilot_working_set_bytes(11, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES ||
        pilot_working_set_bytes(14, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP) !=
            GATE_A_OCCUPANCY_SMALL_BYTES) return 1;
    if (self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_WARM_DECLARATION_SHAM_LOOP)) return 1;
    if (self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP)) return 1;
    if (self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP)) return 1;
    if (self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP)) return 1;
    if (self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP)) return 1;
    return self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP);
}
...
int main(int argc, char **argv) {
    ...
    if (argc == 2 && string_equal(argv[1], "--self-test-coded-preprojection-warm-query-scramble")) {
        return self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_WARM_QUERY_SCRAMBLE_LOOP);
    }
    ...
    if (argc == 2 && string_equal(argv[1], "--self-test-coded-preprojection-warm-phase-local-sham")) {
        return self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP);
    }
    if (argc == 2 && string_equal(argv[1], "--self-test-coded-preprojection-warm-phase-local")) {
        return self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP);
    }
    if (argc == 2 && string_equal(argv[1], "--self-test-coded-preprojection-active-query")) {
        return self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP);
    }
    if (argc == 2 && string_equal(argv[1], "--self-test-coded-preprojection-source-phase-chop")) {
        return self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP);
    }
    if (argc == 2 && string_equal(argv[1], "--self-test-coded-preprojection-active-source-chop")) {
        return self_test(NULL, GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP);
    }
    ...
    if (argc == 3 && string_equal(argv[1], "--self-test-coded-preprojection-warm-query-scramble-retain")) {
        return self_test(argv[2], GATE_A_PILOT_CODED_PREPROJECTION_WARM_QUERY_SCRAMBLE_LOOP);
    }
    ...
    if (argc == 3 && string_equal(argv[1], "--self-test-coded-preprojection-warm-phase-local-sham-retain")) {
        return self_test(argv[2], GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_SHAM_LOOP);
    }
    if (argc == 3 && string_equal(argv[1], "--self-test-coded-preprojection-warm-phase-local-retain")) {
        return self_test(argv[2], GATE_A_PILOT_CODED_PREPROJECTION_WARM_PHASE_LOCAL_LOOP);
    }
    if (argc == 3 && string_equal(argv[1], "--self-test-coded-preprojection-active-query-retain")) {
        return self_test(argv[2], GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_QUERY_LOOP);
    }
    if (argc == 3 && string_equal(argv[1], "--self-test-coded-preprojection-source-phase-chop-retain")) {
        return self_test(argv[2], GATE_A_PILOT_CODED_PREPROJECTION_SOURCE_PHASE_CHOP_LOOP);
    }
    if (argc == 3 && string_equal(argv[1], "--self-test-coded-preprojection-active-source-chop-retain")) {
        return self_test(argv[2], GATE_A_PILOT_CODED_PREPROJECTION_ACTIVE_SOURCE_CHOP_LOOP);
    }
    if (argc >= 2 && string_equal(argv[1], "--execute-authorized")) {
        return execute_authorized(argc, argv);
    }
    fputs("usage: gate_a_worker --validate-only | --self-test | --self-test-cache-response | --self-test-timing-diagnostics | --self-test-coded-preprojection | --self-test-coded-preprojection-restored | --self-test-coded-preprojection-warm-restored | --self-test-coded-preprojection-warm-query-scramble | --self-test-coded-preprojection-warm-query-off | --self-test-coded-preprojection-warm-declaration-sham | --self-test-coded-preprojection-warm-phase-local-sham | --self-test-coded-preprojection-warm-phase-local | --self-test-coded-preprojection-active-query | --self-test-coded-preprojection-source-phase-chop | --self-test-coded-preprojection-active-source-chop | --self-test-retain ABSOLUTE_OUTPUT | --self-test-cache-response-retain ABSOLUTE_OUTPUT | --self-test-readonly-cache-response-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-restored-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-warm-restored-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-warm-query-scramble-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-warm-query-off-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-warm-declaration-sham-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-warm-phase-local-sham-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-warm-phase-local-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-active-query-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-source-phase-chop-retain ABSOLUTE_OUTPUT | --self-test-coded-preprojection-active-source-chop-retain ABSOLUTE_OUTPUT | --execute-authorized ...\n", stderr);
    return 2;
}
