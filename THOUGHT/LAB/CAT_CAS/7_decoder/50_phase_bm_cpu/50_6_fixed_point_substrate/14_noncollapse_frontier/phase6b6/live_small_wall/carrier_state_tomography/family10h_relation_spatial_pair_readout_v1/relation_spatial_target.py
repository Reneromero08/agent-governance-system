#!/usr/bin/env python3
"""Target-side authority gate and offline preflight for relation-spatial v1.
...
AUTHORITY_ENV = "FAMILY10H_RELATION_SPATIAL_LIVE_AUTHORITY"
SOURCE_AUTHORITY_ENV = "FAMILY10H_RELATION_SPATIAL_SOURCE_AUTHORITY_COMMIT"
FREEZE_COMMIT_ENV = "FAMILY10H_RELATION_SPATIAL_FREEZE_COMMIT"
MANIFEST_ENV = "FAMILY10H_RELATION_SPATIAL_MANIFEST_SHA256"
LEGACY_COMMIT_ENV = "FAMILY10H_RELATION_SPATIAL_COMMIT_BINDING"
RUNTIME_AUTHORITY_ENV = "FAMILY10H_RELATION_SPATIAL_RUNTIME_AUTHORITY"
PREFLIGHT_FIXTURE_ENV = "FAMILY10H_RELATION_SPATIAL_PREFLIGHT_FIXTURE"
PREFLIGHT_SYSTEM_MOCK_ENV = "FAMILY10H_RELATION_SPATIAL_PREFLIGHT_SYSTEM_MOCK"
DEPLOYMENT_CUSTODY_ENV = "FAMILY10H_RELATION_SPATIAL_DEPLOYMENT_CUSTODY"
EXECUTION_MODE_ENV = "FAMILY10H_RELATION_SPATIAL_EXECUTION_MODE"
EXECUTION_MODE_SYNTHETIC = "synthetic"
AUTHORITY_VALUE = pub.TRANSACTION_RUN_ID
RUNTIME_BINARY_NAMES = ["relation_spatial_runtime", "relation_spatial_runtime.exe"]
PMU_PREFLIGHT_HELPER_NAMES = ["relation_spatial_pmu_preflight", "relation_spatial_pmu_preflight.exe"]
...
def wsl_distro(source_root: Path) -> str | None:
    build_receipt = source_root / "RELATION_SPATIAL_RUNTIME_BUILD_SELF_TEST.json"
...
def validate_source_root(source_root: Path) -> dict[str, Any]:
    ...
    required = [
        ...
        "RELATION_SPATIAL_TOOLCHAIN_DISCOVERY.json",
        "RELATION_SPATIAL_RUNTIME_BUILD_SELF_TEST.json",
        "RELATION_SPATIAL_SYNTHETIC_EXECUTOR_SELF_TEST.json",
        "RELATION_SPATIAL_PHYSICAL_ADJUDICATOR_SELF_TEST.json",
        "RELATION_SPATIAL_PHYSICAL_THRESHOLD_CONTRACT.json",
    ...
    schedule_manifest = read_json(source_root / "RELATION_SPATIAL_PUBLIC_SCHEDULE.json")
    schedule = pub.build_schedule(grammar)
    manifest = read_json(source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json")
    source_hashes = read_json(source_root / "RELATION_SPATIAL_SOURCE_HASHES.json")
    sensor_binding = read_json(source_root / pub.SENSOR_AUTHORITY_BINDING_FILENAME)
    threshold = read_json(source_root / "RELATION_SPATIAL_PHYSICAL_THRESHOLD_CONTRACT.json")
    readiness = read_json(source_root / "RELATION_SPATIAL_BUILD_READINESS.json") if (source_root / "RELATION_SPATIAL_BUILD_READINESS.json").exists() else {}
    ...
    schedule_tsv_sha = pub.sha256_file(source_root / "RELATION_SPATIAL_PUBLIC_SCHEDULE.tsv")
    if schedule_tsv_sha != (source_root / "RELATION_SPATIAL_PUBLIC_SCHEDULE.sha256").read_text(encoding="utf-8").strip():
        failures.append("expanded schedule TSV sha sidecar mismatch")
    if schedule_manifest.get("schema") != "FAMILY10H_RELATION_SPATIAL_PUBLIC_SCHEDULE_MANIFEST_V3":
    ...
    manifest_file_sha = pub.sha256_file(source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json")
    if manifest_file_sha != (source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.sha256").read_text(encoding="utf-8").strip():
...
def artifact_identity(source_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "schedule_tsv_sha256": pub.sha256_file(source_root / "RELATION_SPATIAL_PUBLIC_SCHEDULE.tsv"),
        ...
        "manifest_file_sha256": pub.sha256_file(source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json"),
...
def runtime_refusal_probe(source_root: Path) -> dict[str, Any]:
    ...
    command, launch_mode = runtime_command(
        source_root,
        runtime,
        ["--execute-schedule", source_root / "RELATION_SPATIAL_PUBLIC_SCHEDULE.tsv", source_root / "_no_live_output"],
    )
    completed = subprocess.run(
        command,
        text=True,
...
def base_preflight_fixture(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    return {
        "schema": "FAMILY10H_RELATION_SPATIAL_PREFLIGHT_FIXTURE_V1",
        "cpu": {
            "vendor": "AuthenticAMD",
            "family": 16,
        ...
        "pmu": {
            "schema": "FAMILY10H_RELATION_SPATIAL_DISABLED_PMU_PREFLIGHT_V1",
            "passed": True,
            "returncode": 0,
...
def pmu_preflight_helper_identity(source_root: Path, source_hashes: dict[str, Any] | None = None) -> dict[str, Any]:
    if source_hashes is None:
        hashes_path = source_root / "RELATION_SPATIAL_SOURCE_HASHES.json"
    ...
    c_source = source_root / "relation_spatial_pmu_preflight.c"
    expected_binary_sha = authority.get("compiled_binary_sha256")
    expected_c_sha = authority.get("helper_c_sha256") or source_hashes.get("files", {}).get("relation_spatial_pmu_preflight.c", {}).get("sha256")
...
def physical_preflight_observation(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    if mock_path and not synthetic:
        return {
            "schema": "FAMILY10H_RELATION_SPATIAL_PHYSICAL_PREFLIGHT_OBSERVATION_V1",
    ...
    return {
        ...
        | {"relation_source_authority_commit": relation_source_authority(read_json(source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json"))},
        "runtime_binary_format_contains": ["ELF 64-bit", "x86-64"],
...
def target_preflight(source_root: Path, output_root: Path, fixture: dict[str, Any] | None) -> dict[str, Any]:
    ...
    manifest = read_json(source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json") if (source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json").exists() else {}
    schedule_manifest = read_json(source_root / "RELATION_SPATIAL_PUBLIC_SCHEDULE.json") if (source_root / "RELATION_SPATIAL_PUBLIC_SCHEDULE.json").exists() else {}
    source_hashes = read_json(source_root / "RELATION_SPATIAL_SOURCE_HASHES.json") if (source_root / "RELATION_SPATIAL_SOURCE_HASHES.json").exists() else {}
    ...
    return {
        "schema": "FAMILY10H_RELATION_SPATIAL_TARGET_PREFLIGHT_V2",
...
def target_preflight_refusal(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    result["schema"] = "FAMILY10H_RELATION_SPATIAL_TARGET_PREFLIGHT_REFUSAL_V2"
...
def target_authority_env(source_root: Path, fixture_path: Path | None, *, synthetic: bool = False) -> dict[str, str | None]:
    manifest_sha = pub.sha256_file(source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json")
    manifest = read_json(source_root / "RELATION_SPATIAL_IMPLEMENTATION_MANIFEST.json")
    return {
...
def execute_authorized(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    return {
        "schema": "FAMILY10H_RELATION_SPATIAL_SYNTHETIC_TARGET_WRAPPER_EXECUTION_V1"
        if synthetic
        else "FAMILY10H_RELATION_SPATIAL_TARGET_WRAPPER_EXECUTION_V1",
...
def run_preflight_fixture_suite(source_root: Path, parent: Path) -> dict[str, Any]:
    ...
    parent.mkdir()
    return {
        "schema": "FAMILY10H_RELATION_SPATIAL_TARGET_PREFLIGHT_FIXTURE_SUITE_V1",
        "results": results,
...
def run_helper_authority_negative_suite(source_root: Path, parent: Path) -> dict[str, Any]:
    ...
    def mutate_helper_authority(candidate: Path) -> None:
        path = candidate / "RELATION_SPATIAL_SOURCE_HASHES.json"
        data = read_json(path)
    ...
    return {
        "schema": "FAMILY10H_RELATION_SPATIAL_HELPER_AUTHORITY_NEGATIVE_SUITE_V1",
        "results": results,
        "passed": all(item["passed"] for item in results.values()),
    }
...
def base_physical_mock(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    mock["schema"] = "FAMILY10H_RELATION_SPATIAL_PHYSICAL_PREFLIGHT_OBSERVATION_V1"
...
def run_physical_preflight_mock_suite(source_root: Path, parent: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="relation_spatial_physical_preflight_",
    ...
    return {
        "schema": "FAMILY10H_RELATION_SPATIAL_PHYSICAL_PREFLIGHT_MOCK_SUITE_V1",
        "results": results,
        "passed": all(item["passed"] for item in results.values()),
...
def run_authority_refusal_tests(source_root: Path, parent: Path) -> dict[str, Any]:
    ...
    return {
        "schema": "FAMILY10H_RELATION_SPATIAL_AUTHORITY_REFUSAL_TESTS_V1",
        "tests": tests,
...
def self_test(source_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        safe_remove_owned_output_parent(source_root)
    result = {
        "schema": "FAMILY10H_RELATION_SPATIAL_TARGET_SELF_TEST_V2",
        "science_package_id": pub.SCIENCE_PACKAGE_ID,
        "transaction_run_id": pub.TRANSACTION_RUN_ID,
        "offline_only": True,
        "target_contact_count": 0,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = {
            "schema": "FAMILY10H_RELATION_SPATIAL_TARGET_REFUSAL_V2",
