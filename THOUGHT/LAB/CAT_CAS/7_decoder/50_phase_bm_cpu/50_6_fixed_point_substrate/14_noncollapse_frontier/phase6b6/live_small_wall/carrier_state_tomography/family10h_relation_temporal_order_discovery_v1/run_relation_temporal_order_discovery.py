...
RUN_ID = "family10h_relation_temporal_order_discovery_v1_0"
...
QUERY_BY_VARIANT = {
    variant: ("query_relation_pair" if variant.endswith("_primary") else "relation_sham")
    for variant in VARIANTS
}
ROWS_PER_VARIANT = 2048
PAIR_SAMPLE_COUNT = 256
EXPECTED_PAIRS_PER_VARIANT = ROWS_PER_VARIANT * PAIR_SAMPLE_COUNT
MATCHED_PERMUTATION_COUNT = 63
MATCHED_PERMUTATION_SEED = "family10h-relation-temporal-order-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "TEMPORAL_ORDER_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_TEMPORAL_ORDER_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_TEMPORAL_ORDER_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_temporal_order() -> None:
    ...
    require(map_function in text, "relation_spatial_map_index anchor not found")
    text = text.replace(map_function, temporal_map_function, 1)
    text = text.replace(
    ...
    text = text.replace(
        ...
        "        parity ^= 1u;\n"
        "    }\n"
        "    if (row->r_query == RELATION_SPATIAL_R1) {\n"
        "        parity ^= 1u;\n"
        "    }\n"
        "    return (parity & 1u) == 0u;\n",
        1,
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        for index, row in enumerate(base_rows):
            ...
            copied["control_semantics_id"] = "none" if variant.endswith("_primary") else query
        ...
        schedules[variant] = {
            "query": query,
            "temporal_relation_coding": "r1_inverts_order_r0_preserves_order",
            "path": f"TEMPORAL_ORDER_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        ...
        "mechanism_question": (
            "Whether encoding r0/r1 as AB-versus-BA temporal order under identical spatial marginals "
            "exposes an oriented relation phase after local primary-minus-sham subtraction."
        ),
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_TEMPORAL_ORDER_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_TEMPORAL_ORDER_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/TEMPORAL_ORDER_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_TEMPORAL_ORDER_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_TEMPORAL_ORDER_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_TEMPORAL_ORDER_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_TEMPORAL_ORDER_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_TEMPORAL_ORDER_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(RUN_ROOT / "RELATION_TEMPORAL_ORDER_DISCOVERY_ANALYSIS.json", result)
    write_json(SUMMARY_JSON, result)
    lines = [
        "# Relation Temporal Order Discovery",
        "",
        f"Run ID: `{RUN_ID}`",
    ...
    lines.extend(
        [
            ...
            f"- temporal order candidate: `{result['interpretation']['shift_sweep_candidate']}`",
            "",
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-redef]
    require(LOCAL_ARCHIVE.exists(), "archive missing for analysis")
    reports = {}
    records_by_variant: dict[str, list[dict[str, Any]]] = {}
    with tarfile.open(LOCAL_ARCHIVE, "r:gz") as tf:
        ...
        for variant in VARIANTS:
            ...
            reports[variant] = {
                "query": QUERY_BY_VARIANT[variant],
                "relation_shift_lines": SHIFT_BY_VARIANT[variant],
                "raw_record_count": len(raw),
                "pair_observation_count": len(pairs),
                "source_death_receipt_count": len(deaths),
                "block_count": len(records),
    ...
    method_pairs = {
        f"shift_{shift:04d}": (f"shift_{shift:04d}_primary", f"shift_{shift:04d}_sham")
        for shift in RELATION_SHIFT_VALUES
    }
    method_reports: dict[str, Any] = {}
    for method, (primary_variant, sham_variant) in method_pairs.items():
        primary = reports[primary_variant]["R_spatial"]["mean"]
        ...
        axis_delta = {
            key: primary_axes[key] - sham_axes[key]
            for key in ["common_axis", "diag_R_axis", "prep_axis", "query_axis", "offdiag_skew_axis"]
        }
        method_reports[method] = {
            "relation_shift_lines": SHIFT_BY_VARIANT[primary_variant],
            "primary_variant": primary_variant,
            "sham_variant": sham_variant,
            "primary_R": primary,
            "sham_R": sham,
    ...
    baseline_method = "shift_0001"
    baseline_d = method_reports[baseline_method]["D_primary_minus_sham"]
    baseline_pq_mag = method_reports[baseline_method]["prep_query_magnitude"]
    baseline_ds_mag = method_reports[baseline_method]["diag_skew_magnitude"]
    for report in method_reports.values():
        report["D_abs_to_shift_0001_D_abs"] = abs(report["D_primary_minus_sham"]) / abs(baseline_d) if baseline_d else None
        report["prep_query_magnitude_abs_to_shift_0001"] = report["prep_query_magnitude"] / baseline_pq_mag if baseline_pq_mag else None
        report["diag_skew_magnitude_abs_to_shift_0001"] = report["diag_skew_magnitude"] / baseline_ds_mag if baseline_ds_mag else None
    ...
    baseline_strata = {factor: d_by_factor(baseline_method, factor) for factor in factors}
    stratum_reports: dict[str, Any] = {}
    for method in method_pairs:
        factor_reports = {}
        ratios: list[float] = []
        ...
        for factor in factors:
            ...
            level_reports = {}
            for level, value in method_values.items():
                baseline_value = baseline_strata.get(factor, {}).get(level)
                ratio = abs(value) / abs(baseline_value) if baseline_value else None
                if ratio is not None:
                    ratios.append(ratio)
                d_values.append(value)
                level_reports[level] = {
                    "D_primary_minus_sham": value,
                    "shift_0001_matched_D": baseline_value,
                    "abs_to_shift_0001_matched_abs": ratio,
                    "D_positive": value > 0,
                    "D_negative": value < 0,
                }
            factor_reports[factor] = {
                "levels": level_reports,
                "all_D_positive": all(item["D_positive"] for item in level_reports.values()),
                "all_D_negative": all(item["D_negative"] for item in level_reports.values()),
            }
        stratum_reports[method] = {
            "factors": factor_reports,
            "all_one_factor_strata_D_positive": all(value > 0 for value in d_values),
            "all_one_factor_strata_D_negative": all(value < 0 for value in d_values),
            "max_abs_to_shift_0001_matched_abs": max(ratios) if ratios else None,
            "min_D_primary_minus_sham_stratum": min(d_values) if d_values else None,
            "max_D_primary_minus_sham_stratum": max(d_values) if d_values else None,
        }

    d_values_by_shift = [
        (shift, method_reports[f"shift_{shift:04d}"]["D_primary_minus_sham"])
        for shift in RELATION_SHIFT_VALUES
    ]
    positive_shifts = [shift for shift, value in d_values_by_shift if value > 0]
    negative_shifts = [shift for shift, value in d_values_by_shift if value < 0]
    best_abs_method = max(method_reports, key=lambda method: abs(method_reports[method]["D_primary_minus_sham"]))
    best_pq_method = max(method_reports, key=lambda method: method_reports[method]["prep_query_magnitude"])
    best_ds_method = max(method_reports, key=lambda method: method_reports[method]["diag_skew_magnitude"])
    log2_shifts = [math.log2(float(shift)) for shift, _ in d_values_by_shift]
    d_values = [value for _, value in d_values_by_shift]
    abs_d_values = [abs(value) for value in d_values]
    ...
    result = {
        ...
        "stratum_reports": stratum_reports,
        "coordinates": {
            "baseline_shift_lines": 1,
            "baseline_D_primary_minus_sham": baseline_d,
            "D_by_shift": [{"shift": shift, "D_primary_minus_sham": value} for shift, value in d_values_by_shift],
            "positive_D_shifts": positive_shifts,
            "negative_D_shifts": negative_shifts,
            "best_abs_D_shift": method_reports[best_abs_method]["relation_shift_lines"],
            "best_abs_D_primary_minus_sham": method_reports[best_abs_method]["D_primary_minus_sham"],
            "best_prep_query_axis_shift": method_reports[best_pq_method]["relation_shift_lines"],
            "best_diag_skew_axis_shift": method_reports[best_ds_method]["relation_shift_lines"],
            "D_min": min(d_values) if d_values else None,
            "D_max": max(d_values) if d_values else None,
            "D_range": (max(d_values) - min(d_values)) if d_values else None,
            "log2_shift_spearman_D": spearman(log2_shifts, d_values),
            "log2_shift_spearman_abs_D": spearman(log2_shifts, abs_d_values),
        },
        "interpretation": {
            "D_sign_changes_across_shifts": bool(positive_shifts and negative_shifts),
            "all_shift_D_positive": len(positive_shifts) == len(RELATION_SHIFT_VALUES),
            "all_shift_D_negative": len(negative_shifts) == len(RELATION_SHIFT_VALUES),
            "any_shift_with_all_one_factor_strata_D_negative": any(
                stratum_reports[method]["all_one_factor_strata_D_negative"] for method in method_reports
            ),
            "any_shift_with_all_one_factor_strata_D_positive": any(
                stratum_reports[method]["all_one_factor_strata_D_positive"] for method in method_reports
            ),
            "candidate_oriented_shift_selectivity": bool(positive_shifts and negative_shifts),
            "exploratory_only": True,
            "small_wall_crossed": False,
        },
        "claim_boundary": {
    ...
    for shift in RELATION_SHIFT_VALUES:
        method = f"shift_{shift:04d}"
        report = method_reports[method]
        stratum = stratum_reports[method]
        lines.append(
            f"| {shift} | {report['primary_R']:.9f} | {report['sham_R']:.9f} | "
            f"{report['D_primary_minus_sham']:.9f} | "
            f"{(report['D_abs_to_shift_0001_D_abs'] or 0.0):.3f} | "
            f"{(stratum['max_abs_to_shift_0001_matched_abs'] or 0.0):.3f} | "
            f"{stratum['all_one_factor_strata_D_positive']} | "
            f"{stratum['all_one_factor_strata_D_negative']} |"
        )
    lines.extend(
        [
            "",
    ...
    for shift in RELATION_SHIFT_VALUES:
        report = method_reports[f"shift_{shift:04d}"]
        lines.append(
            f"| {shift} | {report['prep_query_phase_radians']:.6f} | "
            f"{(report['prep_query_magnitude_abs_to_shift_0001'] or 0.0):.3f} | "
            f"{report['diag_skew_phase_radians']:.6f} | "
            f"{(report['diag_skew_magnitude_abs_to_shift_0001'] or 0.0):.3f} |"
        )
    lines.extend(
        [
            "",
            "Discovery interpretation:",
            f"- D sign changes across shifts: `{result['interpretation']['D_sign_changes_across_shifts']}`",
            f"- all shift D positive: `{result['interpretation']['all_shift_D_positive']}`",
            f"- best absolute-D shift: `{result['coordinates']['best_abs_D_shift']}`",
            f"- log2(shift) Spearman vs D: `{result['coordinates']['log2_shift_spearman_D']:.6f}`",
            f"- log2(shift) Spearman vs abs(D): `{result['coordinates']['log2_shift_spearman_abs_D']:.6f}`",
            f"- oriented shift-selectivity candidate: `{result['interpretation']['candidate_oriented_shift_selectivity']}`",
            "",
            "This is exploratory evidence only. It emits no prospective scientific claim and does not promote `SMALL_WALL_CROSSED`.",
        ]
    )
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-redef]
    ...
    primary_variant = "temporal_primary"
    sham_variant = "temporal_sham"
    primary = reports[primary_variant]["R_spatial"]["mean"]
    sham = reports[sham_variant]["R_spatial"]["mean"]
    delta = primary - sham
    primary_axes = reports[primary_variant]["relation_cell_axes"]
    ...
    result = {
        ...
        "coordinates": {
            "R_temporal_primary": primary,
            "R_temporal_sham": sham,
            "D_temporal_primary_minus_sham": delta,
            "primary_relation_cell_axes": primary_axes,
            "sham_relation_cell_axes": sham_axes,
            "axis_primary_minus_sham": axis_delta,
            "diag_skew_phase_radians": method_report["diag_skew_phase_radians"],
            "diag_skew_magnitude": method_report["diag_skew_magnitude"],
            "prep_query_phase_radians": method_report["prep_query_phase_radians"],
            "prep_query_magnitude": method_report["prep_query_magnitude"],
        },
        "interpretation": {
            "temporal_D_positive": delta > 0,
            "temporal_all_one_factor_strata_D_positive": all(value > 0 for value in d_values),
            "temporal_order_axis_nonzero": method_report["diag_skew_magnitude"] > 0.0 or method_report["prep_query_magnitude"] > 0.0,
            "temporal_order_candidate_for_followup": delta > 0 and all(value > 0 for value in d_values),
            "exploratory_only": True,
            "small_wall_crossed": False,
        },
        "claim_boundary": {
    ...
    lines = [
        ...
        f"Analysis SHA-256: `{result['analysis_sha256']}`",
        "",
        "| Coordinate | Primary R | Sham R | D primary-minus-sham | all one-factor strata D+ |",
        "|---|---:|---:|---:|---:|",
        f"| temporal order | {primary:.9f} | {sham:.9f} | {delta:.9f} | {result['stratum_report']['all_one_factor_strata_D_positive']} |",
        "",
        "| Axis | Primary | Sham | Primary-minus-sham |",
        "|---|---:|---:|---:|",
    ]
    for key in ["common_axis", "diag_R_axis", "prep_axis", "query_axis", "offdiag_skew_axis"]:
        lines.append(f"| {key} | {primary_axes[key]:.9f} | {sham_axes[key]:.9f} | {axis_delta[key]:.9f} |")
    lines.extend(
        [
            "",
            "Discovery interpretation:",
            f"- temporal D positive: `{result['interpretation']['temporal_D_positive']}`",
            f"- temporal all one-factor strata D positive: `{result['interpretation']['temporal_all_one_factor_strata_D_positive']}`",
            f"- temporal order candidate for follow-up: `{result['interpretation']['temporal_order_candidate_for_followup']}`",
            "",
            "This is exploratory evidence only. It emits no prospective scientific claim and does not promote `SMALL_WALL_CROSSED`.",
        ]
    )
...
def prepare() -> dict[str, Any]:
    ...
    patch_runtime_for_temporal_order()
    schedule_manifest = make_rows()
    build = compile_runtime()
    result = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_ORDER_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_TEMPORAL_ORDER_PREPARE_RESULT.json", result)
    return result
