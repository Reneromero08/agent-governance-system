...
RUN_ID = "family10h_relation_prep_mutation_discovery_v1_0"
...
VARIANTS = [
    ...
    "prep_inversion_control",
    "carrier_off_control",
    "relation_sham",
]
QUERY_BY_VARIANT = {
    "primary_relation_pair": "query_relation_pair",
    "prep_inversion_control": "prep_inversion_control",
...
MATCHED_PERMUTATION_SEED = "family10h-prep-mutation-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "PREP_MUTATION_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_PREP_MUTATION_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_PREP_MUTATION_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_transform_kill() -> None:
    ...
    text = text.replace(
        ...
        "RELATION_SPATIAL_CONTROL_DISTANCE_MATCHED = 4,\n"
        "    RELATION_SPATIAL_CONTROL_PREP_INVERSION = 5,\n"
    ...
    text = text.replace(
        ...
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "prep_inversion_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_PREP_INVERSION;\n"
        "        return 1;\n"
        "    }\n"
    ...
    patched_prep_block = (
        ...
        "                prep.bank_a_work = row.bank_a_work;\n"
        "                prep.bank_b_work = row.bank_b_work;\n"
                "                prep.relation = row.control == RELATION_SPATIAL_CONTROL_PREP_INVERSION\n"
                "                    ? (row.r_prepare == RELATION_SPATIAL_R0 ? RELATION_SPATIAL_R1 : RELATION_SPATIAL_R0)\n"
                "                    : row.r_prepare;\n"
        "                prep.source_order = row.source_order;\n"
        "                prep.cyclic_origin = row.cyclic_origin;\n"
        "                shared->preparation_ok = relation_spatial_prepare(prep, &shared->state);\n"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_PREP_MUTATION_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"PREP_MUTATION_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_PREP_MUTATION_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
    ...
    write_json(SOURCE_ROOT / "RELATION_PREP_MUTATION_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_PREP_MUTATION_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_PREP_MUTATION_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_PREP_MUTATION_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_PREP_MUTATION_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/PREP_MUTATION_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_PREP_MUTATION_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_PREP_MUTATION_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_PREP_MUTATION_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_PREP_MUTATION_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_PREP_MUTATION_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_PREP_MUTATION_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_PREP_MUTATION_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_PREP_MUTATION_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_PREP_MUTATION_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_PREP_MUTATION_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    prep_inverted = reports["prep_inversion_control"]["R_spatial"]["mean"]
    carrier_off = reports["carrier_off_control"]["R_spatial"]["mean"]
    sham = reports["relation_sham"]["R_spatial"]["mean"]
    prep_mutation = primary - prep_inverted
    result = {
        "schema": "FAMILY10H_RELATION_PREP_MUTATION_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "coordinates": {
            "R_primary": primary,
            "R_prep_inversion": prep_inverted,
            "R_carrier_off": carrier_off,
            "R_relation_sham": sham,
            "D_primary_minus_sham": primary - sham,
            "prep_mutation_primary_minus_prep_inversion": prep_mutation,
            "prep_mutation_half_difference": 0.5 * prep_mutation,
            "prep_inversion_abs_to_primary_abs": abs(prep_inverted) / abs(primary) if primary else None,
        ...
        "interpretation": {
            "prep_inversion_reverses_primary": primary > 0 and prep_inverted < 0,
            "prep_mutation_nonzero_above_25pct_primary": primary > 0 and abs(prep_mutation) > 0.25 * abs(primary),
    ...
    write_json(RUN_ROOT / "RELATION_PREP_MUTATION_DISCOVERY_ANALYSIS.json", result)
    write_json(SUMMARY_JSON, result)
    lines = [
        "# Relation Prep-Mutation Discovery",
        "",
        f"Run ID: `{RUN_ID}`",
    ...
    lines.extend(
        [
            ...
            f"- prep inversion reverses primary: `{result['interpretation']['prep_inversion_reverses_primary']}`",
            f"- prep mutation above 0.25 x primary: `{result['interpretation']['prep_mutation_nonzero_above_25pct_primary']}`",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_PREP_MUTATION_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_PREP_MUTATION_PREPARE_RESULT.json", result)
    return result
