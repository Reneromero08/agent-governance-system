...
RUN_ID = "family10h_relation_offset_signed_source_off_sweep_discovery_v1_0"
...
OFFSETS = [2, 4, 8, 16, 1024]
VARIANTS = [
    f"{lifecycle}_offset{offset}_signed"
    for offset in OFFSETS
    for lifecycle in ("alive", "source_off")
]
QUERY_BY_VARIANT = {
    f"alive_offset{offset}_signed": f"source_on_offset_{offset}_control" for offset in OFFSETS
}
QUERY_BY_VARIANT.update(
    {f"source_off_offset{offset}_signed": f"source_off_offset_{offset}_control" for offset in OFFSETS}
)
LIFECYCLE_BY_VARIANT = {
    f"{lifecycle}_offset{offset}_signed": lifecycle
    for offset in OFFSETS
    for lifecycle in ("alive", "source_off")
}
OFFSET_BY_VARIANT = {
    f"{lifecycle}_offset{offset}_signed": offset
    for offset in OFFSETS
    for lifecycle in ("alive", "source_off")
}
...
MATCHED_PERMUTATION_SEED = "family10h-offset-signed-source-off-sweep-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "OFFSET_SIGNED_SOURCE_OFF_SWEEP_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY_SUMMARY.md"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        for index, row in enumerate(base_rows):
            ...
            copied["matched_twin_group"] = f"{copied['block_id']}:rmat:o{OFFSET_BY_VARIANT[variant]}:{LIFECYCLE_BY_VARIANT[variant]}"
            copied["matched_twin_pair"] = f"{copied['block_id']}:rp_{copied['block_local_position']}:o{OFFSET_BY_VARIANT[variant]}:{LIFECYCLE_BY_VARIANT[variant]}"
            rows.append(copied)
        ...
        schedules[variant] = {
            ...
            "path": f"OFFSET_SIGNED_SOURCE_OFF_SWEEP_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
    ...
    write_json(SOURCE_ROOT / "RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/OFFSET_SIGNED_SOURCE_OFF_SWEEP_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    source_off_kill_offsets: list[int] = []
    stratum_reports: dict[str, Any] = {}
    for offset in OFFSETS:
        alive_variant = f"alive_offset{offset}_signed"
        source_off_variant = f"source_off_offset{offset}_signed"
        ...
        for variant in [alive_variant, source_off_variant]:
            factor_reports = {}
            ratios: list[float] = []
            values: list[float] = []
            for factor in factors:
                ...
                for level in levels:
                    ...
                    values.append(value)
                    level_reports[level] = {
                        "C_offset_signed": value,
                        "matched_alive_C_offset_signed": alive_value,
                        "abs_to_matched_alive_abs": ratio,
                        "same_sign_as_alive_aggregate": (value > 0) == (alive > 0) if alive else False,
                        "below_25pct_matched_alive": ratio is not None and ratio <= 0.25,
                    }
        ...
        aggregate_ratio = abs(source_off) / abs(alive) if alive else None
        source_off_kill = (
            alive != 0.0
            and aggregate_ratio is not None
            and aggregate_ratio <= 0.25
            and stratum_reports[alive_variant]["all_same_sign_as_alive_aggregate"]
            and stratum_reports[source_off_variant]["all_one_factor_strata_below_25pct_alive_matched"]
        )
        if source_off_kill:
            source_off_kill_offsets.append(offset)
        offset_reports[str(offset)] = {
            "alive_variant": alive_variant,
            "source_off_variant": source_off_variant,
            "C_alive_offset_signed": alive,
            "C_source_off_offset_signed": source_off,
            "source_off_abs_to_alive_abs": aggregate_ratio,
            "alive_all_one_factor_strata_same_sign": stratum_reports[alive_variant]["all_same_sign_as_alive_aggregate"],
            "source_off_all_one_factor_strata_below_25pct_alive": stratum_reports[source_off_variant][
                "all_one_factor_strata_below_25pct_alive_matched"
            ],
            "source_off_screen_hit": source_off_kill,
            "max_source_off_stratum_abs_to_alive_matched_abs": stratum_reports[source_off_variant][
    ...
    best_offset = min(
        ...
        key=lambda offset: (
            float("inf")
            if offset_reports[str(offset)]["source_off_abs_to_alive_abs"] is None
            else offset_reports[str(offset)]["source_off_abs_to_alive_abs"]
        ),
    )
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "receiver_projection": "relation-matrix contrast of per-row mean(B_first_touch_cycles - A_first_touch_cycles) at signed offset sweep",
        "offsets": OFFSETS,
        "variant_reports": reports,
        "stratum_reports": stratum_reports,
        "offset_reports": offset_reports,
        "coordinates": {
            "best_source_off_ratio_offset": best_offset,
            "best_source_off_abs_to_alive_abs": offset_reports[str(best_offset)]["source_off_abs_to_alive_abs"],
            "source_off_screen_hit_offsets": source_off_kill_offsets,
        },
        "interpretation": {
            "any_source_off_screen_hit": bool(source_off_kill_offsets),
            "best_offset_for_followup": best_offset,
            "followup_needed_for_dead_reset_persistence": bool(source_off_kill_offsets),
            "offset_signed_source_off_sweep_candidate": bool(source_off_kill_offsets),
            "exploratory_only": True,
            "small_wall_crossed": False,
        },
        "claim_boundary": {
    ...
    write_json(RUN_ROOT / "RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_DISCOVERY_ANALYSIS.json", result)
    ...
    for variant in VARIANTS:
        ...
        offset = OFFSET_BY_VARIANT[variant]
        if LIFECYCLE_BY_VARIANT[variant] == "alive":
            ratio = 1.0
        else:
            ratio = offset_reports[str(offset)]["source_off_abs_to_alive_abs"] or 0.0
        lines.append(
    ...
    lines.extend(
        [
            ...
            f"- source-off screen hit offsets: `{result['coordinates']['source_off_screen_hit_offsets']}`",
            f"- best source-off ratio offset: `{result['coordinates']['best_source_off_ratio_offset']}`",
            f"- best source-off abs/alive abs: `{result['coordinates']['best_source_off_abs_to_alive_abs']:.3f}`",
            f"- any source-off screen hit: `{result['interpretation']['any_source_off_screen_hit']}`",
            f"- followup needed for dead/reset persistence: `{result['interpretation']['followup_needed_for_dead_reset_persistence']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_OFFSET_SIGNED_SOURCE_OFF_SWEEP_PREPARE_RESULT.json", result)
    return result
