...
RUN_ID = "family10h_relation_temporal_source_off_discovery_v1_0"
...
MATCHED_PERMUTATION_SEED = "family10h-relation-temporal-source-off-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "TEMPORAL_SOURCE_OFF_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_TEMPORAL_SOURCE_OFF_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_TEMPORAL_SOURCE_OFF_DISCOVERY_SUMMARY.md"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"TEMPORAL_SOURCE_OFF_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        ...
        "mechanism_question": (
            "Whether the temporal-order relation coordinate requires source preparation or is replayed by "
            "receiver/query-order structure under no-source-preparation controls."
        ),
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_TEMPORAL_SOURCE_OFF_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_TEMPORAL_SOURCE_OFF_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/TEMPORAL_SOURCE_OFF_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_TEMPORAL_SOURCE_OFF_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_TEMPORAL_SOURCE_OFF_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_TEMPORAL_SOURCE_OFF_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_TEMPORAL_SOURCE_OFF_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_TEMPORAL_SOURCE_OFF_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(RUN_ROOT / "RELATION_TEMPORAL_SOURCE_OFF_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- temporal source-off candidate: `{result['interpretation']['phase_mutation_candidate']}`",
            "",
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-redef]
    ...
    for report in method_reports.values():
        report["D_abs_to_alive_D_abs"] = abs(report["D_primary_minus_sham"]) / abs(alive_d) if alive_d else None
    ...
    result = {
        ...
        "coordinates": {
            ...
            "source_off_D_abs_to_alive_D_abs": abs(source_off_d) / abs(alive_d) if alive_d else None,
            "R_alive_primary": method_reports["alive"]["primary_R"],
            "R_alive_sham": method_reports["alive"]["sham_R"],
        ...
        "interpretation": {
            "alive_D_positive": alive_d > 0,
            "source_off_D_positive": source_off_d > 0,
            "source_off_kills_below_25pct_alive_D": abs(source_off_d) <= 0.25 * abs(alive_d) if alive_d else False,
            "source_off_all_one_factor_strata_below_25pct_alive_D": stratum_reports["source_off"][
                "all_one_factor_strata_below_25pct_alive_matched"
            ],
            "source_written_temporal_candidate": (
                alive_d > 0
    ...
    for method in ["alive", "source_off"]:
        report = method_reports[method]
        stratum = stratum_reports[method]
        lines.append(
            f"| {method} | {report['primary_R']:.9f} | {report['sham_R']:.9f} | "
    ...
    lines.extend(
        [
            ...
            "Discovery interpretation:",
            f"- alive D positive: `{result['interpretation']['alive_D_positive']}`",
            f"- source-off D positive: `{result['interpretation']['source_off_D_positive']}`",
            f"- source-off kills below 0.25 x alive D: `{result['interpretation']['source_off_kills_below_25pct_alive_D']}`",
            f"- source-off all one-factor strata below 0.25 x alive D: `{result['interpretation']['source_off_all_one_factor_strata_below_25pct_alive_D']}`",
            f"- source-written temporal candidate: `{result['interpretation']['source_written_temporal_candidate']}`",
            "",
            "This is exploratory evidence only. It emits no prospective scientific claim and does not promote `SMALL_WALL_CROSSED`.",
        ]
    )
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_TEMPORAL_SOURCE_OFF_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_TEMPORAL_SOURCE_OFF_PREPARE_RESULT.json", result)
    return result
