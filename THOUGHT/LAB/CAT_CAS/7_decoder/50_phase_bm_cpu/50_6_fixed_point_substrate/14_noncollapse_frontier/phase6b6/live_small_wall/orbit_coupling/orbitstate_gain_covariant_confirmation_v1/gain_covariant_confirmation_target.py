...
import gain_covariant_confirmation_public as public


HERE = Path(__file__).resolve().parent
SCIENCE_PACKAGE_ID = public.RUN_ID
TRANSACTION_RUN_ID = "orbitstate_gain_covariant_confirmation_v1_0"
COMMIT_ENV = "ORBITSTATE_GAIN_COVARIANT_V1_COMMIT_BINDING"
MANIFEST_ENV = "ORBITSTATE_GAIN_COVARIANT_V1_MANIFEST_SHA256"
AUTHORITY_ENV = "ORBITSTATE_GAIN_COVARIANT_V1_LIVE_AUTHORITY"
AUTHORITY_VALUE = TRANSACTION_RUN_ID
OUTPUT_ROOT_MARKER = ".gain_covariant_confirmation_v1_owned"

SOURCE_FILE_NAMES = [
    "FINAL_CONFIRMATION_CONTRACT.md",
    "GAIN_COVARIANT_CONTRACT_REFERENCE.json",
    "gain_covariant_confirmation_public.py",
    "gain_covariant_confirmation_runtime.c",
    "gain_covariant_confirmation_runtime.h",
    "gain_covariant_confirmation_target.py",
    "run_gain_covariant_confirmation_v1.py",
    "GAIN_COVARIANT_PUBLIC_SCHEDULE.json",
    "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv",
    "GAIN_COVARIANT_PUBLIC_SCHEDULE.sha256",
    "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json",
    "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.sha256",
]

SUCCESS_EVIDENCE_FILES = [
    "GAIN_COVARIANT_CONFIRMATION_MANIFEST.json",
    "GAIN_COVARIANT_SOURCE_BUNDLE.tar.gz",
    "GAIN_COVARIANT_SOURCE_HASHES.json",
    "RAW_GAIN_COVARIANT_RECEIVER_CAPTURE.jsonl",
    "GAIN_COVARIANT_RECEIVER_SENTINELS.jsonl",
    "GAIN_COVARIANT_STAGE_RECEIPTS.jsonl",
    "GAIN_COVARIANT_SOURCE_RECEIPTS.jsonl",
    "GAIN_COVARIANT_RECEIVER_FEATURES.json",
    "GAIN_COVARIANT_RECEIVER_FEATURES.sha256",
    "GAIN_COVARIANT_FEATURE_FREEZE_RECEIPT.json",
    "GAIN_COVARIANT_ADJUDICATION.json",
    "LIVE_CUSTODY_LOG.json",
    "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json",
...
def validate_public_schedule_only(source_root: Path) -> dict[str, Any]:
    schedule = read_json(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json")
    reference = read_json(source_root / "GAIN_COVARIANT_CONTRACT_REFERENCE.json")
    ...
    require((source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv").read_text(encoding="utf-8") == expected_tsv, "schedule TSV drift")
    schedule_sha_line = read_json(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.sha256")
    require(schedule_sha_line["json_sha256"] == sha256_file(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json"), "schedule JSON sha file drift")
    require(schedule_sha_line["tsv_sha256"] == sha256_file(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv"), "schedule TSV sha file drift")
    return {
        "passed": True,
        "public_schedule_json_sha256": sha256_file(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json"),
        "public_schedule_tsv_sha256": sha256_file(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv"),
...
def private_map_validator_summary(source_root: Path) -> dict[str, Any]:
    schedule = read_json(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json")
    source_map = read_json(source_root / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json")
    public.validate_private_source_map(schedule, source_map)
    private_sha_line = (source_root / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.sha256").read_text(encoding="utf-8").strip()
    canonical = public.digest(source_map)
    require(private_sha_line.startswith(canonical), "private source map sha file drift")
    return {
        "passed": True,
        "file_sha256": sha256_file(source_root / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json"),
...
def public_manifest_blindness(source_root: Path) -> dict[str, Any]:
    schedule_json = (source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json").read_text(encoding="utf-8").lower()
    schedule_tsv = (source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv").read_text(encoding="utf-8").lower()
    forbidden = [
        "pre_projection_d",
        "pre_projection_fold",
        "source_off",
...
def process_boundary_static_proof(source_root: Path) -> dict[str, Any]:
    runtime = (source_root / "gain_covariant_confirmation_runtime.c").read_text(encoding="utf-8")
...
def feature_boundary_static_proof(source_root: Path) -> dict[str, Any]:
    text = (source_root / "gain_covariant_confirmation_target.py").read_text(encoding="utf-8")
    ...
    result = {
        "schema": "GAIN_COVARIANT_FEATURE_BOUNDARY_SELF_TEST_V2",
    ...
    write_json(source_root / "GAIN_COVARIANT_FEATURE_BOUNDARY_SELF_TEST.json", result)
    return result


def custody_gate_static_proof(source_root: Path) -> dict[str, Any]:
    text = (source_root / "gain_covariant_confirmation_target.py").read_text(encoding="utf-8")
    public_text = (source_root / "gain_covariant_confirmation_public.py").read_text(encoding="utf-8")
    exception_cleanup_snapshot = 'best_effort_process_snapshot("exception cleanup when hardware execution began")'
    forbidden_exception_cleanup_gate = "require_clean_process_snapshot(" + exception_cleanup_snapshot + ")"
    raw_exception_cleanup_snapshot = '                process_snapshot("exception cleanup when hardware execution began")'
    hardware_execution_marker = 'state["hardware_execution_began"] = True'
    live_start = text.index("def execute_live(")
    cleanup_start = text.index("    except Exception as exc:", text.index(hardware_execution_marker, live_start))
    cleanup_end = text.index("        seal_failure_evidence(output_root", cleanup_start)
    exception_cleanup_block = text[cleanup_start:cleanup_end]
    checks = {
        "process_snapshot_enforced": "def require_clean_process_snapshot" in text
        and text.count("require_clean_process_snapshot(process_snapshot(") >= 3,
        "process_scan_returncode_gated": 'snapshot.get("scan_returncode") == 0' in text,
        "process_forbidden_hits_gated": 'snapshot.get("forbidden_process_hits") == []' in text,
        "exception_cleanup_cannot_block_failure_seal": "def best_effort_process_snapshot" in text
        and "snapshot_failed" in text
        and exception_cleanup_snapshot in exception_cleanup_block
        and forbidden_exception_cleanup_gate not in exception_cleanup_block
        and raw_exception_cleanup_snapshot not in exception_cleanup_block,
        "source_receipts_passed_to_phase_law": "source_receipts_by_id_component=source_receipts_by_id_component" in public_text,
        "phase_partition_uses_receipt_q": "def receipt_q(row" in public_text
        and 'receipt["q_theta"]' in public_text,
    }
...
def pmu_runtime_static_proof(source_root: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "event_ids_checked": "event ID positivity/distinctness failed" in runtime
        and "measurement.cycles_id == 0u" in runtime
        and "measurement.change_to_dirty_id == measurement.probe_dirty_id" in runtime,
...
def compile_runtime(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    binary = output_root / "gain_covariant_confirmation_runtime"
    source = source_root / "gain_covariant_confirmation_runtime.c"
    include = source_root
    base_flags = [
        "-std=c11",
        "-Wall",
...
def disassembly_receipt(compile_receipt: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    path = output_root / "GAIN_COVARIANT_RUNTIME_DISASSEMBLY_NORMALIZED.txt"
...
def run_runtime_self_test(compile_receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    data = json.loads(completed.stdout)
    return {
        "passed": data["status"] == "GAIN_COVARIANT_RUNTIME_SELF_TEST_OK",
        "returncode": completed.returncode,
        "stdout": completed.stdout,
        "stderr": completed.stderr,
        "data": data,
...
def run_runtime_schedule_validation(compile_receipt: dict[str, Any], source_root: Path) -> dict[str, Any]:
    schedule_arg = str(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv")
    if compile_receipt["runtime_command"][0] == "wsl":
        schedule_arg = windows_to_wsl_path(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv")
    completed = run_command([*compile_receipt["runtime_command"], "--validate-schedule-tsv", schedule_arg], timeout=30.0)
    data = json.loads(completed.stdout)
    return {
        "passed": data["status"] == "GAIN_COVARIANT_PUBLIC_SCHEDULE_TSV_OK",
        "returncode": completed.returncode,
        "stdout": completed.stdout,
        "stderr": completed.stderr,
        "data": data,
...
def baseline_control_audit(public_self: dict[str, Any]) -> dict[str, Any]:
    ...
    required = required_gain_controls | required_geometry_controls | required_strong_controls | required_near_zero_controls
    ...
    return {
        ...
        "required_gain_controls": sorted(required_gain_controls),
        "required_geometry_controls": sorted(required_geometry_controls),
        "required_strong_controls": sorted(required_strong_controls),
        "required_near_zero_controls": sorted(required_near_zero_controls),
...
def create_absent_output_root(output_root: Path) -> None:
    ...
    (output_root / OUTPUT_ROOT_MARKER).write_text("gain_covariant_confirmation_v1_owned\n", encoding="utf-8")
...
def failure_copyback_self_test() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="orbitstate_failure_packet_") as temp:
        ...
        required = {
            "TARGET_FAILURE_GAIN_COVARIANT_CONFIRMATION.json",
            "GAIN_COVARIANT_CONFIRMATION_FAILURE_MANIFEST.json",
            "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json",
            "LIVE_CUSTODY_LOG.json",
        }
        final = read_json(output_root / "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json")
...
def offline_validate(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    custody_gates = custody_gate_static_proof(source_root)
    ...
    bundle = deterministic_source_bundle(source_root, output_root / "GAIN_COVARIANT_SOURCE_BUNDLE.tar.gz")
    hashes = source_hashes(source_root)
    write_json(output_root / "GAIN_COVARIANT_SOURCE_HASHES.json", hashes)
    compile_receipt = compile_runtime(source_root, output_root)
    runtime_self = run_runtime_self_test(compile_receipt)
    write_json(output_root / "GAIN_COVARIANT_RUNTIME_SELF_TEST.json", runtime_self)
    ...
    result = {
        "schema": "GAIN_COVARIANT_TARGET_OFFLINE_VALIDATE_V1",
        "zero_live_contact": True,
        ...
        "custody_gate_static_proof": custody_gates,
    ...
    write_json(output_root / "GAIN_COVARIANT_OFFLINE_VALIDATE.json", result)
    return result
...
def best_effort_process_snapshot(label: str) -> dict[str, Any]:
    try:
        snapshot = process_snapshot(label)
        ...
        return {
            ...
            "snapshot_exception_type": type(exc).__name__,
            "snapshot_exception": str(exc),
        }


def require_clean_process_snapshot(snapshot: dict[str, Any]) -> dict[str, Any]:
    require(snapshot.get("scan_returncode") == 0, f"process scan failed at {snapshot.get('label')}")
    require(snapshot.get("forbidden_process_hits") == [], f"forbidden process hit at {snapshot.get('label')}")
    return snapshot
...
def run_runtime_pmu_preflight(compile_receipt: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    write_json(output_root / "GAIN_COVARIANT_PMU_PREFLIGHT.json", data)
    return {
        "passed": data.get("status") == "GAIN_COVARIANT_RUNTIME_PMU_PREFLIGHT_OK",
        "returncode": completed.returncode,
        "stdout": completed.stdout,
        "stderr": completed.stderr,
        "data": data,
        "sha256": sha256_file(output_root / "GAIN_COVARIANT_PMU_PREFLIGHT.json"),
    }
...
def run_receiver_feature_subprocess(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    inputs = {
        "GAIN_COVARIANT_PUBLIC_SCHEDULE.json": source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json",
        "RAW_GAIN_COVARIANT_RECEIVER_CAPTURE.jsonl": output_root / "RAW_GAIN_COVARIANT_RECEIVER_CAPTURE.jsonl",
        "GAIN_COVARIANT_RECEIVER_SENTINELS.jsonl": output_root / "GAIN_COVARIANT_RECEIVER_SENTINELS.jsonl",
        "GAIN_COVARIANT_STAGE_RECEIPTS.jsonl": output_root / "GAIN_COVARIANT_STAGE_RECEIPTS.jsonl",
    }
    ...
    shutil.copyfile(feature_output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.json", output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.json")
    shutil.copyfile(feature_output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.sha256", output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.sha256")
...
def receiver_features_main(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(args.output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.json", features)
    (args.output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.sha256").write_text(
        features["receiver_features_sha256"] + "  GAIN_COVARIANT_RECEIVER_FEATURES.json\n", encoding="utf-8"
    )
    return {
        "passed": True,
...
def run_adjudication_subprocess(
    ...
    receipt_path = output_root / "GAIN_COVARIANT_FEATURE_FREEZE_RECEIPT.json"
    write_json(
        receipt_path,
        {
            "schema": "GAIN_COVARIANT_FEATURE_FREEZE_RECEIPT_V2",
            "science_package_id": SCIENCE_PACKAGE_ID,
            "transaction_run_id": TRANSACTION_RUN_ID,
            "run_id": TRANSACTION_RUN_ID,
            "events": chronology,
    ...
    command = [
        ...
        "--adjudicate-frozen",
        "--schedule-json",
        str(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json"),
        "--receiver-features",
        str(output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.json"),
        "--receiver-features-sha256",
        receiver_features_sha256,
        "--private-map",
        str(source_root / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json"),
        "--source-receipts",
        str(output_root / "GAIN_COVARIANT_SOURCE_RECEIPTS.jsonl"),
...
def adjudicate_frozen_main(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(args.output_root / "GAIN_COVARIANT_ADJUDICATION.json", adjudication)
    receipt["events"] = events
    receipt["private_map_opened_after_feature_hash_frozen"] = True
    receipt["adjudication_sha256"] = sha256_file(args.output_root / "GAIN_COVARIANT_ADJUDICATION.json")
    ...
    return {
        ...
        "adjudication_sha256": sha256_file(args.output_root / "GAIN_COVARIANT_ADJUDICATION.json"),
...
def write_copyback_manifest(output_root: Path) -> dict[str, Any]:
    ...
    manifest = {
        "schema": "GAIN_COVARIANT_COPYBACK_MANIFEST_V1",
        "science_package_id": SCIENCE_PACKAGE_ID,
        "transaction_run_id": TRANSACTION_RUN_ID,
        "run_id": TRANSACTION_RUN_ID,
        "entries": entries,
...
def success_execution_manifest(
    *,
    ...
    evidence_names = [
        "GAIN_COVARIANT_SOURCE_BUNDLE.tar.gz",
        "GAIN_COVARIANT_SOURCE_HASHES.json",
        "gain_covariant_confirmation_runtime",
        "GAIN_COVARIANT_RUNTIME_DISASSEMBLY_NORMALIZED.txt",
        "RAW_GAIN_COVARIANT_RECEIVER_CAPTURE.jsonl",
        "GAIN_COVARIANT_RECEIVER_SENTINELS.jsonl",
        "GAIN_COVARIANT_STAGE_RECEIPTS.jsonl",
        "GAIN_COVARIANT_SOURCE_RECEIPTS.jsonl",
        ...
        "LIVE_CUSTODY_LOG.json",
        "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json",
        "GAIN_COVARIANT_PMU_PREFLIGHT.json",
    ]
    final_hashes = {name: sha256_file(output_root / name) for name in evidence_names if (output_root / name).exists()}
    raw_count = len(public.read_jsonl(output_root / "RAW_GAIN_COVARIANT_RECEIVER_CAPTURE.jsonl"))
    stage_count = len(public.read_jsonl(output_root / "GAIN_COVARIANT_STAGE_RECEIPTS.jsonl"))
    source_count = len(public.read_jsonl(output_root / "GAIN_COVARIANT_SOURCE_RECEIPTS.jsonl"))
    manifest = {
        "schema": "GAIN_COVARIANT_CONFIRMATION_MANIFEST_V1",
        "science_package_id": SCIENCE_PACKAGE_ID,
        "transaction_run_id": TRANSACTION_RUN_ID,
        "run_id": TRANSACTION_RUN_ID,
        "contract_sha256": sha256_file(source_root / "FINAL_CONFIRMATION_CONTRACT.md"),
        ...
        "public_schedule_json_sha256": sha256_file(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json"),
        "public_schedule_tsv_sha256": sha256_file(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv"),
        "private_source_map_file_sha256": sha256_file(source_root / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json"),
        ...
        "raw_capture_sha256": sha256_file(output_root / "RAW_GAIN_COVARIANT_RECEIVER_CAPTURE.jsonl"),
        "sentinels_sha256": sha256_file(output_root / "GAIN_COVARIANT_RECEIVER_SENTINELS.jsonl"),
        "stage_receipts_sha256": sha256_file(output_root / "GAIN_COVARIANT_STAGE_RECEIPTS.jsonl"),
        "source_receipts_sha256": sha256_file(output_root / "GAIN_COVARIANT_SOURCE_RECEIPTS.jsonl"),
        "receiver_features_sha256": sha256_file(output_root / "GAIN_COVARIANT_RECEIVER_FEATURES.json"),
        "feature_freeze_receipt_sha256": sha256_file(output_root / "GAIN_COVARIANT_FEATURE_FREEZE_RECEIPT.json"),
        "adjudication_sha256": sha256_file(output_root / "GAIN_COVARIANT_ADJUDICATION.json"),
        "final_result_sha256": sha256_file(final_result_path),
        "pmu_preflight_sha256": pmu_preflight["sha256"],
        "source_hashes_sha256": sha256_file(output_root / "GAIN_COVARIANT_SOURCE_HASHES.json"),
    ...
    write_json(output_root / "GAIN_COVARIANT_CONFIRMATION_MANIFEST.json", manifest)
    return manifest
...
def seal_failure_evidence(
    ...
    *,
    ...
    failure = {
        "schema": "TARGET_FAILURE_GAIN_COVARIANT_CONFIRMATION",
        "science_package_id": SCIENCE_PACKAGE_ID,
        "transaction_run_id": TRANSACTION_RUN_ID,
        "run_id": TRANSACTION_RUN_ID,
        "failure_phase": phase,
    ...
    if marker.exists():
        marker.unlink()
    write_json(output_root / "TARGET_FAILURE_GAIN_COVARIANT_CONFIRMATION.json", failure)
    final = {
        "status": "ORBITSTATE_GAIN_COVARIANT_TARGET_FAILED",
        "science_package_id": SCIENCE_PACKAGE_ID,
        "transaction_run_id": TRANSACTION_RUN_ID,
        "run_id": TRANSACTION_RUN_ID,
        "failure_phase": phase,
    ...
    write_json(output_root / "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json", final)
    write_json(output_root / "LIVE_CUSTODY_LOG.json", custody)
    files = {
    ...
    manifest = {
        "schema": "GAIN_COVARIANT_FAILURE_MANIFEST_V1",
        "science_package_id": SCIENCE_PACKAGE_ID,
        "transaction_run_id": TRANSACTION_RUN_ID,
        "run_id": TRANSACTION_RUN_ID,
        "failure_phase": phase,
    ...
    write_json(output_root / "GAIN_COVARIANT_CONFIRMATION_FAILURE_MANIFEST.json", manifest)
...
def execute_live(
    ...
    *,
    ...
    try:
        ...
        manifest_path = source_root / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json"
        ...
        source_bundle = deterministic_source_bundle(source_root, output_root / "GAIN_COVARIANT_SOURCE_BUNDLE.tar.gz")
        source_hash_receipt = source_hashes(source_root)
        write_json(output_root / "GAIN_COVARIANT_SOURCE_HASHES.json", source_hash_receipt)
        compile_receipt = compile_runtime(source_root, output_root)
        ...
        append_custody(custody, "process_snapshots", require_clean_process_snapshot(process_snapshot("before execution")), output_root)
        ...
        schedule_arg = str(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv")
        private_arg = str(source_root / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json")
        output_arg = str(output_root)
        if runtime_command[0] == "wsl":
            schedule_arg = windows_to_wsl_path(source_root / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv")
            private_arg = windows_to_wsl_path(source_root / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json")
        ...
        for replicate in public.REPLICATES:
            ...
            append_custody(
                custody,
                "process_snapshots",
                require_clean_process_snapshot(process_snapshot(f"after replicate {replicate}")),
                output_root,
            )
        ...
        append_custody(custody, "process_snapshots", require_clean_process_snapshot(process_snapshot("final")), output_root)
        ...
        adjudication = read_json(output_root / "GAIN_COVARIANT_ADJUDICATION.json")
        final = {
            "status": "ORBITSTATE_GAIN_COVARIANT_TARGET_COMPLETE",
            "science_package_id": SCIENCE_PACKAGE_ID,
            "transaction_run_id": run_id,
            "run_id": run_id,
        ...
        final_path = output_root / "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json"
        ...
        if state.get("hardware_execution_began"):
            append_custody(
                custody,
                "process_snapshots",
                best_effort_process_snapshot("exception cleanup when hardware execution began"),
                output_root,
            )
...
def self_test(source_root: Path, output_root: Path) -> dict[str, Any]:
    offline_path = output_root / "GAIN_COVARIANT_OFFLINE_VALIDATE.json"
    offline = read_json(offline_path) if offline_path.exists() else offline_validate(source_root, output_root)
    result = {
        "schema": "GAIN_COVARIANT_TARGET_SELF_TEST_V2",
        ...
        "custody_gate_static_proof": offline["custody_gate_static_proof"],
    ...
    write_json(output_root / "GAIN_COVARIANT_TARGET_SELF_TEST.json", result)
    return result
