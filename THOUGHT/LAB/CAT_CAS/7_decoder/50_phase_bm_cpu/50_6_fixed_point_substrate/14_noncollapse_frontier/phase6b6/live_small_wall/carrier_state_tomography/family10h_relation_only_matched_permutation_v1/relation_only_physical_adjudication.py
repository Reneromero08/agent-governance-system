...
def effect_summary(rows: list[dict[str, Any]], threshold_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "passed": not failures and abs(effect) >= floor,
        "failures": failures,
        "block_count": len(values),
        "R_match_mean": effect,
        "R_match_abs_mean": abs(effect),
...
def control_nulls(rows: list[dict[str, Any]], threshold_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in rows:
        if row.get("row_role") == "relation_control":
            by_control[row["query"]].append(float(row["dirty_probe_response"]))
    controls = {}
    for control in pub.CONTROL_ROWS:
        ...
        controls[control] = {
            "count": len(values),
            "mean": mean(values),
            "abs_mean": abs(mean(values)),
            "passed": bool(values) and abs(mean(values)) <= threshold,
        }
...
def label_scramble_test(rows: list[dict[str, Any]], threshold_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in rows:
        item = dict(row)
        if item.get("row_role") == "relation_matrix" and item.get("execution_ordinal", 0) // 12 % 2 == 0:
            item["r_query"] = "relation_r1" if item["r_query"] == "relation_r0" else "relation_r0"
        scrambled.append(item)
    summary = effect_summary(scrambled, threshold_contract)
    threshold = threshold_contract["resampling_thresholds"]["label_scramble_abs_max"]
    return {
        "passed": summary["R_match_abs_mean"] <= threshold,
        "scrambled_R_match_abs_mean": summary["R_match_abs_mean"],
        "threshold": threshold,
    }
...
def bootstrap_stability(rows: list[dict[str, Any]], threshold_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for offset in range(64):
        sample = [values[(offset + step * 37) % len(values)] for step in range(len(values))]
        signs.append(1 if mean(sample) > 0 else -1 if mean(sample) < 0 else 0)
