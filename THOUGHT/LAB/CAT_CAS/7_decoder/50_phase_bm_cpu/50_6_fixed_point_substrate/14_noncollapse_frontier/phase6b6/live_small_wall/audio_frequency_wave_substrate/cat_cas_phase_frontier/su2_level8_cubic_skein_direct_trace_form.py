#!/usr/bin/env python3
"""M223 direct coordinate trace form for M222 weighted residual cells.
...
The calculation retains one rational accumulator and bounded rational
temporaries, never a conjugate, norm-cell, or weighted-cell field value.  All
rational temporaries and loop integers are included in the logical live-state
accounting.  The identical coordinate recurrence remains classical.
"""
...
def install_direct_trace_path() -> None:
    ...
    m222.line_minimum = line_minimum
    m222.balance_resident = balance_resident
    m221.Work = Work
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit(
            "usage: su2_level8_cubic_skein_direct_trace_form.py "
            "SEPARATE_REFERENCE_JSON"
        )
    reference_path = Path(sys.argv[1]).resolve()
    if str(reference_path).startswith(("/dev/shm/", "/run/shm/")):
        raise ValueError("RAM-backed M223 reference is forbidden")
    reference = json.loads(reference_path.read_text())
    if reference.get("schema") != "cat_cas.su2_level8_cubic_skein_direct_trace_form_reference.v1":
        raise RuntimeError("M223 separate-reference schema changed")
    ...
    if [m221.reference_case_view(case) for case in cases] != reference.get("cases"):
        raise RuntimeError("M223 independent case and resource parity failed")
    ...
    for section in ("primary", "reuse", "fresh_reuse"):
        for key in (
            ...
            if reuse[section][key] != reference["reuse"][section][key]:
                raise RuntimeError(
                    f"M223 independent reuse parity failed: {section}.{key}"
                )
        normalized_work = m221.reference_case_view(reuse[section])["work"]
        if normalized_work != reference["reuse"][section]["work"]:
            raise RuntimeError(
                f"M223 independent normalized work parity failed: {section}"
            )
    for key in (
        ...
        if reuse[key] != reference["reuse"][key]:
            raise RuntimeError(f"M223 independent reuse parity failed: {key}")
    ...
    if (
        ...
        raise RuntimeError("M223 control failed")
    selected = [
        {
            ...
            "direct_trace_maximum_declared_live_payload_bits": case["work"][
            ...
            "direct_trace_maximum_context": case["work"][
    ...
    if all_nontrivial_smaller:
        result_name = "PASS_BOUNDED_EXACT_DIRECT_TRACE_FORM_LIFECYCLE_REDUCTION"
        claim = "BOUNDED_EXACT_DIRECT_TRACE_COORDINATE_FORM_ELIMINATES_RESIDUAL_CELL_CONJUGATE_NORM_CELL_AND_WEIGHTED_CELL_CYCLOTOMIC_FIELD_MATERIALIZATION_FROM_THE_ACCEPTED_TRACE_SUBPATH_AND_REDUCES_DECLARED_EXACT_LIVE_PAYLOAD_BELOW_THE_MATCHED_RAW_RECURRENCE_ON_EVERY_DECLARED_DEPTH_ABOVE_ONE_WITH_FINAL_ONLY_BOUNDARY_EXACT_RESTORATION_REUSE_BUT_ONE_SCALE_CONJUGATE_PER_BALANCE_AND_THE_IDENTICAL_CLASSICAL_COORDINATE_RECURRENCE_REMAIN"
    elif every_case_above:
        result_name = "PASS_BOUNDED_EXACT_DIRECT_TRACE_FORM_PERSISTING_HEIGHT_NO_GO"
        claim = "BOUNDED_EXACT_DIRECT_TRACE_COORDINATE_FORM_ELIMINATES_RESIDUAL_CELL_CONJUGATE_NORM_CELL_AND_WEIGHTED_CELL_CYCLOTOMIC_FIELD_MATERIALIZATION_FROM_THE_ACCEPTED_TRACE_SUBPATH_WITH_FINAL_ONLY_BOUNDARY_EXACT_RESTORATION_REUSE_BUT_ONE_SCALE_CONJUGATE_PER_BALANCE_REMAINS_AND_DECLARED_EXACT_LIVE_PAYLOAD_REMAINS_ABOVE_THE_MATCHED_RAW_RECURRENCE_ON_EVERY_DECLARED_CASE_BECAUSE_CALLER_RETAINED_FIELD_SCALARS_PLUS_FIVE_SIMULTANEOUSLY_LIVE_RATIONAL_COORDINATE_VALUES_DOMINATE_AND_THE_IDENTICAL_CLASSICAL_COORDINATE_RECURRENCE_REMAINS"
    else:
        result_name = "PASS_BOUNDED_EXACT_DIRECT_TRACE_FORM_MIXED_LIFECYCLE_RESULT"
        claim = "BOUNDED_EXACT_DIRECT_TRACE_COORDINATE_FORM_ELIMINATES_RESIDUAL_CELL_CONJUGATE_NORM_CELL_AND_WEIGHTED_CELL_CYCLOTOMIC_FIELD_MATERIALIZATION_FROM_THE_ACCEPTED_TRACE_SUBPATH_WITH_FINAL_ONLY_BOUNDARY_EXACT_RESTORATION_REUSE_AND_A_MIXED_DECLARED_LIVE_PAYLOAD_RESULT_AGAINST_THE_MATCHED_RAW_RECURRENCE_WHILE_ONE_SCALE_CONJUGATE_PER_BALANCE_AND_THE_IDENTICAL_CLASSICAL_COORDINATE_RECURRENCE_REMAIN"
    here = Path(__file__).resolve().parent
    result = {
        "schema": "cat_cas.su2_level8_cubic_skein_direct_trace_form.v1",
        "result": result_name,
        "claim": claim,
        "classification": "INDEPENDENTLY_VERIFIED_STRICT_SCOPE",
        "verification_level": "SEPARATE_REFERENCE_PARITY",
        "restoration_classification": "EXACT_ALGEBRAIC_RESTORATION",
        "claim_ceiling": "FORMAL_PRETRUNCATION_QZETA40_M219_CUBIC_SKEIN_M220_UNIT_M221_LEDGER_NATIVE_M222_STREAMED_PARAMETERS3_7_9_11_13_17_19_DIRECT_POWER_BASIS_RAMANUJAN_TRACE_LINE_SEARCH_FAMILY0_STRANDS4_DEPTH1TO4_STRANDS6_DEPTH1TO2_STRANDS8_DEPTH1_PRIMARY4_DEPTH4_REUSE4_DEPTH2_FAMILY1_DIRECT_PROCESS_ONLY",
        "mechanism": {
            ...
            "lifecycle_scale_conjugates_materialized": True,
            "lifecycle_scale_conjugates_per_balance": 1,
        ...
        "lifecycle_law": {
            ...
            "dominant_direct_trace_contexts": contexts,
            ...
            "lifecycle_scale_conjugates_honestly_reported": all(
                case["work"][
                    "lifecycle_scale_conjugate_field_values_materialized"
                ]
                == case["work"]["balance_calls"]
                for case in cases
            ),
        ...
        "matched_classical_baselines": {
            "strongest_compact": "IDENTICAL_LEDGER_NATIVE_DIRECT_POWER_BASIS_RAMANUJAN_TRACE_COORDINATE_RECURRENCE",
        ...
        "source_dependencies": {
            ...
            "m223_production_sha256": sha256_file(Path(__file__).resolve()),
            "m223_separate_reference_code_sha256": sha256_file(
                here / "su2_level8_cubic_skein_direct_trace_form_separate_reference.py"
            ),
            "m223_separate_reference_result_sha256": sha256_file(reference_path),
        },
        "terminal": False,
    }
