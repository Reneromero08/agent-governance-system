...
static int parse_control(const char *query, relation_spatial_control_id *out) {
    ...
    if (strcmp(query, "palindrome_a_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_PALINDROME_A_PRIMARY;
        return 1;
    }
    if (strcmp(query, "palindrome_a_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_PALINDROME_A_SHAM;
        return 1;
    }
    if (strcmp(query, "palindrome_b_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_PALINDROME_B_PRIMARY;
        return 1;
    }
    if (strcmp(query, "palindrome_b_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_PALINDROME_B_SHAM;
        return 1;
    }
    if (strcmp(query, "dead_palindrome_a_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_PALINDROME_A_PRIMARY;
        return 1;
    }
    if (strcmp(query, "dead_palindrome_a_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_PALINDROME_A_SHAM;
        return 1;
    }
    if (strcmp(query, "dead_palindrome_b_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_PALINDROME_B_PRIMARY;
        return 1;
    }
    if (strcmp(query, "dead_palindrome_b_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_PALINDROME_B_SHAM;
        return 1;
    }
    if (strcmp(query, "source_off_palindrome_a_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_PALINDROME_A_PRIMARY;
        return 1;
    }
    if (strcmp(query, "source_off_palindrome_a_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_PALINDROME_A_SHAM;
        return 1;
    }
    if (strcmp(query, "source_off_palindrome_b_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_PALINDROME_B_PRIMARY;
        return 1;
    }
    if (strcmp(query, "source_off_palindrome_b_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_PALINDROME_B_SHAM;
        return 1;
    }
    if (strcmp(query, "block_replay_a_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_BLOCK_REPLAY_A_PRIMARY;
        return 1;
    }
    if (strcmp(query, "block_replay_a_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_BLOCK_REPLAY_A_SHAM;
        return 1;
    }
    if (strcmp(query, "block_replay_b_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_BLOCK_REPLAY_B_PRIMARY;
        return 1;
    }
    if (strcmp(query, "block_replay_b_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_BLOCK_REPLAY_B_SHAM;
        return 1;
    }
    if (strcmp(query, "dead_block_replay_a_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_BLOCK_REPLAY_A_PRIMARY;
        return 1;
    }
    if (strcmp(query, "dead_block_replay_a_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_BLOCK_REPLAY_A_SHAM;
        return 1;
    }
    if (strcmp(query, "dead_block_replay_b_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_BLOCK_REPLAY_B_PRIMARY;
        return 1;
    }
    if (strcmp(query, "dead_block_replay_b_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_DEAD_BLOCK_REPLAY_B_SHAM;
        return 1;
    }
    if (strcmp(query, "source_off_block_replay_a_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_BLOCK_REPLAY_A_PRIMARY;
        return 1;
    }
    if (strcmp(query, "source_off_block_replay_a_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_BLOCK_REPLAY_A_SHAM;
        return 1;
    }
    if (strcmp(query, "source_off_block_replay_b_compose_query_relation_pair_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_BLOCK_REPLAY_B_PRIMARY;
        return 1;
    }
    if (strcmp(query, "source_off_block_replay_b_compose_relation_sham_control") == 0) {
        *out = RELATION_SPATIAL_CONTROL_SOURCE_OFF_BLOCK_REPLAY_B_SHAM;
        return 1;
    }
    return 0;
}
...
static int execute_schedule_common(const char *schedule_path, const char *output_root, int synthetic) {
    ...
    while (fgets(line, sizeof(line), schedule) != NULL) {
        ...
        if (synthetic) {
            ...
            } else if (palindrome_a_control(row.control)) {
                shared->preparation_ok = relation_spatial_prepare_composition4(prep, &shared->state, RELATION_SPATIAL_R0, RELATION_SPATIAL_R1, RELATION_SPATIAL_R1, RELATION_SPATIAL_R0);
            } else if (palindrome_b_control(row.control)) {
                shared->preparation_ok = relation_spatial_prepare_composition4(prep, &shared->state, RELATION_SPATIAL_R1, RELATION_SPATIAL_R0, RELATION_SPATIAL_R0, RELATION_SPATIAL_R1);
            } else if (block_replay_a_control(row.control)) {
                shared->preparation_ok = relation_spatial_prepare_composition4(prep, &shared->state, RELATION_SPATIAL_R0, RELATION_SPATIAL_R0, RELATION_SPATIAL_R1, RELATION_SPATIAL_R1);
            } else if (block_replay_b_control(row.control)) {
                shared->preparation_ok = relation_spatial_prepare_composition4(prep, &shared->state, RELATION_SPATIAL_R1, RELATION_SPATIAL_R1, RELATION_SPATIAL_R0, RELATION_SPATIAL_R0);
