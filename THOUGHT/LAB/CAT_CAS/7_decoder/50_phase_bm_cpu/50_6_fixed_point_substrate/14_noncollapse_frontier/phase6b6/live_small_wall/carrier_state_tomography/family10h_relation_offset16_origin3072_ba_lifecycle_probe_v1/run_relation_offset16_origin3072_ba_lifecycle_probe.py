...
RUN_ID = "family10h_relation_offset16_origin3072_ba_lifecycle_probe_v1_0"
...
OFFSETS = [16]
FOCUS_OFFSET = 16
FOCUS_CYCLIC_ORIGIN = "3072"
FOCUS_QUERY_ORDER = "BA"
VARIANTS = [
...
MATCHED_PERMUTATION_SEED = "family10h-offset16-origin3072-ba-lifecycle-probe-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SUMMARY.md"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            ...
            "path": f"OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
    ...
    write_json(SOURCE_ROOT / "RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "interpretation": {
            ...
            "offset16_origin3072_ba_lifecycle_candidate": lifecycle_candidate,
    ...
    write_json(RUN_ROOT / "RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- lifecycle candidate: `{result['interpretation']['offset16_origin3072_ba_lifecycle_candidate']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_OFFSET16_ORIGIN3072_BA_LIFECYCLE_PROBE_PREPARE_RESULT.json", result)
    return result
