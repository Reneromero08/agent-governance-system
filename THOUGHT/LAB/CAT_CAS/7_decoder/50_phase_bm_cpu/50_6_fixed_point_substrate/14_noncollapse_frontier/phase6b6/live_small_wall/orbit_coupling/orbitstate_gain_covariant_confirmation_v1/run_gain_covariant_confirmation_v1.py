...
import gain_covariant_confirmation_target as target
...
PROTECTED_REMOTE_ROOT_FRAGMENTS = [
    "orbitstate_independent_v2_0",
    "orbitstate_independent_v2_1",
    "independent_window_transducer_v3_0",
    "balanced_transducer_confirmation_v2_0",
    "balanced_transducer_confirmation_v2_1",
]

EXPECTED_STARTING_COMMIT = "4ff588cea11343bf38d4c96c1281d34cbf1961ed"
...
FROZEN_BYTE_FILES = {
    "contract_sha256": HERE / "FINAL_CONFIRMATION_CONTRACT.md",
}
...
def validate_frozen_hashes() -> dict[str, Any]:
    ...
    schedule_sha = read_json(HERE / "GAIN_COVARIANT_PUBLIC_SCHEDULE.sha256")
    observed["public_schedule_json_sha256"] = sha256_file(HERE / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json")
    observed["public_schedule_tsv_sha256"] = sha256_file(HERE / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv")
    observed["public_schedule_canonical_sha256"] = public.digest(read_json(HERE / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json"))
    if observed["public_schedule_json_sha256"] != schedule_sha.get("json_sha256"):
        failures.append("public schedule JSON sidecar mismatch")
    if observed["public_schedule_tsv_sha256"] != schedule_sha.get("tsv_sha256"):
        failures.append("public schedule TSV sidecar mismatch")
    if observed["public_schedule_canonical_sha256"] != schedule_sha.get("canonical_sha256"):
        failures.append("public schedule canonical sidecar mismatch")
    private_map = read_json(HERE / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json")
    observed["private_source_map_canonical_sha256"] = public.digest(private_map)
    observed["private_source_map_file_sha256"] = sha256_file(HERE / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json")
    private_sha_line = (HERE / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.sha256").read_text(encoding="utf-8").strip()
    if not private_sha_line.startswith(observed["private_source_map_canonical_sha256"]):
        failures.append("private source map canonical sidecar mismatch")
...
def fixed_parent_depth_static_rejection(source_root: Path = HERE) -> dict[str, Any]:
    ...
    return {
        "schema": "GAIN_COVARIANT_FIXED_PARENT_DEPTH_STATIC_REJECTION_V1",
...
def copy_shallow_deployment_source(destination: Path) -> None:
    ...
    names = list(target.SOURCE_FILE_NAMES)
    if (HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json").exists():
        names.append("GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json")
    for name in names:
...
def deployment_layout_self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="orbitstate_shallow_deploy_") as temp:
        ...
        import_completed = run(
            [
                ...
                (
                    ...
                    "import gain_covariant_confirmation_target; "
        ...
        offline_completed = run(
            [
                sys.executable,
                str(source / "gain_covariant_confirmation_target.py"),
                "--offline-validate",
                "--source-root",
                str(source),
                "--output-root",
    ...
    result = {
        "schema": "GAIN_COVARIANT_DEPLOYMENT_LAYOUT_SELF_TEST_V1",
...
def build_source_file_map(include_manifest: bool) -> dict[str, dict[str, Any]]:
    ...
    if include_manifest:
        files.append(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json")
...
def build_sol_audit_placeholder() -> dict[str, Any]:
    return {
        "schema": "GAIN_COVARIANT_SOL_IMPLEMENTATION_AUDIT_V2",
        "model": "gpt-5.6-sol",
        "effort": "xhigh",
        "custody": "READ_ONLY",
...
def build_manifest() -> dict[str, Any]:
    ...
    public_audit = ORBIT_COUPLING_ROOT / "orbitstate_gain_covariant_audit/GAIN_COVARIANT_LAW_AUDIT.md"
    offline_validate_path = HERE / "GAIN_COVARIANT_OFFLINE_VALIDATE.json"
    offline = read_json(offline_validate_path) if offline_validate_path.exists() else {}
    transport_path = HERE / "GAIN_COVARIANT_TRANSPORT_SIMULATION.json"
    transport = read_json(transport_path) if transport_path.exists() else {}
    feature_boundary_path = HERE / "GAIN_COVARIANT_FEATURE_BOUNDARY_SELF_TEST.json"
    feature_boundary = read_json(feature_boundary_path) if feature_boundary_path.exists() else {}
    runtime_self_test_path = HERE / "GAIN_COVARIANT_RUNTIME_SELF_TEST.json"
    deployment_layout = read_json(DEPLOYMENT_LAYOUT_TEST_PATH) if DEPLOYMENT_LAYOUT_TEST_PATH.exists() else {}
    implementation_audit_path = HERE / "FINAL_CONFIRMATION_IMPLEMENTATION_AUDIT.md"
    sol_path = HERE / "GAIN_COVARIANT_SOL_IMPLEMENTATION_AUDIT.json"
    if not sol_path.exists():
        write_json(sol_path, build_sol_audit_placeholder())
    sol = read_json(sol_path)
    source_map = read_json(HERE / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json")
    manifest = {
        "schema": "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST_V2",
        "starting_commit": EXPECTED_STARTING_COMMIT,
        ...
        "contract_reference": read_json(HERE / "GAIN_COVARIANT_CONTRACT_REFERENCE.json"),
        "source_file_map": build_source_file_map(include_manifest=False),
        "hashes": {
            "retrospective_gain_covariant_law_audit_sha256": sha256_file(public_audit),
            "implementation_audit_sha256": file_hash_or_none(implementation_audit_path),
            "contract_sha256": sha256_file(HERE / "FINAL_CONFIRMATION_CONTRACT.md"),
            "contract_reference_file_sha256": sha256_file(HERE / "GAIN_COVARIANT_CONTRACT_REFERENCE.json"),
            "public_schedule_json_sha256": sha256_file(HERE / "GAIN_COVARIANT_PUBLIC_SCHEDULE.json"),
            "public_schedule_tsv_sha256": sha256_file(HERE / "GAIN_COVARIANT_PUBLIC_SCHEDULE.tsv"),
            "public_schedule_sha256_file_sha256": sha256_file(HERE / "GAIN_COVARIANT_PUBLIC_SCHEDULE.sha256"),
            "private_source_map_file_sha256": sha256_file(HERE / "GAIN_COVARIANT_PRIVATE_SOURCE_MAP.json"),
            ...
            "runtime_self_test_sha256": file_hash_or_none(runtime_self_test_path),
            "self_test_sha256": file_hash_or_none(HERE / "GAIN_COVARIANT_SELF_TEST.json"),
            "target_self_test_sha256": file_hash_or_none(HERE / "GAIN_COVARIANT_TARGET_SELF_TEST.json"),
            "controller_self_test_sha256": file_hash_or_none(HERE / "GAIN_COVARIANT_CONTROLLER_SELF_TEST.json"),
        ...
        "future_authorization": {
            ...
            "powershell_command": (
                ...
                '"THOUGHT\\LAB\\CAT_CAS\\7_decoder\\50_phase_bm_cpu\\50_6_fixed_point_substrate\\14_noncollapse_frontier\\phase6b6\\live_small_wall\\orbit_coupling\\orbitstate_gain_covariant_confirmation_v1\\run_gain_covariant_confirmation_v1.py" '
                "--execute-authorized"
            ),
        },
        ...
        "small_wall_post_audit_promotion_law": [
            "fresh target result is ORBITSTATE_GAIN_COVARIANT_CONFIRMATION_CONFIRMED",
...
def write_self_test_artifacts() -> dict[str, Any]:
    ...
    write_json(HERE / "GAIN_COVARIANT_SELF_TEST.json", public_self)
...
def validate_source_bundle_against_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="orbitstate_source_bundle_check_") as temp:
        receipt = target.deterministic_source_bundle(HERE, Path(temp) / "GAIN_COVARIANT_SOURCE_BUNDLE.tar.gz")
    return {
...
def pretransport_gate(
    *,
    ...
    actual_manifest_sha = sha256_file(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json")
...
def source_transfer_entries(manifest: dict[str, Any], manifest_sha: str) -> list[dict[str, Any]]:
    ...
    entries.append(
        {
            "name": "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json",
            "path": manifest_path,
            "sha256": manifest_sha,
...
def run_controller_command(
    ...
    *,
    ...
    require(not command_mutates_historical_root(command), f"historical root mutation command blocked: {' '.join(command)}")
    completed = runner(command, text=True, capture_output=True, timeout=timeout, check=False)
    executed.append(command)
    return completed


def command_mutates_historical_root(command: list[str]) -> bool:
    text = " ".join(command)
    matched = [fragment for fragment in PROTECTED_REMOTE_ROOT_FRAGMENTS if fragment in text]
    if not matched:
        return False
...
def verify_success_evidence(local_root: Path, *, manifest_sha: str, target_returncode: int) -> dict[str, Any]:
    ...
    final_path = local_root / "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json"
    execution_manifest_path = local_root / "GAIN_COVARIANT_CONFIRMATION_MANIFEST.json"
    ...
    require(final.get("status") == "ORBITSTATE_GAIN_COVARIANT_TARGET_COMPLETE", "target final status is not complete")
...
def verify_target_failure_evidence(local_root: Path) -> dict[str, Any]:
    ...
    require(not (local_root / "GAIN_COVARIANT_CONFIRMATION_MANIFEST.json").exists(), "success manifest must not exist on target failure")
    final = read_json(local_root / "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json")
    ...
    failure_manifest = read_json(local_root / "GAIN_COVARIANT_CONFIRMATION_FAILURE_MANIFEST.json")
    ...
    *,
    ...
    if completed.returncode != 0:
        return controller_failure(
            status="GAIN_COVARIANT_CONTROLLER_REMOTE_ROOT_NOT_ABSENT",
    ...
    for entry in entries:
        ...
        if completed.returncode != 0:
            return controller_failure(
                status="GAIN_COVARIANT_CONTROLLER_SOURCE_TRANSFER_FAILED",
                remote_retained=True,
                remote_cleaned=False,
                executed=executed,
    ...
    remote_command = (
        ...
        f"python3 {sh_quote(EXPECTED_REMOTE_SOURCE_ROOT + '/gain_covariant_confirmation_target.py')} "
    ...
    if target_completed.returncode != 0:
        if copied:
            try:
                ...
                return controller_failure(
                    status="GAIN_COVARIANT_CONTROLLER_TARGET_FAILED_EVIDENCE_RETAINED",
                    remote_retained=True,
                    remote_cleaned=False,
                    executed=executed,
                ...
                return controller_failure(
                    status="GAIN_COVARIANT_CONTROLLER_TARGET_FAILED_EVIDENCE_INVALID",
                    remote_retained=True,
                    remote_cleaned=False,
                    executed=executed,
                    target_returncode=target_completed.returncode,
        ...
        return controller_failure(
            status="GAIN_COVARIANT_CONTROLLER_TARGET_FAILED_NO_COPYBACK",
            remote_retained=True,
            remote_cleaned=False,
            executed=executed,
            target_returncode=target_completed.returncode,
    ...
    if not copied:
        return controller_failure(
            status="GAIN_COVARIANT_CONTROLLER_COPYBACK_FAILED",
            remote_retained=True,
            remote_cleaned=False,
            executed=executed,
            output_root_exists=output_exists.returncode == 0,
    ...
    try:
        ...
        return controller_failure(
            status="GAIN_COVARIANT_CONTROLLER_SUCCESS_EVIDENCE_INVALID",
            remote_retained=True,
            remote_cleaned=False,
            executed=executed,
            evidence_error=str(exc),
    ...
    if cleanup.returncode != 0:
        return controller_failure(
            status="GAIN_COVARIANT_CONTROLLER_CLEANUP_FAILED",
            remote_retained=True,
            remote_cleaned=False,
            executed=executed,
    ...
    if absence.returncode != 0:
        return controller_failure(
            status="GAIN_COVARIANT_CONTROLLER_CLEANUP_ABSENCE_VERIFY_FAILED",
            remote_retained=True,
            remote_cleaned=False,
            executed=executed,
    ...
    return {
        "status": "GAIN_COVARIANT_CONTROLLER_TARGET_COMPLETE",
...
def write_fake_success_evidence(local_root: Path, manifest_sha: str, corrupt: str | None = None) -> None:
    ...
    required_payloads = {
        ...
        "GAIN_COVARIANT_RECEIVER_FEATURES.sha256": "0" * 64 + "  GAIN_COVARIANT_RECEIVER_FEATURES.json\n",
    }
    ...
    write_json(local_root / "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json", final)
    evidence_hashes = {
        path.relative_to(local_root).as_posix(): sha256_file(path)
        for path in sorted(local_root.rglob("*"))
        if path.is_file() and path.name not in {"COPYBACK_MANIFEST.json", "GAIN_COVARIANT_CONFIRMATION_MANIFEST.json"}
    }
    execution_manifest = {
    ...
    write_json(local_root / "GAIN_COVARIANT_CONFIRMATION_MANIFEST.json", execution_manifest)
...
def write_fake_failure_evidence(local_root: Path, corrupt: str | None = None) -> None:
    local_root.mkdir(parents=True, exist_ok=True)
    write_json(local_root / "TARGET_FAILURE_GAIN_COVARIANT_CONFIRMATION.json", {"failure_phase": "fake_target", "fake_transport": True})
    final = {
    ...
    failure_manifest = {
        ...
        "scientific_classification_emitted": False,
        "failure_files": {
            "TARGET_FAILURE_GAIN_COVARIANT_CONFIRMATION.json": sha256_file(local_root / "TARGET_FAILURE_GAIN_COVARIANT_CONFIRMATION.json"),
            "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json": sha256_file(local_root / "FINAL_RESULT_GAIN_COVARIANT_CONFIRMATION.json"),
    ...
    write_json(local_root / "GAIN_COVARIANT_CONFIRMATION_FAILURE_MANIFEST.json", failure_manifest)
    write_copyback_manifest_for_fake(local_root, corrupt=corrupt)
...
def fake_transport_self_tests() -> dict[str, Any]:
    ...
    manifest_sha = sha256_file(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json") if (HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json").exists() else manifest["manifest_canonical_sha256"]
    ...
    regressions = {
        "existing_remote_root_rejected": results["remote_root_exists"]["status"] == "GAIN_COVARIANT_CONTROLLER_REMOTE_ROOT_NOT_ABSENT"
        and target_invocations["remote_root_exists"] == 0,
        "null_transport_baseline_rejected_without_target_invocation": results["remote_root_exists"]["status"]
        == "GAIN_COVARIANT_CONTROLLER_REMOTE_ROOT_NOT_ABSENT"
        and target_invocations["remote_root_exists"] == 0,
        ...
        "copyback_size_mismatch_fails": results["copyback_size_mismatch"]["status"] == "GAIN_COVARIANT_CONTROLLER_SUCCESS_EVIDENCE_INVALID",
        "copyback_sha_mismatch_fails": results["copyback_sha_mismatch"]["status"] == "GAIN_COVARIANT_CONTROLLER_SUCCESS_EVIDENCE_INVALID",
        "target_failure_evidence_copied": results["target_failure"]["status"] == "GAIN_COVARIANT_CONTROLLER_TARGET_FAILED_EVIDENCE_RETAINED",
        "target_failure_retains_remote_root": results["target_failure"]["remote_retained"] is True,
        "preexecution_no_output_failure_retains_new_root": results["preexecution_no_output"]["status"]
        == "GAIN_COVARIANT_CONTROLLER_TARGET_FAILED_NO_COPYBACK"
        ...
        "historical_root_mutation_rejected": not any(command_mutates_historical_root(command) for command in all_commands),
        "historical_roots_not_used_for_transfer_or_execution": not any(
            fragment in "\n".join(" ".join(command) for command in all_commands)
            for fragment in PROTECTED_REMOTE_ROOT_FRAGMENTS
        ),
    ...
    result = {
        "schema": "GAIN_COVARIANT_TRANSPORT_SIMULATION_V2",
        "zero_live_contact": True,
        "results": sanitize_fake_transport_receipt(results, temp_root_text),
        "regressions": regressions,
        "target_invocations": target_invocations,
    }
    result["passed"] = all(regressions.values()) and results["success"]["status"] == "GAIN_COVARIANT_CONTROLLER_TARGET_COMPLETE"
    ...
    write_json(HERE / "GAIN_COVARIANT_TRANSPORT_SIMULATION.json", result)
    return result
...
def freeze_artifacts() -> dict[str, Any]:
    ...
    write_json(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json", initial_manifest)
    deployment = deployment_layout_self_test()
    transport = fake_transport_self_tests()
    manifest = build_manifest()
    write_json(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json", manifest)
    manifest_file_sha = sha256_file(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json")
    result = {
        "schema": "GAIN_COVARIANT_FREEZE_ARTIFACTS_V1",
...
def validate_only() -> dict[str, Any]:
    ...
    custody_gates = target.custody_gate_static_proof(HERE)
    ...
    frozen = validate_frozen_hashes()
    public_self = public.self_test()
    manifest = read_json(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json")
    source_bundle = validate_source_bundle_against_manifest(manifest)
    result = {
        "schema": "GAIN_COVARIANT_VALIDATE_ONLY_V1",
        "zero_live_contact": True,
        "schedule": schedule,
        "blindness": blindness,
        "process_boundary": boundary,
        "feature_boundary": feature_boundary,
        "custody_gates": custody_gates,
        ...
        "manifest_file_sha256": sha256_file(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json"),
    }
    result["passed"] = all(
        [
            schedule["passed"],
...
def controller_self_test() -> dict[str, Any]:
    ...
    result = {
        "schema": "GAIN_COVARIANT_CONTROLLER_SELF_TEST_V2",
    ...
    write_json(HERE / "GAIN_COVARIANT_CONTROLLER_SELF_TEST.json", result)
    manifest = build_manifest()
    write_json(HERE / "GAIN_COVARIANT_IMPLEMENTATION_MANIFEST.json", manifest)
    return result
...
def controller_result_success(result: dict[str, Any]) -> bool:
    ...
    if result.get("schema") == "GAIN_COVARIANT_FREEZE_ARTIFACTS_V1":
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "GAIN_COVARIANT_CONTROLLER_FAILED", "error": str(exc)}, indent=2, sort_keys=True), file=os.sys.stderr)
        return 1
