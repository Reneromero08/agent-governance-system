...
RUN_ID = "family10h_relation_composition_replay_exclusion_discovery_v1_0"
PACKAGE_ID = "family10h_relation_composition_replay_exclusion_discovery_v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "COMPOSITION_REPLAY_EXCLUSION_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_COMPOSITION_REPLAY_EXCLUSION_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_COMPOSITION_REPLAY_EXCLUSION_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_composition_loop() -> None:
    ...
    text = text.replace(
        ...
        "            : \"source_alive_composition_replay_exclusion_before_spatial_pair_probe\"\n",
        1,
    )
    text = text.replace(
...
def target_script() -> str:
    return (
        BASE_TARGET_SCRIPT()
        .replace("RELATION_TEMPORAL_SOURCE_OFF", "RELATION_COMPOSITION_REPLAY_EXCLUSION")
        .replace("TEMPORAL_SOURCE_OFF_SCHEDULES", "COMPOSITION_REPLAY_EXCLUSION_SCHEDULES")
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        for index, row in enumerate(base_rows):
            ...
            copied["operation_semantics_id"] = f"composition_replay_exclusion:{COMPOSITION_BY_VARIANT[variant]}:{variant}"
            ...
            copied["matched_twin_group"] = f"{copied['block_id']}:composition_replay_exclusion:{variant}"
        ...
        schedules[variant] = {
            ...
            "path": f"COMPOSITION_REPLAY_EXCLUSION_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_COMPOSITION_REPLAY_EXCLUSION_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "matched_marginal_law": {
            ...
            "tested_relation": "adjacent relation composition sequence R0_then_R1 versus R1_then_R0, with gapped and balanced-alternating replay controls",
        ...
        "predeclared_patterns": {
            "H1_adjacent_composition_sensitive_relational_carrier": [
                ...
                "gapped and balanced-alternating composition controls do not replay the directed adjacent-composition difference",
    ...
    write_json(SOURCE_ROOT / "RELATION_COMPOSITION_REPLAY_EXCLUSION_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_COMPOSITION_REPLAY_EXCLUSION_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    denom = abs(adjacent_composition_order)
    ...
    result = {
        "schema": "FAMILY10H_RELATION_COMPOSITION_REPLAY_EXCLUSION_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "coordinates": {
            ...
            "gapped_composition_abs_to_adjacent_composition_abs": abs(gapped_composition_order) / denom if denom else None,
            "balanced_alt_abs_to_adjacent_composition_abs": abs(balanced_alt_composition_order) / denom if denom else None,
        ...
        "interpretation": {
            ...
            "gapped_replay_collapses_below_25pct_adjacent": abs(gapped_composition_order) <= 0.25 * denom if denom else False,
            "balanced_alt_replay_collapses_below_25pct_adjacent": (
                abs(balanced_alt_composition_order) <= 0.25 * denom if denom else False
            ),
            "composition_replay_exclusion_candidate": (
                denom > 0.0
                and abs(dead_composition_order) >= 0.25 * denom
                and abs(source_off_composition_order) <= 0.25 * denom
                and abs(gapped_composition_order) <= 0.25 * denom
                and abs(balanced_alt_composition_order) <= 0.25 * denom
            ),
            "exploratory_only": True,
            "small_wall_crossed": False,
        },
    ...
    write_json(RUN_ROOT / "RELATION_COMPOSITION_REPLAY_EXCLUSION_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- gapped abs/adjacent abs: `{(abs(gapped_composition_order) / denom if denom else 0.0):.3f}`",
            f"- balanced-alt abs/adjacent abs: `{(abs(balanced_alt_composition_order) / denom if denom else 0.0):.3f}`",
            "",
            "Interpretation:",
            f"- composition replay-exclusion candidate: `{result['interpretation']['composition_replay_exclusion_candidate']}`",
            ...
            f"- gapped replay collapses below 0.25 x adjacent: `{result['interpretation']['gapped_replay_collapses_below_25pct_adjacent']}`",
            f"- balanced-alt replay collapses below 0.25 x adjacent: `{result['interpretation']['balanced_alt_replay_collapses_below_25pct_adjacent']}`",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_COMPOSITION_REPLAY_EXCLUSION_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_COMPOSITION_REPLAY_EXCLUSION_PREPARE_RESULT.json", result)
    return result
