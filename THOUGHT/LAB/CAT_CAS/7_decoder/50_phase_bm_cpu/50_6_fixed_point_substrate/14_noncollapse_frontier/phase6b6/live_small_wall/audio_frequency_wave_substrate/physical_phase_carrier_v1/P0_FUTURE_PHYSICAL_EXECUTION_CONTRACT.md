...
Before assignment or any primary source preparation, the separately authorized resonance calibration must use DUT assembly A with its FC135 population, preserve its original native instrument bytes, and produce the implemented SDK-export canonical calibration payload: 143 frequency blocks, 4096 samples per channel per block, 1,000,000 samples/s, little-endian signed int16, frequency-major/sample-major CH0 source monitor then CH1 carrier response. This is one global pre-assignment frequency reference for all later records. Each later role retains its own primary assembly/population custody; B with the resonator removed and C with the 1-pF dummy do not claim that their hardware produced the resonance. The identity adapter is lossless for the selected channels and source-controlled. The artifact binds native, canonical, adapter and analyzer hashes; all sampling/scaling/settling fields; exact frequencies; source and instrument querybacks; DUT-A/FC135; and start/completion UTC. If an instrument emits a proprietary container, those bytes must be preserved separately; this contract does not claim to parse that container. The analyzer independently fits sine/cosine/DC terms in CH0 and CH1, derives `H=Z1/Z0`, propagates uncertainty, fits `H(f)=B+C/(1+i*2Q*(f-f0)/f0)`, and rejects nonconvergence, boundary fits, flat or competing responses, clipping, weak source/resonance, excessive background/noise/residual, and broken custody. The final raw block is the bound post-fit off-resonance control and is excluded from the resonance fit.
