...
def physical_threshold_contract() -> dict[str, Any]:
    contract = {
        ...
        "calibrated_requirements": {
            ...
            "relation_sham_and_scrambled_controls_inside_null": True,
            "A_first_and_B_first_strata_survive": True,
...
def adjudicate_physical_packet(
    ...
    *,
    ...
    null = matched_permutation_null(packet, rows, threshold_contract)
    stability = factor_stability(r_records, ["session", "mapping", "source_order", "query_order", "cyclic_origin"])
    controls = control_results(rows, null["null_distribution"]["max_abs"])
    order_stability = measurement_order_stability(packet, rows)
    marginals = marginal_explanation(rows, null["q99_abs_mean_R_spatial"])
    gates = {
        "validation": validation["passed"],
        "complete_relation_matrix_global": not relation_failures and len(r_records) == max(1, schedule["tuple_count"] // 8),
        "matched_permutation_q99_exceeded": null["passed"],
        "factor_sign_stability": stability["passed"],
        "relation_controls_null": controls["passed"],
        "measurement_order_strata": order_stability["passed"],
        "not_explained_by_A_or_B_marginal_latency_alone": marginals["passed"],
    }
    passed = all(gates.values())
    candidate = (not passed) and gates["complete_relation_matrix_global"] and global_summary["R_spatial_distribution"]["abs_of_mean"] > 0.5 * null["q99_abs_mean_R_spatial"]
    ...
    report = {
        ...
        "measurement_order_stability": order_stability,
        "marginal_explanation": marginals,
        "gates": gates,
    }
...
def fixture_packet(schedule: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    for row in schedule["rows"]:
        ...
        for sample_index, a_index in enumerate(pub.selected_pair_indices(row["execution_ordinal"])):
            ...
            if mode == "positive" and row["row_role"] == "relation_matrix":
...
def run_self_test(schedule: dict[str, Any]) -> dict[str, Any]:
    ...
    fixture_rows = schedule["rows"][:32]
    fixture_schedule = {**schedule, "rows": fixture_rows, "tuple_count": len(fixture_rows), "expected_pair_observation_count": len(fixture_rows) * pub.PAIR_SAMPLE_COUNT}
    ...
    checks = {
        ...
        "matched_permutation_threshold_applied": positive["matched_permutation_null"]["q99_abs_mean_R_spatial"] > 0.0,
    }
    result = {
        ...
        "positive_result": {
            ...
            "R_spatial_abs_of_mean": positive["R_spatial_global"]["R_spatial_distribution"]["abs_of_mean"],
            "matched_permutation_q99": positive["matched_permutation_null"]["q99_abs_mean_R_spatial"],
        },
