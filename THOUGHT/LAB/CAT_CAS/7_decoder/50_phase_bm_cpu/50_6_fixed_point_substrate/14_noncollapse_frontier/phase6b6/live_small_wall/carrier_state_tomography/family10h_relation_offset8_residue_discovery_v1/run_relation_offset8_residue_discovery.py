...
RUN_ID = "family10h_relation_offset8_residue_discovery_v1_0"
...
ROUNDS = [0, 1]
OFFSETS = [4, 8, 16]
SOURCE_STATES = ["on", "off"]
VARIANTS = [
    f"round{round_id}_source_{state}_offset_{offset}"
    for round_id in ROUNDS
    for offset in OFFSETS
    for state in SOURCE_STATES
]
QUERY_BY_VARIANT = {
    variant: f"source_{state}_offset_{offset}_control"
    for round_id in ROUNDS
    for offset in OFFSETS
    for state in SOURCE_STATES
    for variant in [f"round{round_id}_source_{state}_offset_{offset}"]
}
SOURCE_OFF_VARIANTS = {
    f"round{round_id}_source_off_offset_{offset}" for round_id in ROUNDS for offset in OFFSETS
}
...
MATCHED_PERMUTATION_SEED = "family10h-offset8-residue-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "OFFSET8_RESIDUE_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_OFFSET8_RESIDUE_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_OFFSET8_RESIDUE_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_offset8_residue() -> None:
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    source_schedules: dict[int, Path] = {
        round_id: LOCAL_PAIRED_ROOT / "LOCAL_PAIRED_DIFFERENTIAL_SCHEDULES" / f"round{round_id}_query_relation_pair.tsv"
        for round_id in ROUNDS
    }
    source_rows: dict[int, list[dict[str, str]]] = {}
    fields: list[str] = []
    for round_id, source_schedule in source_schedules.items():
        with source_schedule.open(newline="", encoding="utf-8") as handle:
            reader = csv.DictReader(handle, delimiter="\t")
            if not fields:
                fields = reader.fieldnames or []
            source_rows[round_id] = list(reader)[:ROWS_PER_VARIANT]
        require(len(source_rows[round_id]) == ROWS_PER_VARIANT, f"not enough base rows for round {round_id} discovery schedule")
    SCHEDULE_DIR.mkdir(parents=True, exist_ok=True)
    schedules: dict[str, Any] = {}
    for variant in VARIANTS:
        round_id = int(variant.split("_", 1)[0].replace("round", ""))
        query = QUERY_BY_VARIANT[variant]
        rows = []
        for index, row in enumerate(source_rows[round_id]):
            copied = dict(row)
            copied["execution_ordinal"] = str(index)
            copied["query"] = query
            copied["operation_semantics_id"] = query
            copied["control_semantics_id"] = query
        ...
        schedules[variant] = {
            "query": query,
            "path": f"OFFSET8_RESIDUE_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        "source_schedules": {
            f"round{round_id}": {
                "path": str(source_schedule),
                "sha256": sha256_file(source_schedule),
            }
            for round_id, source_schedule in source_schedules.items()
        },
        "rounds": ROUNDS,
    ...
    write_json(SOURCE_ROOT / "RELATION_OFFSET8_RESIDUE_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_OFFSET8_RESIDUE_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/OFFSET8_RESIDUE_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_OFFSET8_RESIDUE_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_OFFSET8_RESIDUE_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_OFFSET8_RESIDUE_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_OFFSET8_RESIDUE_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_OFFSET8_RESIDUE_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    round_offset_reports: dict[str, Any] = {}
    for round_id in ROUNDS:
        round_offset_reports[str(round_id)] = {}
        for offset in OFFSETS:
            source_on_variant = f"round{round_id}_source_on_offset_{offset}"
            source_off_variant = f"round{round_id}_source_off_offset_{offset}"
            ...
            round_offset_reports[str(round_id)][str(offset)] = {
                ...
                "source_dependent_delta_above_25pct_source_on": (
                    abs(delta) > 0.25 * abs(source_on) if source_on else False
                ),
            }
    offset8_reports = [round_offset_reports[str(round_id)]["8"] for round_id in ROUNDS]
    offset8_deltas = [report["source_on_minus_source_off"] for report in offset8_reports]
    offset8_ratios = [report["delta_abs_to_source_on_abs"] for report in offset8_reports]
    neighbor_reports = [
        round_offset_reports[str(round_id)][str(offset)]
        for round_id in ROUNDS
        for offset in OFFSETS
    ...
    strongest_neighbor = max(neighbor_reports, key=lambda report: abs(report["source_on_minus_source_off"]))
    strongest_round_offset = max(
        ((round_id, offset) for round_id in ROUNDS for offset in OFFSETS),
        key=lambda item: abs(round_offset_reports[str(item[0])][str(item[1])]["source_on_minus_source_off"]),
    )
    strongest = round_offset_reports[str(strongest_round_offset[0])][str(strongest_round_offset[1])]
    offset8_survives_both_rounds = all(
        report["source_dependent_delta_positive"] and report["source_dependent_delta_above_25pct_source_on"]
        for report in offset8_reports
    )
    offset8_exceeds_neighbors = min(abs(delta) for delta in offset8_deltas) > abs(
        strongest_neighbor["source_on_minus_source_off"]
    )
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "round_offset_reports": round_offset_reports,
        "coordinates": {
            "round0_offset8_source_on_R": offset8_reports[0]["source_on_R"],
            "round0_offset8_source_off_R": offset8_reports[0]["source_off_R"],
            "round0_offset8_delta": offset8_reports[0]["source_on_minus_source_off"],
            "round0_offset8_delta_abs_to_source_on_abs": offset8_reports[0]["delta_abs_to_source_on_abs"],
            "round1_offset8_source_on_R": offset8_reports[1]["source_on_R"],
            "round1_offset8_source_off_R": offset8_reports[1]["source_off_R"],
            "round1_offset8_delta": offset8_reports[1]["source_on_minus_source_off"],
            "round1_offset8_delta_abs_to_source_on_abs": offset8_reports[1]["delta_abs_to_source_on_abs"],
            "offset8_min_delta": min(offset8_deltas),
            "offset8_max_delta": max(offset8_deltas),
            "offset8_min_delta_abs_to_source_on_abs": min(value for value in offset8_ratios if value is not None),
            "strongest_neighbor_delta": strongest_neighbor["source_on_minus_source_off"],
            "strongest_delta_round": strongest_round_offset[0],
            "strongest_delta_offset": strongest_round_offset[1],
            "strongest_delta": strongest["source_on_minus_source_off"],
        },
        "interpretation": {
            "offset8_survives_both_rounds": offset8_survives_both_rounds,
            "offset8_exceeds_neighbor_offsets_4_and_16": offset8_exceeds_neighbors,
            "offset8_all_deltas_positive": all(delta > 0 for delta in offset8_deltas),
            "offset8_all_deltas_above_25pct_source_on": all(
                report["source_dependent_delta_above_25pct_source_on"] for report in offset8_reports
            ),
            "strongest_delta_round": strongest_round_offset[0],
            "strongest_delta_offset": strongest_round_offset[1],
            "exploratory_only": True,
    ...
    write_json(RUN_ROOT / "RELATION_OFFSET8_RESIDUE_DISCOVERY_ANALYSIS.json", result)
    ...
    for round_id in ROUNDS:
        for offset in OFFSETS:
            report = round_offset_reports[str(round_id)][str(offset)]
            lines.append(
                f"| {round_id} | {offset} | {report['source_on_R']:.9f} | {report['source_off_R']:.9f} | "
    ...
    lines.extend(
        [
            ...
            f"- offset 8 survives both rounds: `{result['interpretation']['offset8_survives_both_rounds']}`",
            f"- offset 8 exceeds neighbor offsets 4 and 16: `{result['interpretation']['offset8_exceeds_neighbor_offsets_4_and_16']}`",
            f"- offset 8 all deltas positive: `{result['interpretation']['offset8_all_deltas_positive']}`",
            f"- offset 8 all deltas above 0.25 x source-on: `{result['interpretation']['offset8_all_deltas_above_25pct_source_on']}`",
            f"- strongest delta round/offset: `{result['interpretation']['strongest_delta_round']}` / `{result['interpretation']['strongest_delta_offset']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    patch_runtime_for_offset8_residue()
    schedule_manifest = make_rows()
    build = compile_runtime()
    result = {
        "schema": "FAMILY10H_RELATION_OFFSET8_RESIDUE_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_OFFSET8_RESIDUE_PREPARE_RESULT.json", result)
    return result
