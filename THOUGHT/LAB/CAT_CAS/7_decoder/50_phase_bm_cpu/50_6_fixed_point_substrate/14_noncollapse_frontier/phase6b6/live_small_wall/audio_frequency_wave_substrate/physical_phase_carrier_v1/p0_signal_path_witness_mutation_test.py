...
def structural_mutations(source: bytes) -> list[dict[str, str]]:
    mutations = {
        ...
        "substitute_pre_source_channel": (b"pre0 = signal_path_tone_fit(ch0, pre_start, pre_stop, thresholds)", b"pre0 = signal_path_tone_fit(ch1, pre_start, pre_stop, thresholds)"),
        "substitute_pre_carrier_channel": (b"pre1 = signal_path_tone_fit(ch1, pre_start, pre_stop, thresholds)", b"pre1 = signal_path_tone_fit(ch0, pre_start, pre_stop, thresholds)"),
        "substitute_open_source_channel": (b"open0 = signal_path_tone_fit(ch0, open_start, open_stop, thresholds)", b"open0 = signal_path_tone_fit(ch1, open_start, open_stop, thresholds)"),
        "substitute_open_carrier_channel": (b"open1 = signal_path_tone_fit(ch1, open_start, open_stop, thresholds)", b"open1 = signal_path_tone_fit(ch0, open_start, open_stop, thresholds)"),
        ...
        "substitute_common_mode_derivation": (b"common_mode_peak = 0.5 * raw_peak", b"common_mode_peak = 0.0 * raw_peak"),
        "remove_pre_window_cycle_gate": (b'if (pre_stop - pre_start) * F_WITNESS / FS < model_number(pre_spec["valid_cycles"], "model.pre_valid_cycles") - 1e-12:', b"if False:"),
        "remove_open_window_cycle_gate": (b'if (open_stop - open_start) * F_WITNESS / FS < model_number(open_spec["valid_cycles"], "model.open_valid_cycles") - 1e-12:', b"if False:"),
        "invert_clipping_gate": (b"if raw_peak > clip:", b"if raw_peak < clip:"),
        "invert_common_mode_gate": (b'if common_mode_peak > model_number(thresholds["common_mode_abs_v_max"], "model.common_mode_abs_v_max"):', b'if common_mode_peak < model_number(thresholds["common_mode_abs_v_max"], "model.common_mode_abs_v_max"):'),
