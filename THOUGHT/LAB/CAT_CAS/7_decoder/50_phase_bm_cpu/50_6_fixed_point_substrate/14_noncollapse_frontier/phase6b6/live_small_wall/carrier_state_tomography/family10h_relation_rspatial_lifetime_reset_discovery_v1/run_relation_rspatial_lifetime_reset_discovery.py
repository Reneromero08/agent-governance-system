...
RUN_ID = "family10h_relation_rspatial_lifetime_reset_discovery_v1_0"
...
QUERY_BY_VARIANT = {
    ...
    "reset_primary": "reset_query_relation_pair_control",
    "reset_sham": "reset_relation_sham_control",
}
...
MATCHED_PERMUTATION_SEED = "family10h-rspatial-lifetime-reset-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "RSPATIAL_LIFETIME_RESET_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_RSPATIAL_LIFETIME_RESET_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_RSPATIAL_LIFETIME_RESET_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_rspatial_lifetime_reset() -> None:
    ...
    text = text.replace(
        ...
        "    RELATION_SPATIAL_CONTROL_RESET_PRIMARY = 7,\n"
        "    RELATION_SPATIAL_CONTROL_RESET_SHAM = 8",
    )
    ...
    text = text.replace(
        ...
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "reset_query_relation_pair_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESET_PRIMARY;\n"
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "reset_relation_sham_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESET_SHAM;\n"
        "        return 1;\n"
        "    }\n"
        "    return 0;\n",
    )
    text = text.replace(
        ...
        "                                  : ((a_index + FAMILY10H_RELATION_SPATIAL_LINE_COUNT - 1u) % FAMILY10H_RELATION_SPATIAL_LINE_COUNT);\n"
        "    }\n"
        "    if (row->control == RELATION_SPATIAL_CONTROL_DEAD_SHAM || row->control == RELATION_SPATIAL_CONTROL_RESET_SHAM) {\n"
    ...
    lifecycle_helpers = (
        ...
        "        || control == RELATION_SPATIAL_CONTROL_RESET_PRIMARY\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESET_SHAM;\n"
        "}\n\n"
        "static int reset_after_source_dead_control(relation_spatial_control_id control) {\n"
        "    return control == RELATION_SPATIAL_CONTROL_RESET_PRIMARY\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESET_SHAM;\n"
        "}\n\n"
        + lifecycle_anchor
    )
    ...
    text = text.replace(
        ...
        "                ? \"source_dead_then_cache_flush_before_spatial_pair_probe\"\n"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"RSPATIAL_LIFETIME_RESET_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        ...
        "mechanism_question": (
            "Does the R-spatial primary-minus-sham relation coordinate survive source death and "
            "explicit cache-line flush reset under the same primary/sham readout?"
        ),
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_RSPATIAL_LIFETIME_RESET_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_RSPATIAL_LIFETIME_RESET_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/RSPATIAL_LIFETIME_RESET_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_RSPATIAL_LIFETIME_RESET_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_RSPATIAL_LIFETIME_RESET_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_RSPATIAL_LIFETIME_RESET_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_RSPATIAL_LIFETIME_RESET_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_RSPATIAL_LIFETIME_RESET_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    lifecycle_pairs = {
        ...
        "reset": ("reset_primary", "reset_sham"),
    }
    lifecycle_reports: dict[str, Any] = {}
    for lifecycle, (primary_variant, sham_variant) in lifecycle_pairs.items():
    ...
    alive_d = lifecycle_reports["alive"]["D_primary_minus_sham"]
    dead_d = lifecycle_reports["dead"]["D_primary_minus_sham"]
    reset_d = lifecycle_reports["reset"]["D_primary_minus_sham"]
    alive_primary = lifecycle_reports["alive"]["primary_R"]
    dead_primary = lifecycle_reports["dead"]["primary_R"]
    reset_primary = lifecycle_reports["reset"]["primary_R"]

    def ratio_to_alive(value: float) -> float | None:
        return abs(value) / abs(alive_d) if alive_d else None

    for report in lifecycle_reports.values():
        report["D_abs_to_alive_D_abs"] = ratio_to_alive(report["D_primary_minus_sham"])

    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "coordinates": {
            ...
            "R_alive_sham": lifecycle_reports["alive"]["sham_R"],
            "D_alive_primary_minus_sham": alive_d,
            "R_dead_primary": dead_primary,
            "R_dead_sham": lifecycle_reports["dead"]["sham_R"],
            "D_dead_primary_minus_sham": dead_d,
            "R_reset_primary": reset_primary,
            "R_reset_sham": lifecycle_reports["reset"]["sham_R"],
            "D_reset_primary_minus_sham": reset_d,
            "dead_D_abs_to_alive_D_abs": ratio_to_alive(dead_d),
            "reset_D_abs_to_alive_D_abs": ratio_to_alive(reset_d),
            "dead_primary_abs_to_alive_primary_abs": abs(dead_primary) / abs(alive_primary) if alive_primary else None,
            "reset_primary_abs_to_alive_primary_abs": abs(reset_primary) / abs(alive_primary) if alive_primary else None,
        },
        "interpretation": {
            "alive_D_positive": alive_d > 0,
            "dead_D_positive": dead_d > 0,
            "reset_D_positive": reset_d > 0,
            "dead_preserves_at_least_25pct_alive_D": abs(dead_d) >= 0.25 * abs(alive_d) if alive_d else False,
            "reset_preserves_at_least_25pct_alive_D": abs(reset_d) >= 0.25 * abs(alive_d) if alive_d else False,
            "source_death_kills_below_25pct_alive_D": abs(dead_d) <= 0.25 * abs(alive_d) if alive_d else False,
            "reset_kills_below_25pct_alive_D": abs(reset_d) <= 0.25 * abs(alive_d) if alive_d else False,
            "reset_reduces_dead_D_abs": abs(reset_d) < abs(dead_d),
            "dead_primary_same_sign_as_alive_primary": (dead_primary > 0) == (alive_primary > 0),
            "reset_primary_same_sign_as_alive_primary": (reset_primary > 0) == (alive_primary > 0),
            "candidate_persistent_source_written_state": (
                alive_d > 0
                and dead_d > 0
                and abs(dead_d) >= 0.25 * abs(alive_d)
                and abs(reset_d) <= 0.25 * abs(alive_d)
            )
            if alive_d
            else False,
            "candidate_receiver_static_geometry": (
                alive_d > 0
                and dead_d > 0
                and reset_d > 0
                and abs(reset_d) >= 0.25 * abs(alive_d)
    ...
    write_json(RUN_ROOT / "RELATION_RSPATIAL_LIFETIME_RESET_DISCOVERY_ANALYSIS.json", result)
    ...
    for lifecycle in ["alive", "dead", "reset"]:
        report = lifecycle_reports[lifecycle]
        lines.append(
            f"| {lifecycle} | {report['primary_R']:.9f} | {report['sham_R']:.9f} | "
            f"{report['D_primary_minus_sham']:.9f} | "
            f"{(report['D_abs_to_alive_D_abs'] or 0.0):.3f} | "
            f"{(report['sham_abs_to_primary_abs'] or 0.0):.3f} |"
        )
    lines.extend(
        [
            "",
            ...
            f"- reset D positive: `{result['interpretation']['reset_D_positive']}`",
            f"- source death kills below 0.25 x alive D: `{result['interpretation']['source_death_kills_below_25pct_alive_D']}`",
            f"- reset kills below 0.25 x alive D: `{result['interpretation']['reset_kills_below_25pct_alive_D']}`",
            f"- reset reduces dead D abs: `{result['interpretation']['reset_reduces_dead_D_abs']}`",
            f"- persistent source-written-state candidate: `{result['interpretation']['candidate_persistent_source_written_state']}`",
            f"- receiver/static-geometry candidate: `{result['interpretation']['candidate_receiver_static_geometry']}`",
            "",
...
def prepare() -> dict[str, Any]:
    ...
    patch_runtime_for_rspatial_lifetime_reset()
    schedule_manifest = make_rows()
    build = compile_runtime()
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_LIFETIME_RESET_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_RSPATIAL_LIFETIME_RESET_PREPARE_RESULT.json", result)
    return result
