#!/usr/bin/env python3
"""Offline-ready controller for the relation-lifetime matched-permutation run.
...
import relation_lifetime_target as target


HERE = Path(__file__).resolve().parent
LIVE_AUTHORITY_ENV = "FAMILY10H_RELATION_LIFETIME_CONTROLLER_LIVE_AUTHORITY"
...
def verify_freeze_authority(repo_root: Path, expected_freeze: str, *, synthetic: bool = False) -> dict[str, Any]:
    ...
    return {
        "schema": "FAMILY10H_RELATION_LIFETIME_CONTROLLER_FREEZE_AUTHORITY_V1",
        "passed": passed,
        "synthetic": synthetic,
...
def copy_package_to_deployment(source_root: Path, deployment_root: Path) -> list[str]:
    ...
    for path in sorted(source_root.iterdir(), key=lambda item: item.name):
        if path.name == pub.OWNED_OUTPUT_PARENT_NAME or path.name.startswith("_relation_lifetime_"):
            continue
        if path.is_file():
...
def write_physical_mock(deployment_root: Path, output_root: Path) -> Path:
    ...
    path = deployment_root / "RELATION_LIFETIME_PHYSICAL_PREFLIGHT_MOCK.json"
...
def synthetic_transport_self_test(source_root: Path, relation_freeze_commit: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="relation_lifetime_controller_", dir="C:/tmp" if os.name == "nt" else None) as temp:
        temp_root = Path(temp)
        deployment_root = temp_root / "remote_relation_lifetime_root"
        ...
        env = {
            ...
            target.MANIFEST_ENV: pub.sha256_file(deployment_root / "RELATION_LIFETIME_IMPLEMENTATION_MANIFEST.json"),
    ...
    passed = (
        ...
        and execution["raw_record_count"] == 21504
        and execution["source_death_receipt_count"] == 21504
