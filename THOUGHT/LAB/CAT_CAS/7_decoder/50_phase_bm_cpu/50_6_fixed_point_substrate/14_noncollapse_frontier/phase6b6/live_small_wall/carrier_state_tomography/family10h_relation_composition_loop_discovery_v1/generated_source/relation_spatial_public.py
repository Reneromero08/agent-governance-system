...
TRANSACTION_RUN_ID = "family10h_relation_spatial_pair_readout_v1_1_segmented_0"
...
PACKAGE_DECISION_BLOCKED = "FAMILY10H_SEGMENTED_SPATIAL_RELATION_READOUT_BUILD_READINESS_BLOCKED"
PACKAGE_DECISION_BUILD_READY = "FAMILY10H_SEGMENTED_SPATIAL_RELATION_READOUT_BUILD_READY_AWAITING_AUTHORIZATION"
...
RELATION_PRIMARY_QUERY = "query_relation_pair"
SEGMENTED_CONTROL_QUERIES = [
    "relation_sham",
    "scrambled_pair_control",
    "distance_matched_control",
    "route_pressure_control",
]
SEGMENTED_RELATION_QUERY_SEGMENTS = [
    ("primary", RELATION_PRIMARY_QUERY, "none"),
    ("control_relation_sham", "relation_sham", "relation_sham"),
    ("control_scrambled_pair", "scrambled_pair_control", "scrambled_pair_control"),
    ("control_distance_matched", "distance_matched_control", "distance_matched_control"),
    ("control_route_pressure", "route_pressure_control", "route_pressure_control"),
]
...
def relation_marginal_equality_proof(
    ...
    for row in rows:
        for idx in selected_pair_indices(int(row["execution_ordinal"])):
            selected[idx] += 1
    coverage_ok = not rows or (len(selected) == LINE_COUNT and min(selected.values()) == max(selected.values()))
    checks = {
...
def build_schedule(grammar: dict[str, Any]) -> dict[str, Any]:
    ...
    for segment_index, (segment_id, query, control_semantics) in enumerate(SEGMENTED_RELATION_QUERY_SEGMENTS):
        for block_index, condition in enumerate(conditions):
            block_id = f"seg_{segment_index:02d}_{segment_id}_block_{block_index:05d}_{stable_token({'segment': segment_id, 'condition': condition}, 10)}"
                ...
                local_rows.append(
                    {
                        **schedule_row_base(condition, hashes),
                        ...
                        "operation_semantics_id": "relation_matrix" if query == RELATION_PRIMARY_QUERY else query,
                        "control_semantics_id": control_semantics,
                        "matched_twin_group": f"{block_id}:relation_matrix:{segment_id}",
                        "matched_twin_pair": f"{block_id}:relation_pair_{local_position}:{segment_id}",
                    }
                )
            for row in sorted(local_rows, key=lambda item: item["block_local_position"]):
                ...
                rows.append(row)
                ordinal += 1
    expected_tuple_count = len(conditions) * len(SEGMENTED_RELATION_QUERY_SEGMENTS) * len(RELATION_CELLS)
    schedule = {
        ...
        "relation_query_segment_count": len(SEGMENTED_RELATION_QUERY_SEGMENTS),
        "relation_query_segments": [
            {"segment_id": segment_id, "query": query, "control_semantics_id": control_semantics}
            for segment_id, query, control_semantics in SEGMENTED_RELATION_QUERY_SEGMENTS
        ],
        "primary_query": RELATION_PRIMARY_QUERY,
        "control_queries": SEGMENTED_CONTROL_QUERIES,
        "rows_per_base_condition": len(SEGMENTED_RELATION_QUERY_SEGMENTS) * len(RELATION_CELLS),
        "pair_sample_count_per_row": PAIR_SAMPLE_COUNT,
        "pair_sample_stride": PAIR_SAMPLE_STRIDE,
        ...
        "expected_tuple_count_derivation": {
            ...
            "relation_query_segments": len(SEGMENTED_RELATION_QUERY_SEGMENTS),
            "rows_per_segment_block": len(RELATION_CELLS),
...
def validate_schedule(schedule: dict[str, Any], grammar: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_tuple_count = len(base_conditions()) * len(SEGMENTED_RELATION_QUERY_SEGMENTS) * len(RELATION_CELLS)
    if len(rows) != expected_tuple_count or schedule.get("tuple_count") != expected_tuple_count:
        failures.append(f"tuple count is not the segmented {expected_tuple_count}-row spatial schedule")
    ...
    for row in rows:
        ...
        query_counts[str(row.get("query"))] += 1
        if row.get("q") != 0:
        ...
        if row.get("row_role") != "relation_matrix":
            failures.append("segmented schedule contains non-relation-matrix row_role")
            break
    expected_block_count = len(base_conditions()) * len(SEGMENTED_RELATION_QUERY_SEGMENTS)
    if len(block_rows) != expected_block_count:
        failures.append(f"block count is not {expected_block_count}")
    for block_id, block in block_rows.items():
        if len(block) != len(RELATION_CELLS):
            failures.append(f"{block_id} row count drift")
            continue
        if sorted((row["r_prepare"], row["r_query"]) for row in block) != sorted(RELATION_CELLS):
            failures.append(f"{block_id} relation cell set mismatch")
        queries = {row["query"] for row in block}
        if len(queries) != 1:
            failures.append(f"{block_id} mixed query segment")
    expected_query_count = len(base_conditions()) * len(RELATION_CELLS)
    expected_queries = [RELATION_PRIMARY_QUERY, *SEGMENTED_CONTROL_QUERIES]
    for query in expected_queries:
        if query_counts.get(query) != expected_query_count:
            failures.append(f"query segment count mismatch: {query}")
    factor_counts = {
        "session": Counter(row["session"] for row in rows if row["block_local_position"] == 0 and row["query"] == RELATION_PRIMARY_QUERY),
        "mapping": Counter(row["mapping"] for row in rows if row["block_local_position"] == 0 and row["query"] == RELATION_PRIMARY_QUERY),
        "source_order": Counter(row["source_order"] for row in rows if row["block_local_position"] == 0 and row["query"] == RELATION_PRIMARY_QUERY),
        "query_order": Counter(row["query_order"] for row in rows if row["block_local_position"] == 0 and row["query"] == RELATION_PRIMARY_QUERY),
        "cyclic_origin": Counter(row["cyclic_origin"] for row in rows if row["block_local_position"] == 0 and row["query"] == RELATION_PRIMARY_QUERY),
    }
