...
#include "gain_covariant_confirmation_runtime.h"
...
#define ORBITSTATE_LIVE_AUTHORITY_VALUE "orbitstate_gain_covariant_confirmation_v1_0"
...
static PmuMeasurement measure_store_window(PmuGroup *group, volatile uint8_t *bank, int sentinel_mode) {
    ...
    if (measurement.cycles_id == 0u || measurement.change_to_dirty_id == 0u || measurement.probe_dirty_id == 0u ||
        measurement.cycles_id == measurement.change_to_dirty_id ||
        measurement.cycles_id == measurement.probe_dirty_id ||
        measurement.change_to_dirty_id == measurement.probe_dirty_id) {
        failf("event ID positivity/distinctness failed");
    }
...
static void receiver_execute_rows(
    ...
    snprintf(raw_path, sizeof(raw_path), "%s/RAW_GAIN_COVARIANT_RECEIVER_CAPTURE.jsonl", output_root);
    snprintf(sentinels_path, sizeof(sentinels_path), "%s/GAIN_COVARIANT_RECEIVER_SENTINELS.jsonl", output_root);
    snprintf(stage_path, sizeof(stage_path), "%s/GAIN_COVARIANT_STAGE_RECEIPTS.jsonl", output_root);
    snprintf(source_path, sizeof(source_path), "%s/GAIN_COVARIANT_SOURCE_RECEIPTS.jsonl", output_root);
...
static int validate_schedule_tsv(const char *path) {
    ...
    printf("{\"status\":\"GAIN_COVARIANT_PUBLIC_SCHEDULE_TSV_OK\",\"rows\":%d,\"replicate_0\":%d,\"replicate_1\":%d,"
...
static int self_test(void) {
    ...
    printf(
        "{\"status\":\"GAIN_COVARIANT_RUNTIME_SELF_TEST_OK\",\"d_q0\":%d,\"fold_q0\":%d,"
...
static int pmu_preflight(void) {
    ...
    free_banks(&banks);
    printf(
        "{\"status\":\"GAIN_COVARIANT_RUNTIME_PMU_PREFLIGHT_OK\","
...
static void require_runtime_live_authority(void) {
    const char *commit_binding = getenv("ORBITSTATE_GAIN_COVARIANT_V1_COMMIT_BINDING");
    const char *manifest_binding = getenv("ORBITSTATE_GAIN_COVARIANT_V1_MANIFEST_SHA256");
    const char *live_authority = getenv("ORBITSTATE_GAIN_COVARIANT_V1_LIVE_AUTHORITY");
...
int main(int argc, char **argv) {
    ...
    if (run_schedule != 0) {
        ...
        printf("{\"status\":\"GAIN_COVARIANT_RUNTIME_REPLICATE_COMPLETE\",\"replicate\":%d,"
               "\"scientific_classification_emitted\":false}\n",
               replicate);
        return 0;
    }
