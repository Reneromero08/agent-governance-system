...
#include "balanced_transducer_runtime.h"
...
static int run_capture(const char *schedule_path, const char *output_root, int replicate) {
    ...
    fprintf(summary, "  \"status\": \"%s\",\n",
            failed_trials == 0 ? "BALANCED_TRANSDUCER_RUNTIME_COMPLETE" : "BALANCED_TRANSDUCER_RUNTIME_REJECTED_WINDOWS");
...
static int self_test(void) {
    ...
    printf("BALANCED_TRANSDUCER_RUNTIME_SELF_TEST_OK\n");
    return 0;
}
