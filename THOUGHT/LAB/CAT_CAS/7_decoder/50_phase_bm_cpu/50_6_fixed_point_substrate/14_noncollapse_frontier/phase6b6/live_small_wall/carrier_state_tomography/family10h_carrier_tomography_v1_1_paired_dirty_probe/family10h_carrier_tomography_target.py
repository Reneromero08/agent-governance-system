...
SOURCE_FILE_NAMES = [
    ...
    "paired_dirty_probe_prospective_adjudication.py",
    "run_family10h_carrier_tomography_v1_1_paired_dirty_probe.py",
]
...
def validate_manifest_authority(source_root: Path) -> dict[str, Any]:
    ...
    approved_identity = temperature_authority.get("approved_sensor_identity") if isinstance(temperature_authority, dict) else None
    if not isinstance(approved_identity, dict) or set(approved_identity) != public.TEMPERATURE_SENSOR_IDENTITY_KEYS:
        failures.append("approved temperature sensor identity missing from v1.1 manifest")
    ...
    if package_decision != public.PACKAGE_DECISION_FROZEN:
        failures.append("v1.1 package is not frozen for live execution")
    if manifest.get("offline_validate", {}).get("passed") is not True:
        failures.append("v1.1 package lacks passing offline validation")
    counters = manifest.get("contact_counter_attestation", {})
    if counters != {"target_contact_count": 0, "sensor_inventory_count": 0, "live_invocation_count": 0, "pmu_acquisition_count": 0}:
        failures.append("v1.1 freeze manifest counters must be exactly 0/0/0/0")
    controller_challenge = temperature_authority.get("controller_challenge") if isinstance(temperature_authority, dict) else None
    if not isinstance(controller_challenge, dict):
        failures.append("v1.1 live authority challenge missing")
    else:
        if controller_challenge.get("science_package_id") != public.SCIENCE_PACKAGE_ID:
            failures.append("v1.1 challenge package mismatch")
        if controller_challenge.get("transaction_run_id") != public.TRANSACTION_RUN_ID:
            failures.append("v1.1 challenge run mismatch")
        if controller_challenge.get("source_hashes_sha256") != manifest_source_hash:
            failures.append("v1.1 challenge source hash mismatch")
        if controller_challenge.get("source_bundle_sha256") != bundle_hash:
            failures.append("v1.1 challenge source bundle mismatch")
        if controller_challenge.get("runtime_binary_sha256") != manifest.get("runtime_binary_authority", {}).get("sha256"):
            failures.append("v1.1 challenge runtime mismatch")
        if not re.fullmatch(r"[0-9a-f]{64}", str(controller_challenge.get("controller_nonce_sha256", ""))):
            failures.append("v1.1 challenge nonce hash missing")
    return {
        "passed": not failures,
        "failures": failures,
...
def execute_authorized(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    result = {
        ...
        "retry_count": 0,
        "live_authority": os.environ.get(AUTHORITY_ENV),
