...
RUN_ID = "family10h_relation_rspatial_origin_reset_tuning_discovery_v1_0"
...
VARIANTS = [
    ...
    "reset_flush_primary",
    "reset_flush_sham",
    "reset_prefault_flush_primary",
    "reset_prefault_flush_sham",
    "reset_double_flush_primary",
    "reset_double_flush_sham",
    "reset_prefault_double_flush_primary",
    "reset_prefault_double_flush_sham",
    "reset_prefault_triple_flush_primary",
    "reset_prefault_triple_flush_sham",
    "reset_lane_a_flush_primary",
    "reset_lane_a_flush_sham",
]
QUERY_BY_VARIANT = {
    ...
    "reset_flush_primary": "reset_flush_query_relation_pair_control",
    "reset_flush_sham": "reset_flush_relation_sham_control",
    "reset_prefault_flush_primary": "reset_prefault_flush_query_relation_pair_control",
    "reset_prefault_flush_sham": "reset_prefault_flush_relation_sham_control",
    ...
    "reset_prefault_double_flush_primary": "reset_prefault_double_flush_query_relation_pair_control",
    "reset_prefault_double_flush_sham": "reset_prefault_double_flush_relation_sham_control",
    "reset_prefault_triple_flush_primary": "reset_prefault_triple_flush_query_relation_pair_control",
    "reset_prefault_triple_flush_sham": "reset_prefault_triple_flush_relation_sham_control",
    "reset_lane_a_flush_primary": "reset_lane_a_flush_query_relation_pair_control",
    "reset_lane_a_flush_sham": "reset_lane_a_flush_relation_sham_control",
}
ORIGIN_FILTER = {"0", "1024"}
...
MATCHED_PERMUTATION_SEED = "family10h-rspatial-origin-reset-tuning-discovery-v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "RSPATIAL_ORIGIN_RESET_TUNING_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY_SUMMARY.md"
...
def patch_runtime_for_rspatial_origin_reset_tuning() -> None:
    ...
    text = text.replace(
        ...
        "    RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_PRIMARY = 13,\n"
        "    RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_SHAM = 14,\n"
        "    RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_PRIMARY = 15,\n"
        "    RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_SHAM = 16,\n"
        "    RELATION_SPATIAL_CONTROL_RESET_LANE_A_FLUSH_PRIMARY = 17,\n"
        "    RELATION_SPATIAL_CONTROL_RESET_LANE_A_FLUSH_SHAM = 18",
    )
    ...
    text = text.replace(
        ...
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "reset_prefault_double_flush_query_relation_pair_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_PRIMARY;\n"
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "reset_prefault_double_flush_relation_sham_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_SHAM;\n"
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "reset_prefault_triple_flush_query_relation_pair_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_PRIMARY;\n"
        "        return 1;\n"
        "    }\n"
        '    if (strcmp(query, "reset_prefault_triple_flush_relation_sham_control") == 0) {\n'
        "        *out = RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_SHAM;\n"
        "        return 1;\n"
        "    }\n"
    ...
    text = text.replace(
        ...
        "        || row->control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_SHAM\n"
        "        || row->control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_SHAM\n"
    ...
    lifecycle_helpers = (
        ...
        "        || control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_PRIMARY\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_SHAM\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_PRIMARY\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_SHAM\n"
        ...
        "        || control == RELATION_SPATIAL_CONTROL_RESET_DOUBLE_FLUSH_SHAM;\n"
        "}\n\n"
        "static int reset_prefault_double_flush_after_source_dead_control(relation_spatial_control_id control) {\n"
        "    return control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_PRIMARY\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_DOUBLE_FLUSH_SHAM;\n"
        "}\n\n"
        "static int reset_prefault_triple_flush_after_source_dead_control(relation_spatial_control_id control) {\n"
        "    return control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_PRIMARY\n"
        "        || control == RELATION_SPATIAL_CONTROL_RESET_PREFAULT_TRIPLE_FLUSH_SHAM;\n"
        "}\n\n"
    ...
    text = text.replace(
        ...
        "                    } else if (reset_prefault_triple_flush_after_source_dead_control(row.control)) {\n"
        "                        relation_spatial_prefault(&shared->state);\n"
        "                        flush_state_lines(&shared->state);\n"
        "                        flush_state_lines(&shared->state);\n"
        "                        flush_state_lines(&shared->state);\n"
        "                    } else if (reset_prefault_double_flush_after_source_dead_control(row.control)) {\n"
        "                        relation_spatial_prefault(&shared->state);\n"
        "                        flush_state_lines(&shared->state);\n"
        "                        flush_state_lines(&shared->state);\n"
    ...
    text = text.replace(
        ...
        "            : (reset_prefault_triple_flush_after_source_dead_control(row.control)\n"
        "                ? \"source_dead_then_prefault_triple_flush_before_spatial_pair_probe\"\n"
        "                : (reset_prefault_double_flush_after_source_dead_control(row.control)\n"
        "                    ? \"source_dead_then_prefault_double_flush_before_spatial_pair_probe\"\n"
...
def compile_runtime() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_RUNTIME_BUILD_V1",
...
def make_rows() -> dict[str, Any]:
    ...
    with source_schedule.open(newline="", encoding="utf-8") as handle:
        ...
        base_rows = [row for row in reader if row["cyclic_origin"] in ORIGIN_FILTER][:ROWS_PER_VARIANT]
    ...
    for variant in VARIANTS:
        ...
        schedules[variant] = {
            "query": query,
            "path": f"RSPATIAL_ORIGIN_RESET_TUNING_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "run_id": RUN_ID,
        "source_schedule": str(source_schedule),
        "source_schedule_sha256": sha256_file(source_schedule),
        "origin_filter": sorted(ORIGIN_FILTER),
        ...
        "mechanism_question": (
            "Can stronger reset recipes kill the source-dead R-spatial primary-minus-sham "
            "coordinate at the previously leaking cyclic origins 0 and 1024?"
        ),
    }
    manifest["schedule_manifest_sha256"] = digest({k: v for k, v in manifest.items() if k != "schedule_manifest_sha256"})
    write_json(SOURCE_ROOT / "RELATION_RSPATIAL_ORIGIN_RESET_TUNING_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_RSPATIAL_ORIGIN_RESET_TUNING_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_CPUFREQ_PREPARE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
for cpu in [4, 5]:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_CPUFREQ_RESTORE_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "cpus": {{}}, "failures": []}}
try:
...
receipt = {{"schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_PREFLIGHT_V1", "passed": not failures and pmu.returncode == 0, "failures": failures, "target_identity": "AuthenticAMD family16 model10 cpus4-5", "sensor": {{"hwmon": "hwmon0", "name": name.read_text().strip() if name.exists() else None, "temp1_input": temp}}, "pmu_preflight": {{"returncode": pmu.returncode, "stdout": pmu.stdout, "stderr": pmu.stderr}}, "small_wall_crossed": False}}
...
for variant in {' '.join(VARIANTS)}; do
  ...
  schedule="$SOURCE_ROOT/RSPATIAL_ORIGIN_RESET_TUNING_SCHEDULES/$variant.tsv"
  if [ -e "$out" ]; then
...
summary = {{"schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_TARGET_SUMMARY_V1", "run_id": "{RUN_ID}", "attempt_consumed": (root / "ATTEMPT_CONSUMED").exists(), "variants": {{}}, "small_wall_crossed": False}}
for variant in variants:
...
(root / "RELATION_RSPATIAL_ORIGIN_RESET_TUNING_TARGET_SUMMARY.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\\n")
PY
...
def build_package() -> dict[str, Any]:
    ...
    with tarfile.open(LOCAL_PACKAGE, "w:gz") as tf:
        ...
        info = tarfile.TarInfo("source/RUN_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY.sh")
        info.size = len(data)
        info.mode = 0o755
        info.mtime = int(time.time())
        tf.addfile(info, io.BytesIO(data))
        files["source/RUN_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY.sh"] = {"sha256": sha256_bytes(data), "size_bytes": len(data)}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_SOURCE_PACKAGE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
...
def deploy_execute_copyback(package: dict[str, Any]) -> dict[str, Any]:
    ...
    extract = run_remote(
        f"set -eu; tar -xzf {REMOTE_PACKAGE} -C {REMOTE_ROOT}; chmod +x {REMOTE_ROOT}/source/relation_spatial_runtime {REMOTE_ROOT}/source/relation_spatial_pmu_preflight {REMOTE_ROOT}/source/RUN_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY.sh",
        timeout=120,
    )
    require(extract["returncode"] == 0, f"extract failed: {extract['stderr']}")
    live = run_remote(f"set +e; cd {REMOTE_ROOT}/source; ./RUN_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY.sh", timeout=3600)
    ...
    copyback = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_COPYBACK_V1",
        "created_at": utc_now(),
    ...
    if copyback["passed"]:
        ...
        cleanup = {"schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_REMOTE_CLEANUP_V1", "cleanup": cleanup_cmd, "passed": cleanup_cmd["returncode"] == 0}
    receipt = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_LIVE_CONTROLLER_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        "package": package,
        "absence": absence,
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    method_pairs = {
        ...
        "reset_flush": ("reset_flush_primary", "reset_flush_sham"),
        "reset_prefault_flush": ("reset_prefault_flush_primary", "reset_prefault_flush_sham"),
        "reset_double_flush": ("reset_double_flush_primary", "reset_double_flush_sham"),
        "reset_prefault_double_flush": ("reset_prefault_double_flush_primary", "reset_prefault_double_flush_sham"),
        "reset_prefault_triple_flush": ("reset_prefault_triple_flush_primary", "reset_prefault_triple_flush_sham"),
        "reset_lane_a_flush": ("reset_lane_a_flush_primary", "reset_lane_a_flush_sham"),
    }
    ...
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "interpretation": {
            ...
            "wrong_lane_a_reset_also_kills_all_strata": stratum_reports["reset_lane_a_flush"][
                "all_one_factor_strata_below_25pct_alive_matched"
            ],
            "exploratory_only": True,
            "small_wall_crossed": False,
    ...
    write_json(RUN_ROOT / "RELATION_RSPATIAL_ORIGIN_RESET_TUNING_DISCOVERY_ANALYSIS.json", result)
    ...
    lines.extend(
        [
            ...
            f"- wrong lane-A reset also kills all strata: `{result['interpretation']['wrong_lane_a_reset_also_kills_all_strata']}`",
...
def prepare() -> dict[str, Any]:
    ...
    patch_runtime_for_rspatial_origin_reset_tuning()
    schedule_manifest = make_rows()
    build = compile_runtime()
    result = {
        "schema": "FAMILY10H_RELATION_RSPATIAL_ORIGIN_RESET_TUNING_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_RSPATIAL_ORIGIN_RESET_TUNING_PREPARE_RESULT.json", result)
    return result
