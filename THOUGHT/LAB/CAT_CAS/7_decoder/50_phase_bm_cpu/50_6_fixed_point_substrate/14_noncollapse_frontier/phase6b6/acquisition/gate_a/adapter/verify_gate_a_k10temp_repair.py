...
def verify_temperature_sources() -> dict[str, Any]:
    ...
    tests = subprocess.run(
        [sys.executable, "-B", "-m", "unittest", "discover", "-s", str(HERE), "-p", "test_gate_a_temperature_custody.py", "-v"],
        cwd=REPO_ROOT,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ...
    require(tests.returncode == 0, f"temperature custody tests failed:\n{tests.stdout}\n{tests.stderr}")
    test_count = sum(1 for line in tests.stderr.splitlines() if line.rstrip().endswith("... ok"))
    require(test_count >= 9, "temperature custody test count below minimum")
    return {
