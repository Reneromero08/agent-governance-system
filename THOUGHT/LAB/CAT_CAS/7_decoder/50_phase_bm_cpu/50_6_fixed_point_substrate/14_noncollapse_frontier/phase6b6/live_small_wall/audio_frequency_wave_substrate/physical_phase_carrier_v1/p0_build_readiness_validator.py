...
def check_analyzer(snapshot: Mapping[str, bytes]) -> dict[str, Any]:
    ...
    if len(fixtures.get("raw_adversary", [])) != 58 or len(fixtures.get("signal_path_positive", [])) != 8 or len(fixtures.get("signal_path_scientific_negative", [])) != 11 or len(fixtures.get("signal_path_custody_negative", [])) != 4 or fixtures.get("scope_law") != expected_scope:
        raise Failure("raw/semantic fixture scope")
    if results.get("schema") != "p0.analyzer-reference-results.v1" or results.get("fixture_count") != 61 or results.get("positive_count") != 8 or results.get("scientific_negative_count") != 33 or results.get("malformed_or_custody_negative_count") != 20 or results.get("raw_adversary_count") != 58 or results.get("signal_path_positive_count") != 8 or results.get("signal_path_scientific_negative_count") != 11 or results.get("signal_path_custody_negative_count") != 4:
    ...
    for role, control in off_resonance.items():
        if set(control) != {"fitted_decay_seconds", "fitted_f_carrier_hz", "fitted_q_factor", "fitted_u95_hz", "linewidth_hz", "probe_frequency_hz", "required_separation_hz", "response_ratio"} or control.get("response_ratio", 1.0) > 0.020 or control.get("required_separation_hz", 0.0) < 20.0 or abs(control.get("fitted_f_carrier_hz", 0.0) - raw["f_ref_hz"]) > control.get("fitted_u95_hz", 0.0) or control.get("fitted_q_factor", 0.0) <= 0.0 or control.get("fitted_decay_seconds", 0.0) <= 0.0 or not math.isclose(control.get("probe_frequency_hz", 0.0) - control.get("fitted_f_carrier_hz", 0.0), control.get("required_separation_hz", -1.0), rel_tol=1e-12, abs_tol=1e-9) or not math.isclose(control.get("required_separation_hz", 0.0), max(20.0, 20.0 * control.get("linewidth_hz", 0.0)), rel_tol=1e-12, abs_tol=1e-9):
    ...
    if dynamic.get("scientific_pass") is not True or dynamic.get("f_ref_hz") != 32800.0 or dynamic.get("f_witness_hz") != 65600.0 or dynamic.get("f_carrier_u95_hz") != 0.025:
        raise Failure("dynamic frequency end-to-end reference")
    return {"fixture_count": 61, "positive": 8, "scientific_negative": 33, "malformed_or_custody_negative": 20, "signal_path_positive": 8, "signal_path_scientific_negative": 11, "signal_path_custody_negative": 4, "raw_adversaries": 58, "fixture_sha256": sha256(snapshot["P0_SCIENTIFIC_FIXTURES.json"]), "result_sha256": sha256(snapshot["P0_ANALYZER_REFERENCE_RESULTS.json"]), "schema_sha256": sha256(snapshot["P0_BUILD_READINESS_SCHEMAS.json"])}
