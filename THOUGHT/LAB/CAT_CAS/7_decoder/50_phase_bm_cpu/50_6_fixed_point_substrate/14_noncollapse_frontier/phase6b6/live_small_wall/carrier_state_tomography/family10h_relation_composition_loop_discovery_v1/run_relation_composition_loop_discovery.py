...
RUN_ID = "family10h_relation_composition_loop_discovery_v1_0"
PACKAGE_ID = "family10h_relation_composition_loop_discovery_v1"
...
SCHEDULE_DIR = SOURCE_ROOT / "COMPOSITION_LOOP_SCHEDULES"
...
SUMMARY_JSON = HERE / "RELATION_COMPOSITION_LOOP_DISCOVERY_SUMMARY.json"
SUMMARY_MD = HERE / "RELATION_COMPOSITION_LOOP_DISCOVERY_SUMMARY.md"
...
def target_script() -> str:
    return (
        BASE_TARGET_SCRIPT()
        .replace("RELATION_TEMPORAL_SOURCE_OFF", "RELATION_COMPOSITION_LOOP")
        .replace("TEMPORAL_SOURCE_OFF_SCHEDULES", "COMPOSITION_LOOP_SCHEDULES")
...
def make_rows() -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        for index, row in enumerate(base_rows):
            ...
            copied["operation_semantics_id"] = f"composition_loop:{COMPOSITION_BY_VARIANT[variant]}:{variant}"
            ...
            copied["matched_twin_group"] = f"{copied['block_id']}:composition_loop:{variant}"
        ...
        schedules[variant] = {
            ...
            "path": f"COMPOSITION_LOOP_SCHEDULES/{variant}.tsv",
            "row_count": len(rows),
    ...
    manifest = {
        "schema": "FAMILY10H_RELATION_COMPOSITION_LOOP_DISCOVERY_SCHEDULE_MANIFEST_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
        ...
        "matched_marginal_law": {
            ...
            "tested_relation": "relation composition sequence only: R0_then_R1 versus R1_then_R0",
    ...
    write_json(SOURCE_ROOT / "RELATION_COMPOSITION_LOOP_SCHEDULE_MANIFEST.json", manifest)
    write_json(HERE / "RELATION_COMPOSITION_LOOP_SCHEDULE_MANIFEST.json", manifest)
    return manifest
...
def analyze_archive(controller: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_COMPOSITION_LOOP_DISCOVERY_ANALYSIS_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(RUN_ROOT / "RELATION_COMPOSITION_LOOP_DISCOVERY_ANALYSIS.json", result)
    ...
    for key in ["r0r1", "r1r0", "dead_r0r1", "dead_r1r0", "source_off_r0r1", "source_off_r1r0", "neutral", "random"]:
    ...
    lines.extend(
        [
            ...
            "",
            "Interpretation:",
            f"- composition-loop candidate: `{result['interpretation']['composition_loop_candidate']}`",
...
def prepare() -> dict[str, Any]:
    ...
    result = {
        "schema": "FAMILY10H_RELATION_COMPOSITION_LOOP_PREPARE_V1",
        "created_at": utc_now(),
        "run_id": RUN_ID,
    ...
    write_json(HERE / "RELATION_COMPOSITION_LOOP_PREPARE_RESULT.json", result)
    return result
