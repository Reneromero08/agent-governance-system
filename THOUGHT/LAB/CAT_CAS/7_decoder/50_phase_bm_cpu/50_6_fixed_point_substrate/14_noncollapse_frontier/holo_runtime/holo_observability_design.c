...
int holo_observability_design_populate_current(HoloObservabilityDesign *d) {
    ...
    if(add_calibration(d,"C0","instrument idle baseline","constant_idle","60 s blocks","3 blocks per session","noise floor;drift","raw_capture;calibration_metadata","baseline nonstationary beyond approved drift bound") ||
       ...
       add_calibration(d,"C7","repeatability","binary_mode_modulation","256 windows","30 identical schedules across sessions","predeclared whitened trajectory-distance statistics","held_out_prediction","session-block-bootstrap repeatability acceptance rule fails") ||
    ...
    add_observability_test(d,"repeatability","distance=median over fixed windows of training-idle-covariance-whitened Euclidean [I,Q,ring_period];alignment=absolute-TSC window index;no dynamic warping","unit=complete held-out session-route-schedule replicate;within=median same-input distance;between=q05 different-input distance","accept iff 10000-resample session-block-bootstrap upper95(within)<lower95(between);freeze all choices before acquisition","defines measured-trajectory repeatability only");
    add_observability_test(d,"state_distinguishability","classes=idle,low_load,high_load,post_impulse_history;balance=1:1:1:1;classifier=L2 multinomial logistic,C={0.01,0.1,1,10};metric=balanced_accuracy","session-level split;chance=0.25;freeze delta_power before acquisition from alpha=0.05,power=0.80,held-out class counts;require lower95(BA)>0.25+delta_power","confidence=10000-resample session-block bootstrap;classifier grid and margin fixed before data;otherwise merge equivalence classes or block","does not identify hidden semiconductor state");
    add_observability_test(d,"delay_embedding_gain","held-out NRMSE for S0,S1,S2 at L=1,2,4,8,16,32","not an authorization gate;10 percent gain only supports S2 over an already-sufficient S1","if S1 passes sufficiency retain S1 unless S2 gains >=10 percent;otherwise choose smallest sufficient S2;block only if none pass","test set cannot select L");
    ...
    add_falsification(d,"F1","identical inputs are not repeatable","predeclared whitened trajectory distance with fixed TSC-window alignment","session-block-bootstrap upper95(within)>=lower95(between)","improve preparation/instrumentation;do not identify operator");
    ...
    add_falsification(d,"F6","no tested measured-state representation is predictively sufficient","S1 sufficiency gates plus S2 gates for L={1,2,4,8,16,32}","S1 fails repeatability,distinguishability,held-out prediction,or stability AND every S2 L fails predictive sufficiency","if S1 passes select S1 when S2 gain<10 percent;if S1 fails select smallest sufficient S2 L;otherwise block");
    add_falsification(d,"F7","prepared states indistinguishable","session-held-out balanced accuracy for four balanced prepared classes","session-block-bootstrap lower95(BA)<=0.25+predeclared delta_power","merge observable equivalence classes or block");
