...
static double alu_burst(uint64_t *state) {
    double f0 = 1.000001, f1 = 1.000002, f2 = 1.000003, f3 = 1.000004;
    double f4 = 1.000005, f5 = 1.000006, f6 = 1.000007, f7 = 1.000008;
    uint64_t x0 = *state, x1 = x0 ^ 0x9e3779b97f4a7c15ULL;
    uint64_t x2 = x0 ^ 0xd1b54a32d192ed03ULL;
    uint64_t x3 = x0 ^ 0x94d049bb133111ebULL;
    for (int i = 0; i < 64; i++) {
        f0 = f0 * 1.00000007 + .99999993; f1 = f1 * 1.00000009 + .99999991;
        f2 = f2 * 1.00000011 + .99999989; f3 = f3 * 1.00000013 + .99999987;
        f4 = f4 * 1.00000015 + .99999985; f5 = f5 * 1.00000017 + .99999983;
        f6 = f6 * 1.00000019 + .99999981; f7 = f7 * 1.00000021 + .99999979;
        x0 = x0 * 6364136223846793005ULL + 1;
        x1 = x1 * 2862933555777941757ULL + 3037000493ULL;
        x2 = x2 * 3202034522624059733ULL + 1;
        x3 = x3 * 3935559000370003845ULL + 2691343689449507681ULL;
    }
    __asm__ volatile("" : "+x"(f0), "+x"(f1), "+x"(f2), "+x"(f3),
                         "+x"(f4), "+x"(f5), "+x"(f6), "+x"(f7),
                         "+r"(x0), "+r"(x1), "+r"(x2), "+r"(x3));
    *state = x0 ^ x1 ^ x2 ^ x3;
