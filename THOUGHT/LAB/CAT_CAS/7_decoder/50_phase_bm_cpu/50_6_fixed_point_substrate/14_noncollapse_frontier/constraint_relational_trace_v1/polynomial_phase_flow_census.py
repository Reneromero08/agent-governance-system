...
class PolynomialPhaseFlowCensusFailure:
    clause_mask: int
...
@dataclass(frozen=True)
class ThreeVariablePolynomialPhaseFlowCensus:
    total_formulae: int
    ...
    failures: tuple[PolynomialPhaseFlowCensusFailure, ...]
    status: str
...
def run_three_variable_polynomial_phase_flow_census(
    ...
    failures: list[PolynomialPhaseFlowCensusFailure] = []
        ...
        if reference.satisfiable:
            ...
            if run.converged_to_public_solution:
                ...
                maximum_solution_steps = max(
                    maximum_solution_steps, run.steps_executed
                )
            else:
                failures.append(
                    PolynomialPhaseFlowCensusFailure(
    ...
    return ThreeVariablePolynomialPhaseFlowCensus(
        ...
        status=(
            "EXHAUSTIVE_THREE_VARIABLE_POLYNOMIAL_PHASE_FLOW_CENSUS_PASS"
            if satisfiable_converged == satisfiable_formulae
            and unsat_false_solutions == 0
            else "EXHAUSTIVE_THREE_VARIABLE_POLYNOMIAL_PHASE_FLOW_CENSUS_EXPOSED_FAILURES"
        ),
    )
