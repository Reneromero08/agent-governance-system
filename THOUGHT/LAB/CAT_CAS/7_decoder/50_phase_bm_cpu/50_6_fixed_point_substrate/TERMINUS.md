...
**Role:** Construct-side terminus / pre-noncollapse interpretation.
**Claim ceiling:** L4-5 (built, run, smuggle-gated, cost-scaled on the real construction). ASCII only.

**Note:** Phase 6B (14_noncollapse_frontier) reopens the campaign by changing the
primitive from scalar candidate recovery to non-collapse OrbitState evolution --
not by continuing forward detector logic. The TERMINUS verdict on the construct
side stands. The noncollapse frontier tests whether a representation change
crosses the same wall from a different angle.
...
- `SPEC.md` sec 1B.1 - the equivariance theorem (no scalar lift synthesizes orientation).
