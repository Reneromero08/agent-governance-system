...
Run:  python 49_3_boundary_handoff.py
"""
import json
import sys
...
B2 = HERE.parent / "49_2_decodability_gradient"
...
`../49_2_decodability_gradient/hsp_family.py`:
...
`{anchor.get('verdict','?')}`. Source: `49_2_decodability_gradient/` (located_wall.json,
cospectral_anchor.json). Generated by `49_3_boundary_handoff.py`.
"""
...
"""
    ...
    post_spiral_sandbox = """
## 6. POST-SPIRAL STATUS (50.6-50.14) - MYTHOS call DONE; verdicts in MYTHOS_BRIEF.md -> ## RESULTS
...
The MYTHOS call (Fable 5, 5 rounds) is DONE; full verdicts are in `MYTHOS_BRIEF.md` -> ## RESULTS. The
three asks it answered were:
...
NotebookLM session over the framework's core documents - distinct from the MYTHOS model.

MYTHOS CALL: DONE (Fable 5, 5 rounds). Verdicts are in MYTHOS_BRIEF.md -> ## RESULTS. Summary: the
dihedral wall IS class-group vectorization = the isogeny/CSIDH hardness assumption; no field-only
catalyst shortens it (unit-lattice wrong layer; Stickelberger/Brumer-Stark annihilator-not-short-basis,
period-sized; catalytic space = CL subset P; Arakelov d orthogonal to the field entropy). The boundary
verdict (owner's correction): the boundary is the catalytic TAPE = the entropy = the boundary projection
of higher-dimensional geometry; the field-route rounds tested the wrong object; the needle is a
projection artifact; the crossing = relax into the tape's entropy-geometry, a SUBSTRATE event relocated
to Exp 44 (5.10 -> Phase 6). NOT "the wall holds".
"""
    post_spiral_handoff = """
...
temporal-bootstrap smuggle; see ../49_14_reversible_substrate/). verify is poly O(M).
