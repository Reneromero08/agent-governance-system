...
49_1_extractive_proof/       -> 49_1_extractive_proof.py, testbed_synth.py, testbed_zeta.py, wrong_answer_control.py
49_2_decodability_gradient/  -> hsp_family.py, 49_2_decodability_gradient.py, 49_2b_nonabelian_reframe.py,
                                49_2c_strong_sampling.py, 49_2_anchor_cospectral.py, *_result.json, located_wall.json
49_3_boundary_handoff/       -> 49_3_boundary_handoff.py, MYTHOS_SANDBOX.md, EXP44_PHASE6_HANDOFF.md
```
Re-run any brick directly; `49_3` regenerates the handoffs from the JSON outputs of 50.2/2b/2c.
...
| Item | Status | Result |
|---|---|---|
| #1 Lattice audit (Exp 25) | **DONE** - `49_4_lattice_audit/` | `EXP25_TOY_SCALE_ONLY` - attack does not cross the bedrock |
| #2 Kuperberg subexponential rung | **DONE** - `49_2_decodability_gradient/49_2d_kuperberg_sieve.py` | `DIHEDRAL_BARRIER_SUBEXPONENTIAL_UPPER_BOUND` (sandwich with 50.2c) |
| #3 Call Mythos | **DONE** (Fable 5, 5 rounds) - `49_3_boundary_handoff/MYTHOS_BRIEF.md` -> ## RESULTS | wall **IS** class-group vectorization = isogeny/CSIDH hardness; no field-only catalyst shortens it; boundary verdict = the catalytic tape (entropy) is the boundary, field-route tested the wrong object, crossing is a SUBSTRATE event relocated to Exp 44. NOT "the wall holds". |
...
| #5 Generalize the gradient | **DONE** - `49_2_decodability_gradient/49_2e_gradient_coverage.py` | `WALL_IS_NON_NORMAL_SUBGROUPS` (Q_8 decisive) |
| #6 Connect to bigger claims | **DONE** - `49_5_decoder_class_map/` | `DECODER_MAP_CONSISTENT` (9 decodable, 3 bounded/wall) |
...
The lattice fixed-point target is added to `49_3_boundary_handoff/EXP44_PHASE6_HANDOFF.md`. This is the
...
### 1. [DONE - 50.4] Point the located barrier at Exp 25 (LWE/SVP)  — `49_4_lattice_audit/`
...
- Build `49_4_lattice_audit/49_4_lwe_audit.py` that runs the Exp 25 solver against the
  SAME null discipline used here: matched random-secret null, recovery vs `n` (lattice
  dimension) and noise sigma, and the budgeted-search control from `49_2c`.
...
> **DONE (this session).** `49_2d_kuperberg_sieve.py`, verdict
...
`49_2c` showed brute-force recovery is O(2^n) and poly(n) fails. The honest middle
...
### 3. [EXECUTED] Call Mythos — done (Fable, 5 rounds); verdicts recorded  — `49_3_boundary_handoff/MYTHOS_BRIEF.md` (## RESULTS)
...
The pre-compressed call is `49_3_boundary_handoff/MYTHOS_BRIEF.md` (self-contained). The three asks:
...
### 4. [HANDOFF VALIDATED; silicon run HARDWARE-BLOCKED] Bare-metal handoff to Exp 44 Phase 6  — `49_3_boundary_handoff/EXP44_PHASE6_HANDOFF.md`
...
> <0.03% error (see `49_5_decoder_class_map/`). The silicon acceptance RUN itself is blocked
...
> **DONE (this session).** `49_2e_gradient_coverage.py`, verdict `WALL_IS_NON_NORMAL_SUBGROUPS`:
...
> **DONE (this session).** `49_5_decoder_class_map/`, verdict `DECODER_MAP_CONSISTENT`: mapped
...
- **`49_2c` measures compute-hardness via a random-budget search** at N up to 2048; the
...
- Stay in `THOUGHT/LAB/CAT_CAS/`. Lab critic must stay clean (`python CAPABILITY/TOOLS/
  governance/critic.py`, grep for `49_the_decoder`). M-4 fires on `(SAT|satisfiab|
