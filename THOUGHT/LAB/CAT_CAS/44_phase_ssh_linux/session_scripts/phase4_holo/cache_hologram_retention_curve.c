...
static void apply_schedule(tape_t *t, int family, int declared_mode, int actual_mode, int trial,
    ...
    uint64_t mask = 0x524554454E54484FULL ^ (uint64_t)family ^
...
int main(void) {
    ...
    printf("delay_class,delay_pauses,family,declared_mode,actual_mode,trial,hash_restored");
        ...
        for (int trial = 0; trial < TRIALS; trial++) {
            for (int family = 0; family < FAMILIES; family++) {
                for (int declared = 0; declared < MODES; declared++) {
                    ...
                    init_tape(&t, 0x4404B000E0000000ULL ^ (uint64_t)trial ^ ((uint64_t)delay_class << 48));
                    ...
                    for (int probe = 0; probe < LINES; probe++) {
                        int line = (probe * 29 + trial * 13 + family * 7 + declared * 3 + delay_class * 11) &
                                   (LINES - 1);
                    ...
                    printf("%d,%d,%s,%s,%s,%d,%d", delay_class, delay_pauses[delay_class],
                           family_name(family), mode_name(declared),
    ...
    fprintf(stderr, "PHASE4B_CACHE_HOLOGRAM_RETENTION_CURVE restored=%d/%d sink=%llu\n",
