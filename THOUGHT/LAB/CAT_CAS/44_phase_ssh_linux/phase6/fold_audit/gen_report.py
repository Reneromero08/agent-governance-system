...
md = """# REPORT - Stage 1 Fold Audit (Exp 44 Phase 6)
...
- `THOUGHT/LAB/CAT_CAS/50_the_decoder/50_14_reversible_substrate/50_14_substrate.py`
  - functions `coset_samples` (lines 51-55) and `make_verify` (lines 58-69)
- `THOUGHT/LAB/CAT_CAS/44_phase_ssh_linux/phase6/SPEC_PHASE6_FIXED_POINT_SUBSTRATE.md`
...
|---|---|---|
| Modulus | `N = 2^n` | 50_14 main(); SPEC Sec 2 |
...
| Sample count | `M = max(4*ceil(sqrt(N)), 48*n)` (`M ~ O(sqrt N)`) | 50_14 main() |
...
| Fixed-point map | `f(x) = x if accept(x) else (x+1) mod N` | 50_14 `f_map` |
...
(50_14_substrate.py loops `n in (8,10,12,14)`; SPEC Sec 6 lists up to 16). The audit runs
