...
def claim_task(task_id: str, agent_name: str = "Antigravity") -> bool:
    ...
    emit_event("task_claimed", {"task_id": task_id, "agent": agent_name, "target": task['target_file']}, INBOX_ROOT)
    return True
...
def update_task(task_id: str, progress: str, details: Optional[dict] = None) -> bool:
    ...
    emit_event("task_updated", {"task_id": task_id, "message": progress}, INBOX_ROOT)
    return True
...
def complete_task(task_id: str, result: dict) -> bool:
    ...
    emit_event("task_completed", {"task_id": task_id, "result": result}, INBOX_ROOT)
    return True
...
def fail_task(task_id: str, error: str, retry: bool = True) -> bool:
    ...
    emit_event("task_failed", {"task_id": task_id, "error": error, "retry": retry}, INBOX_ROOT)
    return True
