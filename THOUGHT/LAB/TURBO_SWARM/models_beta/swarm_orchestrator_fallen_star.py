...
def solve_one(cfg: ClawConfig, client: OllamaClient, logger: logging.Logger, item: Dict) -> Dict:
    ...
    # Max steps reached
    logger.error(f"Max steps ({cfg.max_steps}) reached without fix")
    return {"status": "failed", "reason": "max_steps", "file": rel, "tools": history}
...
def main():
    ...
    p.add_argument("--model", default="starcoder2:15b")
    ...
    logger.info(f"Unleashing {args.workers} Claws on {len(items)} files...")
