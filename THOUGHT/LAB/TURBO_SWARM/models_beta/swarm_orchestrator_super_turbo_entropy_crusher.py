...
def ant_execute(cfg: SwarmConfig, file_path: str, instruction: str, plan: Optional[str], temperature: float, logger: logging.Logger) -> str:
    ...
    timeout = 55 + min(75, len(content) // 1200)
...
def foreman_direct_fix(cfg: SwarmConfig, file_path: str, instruction: str, plan: str, last_error: str, failed_snippet: str, logger: logging.Logger) -> str:
    """Foreman (medium 7B) direct code generation - intermediate fallback between Ant+plan and Architect."""
    wait_model_ready("foreman", timeout=2.0)
    ...
    timeout = 50 + min(70, len(original) // 1500)
    return run_agent_raw(
        model=_MODEL_FALLBACKS["foreman"],
        prompt=prompt,
        timeout=int(timeout),
...
def architect_direct_fix(cfg: SwarmConfig, file_path: str, instruction: str, last_error: str, failed_snippet: str, logger: logging.Logger) -> str:
    ...
    timeout = 55 + min(75, len(original) // 1200)
    return run_agent_raw(
        model=_MODEL_FALLBACKS["architect"],
        prompt=prompt,
        timeout=int(timeout),
...
def process_task(cfg: SwarmConfig, task: Dict[str, Any], logger: logging.Logger) -> Dict[str, Any]:
    ...
    T_ANT1 = 15      # Ant temp 0.18
    T_ANT2 = 30      # Ant temp 0.35
    T_PLAN = 45      # Architect makes plan
    T_ANT_PLAN = 60  # Ant with plan
    T_FOREMAN = 75   # Foreman direct
    T_ARCHITECT = 90 # Architect direct
    T_TIMEOUT = 105  # Hard timeout
    ...
    def try_code(code: Optional[str], step: str, t0: float) -> Optional[str]:
        if not code:
            record(step, False, "no_code", elapsed() - t0)
            return None
    ...
    # TIER 1: Ant direct (temp 0.18) - 0 to 15s
    if elapsed() < T_ANT1 and fixed is None:
        t0 = elapsed()
        resp = ant_execute(cfg, fp, instruction, None, 0.18, logger)
        fixed = try_code(extract_code(resp), "ant_t0.18", t0)

    # TIER 2: Ant direct (temp 0.35) - 15 to 30s
    if elapsed() < T_ANT2 and fixed is None:
        t0 = elapsed()
        resp = ant_execute(cfg, fp, instruction, None, 0.35, logger)
        fixed = try_code(extract_code(resp), "ant_t0.35", t0)

    # TIER 3: Architect plan - 30 to 45s
    if elapsed() < T_PLAN and fixed is None:
        t0 = elapsed()
        plan = architect_plan(cfg, fp, instruction, last_error, "", logger)
        res["plan"] = plan or ""
        record("architect_plan", bool(plan), "ok" if plan else "no_plan", elapsed() - t0)

    # TIER 4: Ant with plan - 45 to 60s
    if elapsed() < T_ANT_PLAN and fixed is None and plan:
        t0 = elapsed()
        resp = ant_execute(cfg, fp, instruction, plan, 0.25, logger)
        fixed = try_code(extract_code(resp), "ant_planned", t0)

    # TIER 5: Foreman direct - 60 to 75s
    if elapsed() < T_FOREMAN and fixed is None and cfg.allow_foreman_direct_fallback:
        t0 = elapsed()
        resp = foreman_direct_fix(cfg, fp, instruction, plan or "", last_error, "", logger)
        fixed = try_code(extract_code(resp), "foreman_direct", t0)

    # TIER 6: Architect direct - 75 to 90s
    if elapsed() < T_ARCHITECT and fixed is None and cfg.allow_architect_direct_fallback:
        t0 = elapsed()
        resp = architect_direct_fix(cfg, fp, instruction, last_error, "", logger)
        fixed = try_code(extract_code(resp), "architect_direct", t0)

    # TIMEOUT
    if elapsed() >= T_TIMEOUT or fixed is None:
        res["reason"] = "timeout" if elapsed() >= T_TIMEOUT else "no_valid_candidate"
...
def main() -> None:
    ...
    # NO WARMUP - start instantly! Models warm on first use.
    logger.info("Starting swarm immediately (no warmup delay)...")
    
    # Set all models as "ready" immediately
