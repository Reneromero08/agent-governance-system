...
def ant_execute(file_path: str, plan: str, instruction: str) -> str:
    ...
    prompt = f"""[MANDATORY CODING TASK]
Fix this file following the plan and instructions.
...
RULES:
1. You MUST include all necessary imports: 'import pytest', 'import sys', 'from pathlib import Path', 'import json'.
2. Use 'from CAPABILITY.PRIMITIVES import ...' and 'from CAPABILITY.PIPELINES import ...'.
3. REPO_ROOT MUST be 'Path(__file__).resolve().parents[N]' where N=3 (usually).
4. Return ONLY the complete, fixed Python code in a ```python block.
5. NO EXPLANATIONS. NO COMMENTS outside the code."""
    
    return run_agent_raw(TIER_1_ANT, prompt, timeout=90)
