...
def main() -> None:
    ...
    if cfg.max_workers == 1:
        for i, item in enumerate(tqdm(work, desc="Hackers", ncols=100, ascii=True)):
            r = process_one(item)
            results.append(r)
            if (i + 1) % save_every == 0:
        ...
        with ThreadPoolExecutor(max_workers=cfg.max_workers) as ex:
            ...
            for i, f in enumerate(tqdm(as_completed(futs), total=len(futs), desc="Hackers", ncols=100, ascii=True)):
