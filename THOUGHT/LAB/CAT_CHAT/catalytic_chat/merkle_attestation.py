...
def sign_merkle_root(merkle_root_hex: str, signing_key_hex: str) -> Dict[str, str]:
    """Sign Merkle root with Ed25519 private key.

    Args:
        merkle_root_hex: Merkle root hex string (64 hex chars)
        signing_key_hex: Ed25519 signing key hex (64 hex chars for seed+pub)

    Returns:
        Attestation dict with scheme, merkle_root, public_key, signature
    ...
    if len(signing_key_hex) != 64:
        raise MerkleAttestationError("signing_key must be 64 hex characters (32 bytes)")
    ...
    msg = b"CAT_CHAT_MERKLE_V1:" + merkle_root_bytes
