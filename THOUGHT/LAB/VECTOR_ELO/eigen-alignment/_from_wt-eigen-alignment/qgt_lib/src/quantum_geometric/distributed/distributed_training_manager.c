...
static inline int MPI_Iprobe(int source, int tag, MPI_Comm comm, int* flag, MPI_Status* status) { (void)source; (void)tag; (void)comm; *flag = 0; (void)status; return 0; }
...
int distributed_manager_handle_failure(distributed_manager_t* manager,
    ...
    // If probe found a message, the rank might still be alive
#ifndef NO_MPI
    // In real MPI builds, check the message source
    if (flag && status.MPI_SOURCE == (int)failed_rank) {
        // Rank responded, not actually failed
        return 0;
    }
#else
    // In stub builds, just check if any message was found
    if (flag) {
        // Message found, rank not actually failed
        return 0;
    }
#endif
