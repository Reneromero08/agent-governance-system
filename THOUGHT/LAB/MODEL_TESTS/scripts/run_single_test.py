#!/usr/bin/env python3
"""
Run a single WTF test by name.

Usage:
  python run_single_test.py floating-point
  python run_single_test.py capability-probe
  python run_single_test.py integer-partition
  python run_single_test.py ambiguity
  python run_single_test.py final-boss
"""
...
def main():
    if len(sys.argv) < 2:
        ...
        for name in TESTS:
            print(f"  - {name}")
        print("\nUsage: python run_single_test.py <test-name>")
        sys.exit(1)
    ...
    print()
    print("Running... (timeout: 900s per iteration, max 10 iterations)")
    print()
    ...
    save_result(test_name, prompt, result, status)
