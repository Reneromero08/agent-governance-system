"""Atomic Catalytic Core — multiple rounds, no head splitting.

An atom: electrons in standing wave orbitals, phase persists.
The Core: attention rounds on same input, si accumulates across rounds.
No Feistel splitting — all heads operate on shared phase substrate.
Just repeat the forward pass, si feeds forward each round.
"""
...
class AtomicCore(nn.Module):
    """Repeated attention on same input. si accumulates across rounds.
    No head splitting — all heads share the phase substrate (like electrons
    sharing nuclear potential). Each round deepens the geodesic."""
    def __init__(self,d=64,H=8,L=2,rounds_per_layer=3):
        super().__init__()
        self.rounds=rounds_per_layer
    ...
    def forward(self,z):
        for attn,curve,phase in zip(self.attns,self.curves,self.phases):
            ...
            for r in range(self.rounds):
                z,si=attn(z+z_init)  # residual from initial state
                z=curve(z,si)
                z=phase(z)
        return z
...
class AtomSys(nn.Module):
    def __init__(self,mv=30,d=64,H=8,L=2,r=3):
        super().__init__()
        self.er=nn.Embedding(mv+1,d);self.ei=nn.Embedding(mv+1,d)
        self.pe=CPE(d);self.core=AtomicCore(d,H,L,r)
...
print("="*60)
print("ATOMIC CORE: repeated rounds, shared phase substrate")
print("="*60)

for d,H,L,r in [(48,6,2,3),(64,8,2,3)]:
    m=AtomSys(me,d,H,L,r)
