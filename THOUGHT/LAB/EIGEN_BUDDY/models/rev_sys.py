"""Reversible Attention Core — inspired by CAT_CAS quantum simulator.

Each attention round: z = z + attn(z) [residual, reversible by subtraction]
si matrix = catalytic tape = quantum state of phase relationships.
Multiple rounds deepen the geodesic without consuming si.
6 rounds (matching quantum scrambler pattern).
"""
...
class ReversibleCore(nn.Module):
    """Repeated residual attention. Each round is reversible:
    z_new = z_old + attn(z_old). Can be reversed: z_old = z_new - attn(z_old).
    si accumulates across rounds — the quantum state of phase relationships.
    Pattern from CAT_CAS 07: 6-round reversible scrambler on catalytic tape."""
    def __init__(self,d=64,H=8,L=2,rounds=6):
        super().__init__()
        self.rounds=rounds
        self.attns=nn.ModuleList([MultiHeadComplexAttention(d,H,'concat',True) for _ in range(L)])
        self.curves=nn.ModuleList([CurvatureModulator(d) for _ in range(L)])
        self.phases=nn.ModuleList([PhaseAccumulator(d) for _ in range(L)])

    def forward(self,z):
        for attn,curve,phase in zip(self.attns,self.curves,self.phases):
            for r in range(self.rounds):
                z,si=attn(z)  # si = catalytic tape = quantum phase state
                z=curve(z,si)  # curvature modulates via si (doesn't consume it)
                z=phase(z)     # phase rotation (reversible: e^(i*theta) <-> e^(-i*theta))
        return z
...
class RevSys(nn.Module):
    def __init__(self,mv=30,d=64,H=8,L=1,rounds=6):
        super().__init__()
        self.er=nn.Embedding(mv+1,d);self.ei=nn.Embedding(mv+1,d)
        self.pe=CPE(d);self.core=ReversibleCore(d,H,L,rounds)
...
mv=6; data=gen(10000,mv)
random.shuffle(data); n=len(data)*4//5; tr,te=data[:n],data[n:]
me=mv*mv*2+mv

print("="*60)
print("REVERSIBLE CORE: 6-round quantum scrambler pattern")
print("="*60)

for d,H,L,r in [(48,6,1,6),(64,8,1,6),(64,8,2,3)]:
    m=RevSys(me,d,H,L,r)
    ...
    print(f"  d={d} L={L} rounds={r}: xy={xy:.1%} P={P:>6,} {'PASS' if xy>.90 else ''}")
