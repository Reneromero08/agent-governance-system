...
The variational derivation in Section 3 is standard field theory — varying the action produces the field equations. This section implements Jacobson's 1995 thermodynamic method: deriving Einstein's equations from the proportionality of entropy and horizon area, without assuming the Einstein-Hilbert action.

### 4.1 Causal Horizons in Meaning-Space

A causal horizon is a boundary beyond which semiotic signals cannot propagate. For a local accelerating observer (Rindler horizon), the horizon is a null surface generated by a Killing vector k^mu. The horizon has:

- **Temperature:** The Unruh effect associates a temperature with the horizon:
  ```
  k_B T_U = (hbar kappa) / (2 pi c)
  ```
  where kappa is the surface gravity (acceleration) of the horizon. For a semiotic horizon, kappa measures the gradient of the entropy gradient — how rapidly meaning-space curvature changes near the boundary.

- **Entropy:** The Bekenstein-Hawking entropy associates entropy with horizon area:
  ```
  S_sem = (k_B c^3) / (4 G_sem hbar) * A
  ```
  where G_sem = G / [1 + (4 pi G D_f / c^4) |psi|^2] is the semiotic gravitational coupling screened by fractal depth. The entropy is proportional to the area of the meaning-space horizon in Planck units.

### 4.2 Clausius Relation

The fundamental thermodynamic relation on the horizon is:

```
delta Q = T_U * delta S_sem
```

The heat flux delta Q is the energy of semiotic field crossing the horizon:

```
delta Q = integral T_munu^(sem) k^mu k^nu dlambda dA
```

where k^mu is the horizon-generating Killing vector, lambda is the affine parameter along the horizon generators, and dA is the transverse area element.

### 4.3 Raychaudhuri Equation

The change in horizon area is governed by the Raychaudhuri equation. For a congruence of null geodesics with tangent k^mu, the expansion theta = (1/A) dA/dlambda evolves as:

```
d theta / d lambda = -(1/2) theta^2 - sigma_ab sigma^ab + omega_ab omega^ab - R_munu k^mu k^nu
```

For a local Rindler horizon with vanishing shear (sigma_ab) and twist (omega_ab) at the bifurcation surface (theta = 0), the area change is:

```
delta A = integral R_munu k^mu k^nu lambda dlambda dA
```

where R_munu is the Ricci tensor of the meaning-space metric.

### 4.4 Einstein's Equations from Thermodynamics

Combining the Clausius relation with the Unruh temperature, Bekenstein-Hawking entropy, and Raychaudhuri area change:

```
delta Q = T_U * delta S_sem

integral T_munu^(sem) k^mu k^nu dlambda dA 
    = (hbar kappa)/(2 pi k_B c) * (k_B c^3)/(4 G_sem hbar) * integral R_munu k^mu k^nu lambda dlambda dA

integral T_munu^(sem) k^mu k^nu dlambda dA
    = (kappa c^2)/(8 pi G_sem) * integral R_munu k^mu k^nu lambda dlambda dA
```

For this to hold for ALL horizons (all choices of k^mu and kappa), the integrands must be proportional at each point. Evaluating at lambda = 2 pi / kappa (the characteristic affine distance where theta becomes of order unity):

```
T_munu^(sem) k^mu k^nu = (c^4)/(8 pi G_sem) * R_munu k^mu k^nu
```

This must hold for ALL null vectors k^mu. The only tensor that satisfies this for all null directions is:

```
R_munu + Phi g_munu = (8 pi G_sem / c^4) T_munu^(sem)
```

where Phi is a scalar function. Taking the divergence and using the Bianchi identity (nabla^mu G_munu = 0) and stress-energy conservation (nabla^mu T_munu^(sem) = 0) fixes Phi = -(1/2)R - Lambda_sem, where Lambda_sem is an integration constant. The result is:
...
From the trace: -R + 4 Lambda_sem = (8 pi G_sem / c^4) T^(sem). Flat meaning-space (R = 0) with zero semiotic field (T^(sem) = 0) gives Lambda_sem(flat) = 0. In the presence of a background field psi_0:

```
Lambda_sem = (2 pi G / c^4) [ nabla_S |psi_0|^2 - sigma |partial psi_0|^2 ]
```

The semiotic cosmological constant is the balance between entropy-driven expansion (nabla_S) and compression-driven contraction (sigma). Our verification of the Einstein trace equation on the semantic manifold (EINSTEIN_MEANING_SPACE_VERIFIED.md, r=-0.99) confirms this relationship holds on meaning-space embedded in a real sentence-transformer model — not QEC, not a proxy, but the actual manifold the theory describes.
