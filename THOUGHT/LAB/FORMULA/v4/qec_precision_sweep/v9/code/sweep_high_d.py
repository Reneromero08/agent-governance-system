"""Extended sweep: d=13,15 for alpha convergence. Same methodology as task1 sweep."""
...
def seed(base, basis, d, p, geom):
    key = f"highd|{base}|{geom}|{basis}|{d}|{p:.8f}".encode()
...
def main():
    ...
    a.add_argument("--shots", type=int, default=100000)
    ...
    conds = [
        Cond(
            basis=b, distance=d, rounds=d, physical_error_rate=p,
            shots=args.shots, seed=seed(20260517, b, d, p, args.geom), geom=args.geom,
        )
    ...
    for i, c in enumerate(conds, 1):
        print(
            f"[{i}/{len(conds)}] {args.geom} d={c.distance} p={c.physical_error_rate} {c.basis}",
            flush=True,
        )
        rows.append(run(c))
    ...
    payload = {
        ...
        "config": {
            "geom": args.geom, "mode": "high_d",
            "shots": args.shots, "distances": dists,
            "bases": bases, "physical_error_rates": ps,
            "noise_model": "depol",
        },
        "conditions": rows,
    }
