# Q41 Geometric Shape Diagnostics - Test Report v3

**Generated:** 2026-01-11T17:32:14.398141+00:00
**Suite Version:** 3.0.0

## Configuration

- Seed: 42
- k-neighbors: 10
- Preprocessing: l2
- Distance metric: euclidean
- Corpus size: 64

---

## Summary

**Identity Tests:** 4/4 passed
**Diagnostic Tests:** 2/6 passed

| Test | Type | Result |
|------|------|--------|
| kernel_trace_identity | identity | PASS |
| laplacian_properties | identity | PASS |
| heat_trace_consistency | identity | PASS |
| rotation_invariance | identity | PASS |
| spectral_signature_diagnostic | diagnostic | FAIL |
| heat_trace_fingerprint_diagnostic | diagnostic | FAIL |
| distance_correlation_diagnostic | diagnostic | PASS |
| covariance_spectrum_diagnostic | diagnostic | FAIL |
| sparse_coding_stability_diagnostic | diagnostic | FAIL |
| persistent_homology_diagnostic | diagnostic | PASS |

---

## kernel_trace_identity

**Type:** identity
**Result:** PASS

### Notes
trace(K) = sum(eigenvalues(K)) - basic linear algebra identity

### Metrics
```json
{
  "trace_direct": 64.0,
  "trace_eigensum": 64.0,
  "absolute_error": 0.0,
  "relative_error": 0.0
}
```

### Thresholds
```json
{
  "relative_error": 1e-08
}
```

### Controls
```json
{}
```

---

## laplacian_properties

**Type:** identity
**Result:** PASS

### Notes
Normalized Laplacian must be symmetric PSD with eigenvalues in [0,2]

### Metrics
```json
{
  "symmetry_error": 0.0,
  "min_eigenvalue": 9.52317302121147e-17,
  "max_eigenvalue": 1.5358201404679448,
  "n_components": 1,
  "near_zero_eigenvalues": 1,
  "eigenvalue_range": [
    9.52317302121147e-17,
    1.5358201404679448
  ]
}
```

### Thresholds
```json
{
  "symmetry_error": 1e-08,
  "min_eigenvalue": -1e-08,
  "max_eigenvalue": 2.00000001
}
```

### Controls
```json
{}
```

---

## heat_trace_consistency

**Type:** identity
**Result:** PASS

### Notes
trace(exp(-tL)) = sum(exp(-t*lambda_i)) - eigendecomposition identity

### Metrics
```json
{
  "trace_matrix_exp": 25.393374389497595,
  "trace_eigensum": 25.393374389497588,
  "absolute_error": 7.105427357601002e-15,
  "relative_error": 2.798142243163927e-16,
  "t_value": 1.0
}
```

### Thresholds
```json
{
  "relative_error": 1e-06
}
```

### Controls
```json
{}
```

---

## rotation_invariance

**Type:** identity
**Result:** PASS

### Notes
Euclidean distance-based constructions must be rotation-invariant

### Metrics
```json
{
  "distance_matrix_error": 1.5543122344752192e-15,
  "eigenvalue_error": 0.0,
  "heat_trace_error": 0.0
}
```

### Thresholds
```json
{
  "distance_matrix_error": 1e-08,
  "eigenvalue_error": 1e-06,
  "heat_trace_error": 1e-06
}
```

### Controls
```json
{
  "rotation_seed": 1042
}
```

---

## spectral_signature_diagnostic

**Type:** diagnostic
**Result:** FAIL

### Notes
Graph Laplacian spectral signatures. High cross-model correlation suggests similar local geometry. NOT testing Ramanujan bounds.

### Metrics
```json
{
  "per_model": {
    "MiniLM": {
      "spectral_gap": 0.10923771597746167,
      "max_eigenvalue": 1.5358201404679448,
      "n_components": 1,
      "mean_degree": 7.46875
    },
    "MPNet": {
      "spectral_gap": 0.06629674632239668,
      "max_eigenvalue": 1.6583579000793651,
      "n_components": 1,
      "mean_degree": 7.5
    },
    "Paraphrase": {
      "spectral_gap": 0.06550561201688526,
      "max_eigenvalue": 1.618909068594788,
      "n_components": 1,
      "mean_degree": 6.84375
    },
    "BERT": {
      "spectral_gap": -8.180015971444755e-17,
      "max_eigenvalue": 1.999999999999998,
      "n_components": 17,
      "mean_degree": 3.5625
    }
  },
  "spectral_correlations": [
    [
      "MiniLM",
      "MPNet",
      0.9999999999999998
    ],
    [
      "MiniLM",
      "Paraphrase",
      0.9999999999999998
    ],
    [
      "MiniLM",
      "BERT",
      0.9999771059650376
    ],
    [
      "MPNet",
      "Paraphrase",
      0.9999999999999998
    ],
    [
      "MPNet",
      "BERT",
      0.9999771059650376
    ],
    [
      "Paraphrase",
      "BERT",
      0.9999771059650376
    ]
  ],
  "heat_correlations": [
    [
      "MiniLM",
      "MPNet",
      1.0
    ],
    [
      "MiniLM",
      "Paraphrase",
      1.0
    ],
    [
      "MiniLM",
      "BERT",
      1.0
    ],
    [
      "MPNet",
      "Paraphrase",
      1.0
    ],
    [
      "MPNet",
      "BERT",
      1.0
    ],
    [
      "Paraphrase",
      "BERT",
      1.0
    ]
  ],
  "mean_spectral_correlation": 0.9999885529825189,
  "mean_heat_correlation": 1.0
}
```

### Thresholds
```json
{
  "positive_control_error": 1e-06,
  "negative_control_decorrelation": 0.9
}
```

### Controls
```json
{
  "positive_rotated_error": 0.0,
  "positive_control_pass": "True",
  "negative_shuffled_correlation": 0.9999885530480366,
  "negative_control_pass": "False"
}
```

---

## heat_trace_fingerprint_diagnostic

**Type:** diagnostic
**Result:** FAIL

### Notes
Heat trace curves encode multi-scale geometry. Similar curves suggest similar structure.

### Metrics
```json
{
  "heat_curve_distances": [
    [
      "MiniLM",
      "MPNet",
      0.001335409491137795
    ],
    [
      "MiniLM",
      "Paraphrase",
      0.006616822039827644
    ],
    [
      "MiniLM",
      "BERT",
      0.053200047139933336
    ],
    [
      "MPNet",
      "Paraphrase",
      0.005543143546339252
    ],
    [
      "MPNet",
      "BERT",
      0.05202759817381133
    ],
    [
      "Paraphrase",
      "BERT",
      0.046610460869503864
    ]
  ],
  "mean_distance": 0.0275555802100922,
  "t_grid": [
    0.01,
    0.1,
    0.5,
    1.0,
    2.0,
    5.0
  ]
}
```

### Thresholds
```json
{
  "diagnostic_threshold": 0.1
}
```

### Controls
```json
{
  "positive_rotated_distance": 0.0,
  "positive_control_pass": "True",
  "negative_gaussian_distance": 0.01454369294839969,
  "negative_control_pass": "False"
}
```

---

## distance_correlation_diagnostic

**Type:** diagnostic
**Result:** PASS

### Notes
Distance matrix correlation measures agreement on relative distances. NOT claimed to test Langlands duality.

### Metrics
```json
{
  "pairwise_correlations": [
    [
      "MiniLM",
      "MPNet",
      0.6073899782253773
    ],
    [
      "MiniLM",
      "Paraphrase",
      0.3480961338254987
    ],
    [
      "MiniLM",
      "BERT",
      0.18817877563339522
    ],
    [
      "MPNet",
      "Paraphrase",
      0.30017881945329283
    ],
    [
      "MPNet",
      "BERT",
      0.16815862580332328
    ],
    [
      "Paraphrase",
      "BERT",
      0.07237196168617795
    ]
  ],
  "mean_correlation": 0.28072904910451085
}
```

### Thresholds
```json
{
  "positive_threshold": 0.999,
  "negative_threshold": 0.5
}
```

### Controls
```json
{
  "positive_rotated_correlation": 1.0,
  "positive_control_pass": "True",
  "negative_shuffled_correlation": 0.023374992113297164,
  "negative_control_pass": "True"
}
```

---

## covariance_spectrum_diagnostic

**Type:** diagnostic
**Result:** FAIL

### Notes
Covariance eigenspectrum comparison. High correlation suggests similar variance structure. Alpha CV measures decay consistency. NOT claimed as Ramanujan bound.

### Metrics
```json
{
  "per_model": {
    "MiniLM": {
      "participation_ratio": 33.81423725725791,
      "decay_exponent_alpha": 0.5464959565267073,
      "decay_prefactor_C": 0.9876402668329037,
      "top_5_eigenvalues": [
        0.9999999983991184,
        0.684371973136147,
        0.5009282464962311,
        0.4163385994757173,
        0.3837448018848588
      ]
    },
    "MPNet": {
      "participation_ratio": 34.556084397547025,
      "decay_exponent_alpha": 0.555758152421964,
      "decay_prefactor_C": 1.2626991846951565,
      "top_5_eigenvalues": [
        0.9999999981933021,
        0.9580781011623044,
        0.6736919608499428,
        0.5993383196267391,
        0.5336697293234858
      ]
    },
    "Paraphrase": {
      "participation_ratio": 31.842866712959427,
      "decay_exponent_alpha": 0.5997881940408291,
      "decay_prefactor_C": 1.465863117362368,
      "top_5_eigenvalues": [
        0.9999999982721646,
        0.9283948254374179,
        0.7227181690885415,
        0.6647758145792789,
        0.5863002172534412
      ]
    },
    "BERT": {
      "participation_ratio": 17.48249333159425,
      "decay_exponent_alpha": 0.91237447534555,
      "decay_prefactor_C": 1.2462983868460957,
      "top_5_eigenvalues": [
        0.9999999891679903,
        0.6183622552681503,
        0.45687999093383247,
        0.37011038872858987,
        0.290378451373423
      ]
    }
  },
  "spectral_correlations": [
    [
      "MiniLM",
      "MPNet",
      1.0
    ],
    [
      "MiniLM",
      "Paraphrase",
      1.0
    ],
    [
      "MiniLM",
      "BERT",
      1.0
    ],
    [
      "MPNet",
      "Paraphrase",
      1.0
    ],
    [
      "MPNet",
      "BERT",
      1.0
    ],
    [
      "Paraphrase",
      "BERT",
      1.0
    ]
  ],
  "mean_spectral_correlation": 1.0,
  "alpha_mean": 0.6536041945837625,
  "alpha_std": 0.1507516523511323,
  "alpha_cv": 0.23064670266394394
}
```

### Thresholds
```json
{
  "positive_control_error": 1e-06,
  "alpha_cv_threshold": 0.3
}
```

### Controls
```json
{
  "positive_rotated_error": 1.536818561298503e-09,
  "positive_control_pass": "True",
  "negative_gaussian_correlation": 1.0,
  "negative_control_pass": "False"
}
```

---

## sparse_coding_stability_diagnostic

**Type:** diagnostic
**Result:** FAIL

### Notes
Sparse coding stability test. High stability means SVD basis is robust. NOT testing UFD or semantic primes.

### Metrics
```json
{
  "n_components": 30,
  "mean_stability": 0.6026932597160339,
  "std_stability": 0.016864843666553497,
  "mean_reconstruction_error": 0.47177064390248213,
  "n_stability_trials": 10
}
```

### Thresholds
```json
{
  "stability_threshold": 0.8,
  "reconstruction_threshold": 0.5
}
```

### Controls
```json
{
  "positive_sv_error": 4.962773991756819e-08,
  "positive_control_pass": "False",
  "negative_sv_correlation": 1.0,
  "negative_control_pass": "False"
}
```

---

## persistent_homology_diagnostic

**Type:** diagnostic
**Result:** PASS

### Notes
Simplified connectivity analysis (ripser unavailable). Install ripser+persim for proper persistent homology.

### Metrics
```json
{
  "per_model": {
    "MiniLM": {
      "scales": [
        1.0899760574601516,
        1.1360687335490958,
        1.1833030764842243,
        1.2245427259941566,
        1.25815608520118
      ],
      "components": [
        1,
        1,
        1,
        1,
        1
      ]
    },
    "MPNet": {
      "scales": [
        1.1523745008690935,
        1.2057705889615098,
        1.245132853553058,
        1.2778406838502294,
        1.3038309214003887
      ],
      "components": [
        1,
        1,
        1,
        1,
        1
      ]
    },
    "Paraphrase": {
      "scales": [
        1.1454974784176029,
        1.2237184245479957,
        1.2758061587942573,
        1.3208985906660886,
        1.3525826623813417
      ],
      "components": [
        3,
        1,
        1,
        1,
        1
      ]
    },
    "BERT": {
      "scales": [
        0.23635961625123214,
        0.2762953558824421,
        0.3299723982401518,
        0.3853966533233336,
        0.4249387843306757
      ],
      "components": [
        27,
        16,
        2,
        1,
        1
      ]
    }
  },
  "component_correlations": [
    [
      "MiniLM",
      "MPNet",
      NaN
    ],
    [
      "MiniLM",
      "Paraphrase",
      NaN
    ],
    [
      "MiniLM",
      "BERT",
      NaN
    ],
    [
      "MPNet",
      "Paraphrase",
      NaN
    ],
    [
      "MPNet",
      "BERT",
      NaN
    ],
    [
      "Paraphrase",
      "BERT",
      0.7254762501100117
    ]
  ],
  "mean_correlation": NaN,
  "method": "simplified_connectivity_fallback"
}
```

### Thresholds
```json
{}
```

### Controls
```json
{
  "positive_control_pass": true,
  "negative_control_pass": true,
  "note": "Using simplified fallback - install ripser for proper persistent homology"
}
```

---

## What This Does NOT Prove

This test suite provides geometric diagnostics on embedding spaces. It does **NOT**:

1. **Prove Langlands correspondence** - No Hecke operators, automorphic forms, or L-functions are constructed
2. **Test Ramanujan bounds** - Covariance eigenvalue decay is measured but not claimed as Ramanujan
3. **Verify trace formula** - No Arthur-Selberg trace formula is implemented
4. **Establish UFD structure** - Sparse coding stability is not unique factorization
5. **Construct TQFT** - No categorical composition or cobordism maps are tested

The tests measure **geometric properties** that may or may not relate to deeper algebraic structures.
Any claims about Langlands program applicability require additional theoretical work beyond these diagnostics.
