"""
Q03 v4: Does R = (E/grad_S) * sigma^Df generalize across domains?
...
v4 fixes from AUDIT_v3.md:
  DOMAIN-01 [CRITICAL]: text_alt replaced with AG News (genuinely different
      corpus, NOT 20 Newsgroups with a different seed).
...
All other v3 fixes retained:
  ...
  STAT-V1a: R-vs-E correlation (r_xy) now logged in results
...
Domains:
  1. text_20ng  -- 20 Newsgroups (sklearn)
  2. text_agnews -- AG News (HuggingFace datasets) -- genuinely different corpus
  3. financial  -- Yahoo Finance sector purity
...
RESULTS = {
    ...
    "pre_registration": {
        ...
        "v4_fix": (
            "DOMAIN-01 from AUDIT_v3: text_alt replaced with AG News, a genuinely "
            "different text corpus (4-class news: World, Sports, Business, Sci/Tech) "
            "instead of 20 Newsgroups with a different random seed."
        ),
        "decision_rule": {
        ...
        "domains": ["text_20ng", "text_agnews", "financial"],
        "text_ground_truth": "cluster purity (label-based, not geometric)",
        "text_agnews_ground_truth": "cluster purity on AG News (genuinely different corpus)",
...
# =========================================================================
# TEXT DOMAIN RUNNER (shared by 20NG and AG News)
# =========================================================================
def run_text_domain(model_name, corpus_name, texts, labels, categories, rng):
    ...
    for cat_id in base_cats:
        ...
        for noise_frac in noise_fractions:
            ...
            clusters.append({
                "category": str(categories[cat_id]),
    ...
    # STAT-V1a fix: log this for transparency
    r_full_e_corr = safe_spearman(R_full_vals, E_vals)
    corr["R_full_vs_E"] = r_full_e_corr
    ...
    corr["R_simple_vs_E"] = r_simple_e_corr
...
def run_financial_domain():
    ...
    """
    print("\n" + "=" * 70)
    print("DOMAIN 3: FINANCIAL (Sector Purity, Non-overlapping Windows)")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------
    # DOMAIN 1: Text -- 20 Newsgroups, 3 diverse models
    # ---------------------------------------------------------------
    print("=" * 70)
    ...
    # ---------------------------------------------------------------
    # DOMAIN 2: AG News (genuinely different text corpus)
    # ---------------------------------------------------------------
    print("\n" + "=" * 70)
    print("DOMAIN 2: TEXT (AG News -- genuinely different corpus)")
    print("=" * 70)
    ...
    print(f"  AG News: {len(ag_texts)} documents, {len(ag_categories)} categories")
    for i, cat in enumerate(ag_categories):
        count = int(np.sum(ag_labels == i))
        print(f"    {cat}: {count} docs")

    rng_ag = np.random.RandomState(42)

    ag_result = run_text_domain(
    ...
    RESULTS["domains"]["text_agnews"] = {
        "dataset": "ag_news",
        "dataset_source": "HuggingFace datasets (fancyzhx/ag_news)",
        "dataset_note": (
            "AG News: 4-class news classification (World, Sports, Business, "
            "Sci/Tech). 120,000 training documents. This is a GENUINELY "
            "DIFFERENT corpus from 20 Newsgroups (DOMAIN-01 fix from AUDIT_v3)."
        ),
        "per_model": {"all-mpnet-base-v2": ag_result},
        "correlations": ag_result["correlations"],
    }

    for cl in ag_result.get("clusters", []):
        all_cluster_metrics.append((f"ag_{cl['cluster_id']}", cl["metrics"]))
    ...
    output_path = (
        ...
        r"\v2\q03_generalization\results\test_v4_q03_results.json"
    )
