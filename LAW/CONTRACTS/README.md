...
- **Fixtures** (`CONTRACTS/fixtures/`, `SKILLS/*/fixtures/`) - Concrete test cases that capture laws and precedents. Each fixture directory typically contains an `input.json` and an `expected.json`. The runner executes the relevant skill or validation script to ensure the actual output matches expectations.
